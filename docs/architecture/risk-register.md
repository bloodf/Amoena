...
This document catalogs all identified risks for the Amoena project — a universal GUI for AI coding TUIs (Claude Code, OpenCode, Codex CLI, Gemini CLI). Risks were identified through:
...
| R9 | System tray missing on Wayland/i3/sway/Hyprland Linux DEs — `tray-icon` crate depends on platform support. | Medium | Medium | Low | Background daemon fallback: headless process managed via CLI (`amoena status/show/quit`). Detect tray availability at startup, fall back gracefully. Add "minimize to background" without tray. | Keep window open on affected DEs (no background mode). Document supported DEs. Investigate D-Bus StatusNotifierItem protocol for broader Wayland support. | Core |
...
These risks stem from Amoena's core challenge: wrapping 4 different TUI applications with varying API maturity.
...
Search engines, package registries, and AI assistants may conflate the two. The Go version's documentation still ranks highly in search results. Amoena targets the active TypeScript version exclusively.
...
Tauri 2 represents a major architectural shift from Tauri 1. Key Tauri plugins Amoena depends on:
...
A compromised package at any level could affect all Amoena users. The npm ecosystem has seen multiple supply chain attacks (event-stream, ua-parser-js, colors.js).

### Agent Runtime Risks

These risks stem from Amoena's multi-agent architecture where agents spawn subagents, accumulate memory, and operate with delegated permissions.
