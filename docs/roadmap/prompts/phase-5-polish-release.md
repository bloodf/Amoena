...
- **amoena-service** (new) — memory, orchestration, remote access, extensions, autopilot, kanban, opinions, CLI integration, replay, diagnostics

Both use a **single SQLite database** (better-sqlite3 + Drizzle ORM) with 43 migrations. The renderer communicates with the main process via **trpc-electron** (direct IPC). Amoena-specific routers are namespaced under `trpc.amoena.*`.
...
**What "Superset-derived screens" means:** The workspace dashboard, terminal tabs, git changes panel, settings page, and agent preset selector all came from the Superset fork. Amoena added 11 new screens on top (Memory Graph Home, Memory Browser, Agent Management, Autopilot, Kanban, Marketplace, Remote Access, Visual Editor, Opinions, Session Replay, Diagnostics).

All 15 features are functional after Phases 1-4. This phase is about polish, completeness, and release readiness.

### Architecture Decisions

- Full app i18n — ALL screens (Superset + Amoena), ~500+ strings
- Mobile app connects to host-service via HTTP/WebSocket
- Cloud removal regression test suite required
- File-based session recordings (~/.amoena/recordings/, gzip compressed)
- Structured logging to ~/.amoena/logs/ (rotated, 50MB max)
...
For QR code pairing, the QR encodes: `amoena://pair?host=192.168.1.x&port=3847&pin=123456`
The mobile app scans this with expo-camera, extracts host/port/pin, and calls
`trpc.amoena.remoteAccess.completePairing.mutate({ pin })`.
...
- Apply Amoena design tokens from packages/tokens
...
- Amoena screens: all user-facing strings (search placeholders, tier labels, phase names, etc.)
...
- Unit tests: all amoena-service services (80%+ coverage)
...
- Amoena logo in sidebar and about dialog
- No Superset references anywhere
- Memory Graph Home as default view
- StatusBar with Amoena widgets
...
Update auto-updater URLs to Amoena release server.
...
- API Reference → tRPC router docs including amoena.\* namespace
...
- [ ] Unit tests: 80%+ coverage on amoena-service
