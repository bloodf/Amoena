...
describe("useTerminalSession", () => {
  ...
  it("returns null terminalSessionId when activeSession is null", async () => {
    const { result } = renderHook(() =>
      useTerminalSession({ activeSession: null, request }),
    );
    await act(async () => {});
  ...
  it("sends shell, cwd, cols, rows in terminal creation payload", async () => {
    ...
    const call = request.mock.calls.find(([p, i]) => p === "/api/v1/terminal/sessions" && i?.method === "POST");
  ...
  it("creates a new terminal session when activeSession changes to a new session", async () => {
    ...
    const session2: SessionSummary = { ...mockSession, id: "session-2", workingDir: "/tmp/project2" };
