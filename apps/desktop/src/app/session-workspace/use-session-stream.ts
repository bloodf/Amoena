  ...
  useEffect(() => {
    ...
    const onEvent = (event: Event) => {
      ...
      switch (payload.eventType) {
        ...
        case "message.delta": {
          setStreamingMessage(
            (previous) => `${previous}${String(payload.payload.text ?? "")}`,
          );
          break;
        }
        case "message.complete": {
          ...
          setMessages((previous) => {
            const withoutStreamingDuplicate = previous.filter(
              (message) =>
                !(message.role === "assistant" && message.content === completed.content),
            );
        ...
        case "agent.status":
        case "agent.spawned": {
          void request<AgentRecord[]>(
            `/api/v1/sessions/${activeSession.id}/agents/list`,
          ).then((nextAgents) => {
