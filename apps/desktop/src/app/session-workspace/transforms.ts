...
export function toWorkspaceSession(summary: SessionSummary) {
  return {
    ...
    model: summary.tuiType,
    provider: "lunaria" as const,
    hasActivity: true,
    permission: "default",
  ...
  for (const agent of records) {
    const mapped: ManagedAgent = {
      ...
      communicationPreference:
        (agent.communicationPreference as ManagedAgent["communicationPreference"]) ??
        undefined,
...
export function toTimelineMessages(
  ...
  const permissionMessages = transcriptEvents
    ...
    .map((event) => ({
      ...
      content: String(
        event.payload.message ?? event.payload.toolName ?? "Permission requested",
      ),
...
export function toMemoryEntries(
  memory: SessionMemoryResponse | null,
): MemoryTabEntry[] {
  if (!memory) {
    return [];
  }
