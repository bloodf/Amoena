...
describe("App shell", () => {
  beforeEach(() => {
    ...
    fetchMock.mockImplementation(async (input: string, init?: RequestInit) => {
      ...
      if (input.endsWith("/api/v1/settings") && typeof init?.method === "string" && init.method === "POST") {
        return {
          ok: true,
          status: 204,
          json: async () => ({}),
  ...
  it("persists general remote settings through the runtime API", async () => {
    ...
    await userEvent.click(screen.getAllByRole("button").find((button) =>
      button.className.includes("relative h-5 w-10 rounded-full"),
    )!);
  ...
  it("sends structured folder_ref attachments from the workspace composer", async () => {
    ...
    await waitFor(() => {
      const messageCall = fetchMock.mock.calls.find(
        ([input, init]) =>
          String(input).includes("/api/v1/sessions/session-1/messages") &&
          init?.method === "POST",
      );
