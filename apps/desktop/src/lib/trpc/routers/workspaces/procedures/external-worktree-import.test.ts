...
const TEST_DIR = join(realpathSync(tmpdir()), `amoena-test-external-wt-${process.pid}`);
...
function createExternalWorktree(mainRepoPath: string, branch: string, worktreePath: string): void {
  ...
  writeFileSync(join(worktreePath, 'test.txt'), 'Important work in external worktree\n');
...
describe('External worktree detection and import', () => {
  ...
  test('external worktree data survives simulated deletion', () => {
    ...
    expect(existsSync(join(externalWorktreePath, 'important-data.txt'))).toBe(true);
