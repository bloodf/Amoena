...
type TableName = 'projects' | 'workspaces' | 'worktrees' | 'settings' | 'workspaceSections';
...
function createTable<Key extends string>(tableName: TableName, keys: readonly Key[]): Table<Key> {
  const table = {
    __tableName: tableName,
  } as {
    __tableName: TableName;
...
function projectSelection(row: Row, selection?: Record<string, Column>): Row | undefined {
  if (!row) {
    return undefined;
  }
...
function runSelect(
  ...
  if (orderBy?.kind === 'desc') {
    rows.sort((a, b) => Number(b[orderBy.column.key] ?? 0) - Number(a[orderBy.column.key] ?? 0));
  }
...
function createSelectResult(
  ...
  return {
    ...
    orderBy: (nextOrderBy: OrderBy) => createSelectResult(table, selection, predicate, nextOrderBy),
  };
}
...
const localDb = {
  select: (selection?: Record<string, Column>) => ({
    from: (table: { __tableName: TableName }) => ({
      ...
      where: (predicate: Predicate) => createSelectResult(table, selection, predicate),
      orderBy: (orderBy: OrderBy) => createSelectResult(table, selection, undefined, orderBy),
    }),
  }),
  insert: (table: { __tableName: TableName }) => ({
    values: (value: Row) => {
      ...
      return {
        ...
        onConflictDoUpdate: ({ target, set }: { target: Column; set: Row }) => ({
          run: () => {
            const rows = getTableRows(table);
            const existingRow = rows.find((row) => row[target.key] === value[target.key]);
            if (existingRow) {
...
vi.mock('@lunaria/local-db/schema', () => ({
  projects,
  settings,
  workspaces,
  workspaceSections,
...
const TEST_DIR = join(realpathSync(tmpdir()), `amoena-test-create-${process.pid}`);
...
function createExternalWorktree(mainRepoPath: string, branch: string, worktreePath: string): void {
...
describe('Workspace creation with external worktree auto-import', () => {
  ...
  afterEach(() => {
    // Clean up database
    if (projectId) {
      localDb.delete(workspaces).where(eq(workspaces.projectId, projectId)).run();
  ...
  test('should auto-import external worktree when creating workspace for existing branch', async () => {
    ...
    const { createWorkspaceFromExternalWorktree } = await import('../utils/workspace-creation');
  ...
  test('should preserve external worktree on disk when removing the workspace record', async () => {
    ...
    const { deleteWorkspace, deleteWorktreeRecord } = await import('../utils/db-helpers');
...
describe('External worktree import via openExternalWorktree', () => {
  ...
  test('should mark worktree as external when using openExternalWorktree', async () => {
    // Create external worktree
    createExternalWorktree(mainRepoPath, 'feature-manual', externalWorktreePath);

    const { openExternalWorktree } = await import('../utils/workspace-creation');
