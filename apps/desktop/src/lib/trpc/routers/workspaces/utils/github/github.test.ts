...
describe("getPullRequestRepoArgs", () => {
	test("returns upstream repo args for forks", () => {
		expect(
			getPullRequestRepoArgs({
				isFork: true,
				upstreamUrl: "git@github.com:LunariaAi/superset.git",
			}),
		).toEqual(["--repo", "LunariaAi/superset"]);
	});
	...
	test("returns no repo args for non-forks", () => {
		expect(
			getPullRequestRepoArgs({
				isFork: false,
				upstreamUrl: "https://github.com/LunariaAi/superset",
			}),
		).toEqual([]);
	});
						...
						nodes: [
							{
								...
								url: "https://github.com/LunariaAi/superset/pull/2681#discussion_r42",
			...
			parseConversationCommentsResponse([
				{
					...
					html_url:
						"https://github.com/LunariaAi/superset/pull/2681#issuecomment-7",
				},
			]),
		).toEqual([
			{
				...
				url: "https://github.com/LunariaAi/superset/pull/2681#issuecomment-7",
				kind: "conversation",
				isResolved: false,
			},
		]);
