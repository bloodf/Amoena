...
const TEST_DIR = join(tmpdir(), `amoena-test-setup-${process.pid}`);
...
describe("loadSetupConfig", () => {
	...
	test("prefers worktree config over main repo config", () => {
		const mainConfig = { setup: ["./.amoena/setup.sh"] };
...
describe("config.local.json", () => {
	...
	test("local config with before/after merges with base setup", () => {
		...
		writeFileSync(
			join(MAIN_REPO, ".amoena", "config.local.json"),
			JSON.stringify({
	...
	test("worktree local config takes priority over main repo local config", () => {
		...
		mkdirSync(join(WORKTREE, ".amoena"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".amoena", "config.local.json"),
