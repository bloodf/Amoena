...
const callSmallModelMock = vi.hoisted(() =>
  vi.fn((async () => ({
    result: null,
    attempts: [],
...
const generateTitleFromMessageMock = vi.hoisted(() =>
  vi.fn((async () => null) as (...args: unknown[]) => Promise<string | null>),
);
const generateTitleFromMessageWithStreamingModelMock = vi.hoisted(() =>
  vi.fn((async () => null) as (...args: unknown[]) => Promise<string | null>),
);
...
vi.mock('lib/ai/call-small-model', () => ({
  callSmallModel: callSmallModelMock,
}));

vi.mock('@lunaria/chat/server/desktop', () => ({
  __esModule: true,
  generateTitleFromMessage: generateTitleFromMessageMock,
  generateTitleFromMessageWithStreamingModel: generateTitleFromMessageWithStreamingModelMock,
}));
...
const selectGetMock = vi.hoisted(() => vi.fn((): SelectedWorkspace => null));
const updateRunMock = vi.hoisted(() => vi.fn(() => ({ changes: 1 })));
const localDbMock = {
  select: vi.fn(() => ({
    from: () => ({
      where: () => ({
...
const { attemptWorkspaceAutoRenameFromPrompt, generateWorkspaceNameFromPrompt } =
...
describe('generateWorkspaceNameFromPrompt', () => {
  ...
  it('falls back to a prompt-derived title when no providers are available', async () => {
    await expect(
      ...
      warning: 'No model account was connected, so a prompt-based title was used.',
    });
  });
  ...
  it('uses the last relevant provider issue in the fallback warning', async () => {
    ...
    await expect(
      ...
      usedPromptFallback: true,
      warning: 'OpenAI needs permission model.request, so a prompt-based title was used.',
    });
  });

  it('uses streaming title generation for OpenAI OAuth naming', async () => {
    generateTitleFromMessageWithStreamingModelMock.mockResolvedValue('Checking In');
    ...
    await expect(generateWorkspaceNameFromPrompt('hey boss how are you')).resolves.toEqual({
      name: 'Checking In',
  ...
  it('preserves empty-string model results instead of forcing fallback', async () => {
    ...
    await expect(generateWorkspaceNameFromPrompt('name this workspace')).resolves.toEqual({
      name: '',
      usedPromptFallback: false,
    });
  });
