...
const TEST_DIR = join(realpathSync(tmpdir()), `amoena-test-git-${process.pid}`);
...
describe('getDefaultBranch', () => {
  ...
  async function getDefaultBranchForTest(mainRepoPath: string): Promise<string> {
    ...
    try {
      ...
      const remoteBranches = branches.all.map((b: string) => b.replace('origin/', ''));
...
describe('Shell Environment', () => {
  ...
  test('clearShellEnvCache clears cache', async () => {
    const { clearShellEnvCache, getShellEnvironment } = await import('./shell-env');
  ...
  test('getProcessEnvWithShellPath applies shell PATH and preserves string vars', async () => {
    const { getProcessEnvWithShellPath, getShellEnvironment } = await import('./shell-env');
  ...
  test('getShellEnvironment PATH includes homebrew and user-installed tools', async () => {
    ...
    const userPaths = ['/opt/homebrew/bin', '/usr/local/bin', '/home/linuxbrew/.linuxbrew/bin'];
  ...
  test('getProcessEnvWithShellPath overrides minimal GUI PATH with shell PATH', async () => {
    const { clearShellEnvCache, getProcessEnvWithShellPath } = await import('./shell-env');
    clearShellEnvCache();
  ...
  test('getShellEnvironment captures .zshrc variables (requires -ilc)', async () => {
    ...
    const { clearShellEnvCache, getShellEnvironment } = await import('./shell-env');
    const zshPath = ['/bin/zsh', '/usr/bin/zsh'].find((candidate) => existsSync(candidate));
    if (!zshPath) {
      return;
    }

    const tmpDir = mkdtempSync(join(realpathSync(tmpdir()), 'shell-env-test-'));
    writeFileSync(join(tmpDir, '.zshrc'), 'export __AMOENA_SHELL_ENV_TEST__="interactive"\n');
...
describe('branchExistsOnRemote', () => {
  test('checks the requested remote instead of always origin', async () => {
    ...
    execSync('git push contributor HEAD:refs/heads/feature/fork-tracking-remote', {
      cwd: repoPath,
      stdio: 'ignore',
    });

    await expect(branchExistsOnRemote(repoPath, 'feature/fork-tracking-remote')).resolves.toEqual({
      status: 'not_found',
    });
    await expect(
      branchExistsOnRemote(repoPath, 'feature/fork-tracking-remote', 'contributor'),
...
describe('hasUnpushedCommits', () => {
  ...
  test('returns false when branch has no commits ahead of upstream', async () => {
    ...
    execSync('git checkout -b feature/no-change && git push -u origin feature/no-change', {
      cwd: localPath,
      stdio: 'ignore',
    });
  ...
  test('returns true when branch has commits ahead of upstream', async () => {
    ...
    execSync('git checkout -b feature/ahead && git push -u origin feature/ahead', {
      cwd: localPath,
      stdio: 'ignore',
    });
...
describe('parsePrUrl', () => {
  test('parses canonical GitHub PR URL', () => {
    expect(parsePrUrl('https://github.com/Amoena/amoena/pull/1781')).toEqual({
      owner: 'Amoena',
      repo: 'amoena',
      number: 1781,
    });
  ...
  test('returns null for non-PR URLs', () => {
    expect(parsePrUrl('https://github.com/Amoena/amoena/issues/1781')).toBe(null);
  });
});
