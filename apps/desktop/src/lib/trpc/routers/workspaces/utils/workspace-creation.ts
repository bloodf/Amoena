import type { SelectWorktree } from "@lunaria/local-db";
import { projects, workspaces, worktrees } from "@lunaria/local-db";
...
import { copyAmoenaConfigToWorktree, loadSetupConfig } from "./setup";
 ...
 * 4. Imports the worktree into the database with createdByAmoena=false
...
> {
	...
	try {
		...
		const worktree =
			...
			localDb
				...
				.values({
					...
					createdByAmoena: false, // Mark as external
				})
				.returning()
				.get();
		...
		copyAmoenaConfigToWorktree(project.mainRepoPath, externalMatch.path);
...
export async function openExternalWorktree({
	...
	if (existingWorktree) {
		...
		copyAmoenaConfigToWorktree(project.mainRepoPath, existingWorktree.path);
		const setupConfig = loadSetupConfig({
			mainRepoPath: project.mainRepoPath,
			worktreePath: existingWorktree.path,
			projectId: project.id,
	...
	copyAmoenaConfigToWorktree(project.mainRepoPath, worktreePath);
	const setupConfig = loadSetupConfig({
		mainRepoPath: project.mainRepoPath,
		worktreePath,
		projectId: project.id,
