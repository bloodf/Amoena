...
type WorkerBehavior = 'boot-fail' | 'fail-on-task' | 'mismatched-only' | 'success';
...
const { WorkerTaskError, WorkerTaskRunner } = await import('./WorkerTaskRunner');
...
describe('WorkerTaskRunner failure handling', () => {
  ...
  test('ignores mismatched worker task results without freeing the slot', async () => {
    ...
    const first = runner.runTask('status', { id: 'first' }, { timeoutMs: 1_000 });
    const second = runner.runTask('status', { id: 'second' }, { timeoutMs: 1_000 });
    await flushMicrotasks(10);
