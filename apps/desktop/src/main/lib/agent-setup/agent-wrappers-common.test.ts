...
const TEST_ROOT = path.join(tmpdir(), `amoena-common-test-${process.pid}-${Date.now()}`);
...
describe('agent-wrappers-common', () => {
  ...
  describe('isAmoenaManagedHookCommand', () => {
    ...
    it('detects Amoena-managed hook command', () => {
      expect(isAmoenaManagedHookCommand('/home/user/.amoena/hooks/notify.sh', 'notify.sh')).toBe(
        true,
      );
    });
    ...
    it('detects environment-scoped Amoena paths', () => {
      expect(
        isAmoenaManagedHookCommand('/home/user/.amoena-dev/hooks/notify.sh', 'notify.sh'),
      ).toBe(true);
    });

    it('handles backslash paths (Windows)', () => {
      expect(
        isAmoenaManagedHookCommand('C:\\Users\\user\\.amoena\\hooks\\notify.sh', 'notify.sh'),
      ).toBe(true);
    });
  });
