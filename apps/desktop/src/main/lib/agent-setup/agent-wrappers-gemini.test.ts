...
const TEST_ROOT = path.join(realOs.tmpdir(), `amoena-gemini-test-${process.pid}-${Date.now()}`);
...
vi.mock('./agent-wrappers-common', () => ({
  ...
  createWrapper: vi.fn(() => {}),
  isAmoenaManagedHookCommand: vi.fn(() => false),
  reconcileManagedEntries: vi.fn(
    ({
      current,
      desired,
    }: {
...
describe('agent-wrappers-gemini', () => {
  ...
  describe('getGeminiSettingsJsonContent', () => {
    it('returns JSON with BeforeAgent, AfterAgent, AfterTool hooks', () => {
      const content = getGeminiSettingsJsonContent(path.join(TEST_HOOKS_DIR, 'gemini-hook.sh'));
    ...
    it('preserves existing settings', () => {
      ...
      writeFileSync(settingsPath, JSON.stringify({ customSetting: true, hooks: {} }));
      const content = getGeminiSettingsJsonContent(path.join(TEST_HOOKS_DIR, 'gemini-hook.sh'));
      const parsed = JSON.parse(content);
