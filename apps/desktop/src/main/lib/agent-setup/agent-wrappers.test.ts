...
const TEST_ROOT = path.join(realOs.tmpdir(), `amoena-agent-wrappers-${process.pid}-${Date.now()}`);
...
describe('reconcileManagedEntries', () => {
  it('preserves user-managed entries while replacing stale managed entries', () => {
    const result = reconcileManagedEntries({
      current: ['/usr/local/bin/custom-hook Start', '/tmp/.amoena-old/hooks/notify.sh Start'],
    ...
    expect(result.replacedManagedEntries).toEqual(['/tmp/.amoena-old/hooks/notify.sh Start']);
  });
  ...
  it('reconciles edited managed entries even when a managed hook already exists', () => {
    ...
    expect(result.entries).toEqual(['/tmp/.amoena-current/hooks/notify.sh Start']);
...
describe('agent-wrappers copilot', () => {
  ...
  it('rewrites stale amoena-notify.json with current hook path', () => {
    ...
    const wrapperScript = buildWrapperScript('copilot', buildCopilotWrapperExecLine());
  ...
  it('injects codex start + permission watchers and completion notifications in wrapper', () => {
    ...
    expect(wrapper).toContain(`approval_request_\${_amoena_approval_fallback_seq}`);
    ...
    const execLine = buildCodexWrapperExecLine(path.join(TEST_HOOKS_DIR, 'notify.sh'));
  ...
  it('replaces stale Cursor hook commands from old amoena paths', () => {
    ...
    expect(beforeSubmitPrompt.some((entry) => entry.command === `${currentHookPath} Start`)).toBe(
      true,
    );
    expect(beforeSubmitPrompt.some((entry) => entry.command.includes(staleHookPath))).toBe(false);
    expect(
      beforeSubmitPrompt.some((entry) => entry.command === '/usr/local/bin/custom-hook Start'),
    ).toBe(true);
  ...
  it('replaces stale Gemini hook commands from old amoena paths', () => {
    const geminiSettingsPath = path.join(mockedHomeDir, '.gemini', 'settings.json');
    ...
    for (const eventName of eventNames) {
      ...
      expect(
        hooks.some((def) => def.hooks?.length === 1 && def.hooks[0]?.command === currentHookPath),
      ).toBe(true);
      expect(
        hooks.some((def) => def.hooks.some((hook) => hook.command.includes(staleHookPath))),
      ).toBe(false);
    }

    const beforeAgent = parsed.hooks.BeforeAgent;
    expect(
      beforeAgent.some((def) => def.hooks.some((hook) => hook.command === '/opt/custom-hook.sh')),
    ).toBe(true);
  ...
  it('replaces stale Mastra hook commands from old amoena paths', () => {
    ...
    const parsed = JSON.parse(content) as Record<string, Array<{ type: string; command: string }>>;
    ...
    for (const eventName of managedEvents) {
      ...
      expect(
        hooks.some(
          (entry) => entry.type === 'command' && entry.command === `bash '${currentHookPath}'`,
        ),
      ).toBe(true);
      expect(hooks.some((entry) => entry.command.includes(staleHookPath))).toBe(false);
    }

    expect(
      parsed.UserPromptSubmit.some((entry) => entry.command === '/usr/local/bin/custom-hook'),
    ).toBe(true);
    expect(JSON.parse(content2)).toEqual(JSON.parse(content));
  });

  it('replaces stale Droid hook commands from old amoena paths', () => {
    const droidSettingsPath = path.join(mockedHomeDir, '.factory', 'settings.json');
    const staleHookPath = '/tmp/.amoena-old/hooks/notify.sh';
    const currentHookPath = '/tmp/.amoena-new/hooks/notify.sh';
    ...
    const managedEvents = ['UserPromptSubmit', 'Notification', 'Stop', 'PostToolUse'] as const;

    for (const eventName of managedEvents) {
      const hooks = parsed.hooks[eventName];
      expect(Array.isArray(hooks)).toBe(true);
      expect(hooks.some((def) => def.hooks.some((hook) => hook.command === currentHookPath))).toBe(
        true,
      );
      expect(
        hooks.some((def) => def.hooks.some((hook) => hook.command.includes(staleHookPath))),
    ...
    expect(parsed.hooks.PostToolUse.some((def) => def.matcher === '*')).toBe(true);
    expect(JSON.parse(content2)).toEqual(JSON.parse(content));
  });
  ...
  it('skips Droid settings writes when the existing JSON is invalid', () => {
    ...
    expect(getDroidSettingsJsonContent('/tmp/.amoena-new/hooks/notify.sh')).toBeNull();
...
describe('agent-wrappers claude settings.json', () => {
  ...
  it('creates Claude settings.json with hooks when no file exists', () => {
    ...
    for (const eventName of managedEvents) {
      ...
      expect(Array.isArray(hooks)).toBe(true);
      expect(
        hooks.some((def) => def.hooks.some((hook) => hook.command === managedClaudeHookCommand)),
      ).toBe(true);
    }
  ...
  it('preserves user hooks and non-hook settings when merging', () => {
    const claudeSettingsPath = path.join(mockedHomeDir, '.claude', 'settings.json');
    ...
    expect(
      parsed.hooks.UserPromptSubmit.some((def: { hooks: Array<{ command: string }> }) =>
        def.hooks.some((hook: { command: string }) => hook.command === '/opt/my-custom-hook.sh'),
      ),
    ).toBe(true);
    ...
    expect(
      parsed.hooks.UserPromptSubmit.some((def: { hooks: Array<{ command: string }> }) =>
        def.hooks.some((hook: { command: string }) => hook.command === managedClaudeHookCommand),
      ),
    ).toBe(true);
  });
  ...
  it('skips Claude settings writes when existing JSON is invalid', () => {
    ...
    expect(getClaudeGlobalSettingsJsonContent('/tmp/.amoena/hooks/notify.sh')).toBeNull();
...
describe('agent-wrappers codex hooks.json', () => {
  ...
  it('creates Codex hooks.json with SessionStart and Stop when no file exists', () => {
    ...
    for (const eventName of ['SessionStart', 'Stop'] as const) {
      const hooks = parsed.hooks[eventName];
      expect(Array.isArray(hooks)).toBe(true);
      expect(hooks.some((def) => def.hooks.some((hook) => hook.command === notifyPath))).toBe(true);
    }
  });
  ...
  it('skips Codex hooks writes when existing JSON is invalid', () => {
    ...
    expect(getCodexGlobalHooksJsonContent('/tmp/.amoena/hooks/notify.sh')).toBeNull();
