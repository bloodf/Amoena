...
import { TERMINAL_ATTACH_CANCELED_MESSAGE, TerminalAttachCanceledError } from '../errors';
...
class MockTerminalHostClient extends EventEmitter {
  ...
  async createOrAttach(params: { sessionId: string; requestId?: string }, signal?: AbortSignal) {
    if (this.createOrAttachGate) {
...
describe('DaemonTerminalManager kill tracking', () => {
  ...
  it('waits for daemon exit and labels killed sessions', async () => {
    ...
    const sessions = (manager as unknown as { sessions: Map<string, SessionInfo> }).sessions;
    sessions.set(paneId, {
      paneId,
      workspaceId: 'ws-1',
      isAlive: true,
  ...
  it('supersedes older createOrAttach requests for the same pane', async () => {
    ...
    await expect(firstPromise).rejects.toThrow(TERMINAL_ATTACH_CANCELED_MESSAGE);
  ...
  it('does not dispatch stale daemon work after canceling before dispatch', async () => {
    ...
    await expect(attachPromise).rejects.toThrow(TERMINAL_ATTACH_CANCELED_MESSAGE);
