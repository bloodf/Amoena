...
const TEST_DIR = join(tmpdir(), `amoena-test-loader-${process.pid}`);
const WORKTREE_PATH = join(TEST_DIR, "worktree");
const AMOENA_DIR = join(WORKTREE_PATH, ".amoena");
const PORTS_FILE = join(AMOENA_DIR, "ports.json");

describe("loadStaticPorts", () => {
	beforeEach(() => {
		mkdirSync(AMOENA_DIR, { recursive: true });
	});
