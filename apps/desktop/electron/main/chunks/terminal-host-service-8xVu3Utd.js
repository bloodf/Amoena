...
async function startTerminalHost() {
  ...
  const servePath = isDev ? path__default.resolve(app.getAppPath(), "../../packages/terminal-host/src/serve.ts") : path__default.join(process.resourcesPath, "terminal-host", "serve.js");
  console.log("[terminal-host] Starting from:", servePath);
  const env = {
    ...process.env,
    PORT: String(state.port),
    HOST_DB_PATH: path__default.join(app.getPath("userData"), "host.db"),
    NODE_ENV: isDev ? "development" : "production"
  };
  const proc = isDev ? childProcess.spawn("bun", ["run", servePath], { env, stdio: ["ignore", "pipe", "pipe"] }) : childProcess.spawn("node", [servePath], { env, stdio: ["ignore", "pipe", "pipe"] });
  state.process = proc;
  ...
  proc.on("exit", (code, signal) => {
    ...
    if (state.restartCount < MAX_RESTARTS) {
      ...
      setTimeout(() => startTerminalHost(), 1e3 * state.restartCount);
    }
  });
...
function getTerminalHostStatus() {
...
export {
  getTerminalHostStatus,
  startTerminalHost,
  stopTerminalHost
};
//# sourceMappingURL=terminal-host-service-8xVu3Utd.js.map
