...
export async function POST(request: NextRequest) {
	...
	const gateway = db
		.prepare(
			"SELECT id, host, port, token, is_primary FROM gateways WHERE id = ?",
		)
	...
	const explicitBrowserWsUrl = String(
	...
	const remoteUrl =
		explicitBrowserWsUrl || resolveRemoteGatewayUrl(gateway, request);
	const ws_url =
		remoteUrl ||
	...
	if (gateway.is_primary === 1 && detectedToken && detectedToken !== dbToken) {
		try {
			db.prepare(
				"UPDATE gateways SET token = ?, updated_at = (unixepoch()) WHERE id = ?",
			).run(detectedToken, gateway.id);
		} catch {
