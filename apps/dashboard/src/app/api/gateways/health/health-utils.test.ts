...
function isBlockedUrl(
	urlStr: string,
	userConfiguredHosts: Set<string>,
): boolean {
	try {
...
describe("isBlockedUrl", () => {
	...
	it("blocks cloud metadata hostnames", () => {
		expect(isBlockedUrl("http://metadata.google.internal/", new Set())).toBe(
			true,
		);
	...
	it("allows user-configured hosts even if private", () => {
		...
		expect(
			isBlockedUrl("http://192.168.1.100:3000/", new Set(["192.168.1.100"])),
		).toBe(false);
	...
	it("allows public external hosts", () => {
		expect(
			isBlockedUrl("https://example.tailnet.ts.net:4443/", new Set()),
		).toBe(false);
...
describe("buildGatewayProbeUrl", () => {
	it("builds URL from bare host + port", () => {
		expect(buildGatewayProbeUrl("example.com", 8080)).toBe(
	...
	it("does not overwrite existing port in URL", () => {
		const result = buildGatewayProbeUrl(
	...
	it("handles URL with query string token", () => {
		const result = buildGatewayProbeUrl(
			"https://gw.example.com/sessions?token=abc",
			0,
		);
