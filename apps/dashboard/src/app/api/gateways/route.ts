...
export async function GET(request: NextRequest) {
	...
	const gateways = db
		.prepare("SELECT * FROM gateways ORDER BY is_primary DESC, name ASC")
		.all() as GatewayEntry[];
      ...
      `);
			for (const agent of agents.slice(0, 50)) {
				...
				const agentRole =
					typeof agent?.role === "string"
						? agent.role.trim().substring(0, 100)
						: "agent";
		...
		try {
			db.prepare(
				...
				`Added gateway: ${name} (${host}:${port})${agentsRegistered ? `, registered ${agentsRegistered} agent(s)` : ""}`,
			);
		} catch {
			/* audit might not exist */
		}

		const gw = db
			.prepare("SELECT * FROM gateways WHERE id = ?")
			.get(result.lastInsertRowid) as GatewayEntry;
		return NextResponse.json(
			{ gateway: redactToken(gw), agents_registered: agentsRegistered },
			{ status: 201 },
		);
	} catch (err: any) {
		if (err.message?.includes("UNIQUE")) {
			return NextResponse.json(
				{ error: "A gateway with that name already exists" },
				{ status: 409 },
			);
		}
		return NextResponse.json(
			{ error: err.message || "Failed to add gateway" },
			{ status: 500 },
		);
	}
}
...
export async function PUT(request: NextRequest) {
	...
	const existing = db.prepare("SELECT * FROM gateways WHERE id = ?").get(id) as
		| GatewayEntry
		| undefined;
	if (!existing)
	...
	if (sets.length === 0 && !Array.isArray(updates.agents))
		return NextResponse.json(
	...
	if (sets.length > 0) {
		...
		values.push(id);
		db.prepare(`UPDATE gateways SET ${sets.join(", ")} WHERE id = ?`).run(
			...values,
		);
	}
	...
	const updated = db
		.prepare("SELECT * FROM gateways WHERE id = ?")
		.get(id) as GatewayEntry;
	return NextResponse.json({
		gateway: redactToken(updated),
...
export async function DELETE(request: NextRequest) {
	...
	const gw = db.prepare("SELECT * FROM gateways WHERE id = ?").get(id) as
		| GatewayEntry
		| undefined;
	if (gw?.is_primary) {
		return NextResponse.json(
			{ error: "Cannot delete the primary gateway" },
			{ status: 400 },
		);
	}
