...
async function getMemorySnapshot() {
	...
	if (process.platform === "darwin") {
		try {
			const { stdout } = await runCommand("sysctl", ["-n", "vm.swapusage"], {
				timeoutMs: 3000,
			});
	...
	try {
		...
		if (code === 0 && stdout.trim()) {
			const gpus = stdout
				...
				.split("\n")
				.map((line) => {
					const [name, totalStr, usedStr] = line
						.split(",")
						.map((s) => s.trim());
				...
				const gpus = displays
					.map((gpu: any) => {
						...
						const vramStr: string =
							gpu.spdisplays_vram || gpu.spdisplays_vram_shared || "";
						let memoryTotalMB = 0;
...
async function getNetworkSnapshot(): Promise<
	Array<{
		interface: string;
		rxBytes: number;
		txBytes: number;
...
async function getProcessSnapshot(): Promise<
	Array<{
		pid: number;
		name: string;
		cpuPercent: number;
	...
	function parsePsOutput(stdout: string) {
		...
		for (const line of lines) {
			...
			results.push({
				...
				cpuPercent: Number.isFinite(rawCpu)
					? Math.round((rawCpu / coreCount) * 10) / 10
					: 0,
