...
export async function GET(
    ...
    `)
		...
		const project = {
			...row,
			assigned_agents: row.assigned_agents_csv
				? String(row.assigned_agents_csv).split(",")
				: [],
...
export async function PATCH(
    ...
    `)
		...
		if (
			typeof body?.ticket_prefix === "string" ||
			typeof body?.ticketPrefix === "string"
		) {
      ...
      `)
			...
			if (conflict)
				return NextResponse.json(
					{ error: "Ticket prefix already in use" },
					{ status: 409 },
				);
		...
		if (body?.github_default_branch !== undefined) {
			...
			paramsList.push(
				typeof body.github_default_branch === "string"
...
export async function DELETE(
    ...
    `)
		...
		if (mode !== "delete") {
			db.prepare(
				`UPDATE projects SET status = 'archived', updated_at = unixepoch() WHERE id = ? AND workspace_id = ?`,
    ...
    `)
      ...
      `).run(fallback.id, workspaceId, projectId);
			...
			db.prepare(`DELETE FROM projects WHERE id = ? AND workspace_id = ?`).run(
				projectId,
				workspaceId,
			);
		});
