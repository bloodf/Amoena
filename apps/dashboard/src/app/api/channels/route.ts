...
function transformGatewayChannels(data: GatewayData): ChannelsSnapshot {
	...
	const labels: Record<string, string> = Object.fromEntries(
		Object.entries(channelLabels ?? {}).flatMap(([key, value]) =>
	...
	for (const key of order) {
		...
		const accountEntries = (
			Array.isArray(accounts) ? accounts : Object.values(accounts)
		) as GatewayData[];
...
export async function GET(request: NextRequest) {
	...
	if (action === "probe") {
		...
		try {
			...
			if (!res.ok) {
				if (res.status === 404) {
					return NextResponse.json(
						await loadChannelsViaRpc(true).catch(() =>
							loadChannelsViaCli(true),
						),
					);
				}
				throw new Error(
					`Gateway channel probe failed with status ${res.status}`,
				);
			}
			...
			try {
				return NextResponse.json(
					await loadChannelsViaRpc(true).catch(() => loadChannelsViaCli(true)),
				);
			} catch (cliErr) {
	...
	try {
		...
		if (!res.ok) {
			if (res.status === 404) {
				return NextResponse.json(
					await loadChannelsViaRpc(false).catch(() =>
						loadChannelsViaCli(false),
					),
				);
			}
			throw new Error(
				`Gateway channel status failed with status ${res.status}`,
			);
		}
		...
		try {
			return NextResponse.json(
				await loadChannelsViaRpc(false).catch(() => loadChannelsViaCli(false)),
			);
		} catch (cliErr) {
		...
		switch (action) {
			case "whatsapp-link": {
				...
				try {
					...
					const res = await fetch(
						`${gatewayInternalUrl}/api/channels/whatsapp/link`,
						{
							method: "POST",
							headers: gatewayHeaders(),
			...
			case "whatsapp-wait": {
				try {
					...
					const res = await fetch(
						`${gatewayInternalUrl}/api/channels/whatsapp/wait`,
						{
							method: "POST",
							headers: gatewayHeaders(),
							signal: controller.signal,
			...
			case "whatsapp-logout": {
				try {
					...
					const res = await fetch(
						`${gatewayInternalUrl}/api/channels/whatsapp/logout`,
						{
							method: "POST",
							headers: gatewayHeaders(),
							signal: controller.signal,
