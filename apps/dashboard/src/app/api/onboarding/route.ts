...
function readUserOnboardingSetting(
	key: OnboardingSettingKey,
	username: string,
): string {
...
function writeUserOnboardingSetting(
	key: OnboardingSettingKey,
	value: string,
	actor: string,
) {
...
export async function GET(request: NextRequest) {
	...
	try {
		const completed =
			readUserOnboardingSetting(
				ONBOARDING_SETTING_KEYS.completed,
				auth.user.username,
			) === "true";
		const skipped =
			readUserOnboardingSetting(
				ONBOARDING_SETTING_KEYS.skipped,
				auth.user.username,
			) === "true";
		const checklistDismissed =
			readUserOnboardingSetting(
				ONBOARDING_SETTING_KEYS.checklistDismissed,
				auth.user.username,
			) === "true";
		const completedStepsRaw = readUserOnboardingSetting(
			ONBOARDING_SETTING_KEYS.completedSteps,
			auth.user.username,
		);
		...
		const currentStep = nextIncompleteStepIndex(
			ONBOARDING_STEPS,
			completedSteps,
		);
	...
	try {
		...
		switch (action) {
			...
			case "complete": {
				...
				writeUserOnboardingSetting(
					ONBOARDING_SETTING_KEYS.completedAt,
					String(Date.now()),
					auth.user.username,
				);
