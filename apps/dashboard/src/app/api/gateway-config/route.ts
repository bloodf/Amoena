...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (err.code === "ENOENT") {
			return NextResponse.json(
				{ error: "Config file not found", path: configPath },
				{ status: 404 },
			);
		}
		return NextResponse.json(
...
async function getSchema(): Promise<NextResponse> {
	...
	try {
		...
		return NextResponse.json(
			{
				error:
					err.name === "AbortError" ? "Gateway timeout" : "Gateway unreachable",
			},
			{ status: 502 },
		);
	}
		...
		if (clientHash) {
			...
			if (clientHash !== serverHash) {
				return NextResponse.json(
					{
						error:
							"Config has been modified by another user. Please reload and try again.",
						code: "CONFLICT",
					},
					{ status: 409 },
				);
...
function redactSensitive(obj: any, _parentKey = ""): any {
