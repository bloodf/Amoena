...
import { createWorkflowSchema, validateBody } from "@/lib/validation";
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const injectionReport = scanForInjection(task_prompt, {
			context: "prompt",
		});
		if (!injectionReport.safe) {
			...
			if (criticals.length > 0) {
				logger.warn(
					{ name, rules: criticals.map((m) => m.rule) },
					"Blocked workflow: injection detected in task_prompt",
				);
				return NextResponse.json(
					{
						error: "Task prompt blocked: potentially unsafe content detected",
...
export async function PUT(request: NextRequest) {
	...
	try {
		...
		db.prepare(
			`UPDATE workflow_templates SET ${fields.join(", ")} WHERE id = ? AND workspace_id = ?`,
		).run(...params);
