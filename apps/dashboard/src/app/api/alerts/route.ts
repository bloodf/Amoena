...
export async function PUT(request: NextRequest) {
	...
	for (const key of allowed) {
		if (key in updates) {
			sets.push(`${key} = ?`);
			values.push(
				key === "action_config" ? JSON.stringify(updates[key]) : updates[key],
			);
		}
	}
	...
	db.prepare(
		`UPDATE alert_rules SET ${sets.join(", ")} WHERE id = ? AND workspace_id = ?`,
	).run(...values);
...
export async function DELETE(request: NextRequest) {
	...
	const result = db
		.prepare("DELETE FROM alert_rules WHERE id = ? AND workspace_id = ?")
		.run(id, workspaceId);
...
function evaluateRules(
	...
	for (const rule of rules) {
		...
		if (
			...
			now - rule.last_triggered_at < rule.cooldown_minutes * 60
		) {
			results.push({
				rule_id: rule.id,
				rule_name: rule.name,
        ...
        `).run(
					recipient,
					`Alert: ${rule.name}`,
					rule.description || `Rule "${rule.name}" triggered`,
					rule.id,
					workspaceId,
				);
			} catch {
...
function evaluateRule(
	...
	try {
		switch (rule.entity_type) {
			case "agent":
				return evaluateAgentRule(db, rule, now, workspaceId);
			case "task":
				return evaluateTaskRule(db, rule, now, workspaceId);
			case "session":
				return evaluateSessionRule(db, rule, now, workspaceId);
			case "activity":
				return evaluateActivityRule(db, rule, now, workspaceId);
			default:
				return false;
		}
	} catch {
...
function evaluateAgentRule(
	...
	if (
		...
		condition_operator === "count_below"
	) {
		const count =
			(
				db
					...
					.get(workspaceId, condition_value) as any
			)?.c || 0;
		return condition_operator === "count_above"
			? count > parseInt(condition_value, 10)
			: count < parseInt(condition_value, 10);
	}

	if (condition_operator === "age_minutes_above") {
		// Check agents where field value is older than N minutes (e.g., last_seen)
		const threshold = now - parseInt(condition_value, 10) * 60;
		const count =
			(
				db
					.prepare(
					...
					.get(workspaceId, threshold) as any
			)?.c || 0;
		return count > 0;
	}
	...
	return agents.some((a) =>
		compareValue(a.val, condition_operator, condition_value),
	);
}
...
function evaluateTaskRule(
	...
	if (condition_operator === "count_above") {
		...
		return count > parseInt(condition_value, 10);
	}
	...
	const tasks = db
		...
		.all(workspaceId) as any[];
	return tasks.some((t) =>
		compareValue(t.val, condition_operator, condition_value),
	);
}

function evaluateSessionRule(
	db: ReturnType<typeof getDatabase>,
	rule: AlertRule,
	_now: number,
	workspaceId: number,
		...
		const count =
			(
				db
					...
					.get(workspaceId, hourAgo, condition_value) as any
			)?.c || 0;
		return count > parseInt(condition_value, 10);
	}
...
function compareValue(
	...
	switch (operator) {
		...
		case "less_than":
			return Number(actual) < Number(expected);
		case "contains":
			return strActual.toLowerCase().includes(expected.toLowerCase());
		default:
			return false;
	}
}
