...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const fleetTokens = fleet
			...
			.filter((t) => t > 0)
			.sort((a, b) => a - b);
		const agentTokensPerTask =
			efficiency.sessionsCount > 0 ? efficiency.avgTokensPerSession : 0;
		const percentile =
			fleetTokens.length > 0
				? Math.round(
						(fleetTokens.filter((t) => t >= agentTokensPerTask).length /
							fleetTokens.length) *
							100,
					)
				: 50;
		...
		return NextResponse.json({
			...
			efficiency: {
				...
				fleetAverage:
					fleetTokens.length > 0
						? Math.round(
								fleetTokens.reduce((a, b) => a + b, 0) / fleetTokens.length,
							)
						: 0,
				percentile,
			...
			performance: {
				...
				fleetRanking:
					fleet.findIndex((f) => f.agentName === agent) + 1 || fleet.length + 1,
			},
