			...
			for (const l of layers) {
				...
				switch (l) {
					...
					case "drift": {
						const driftResults = runDriftCheck(agent, workspaceId);
						const driftScore =
							driftResults.filter((d) => !d.drifted).length /
							Math.max(driftResults.length, 1);
						evalResults = [
							{
								layer: "drift",
								...
								detail: driftResults
									.map(
										(d) =>
											`${d.metric}: ${d.drifted ? "DRIFTED" : "stable"} (delta=${d.delta})`,
									)
									.join("; "),
							},
						];
