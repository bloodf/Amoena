...
function parseHoursParam(raw: string | null): {
	value?: number;
	error?: string;
} {
...
function parseSectionsParam(raw: string | null): {
	value?: Set<DiagnosticsSection>;
	error?: string;
} {
	...
	const invalid = requested.filter(
		(section) => !ALLOWED_SECTIONS.includes(section as DiagnosticsSection),
	);
	if (invalid.length > 0) {
...
export async function GET(
	...
	try {
		...
		if (/^\d+$/.test(agentId)) {
			agent = db
				.prepare(
					"SELECT id, name, role, status, last_seen, created_at FROM agents WHERE id = ? AND workspace_id = ?",
				)
				.get(Number(agentId), workspaceId);
		} else {
			agent = db
				.prepare(
					"SELECT id, name, role, status, last_seen, created_at FROM agents WHERE name = ? AND workspace_id = ?",
				)
		...
		const isSelfRequest =
			(requesterAgentName || auth.user.username) === agent.name;
		...
		if (!isSelfRequest && !(privileged && auth.user.role === "admin")) {
			return NextResponse.json(
				{
					error:
						"Diagnostics are self-scoped. Use privileged=1 with admin role for cross-agent access.",
				},
				{ status: 403 },
			);
		}
		...
		if (sections.has("activity")) {
			result.activity = buildActivityBreakdown(
				db,
				agent.name,
				workspaceId,
				since,
...
function buildSummary(
	...
	const errorRate =
		activityCount > 0
			? Math.round((errorCount / activityCount) * 10000) / 100
			: 0;
...
function buildTaskMetrics(
	...
	const throughputPerDay =
		Math.round((completedInWindow / windowDays) * 100) / 100;
	...
	const periodMetrics = (since: number, until: number) => {
		const activities = (
			db
				...
				.get(agentName, workspaceId, since, until) as any
		).c;
...
function buildTrendAlerts(
	current: { activities: number; errors: number; tasks_completed: number },
	previous: { activities: number; errors: number; tasks_completed: number },
) {
	...
	if (current.errors > 0 && previous.errors > 0) {
		...
		if (errorIncrease > 0.5) {
			alerts.push({
				level: "warning",
				message: `Error count increased ${Math.round(errorIncrease * 100)}% vs previous period`,
			});
		}
	} else if (current.errors > 3 && previous.errors === 0) {
		alerts.push({
			level: "warning",
			message: `New error pattern: ${current.errors} errors (none in previous period)`,
		});
	}
	...
	if (previous.tasks_completed > 0 && current.tasks_completed === 0) {
		alerts.push({
			level: "info",
			message: "No tasks completed in current period (possible stall)",
		});
	} else if (
		previous.tasks_completed > 2 &&
		current.tasks_completed < previous.tasks_completed * 0.5
	) {
		alerts.push({
			level: "info",
			message: `Task throughput dropped ${Math.round((1 - current.tasks_completed / previous.tasks_completed) * 100)}%`,
		});
	}
	...
	if (
		...
		current.activities < previous.activities * 0.25
	) {
		alerts.push({
			level: "warning",
			message: `Activity dropped ${Math.round((1 - current.activities / previous.activities) * 100)}% — agent may be stalled`,
		});
	}
