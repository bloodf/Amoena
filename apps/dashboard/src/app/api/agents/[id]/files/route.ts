...
export async function PUT(
	...
	try {
		...
		if (content.length > MAX_WORKSPACE_FILE_SIZE) {
			return NextResponse.json(
				{
					error: `File content too large (max ${MAX_WORKSPACE_FILE_SIZE} bytes)`,
				},
				{ status: 413 },
			);
		}
		...
		if (file === "soul.md") {
			db.prepare(
				"UPDATE agents SET soul_content = ?, updated_at = unixepoch() WHERE id = ? AND workspace_id = ?",
			).run(content, agent.id, workspaceId);
		}
		if (file === "WORKING.md") {
			db.prepare(
				"UPDATE agents SET working_memory = ?, updated_at = unixepoch() WHERE id = ? AND workspace_id = ?",
			).run(content, agent.id, workspaceId);
		}
		...
		return NextResponse.json(
			{ error: "Failed to save workspace file" },
			{ status: 500 },
		);
	}
}
