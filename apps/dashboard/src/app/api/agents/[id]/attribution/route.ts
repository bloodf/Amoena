	...
	try {
		...
		if (!isSelf && !isPrivileged) {
			return NextResponse.json(
				{
					error:
						"Forbidden: attribution is self-scope by default. Admin can use ?privileged=1 override.",
				},
				{ status: 403 },
			);
		}
		...
		if (!hours) {
			return NextResponse.json(
				{ error: "Invalid hours. Expected integer 1..720." },
				{ status: 400 },
			);
		}
...
function parseSections(
	...
	if (parsed.length === 0) {
		return {
			error:
				"Invalid section. Expected one or more of identity,audit,mutations,cost.",
		};
	}
