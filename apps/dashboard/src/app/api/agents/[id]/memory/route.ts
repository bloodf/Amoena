...
export async function GET(
	...
	try {
		...
		let agent: any;
		if (Number.isNaN(Number(agentId))) {
			agent = db
		...
		const hasWorkingMemory = columns.some(
			(col: any) => col.name === "working_memory",
		);
		...
		try {
			...
			const match = readAgentWorkspaceFile(candidates, [
				"WORKING.md",
				"working.md",
				"MEMORY.md",
				"memory.md",
			...
			logger.warn(
				...
				"Failed to read WORKING.md from workspace",
			);
		}

		// Get working memory content
		const memoryStmt = db.prepare(
			`SELECT working_memory FROM agents WHERE ${Number.isNaN(Number(agentId)) ? "name" : "id"} = ? AND workspace_id = ?`,
		);
...
export async function PUT(
	...
	try {
		...
		try {
			...
			logger.warn(
				{ err, agent: agent.name },
				"Failed to write WORKING.md to workspace",
			);
		}
      ...
      WHERE ${Number.isNaN(Number(agentId)) ? "name" : "id"} = ? AND workspace_id = ?
    `);
...
export async function DELETE(
	...
	try {
		...
		try {
			...
			logger.warn(
				{ err, agent: agent.name },
				"Failed to clear WORKING.md in workspace",
			);
		}
