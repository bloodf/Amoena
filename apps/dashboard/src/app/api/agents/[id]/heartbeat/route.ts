...
export async function GET(
    ...
    `)
		...
		const notifications = db_helpers.getUnreadNotifications(
			agent.name,
			workspaceId,
		);
...
export async function POST(
	...
	if (
		token_usage?.model &&
		token_usage.inputTokens != null &&
		token_usage.outputTokens != null
	) {
		const resolvedParams = await params;
		const agentId = resolvedParams.id;
		let agent: any;
		...
		if (agent) {
			...
			const parsedTaskId =
				token_usage.taskId != null &&
				Number.isFinite(Number(token_usage.taskId))
			...
			if (parsedTaskId && parsedTaskId > 0) {
				...
				if (taskRow?.id) {
					...
					logger.warn(
						{ taskId: parsedTaskId, workspaceId, agent: agent.name },
						"Ignoring token usage with unknown taskId",
					);
				}
			}
