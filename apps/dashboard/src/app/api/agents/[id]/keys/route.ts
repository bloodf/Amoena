...
export async function GET(
	...
	try {
      ...
      `)
		...
		return NextResponse.json(
			{ error: "Failed to list agent API keys" },
			{ status: 500 },
		);
	}
}
...
export async function POST(
	...
	try {
      ...
      `)
		...
		return NextResponse.json(
			{ error: "Failed to create agent API key" },
			{ status: 500 },
		);
	}
}
...
export async function DELETE(
	...
	try {
		...
		if (!Number.isInteger(keyId) || keyId <= 0) {
			return NextResponse.json(
				{ error: "key_id must be a positive integer" },
				{ status: 400 },
			);
		}
      ...
      `)
		...
		if (result.changes < 1) {
			return NextResponse.json(
				{ error: "Active key not found for this agent" },
				{ status: 404 },
			);
		}
		...
		return NextResponse.json(
			{ error: "Failed to revoke agent API key" },
			{ status: 500 },
		);
	}
}
