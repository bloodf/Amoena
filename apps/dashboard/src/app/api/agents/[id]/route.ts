	...
	try {
		...
		const parsed = {
			...
			config: enrichAgentConfigFromWorkspace(
				(agent as any).config ? JSON.parse((agent as any).config) : {},
			),
		};
...
export async function PUT(
	...
	try {
		...
		if (shouldWriteToGateway) {
			try {
				...
				try {
					...
					db.prepare(
						`UPDATE agents SET ${revertFields.join(", ")} WHERE id = ? AND workspace_id = ?`,
					).run(...revertValues);
				} catch (revertErr: any) {
					logger.error(
						{ err: revertErr, agent: agent.name },
						"Failed to revert DB after gateway write failure",
					);
				}
				return NextResponse.json(
					{
		...
		db_helpers.logActivity(
			...
			{
				fields: Object.keys(gateway_config || {}),
				write_to_gateway: shouldWriteToGateway,
			},
			workspaceId,
		);
...
export async function DELETE(
	...
	try {
		...
		db.prepare("DELETE FROM agents WHERE id = ? AND workspace_id = ?").run(
			agent.id,
			workspaceId,
		);
