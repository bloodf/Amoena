	...
	try {
		...
		try {
			...
			logger.warn(
				...
				"Failed to read soul.md from workspace",
			);
		}
		...
		return NextResponse.json(
			{ error: "Failed to fetch SOUL content" },
			{ status: 500 },
		);
	}
}
...
export async function PUT(
	...
	try {
		...
		if (template_name) {
			...
			try {
				templatePath = resolveWithin(
					config.soulTemplatesDir,
					`${template_name}.md`,
				);
			} catch (_pathError) {
				return NextResponse.json(
		...
		try {
			...
			logger.warn(
				{ err, agent: agent.name },
				"Failed to write soul.md to workspace, saving to DB only",
			);
		}
    ...
    `);
		...
		db_helpers.logActivity(
			...
			`SOUL content updated for agent ${agent.name}${template_name ? ` using template: ${template_name}` : ""}${savedToWorkspace ? " (synced to workspace)" : ""}`,
			{
				...
				previous_content_length: agent.soul_content
					? agent.soul_content.length
					: 0,
		...
		return NextResponse.json(
			{ error: "Failed to update SOUL content" },
			{ status: 500 },
		);
	}
}
