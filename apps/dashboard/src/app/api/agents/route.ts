...
import { createAgentSchema, validateBody } from "@/lib/validation";
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const agentNames = agentsWithParsedData
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (template) {
			...
			if (tpl) {
				const builtConfig = buildAgentConfig(
					tpl,
					(gateway_config || {}) as any,
				);
		...
		if (write_to_gateway && finalConfig) {
			try {
				await writeAgentToConfig({
					...
					...(finalConfig.memorySearch && {
						memorySearch: finalConfig.memorySearch,
					}),
				});
