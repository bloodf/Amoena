...
export async function POST(request: NextRequest) {
	...
	const capabilities = Array.isArray(body?.capabilities)
		? body.capabilities.filter((c: any) => typeof c === "string")
		: [];
	const framework =
		typeof body?.framework === "string" ? body.framework.trim() : null;

	if (!name || !NAME_RE.test(name)) {
		return NextResponse.json(
			{
				error:
					"Invalid agent name. Use 1-63 alphanumeric characters, dots, hyphens, or underscores. Must start with alphanumeric.",
			},
			{ status: 400 },
		);
	}
