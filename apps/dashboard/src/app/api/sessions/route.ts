		...
		switch (action) {
			...
			case "set-label": {
				...
				if (typeof label !== "string" || label.length > 100) {
					return NextResponse.json(
						{ error: "Label must be a string up to 100 characters" },
						{ status: 400 },
					);
				}
			...
			default:
				return NextResponse.json(
					{
						error:
							"Invalid action. Must be: set-thinking, set-verbose, set-reasoning, set-label",
					},
					{ status: 400 },
				);
		}
...
export async function DELETE(request: NextRequest) {
	...
	try {
		...
		return NextResponse.json(
			{ error: error.message || "Session deletion failed" },
			{ status: 500 },
		);
	}
}
...
function getLocalClaudeSessions() {
	try {
		...
		return rows.map((s) => {
			const _total = (s.input_tokens || 0) + (s.output_tokens || 0);
			...
			const derivedActive =
				lastMsg > 0 && Date.now() - lastMsg < LOCAL_SESSION_ACTIVE_WINDOW_MS;
			...
			return {
				...
				active: isActive,
				startTime: s.first_message_at
					? new Date(s.first_message_at).getTime()
					: 0,
...
function getLocalCodexSessions() {
	try {
		...
		return rows.map((s) => {
			...
			const firstMsg = s.firstMessageAt
				? new Date(s.firstMessageAt).getTime()
				: 0;
