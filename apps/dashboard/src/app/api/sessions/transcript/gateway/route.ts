...
export async function GET(request: NextRequest) {
	...
	try {
		try {
			...
			const liveMessages = parseGatewayHistoryTranscript(
			...
			logger.warn(
				{ err: rpcErr, sessionKey },
				"Gateway chat.history failed, falling back to disk transcript",
			);
		}
		...
		if (!agentName) {
			return NextResponse.json({
				messages: [],
				source: "gateway",
				error: "Could not determine agent from session key",
			});
		}
		...
		if (!existsSync(sessionsFile)) {
			return NextResponse.json({
				messages: [],
				source: "gateway",
				error: "Agent sessions file not found",
			});
		}
		...
		if (!existsSync(jsonlPath)) {
			return NextResponse.json({
				messages: [],
				source: "gateway",
				error: "Session JSONL file not found",
			});
		}
