		...
		for (const line of lines) {
			...
			if (parsed?.type === "user") {
				...
				if (
					...
					rawContent.some((b: any) => b?.type === "tool_result")
				) {
...
function readHermesTranscriptFromDbPath(
	dbPath: string,
	sessionId: string,
	limit: number,
): TranscriptMessage[] {
    ...
    `)
					...
					[
						{
							...
							content: String(row.content || "")
								...
								.slice(0, 8000),
							isError:
								row.content?.includes('"success": false') ||
								row.content?.includes('"error"'),
						},
					],
					timestamp,
				);
		...
		logger.warn(
			{ err: error, dbPath, sessionId },
			"Failed to read Hermes transcript",
		);
		return [];
	} finally {
		try {
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const limit = Math.min(
			parseInt(searchParams.get("limit") || "40", 10),
			200,
		);

		if (
			!sessionId ||
			(kind !== "claude-code" && kind !== "codex-cli" && kind !== "hermes")
		) {
