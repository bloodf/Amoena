...
import { type Comment, db_helpers, getDatabase } from "@/lib/db";
import { logger } from "@/lib/logger";
import { resolveMentionRecipients } from "@/lib/mentions";
import { mutationLimiter } from "@/lib/rate-limit";
import { createCommentSchema, validateBody } from "@/lib/validation";
...
export async function POST(
    ...
    `);
		...
		const insertResult = stmt.run(
			...
			now,
			parent_id || null,
			mentionResolution.tokens.length > 0
				? JSON.stringify(mentionResolution.tokens)
				: null,
			workspaceId,
		);
		...
		const subscribers = new Set(
			db_helpers.getTaskSubscribers(taskId, workspaceId),
		);
		...
		return NextResponse.json(
			{
				comment: {
					...
					mentions: createdComment.mentions
						? JSON.parse(createdComment.mentions)
						: [],
