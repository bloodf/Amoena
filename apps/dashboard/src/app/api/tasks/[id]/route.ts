...
import { db_helpers, getDatabase, type Task } from "@/lib/db";
...
import { updateTaskSchema, validateBody } from "@/lib/validation";
...
function mapTaskRow(
	task: any,
): Task & { tags: string[]; metadata: Record<string, unknown> } {
	return {
		...task,
...
export async function PUT(
	...
	try {
		...
		const descriptionMentionResolution =
			description !== undefined
				? resolveMentionRecipients(description || "", db, workspaceId)
				: null;
		if (
			descriptionMentionResolution &&
			descriptionMentionResolution.unresolved.length > 0
		) {
			return NextResponse.json(
				{
		...
		const previousDescriptionMentionRecipients = resolveMentionRecipients(
      ...
      `)
        ...
        `).run(project_id, workspaceId);
          ...
          WHERE id = ? AND workspace_id = ?
        `)
					.get(project_id, workspaceId) as
					| { ticket_counter: number }
					| undefined;
				if (!row || !row.ticket_counter) {
					return NextResponse.json(
						{ error: "Failed to allocate project ticket number" },
						{ status: 500 },
					);
				}
    ...
    `);
		...
		if (assigned_to !== undefined && assigned_to !== currentTask.assigned_to) {
			changes.push(
				`assigned: ${currentTask.assigned_to || "unassigned"} → ${assigned_to || "unassigned"}`,
			);
		...
		if (project_id !== undefined && project_id !== currentTask.project_id) {
			changes.push(
				`project: ${currentTask.project_id || "none"} → ${project_id}`,
			);
		}
		if (outcome !== undefined && outcome !== currentTask.outcome) {
			changes.push(
				`outcome: ${currentTask.outcome || "unset"} → ${outcome || "unset"}`,
			);
		}

		if (descriptionMentionResolution) {
			const newMentionRecipients = new Set(
				descriptionMentionResolution.recipients,
			);
    ...
    `)
		...
		const syncRelevantChanges = changes.some(
			(c) =>
				c.startsWith("status:") ||
				c.startsWith("priority:") ||
				c.includes("title") ||
				c.includes("assigned"),
		);
      ...
      `)
		...
		if (config.gnap.enabled && config.gnap.autoSync && changes.length > 0) {
			try {
				pushTaskToGnap(updatedTask as any, config.gnap.repoPath);
			} catch (err) {
				logger.warn({ err, taskId }, "GNAP sync failed for task update");
			}
		}
...
export async function DELETE(
	...
	try {
		...
		if (config.gnap.enabled && config.gnap.autoSync) {
			try {
				removeTaskFromGnap(taskId, config.gnap.repoPath);
			} catch (err) {
				logger.warn({ err, taskId }, "GNAP sync failed for task deletion");
			}
		}
