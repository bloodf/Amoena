...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const betaStart = parseTimestamp(
			searchParams.get("beta_start") || searchParams.get("cutover"),
		);
		if (!betaStart) {
			return NextResponse.json(
				{
					error:
						"beta_start query parameter is required (unix seconds or ISO timestamp)",
				},
				{ status: 400 },
			);
		}
		if (betaStart > now) {
			return NextResponse.json(
				{ error: "beta_start must not be in the future" },
				{ status: 400 },
			);
		}

		const maxLookbackSeconds = 30 * 24 * 60 * 60;
		const lookbackSecondsRaw = Number(
			searchParams.get("lookback_seconds") || 7 * 24 * 60 * 60,
		);
		const lookbackSeconds = Math.min(
			...
			Math.max(
				60,
				Math.floor(
					Number.isFinite(lookbackSecondsRaw)
						? lookbackSecondsRaw
						: 7 * 24 * 60 * 60,
				),
			),
    ...
    `)
		...
		const baseline = buildWindowStats(
			"baseline",
			baselineStart,
			baselineEnd,
			rows,
		...
		const p95Delta =
			post.latency_seconds.p95 !== null && baseline.latency_seconds.p95 !== null
				? post.latency_seconds.p95 - baseline.latency_seconds.p95
				: null;
		const interventionRateDelta =
			post.interventions.rate - baseline.interventions.rate;

		return NextResponse.json({
			metric_definitions: {
				p95_task_latency_seconds:
					"95th percentile of (completed_at - created_at) for done tasks in the window",
				intervention_rate:
					"intervened_task_count / sample_size where intervened = retry_count>0 OR outcome in {failed,partial,abandoned} OR error_message not empty",
			},
			params: {
		...
		return NextResponse.json(
			{ error: "Failed to compute regression metrics" },
			{ status: 500 },
		);
	}
}
