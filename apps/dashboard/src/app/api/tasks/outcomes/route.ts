...
export async function GET(request: NextRequest) {
    ...
    `)
		...
		const byAgent: Record<
			string,
			ReturnType<typeof outcomeBuckets> & {
				total: number;
				success_rate: number;
			}
		> = {};
		...
		for (const row of rows) {
			...
			const retryCount = Number.isFinite(row.retry_count)
			...
			if (outcome in byAgent[assignedTo]) {
				byAgent[assignedTo][
					outcome as keyof ReturnType<typeof outcomeBuckets>
				] += 1;
			} else {
		...
		summary.avg_retry_count =
			rows.length > 0 ? totalRetryCount / rows.length : 0;
		summary.avg_time_to_resolution_seconds =
			resolutionCount > 0 ? totalResolutionSeconds / resolutionCount : 0;
		summary.success_rate =
			summary.with_outcome > 0
				? summary.by_outcome.success / summary.with_outcome
				: 0;
		...
		for (const priority of Object.values(byPriority)) {
			...
			priority.success_rate =
				withOutcome > 0 ? priority.success / withOutcome : 0;
		}
		...
		return NextResponse.json(
			{ error: "Failed to fetch task outcomes" },
			{ status: 500 },
		);
	}
}
