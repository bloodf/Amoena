...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const resolvedCompletedAt =
			completed_at ?? (normalizedStatus === "done" ? now : null);
      ...
      `).run(resolvedProjectId, workspaceId);
        ...
        WHERE id = ? AND workspace_id = ?
      `)
				.get(resolvedProjectId, workspaceId) as
      ...
      `)
			...
			if (project?.github_sync_enabled && project?.github_repo) {
				pushTaskToGitHub(parsedTask as any, project).catch((err) =>
					logger.error(
						{ err, taskId },
						"Outbound GitHub sync failed for new task",
					),
				);
			}
		}
		...
		if (config.gnap.enabled && config.gnap.autoSync) {
			try {
				pushTaskToGnap(parsedTask as any, config.gnap.repoPath);
			} catch (err) {
				logger.warn({ err, taskId }, "GNAP sync failed for new task");
			}
		}
		...
		const transaction = db.transaction((tasksToUpdate: any[]) => {
			for (const task of tasksToUpdate) {
				const oldTask = db
					...
					.get(task.id, workspaceId) as Task;
				if (!oldTask) continue;

				if (
					task.status === "done" &&
					!hasAegisApproval(db, task.id, workspaceId)
				) {
