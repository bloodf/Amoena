...
import { getDatabase, type Notification } from "@/lib/db";
import { logger } from "@/lib/logger";
import { mutationLimiter } from "@/lib/rate-limit";
import { notificationActionSchema, validateBody } from "@/lib/validation";
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		// Build dynamic query
		let query =
			"SELECT * FROM notifications WHERE recipient = ? AND workspace_id = ?";
		...
		const taskDetailStmt = db.prepare(
    ...
    `);
		const agentDetailStmt = db.prepare(
			...
			try {
				if (notification.source_type && notification.source_id) {
					switch (notification.source_type) {
						...
						case "comment": {
							const comment = commentDetailStmt.get(
								notification.source_id,
								workspaceId,
								workspaceId,
							) as any;
						...
						case "agent": {
							const agent = agentDetailStmt.get(
								notification.source_id,
								workspaceId,
							) as any;
							if (agent) {
				...
				logger.warn(
					...
					"Failed to fetch source details for notification",
				);
			}
    ...
    `)
		...
		let countQuery =
			"SELECT COUNT(*) as total FROM notifications WHERE recipient = ? AND workspace_id = ?";
	...
	try {
      ...
      `);
			...
			return NextResponse.json(
				{
					error:
						"Either provide ids array or recipient with olderThan timestamp",
				},
				{ status: 400 },
			);
		}
...
export async function POST(request: NextRequest) {
	...
	try {
      ...
      `);
			...
			const deliveredNotifications = db
				.prepare(`
