...
export async function POST(request: NextRequest) {
    ...
    `;
		...
		const undeliveredNotifications = db
			.prepare(query)
			.all(...params) as (Notification & { session_key?: string })[];
		...
		const markDeliveredStmt = db.prepare(
			"UPDATE notifications SET delivered_at = ? WHERE id = ? AND workspace_id = ?",
		);
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		let baseQuery =
			"SELECT COUNT(*) as count FROM notifications WHERE workspace_id = ?";
		const params: any[] = [workspaceId];
		...
		const totalNotifications = db.prepare(baseQuery).get(...params) as {
			count: number;
		};

		const undeliveredCount = db
			.prepare(`${baseQuery} AND delivered_at IS NULL`)
			.get(...params) as { count: number };

		const deliveredCount = db
			.prepare(`${baseQuery} AND delivered_at IS NOT NULL`)
			.get(...params) as { count: number };
			...
			statistics: {
				...
				delivery_rate:
					totalNotifications.count > 0
						? Math.round(
								(deliveredCount.count / totalNotifications.count) * 100,
							)
						: 0,
			},
