	...
	for (const { table, column, days, label, scoped } of getRetentionTargets()) {
		if (days <= 0) {
			preview.push({
				...
				retention_days: 0,
				stale_count: 0,
				note: "Retention disabled (keep forever)",
			});
			continue;
		}
		...
		try {
			...
			const row = db
				.prepare(
					`SELECT COUNT(*) as c FROM ${table} WHERE ${column} < ?${wsClause}`,
				)
				.get(...params) as any;
			preview.push({
				table: label,
...
export async function POST(request: NextRequest) {
	...
	for (const { table, column, days, label, scoped } of getRetentionTargets()) {
		...
		try {
			if (dryRun) {
				...
				const res = db
					.prepare(`DELETE FROM ${table} WHERE ${column} < ?${wsClause}`)
					.run(...params);
				results.push({
					table: label,
					deleted: res.changes,
	...
	if (ret.gatewaySessions > 0) {
		const sessionPrune = dryRun
			? {
					deleted: countStaleGatewaySessions(ret.gatewaySessions),
					filesTouched: 0,
				}
		...
		results.push({
			...
			cutoff_date: new Date(Date.now() - ret.gatewaySessions * 86400000)
				.toISOString()
				.split("T")[0],
			retention_days: ret.gatewaySessions,
		});
...
function getRetentionTargets() {
	...
	return [
		...
		},
		{
			...
			label: "Audit Log",
			scoped: false,
		}, // instance-global, admin-only
		{
			table: "notifications",
			column: "created_at",
			days: ret.notifications,
			label: "Notifications",
			scoped: true,
		},
		{
