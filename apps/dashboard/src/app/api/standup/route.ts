...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const startOfDay = Math.floor(
			new Date(`${targetDate}T00:00:00Z`).getTime() / 1000,
		);
		const endOfDay = Math.floor(
			new Date(`${targetDate}T23:59:59Z`).getTime() / 1000,
		);
    ...
    `);
		...
		const standupData = agents.map((agent) => {
			const completedTasks = completedTasksStmt.all(
				agent.name,
				workspaceId,
				startOfDay,
				endOfDay,
			...
			const activityCount = activityCountStmt.get(
				agent.name,
				workspaceId,
				startOfDay,
				endOfDay,
			) as { count: number };
			const commentsToday = commentCountStmt.get(
				agent.name,
				workspaceId,
				startOfDay,
				endOfDay,
		...
		const totalCompleted = standupData.reduce(
			(sum, agent) => sum + agent.completedToday.length,
			0,
		);
		const totalInProgress = standupData.reduce(
			(sum, agent) => sum + agent.inProgress.length,
			0,
		);
		const totalAssigned = standupData.reduce(
			(sum, agent) => sum + agent.assigned.length,
			0,
		);
		const totalReview = standupData.reduce(
			(sum, agent) => sum + agent.review.length,
			0,
		);
		const totalBlocked = standupData.reduce(
			(sum, agent) => sum + agent.blocked.length,
			0,
		);
		const totalActivity = standupData.reduce(
			(sum, agent) => sum + agent.activity.actionCount,
			0,
		);
    ...
    `)
		...
		const standupHistory = standupRows.map((row, index) => {
			...
			return {
				...
				generatedAt:
					report.generatedAt || new Date(row.created_at * 1000).toISOString(),
		...
		const countRow = db
			.prepare(
				"SELECT COUNT(*) as total FROM standup_reports WHERE workspace_id = ?",
			)
