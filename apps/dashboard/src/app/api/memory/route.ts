...
import { extractWikiLinks, validateSchema } from "@/lib/memory-utils";
...
if (MEMORY_PATH && !existsSync(MEMORY_PATH)) {
	try {
		mkdirSync(MEMORY_PATH, { recursive: true });
	} catch {
		/* ignore */
	}
}
...
function isPathAllowed(relativePath: string): boolean {
	...
	return MEMORY_ALLOWED_PREFIXES.some(
		(prefix) =>
...
async function resolveSafeMemoryPath(
	baseDir: string,
	relativePath: string,
): Promise<string> {
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const maxDepth = Number.isFinite(depthParam)
			? Math.max(0, Math.min(depthParam, 8))
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (action === "save") {
			...
			const schemaResult = path.endsWith(".md")
