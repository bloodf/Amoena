...
import { qualityReviewSchema, validateBody } from "@/lib/validation";
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (taskIdsParam) {
			...
			if (ids.length === 0) {
				return NextResponse.json(
					{ error: "taskIds must include at least one numeric id" },
					{ status: 400 },
				);
			}
      ...
      `)
			...
			const byTask: Record<
				number,
				{ status?: string; reviewer?: string; created_at?: number } | null
			> = {};
			for (const id of ids) {
				byTask[id] = null;
			}
    ...
    `)
		...
		return NextResponse.json(
			{ error: "Failed to fetch quality reviews" },
			{ status: 500 },
		);
	}
}
...
export async function POST(request: NextRequest) {
    ...
    `)
		...
		if (status === "approved") {
			db.prepare(
				"UPDATE tasks SET status = ?, updated_at = unixepoch() WHERE id = ? AND workspace_id = ?",
			...
			db.prepare(
				"UPDATE tasks SET status = ?, error_message = ?, updated_at = unixepoch() WHERE id = ? AND workspace_id = ?",
			).run(
				"in_progress",
				`Quality review rejected by ${reviewer}: ${notes}`,
				taskId,
				workspaceId,
			);
			eventBus.broadcast("task.status_changed", {
		...
		return NextResponse.json(
			{ error: "Failed to create quality review" },
			{ status: 500 },
		);
	}
}
