...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const blendedScore = Math.round(
			scan.score * 0.7 + eventPosture.score * 0.3,
		);
		const level =
			blendedScore >= 90
				? "hardened"
    ...
    `)
		...
		const loginFailures = authEventsQuery.filter(
			(e) => e.event_type === "auth.failure",
		).length;
		const tokenRotations = authEventsQuery.filter(
			(e) => e.event_type === "auth.token_rotation",
		).length;
		const accessDenials = authEventsQuery.filter(
			(e) => e.event_type === "auth.access_denied",
		).length;
