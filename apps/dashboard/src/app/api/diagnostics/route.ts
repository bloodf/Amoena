...
function getSecurityInfo() {
	...
	checks.push({
		...
		detail: !authPass
			...
			: INSECURE_PASSWORDS.has(authPass)
				? "AUTH_PASS is a known insecure password"
				: "AUTH_PASS is not a common default",
	});
	...
	checks.push({
		...
		detail: allowedHosts.trim()
			? "MC_ALLOWED_HOSTS is configured"
	...
	checks.push({
		...
		detail: sameSite
			? `MC_COOKIE_SAMESITE is '${sameSite}'`
			: "MC_COOKIE_SAMESITE is not set",
	});
...
function getSessionInfo() {
	try {
		const db = getDatabase();
		const totalRow = db
			.prepare("SELECT COUNT(*) as c FROM claude_sessions")
