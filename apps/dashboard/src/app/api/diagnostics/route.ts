...
import { getReplayStorageInfo, DEFAULT_RETENTION_MS } from '@/lib/cleanup';
  ...
  try {
    const [version, security, database, agents, sessions, gateway, replayStorage] =
      await Promise.all([
        ...
        getSessionInfo(),
        getGatewayInfo(),
        getReplayStorageInfo(getReplayRetentionMs()),
      ]);
...
function getSecurityInfo() {
  ...
  let apiKeyDetail: string;
  if (!apiKey) {
    apiKeyDetail = 'API_KEY is not set';
  } else if (apiKey === 'generate-a-random-key') {
    apiKeyDetail = 'API_KEY is default value';
  } else {
    apiKeyDetail = 'API_KEY is set';
  }
  checks.push({
  ...
  let authPassDetail: string;
  if (!authPass) {
    authPassDetail = 'AUTH_PASS is not set';
  } else if (INSECURE_PASSWORDS.has(authPass)) {
    authPassDetail = 'AUTH_PASS is a known insecure password';
  } else {
    authPassDetail = 'AUTH_PASS is not a common default';
  }
  checks.push({
    name: 'Auth password secure',
    pass: Boolean(authPass) && !INSECURE_PASSWORDS.has(authPass),
    detail: authPassDetail,
  });
...
function getDatabaseInfo() {
  try {
    ...
    try {
      ...
      if (row?.name) {
        const latest = db
          .prepare('SELECT version FROM migrations ORDER BY rowid DESC LIMIT 1')
...
function getSessionInfo() {
  try {
    const db = getDatabase();
    const totalRow = db.prepare('SELECT COUNT(*) as c FROM claude_sessions').get() as
      | { c: number }
      | undefined;
    const activeRow = db
...
function getReplayRetentionMs(): number {
  try {
    const db = getDatabase();
    const row = db
      .prepare('SELECT value FROM settings WHERE key = ?')
      .get('sessionReplay.retentionMs') as { value: string } | undefined;
    if (row) {
