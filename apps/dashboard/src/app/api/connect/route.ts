...
import { connectSchema, validateBody } from "@/lib/validation";
...
export async function POST(request: NextRequest) {
	...
	const { tool_name, tool_version, agent_name, agent_role, metadata } =
		validation.data;
	...
	let agent = db
		...
		.get(agent_name, workspaceId) as any;
	if (!agent) {
		const result = db
			.prepare(
       ...
       VALUES (?, ?, 'online', ?, ?, ?)`,
		...
		// Set agent online
		db.prepare(
			"UPDATE agents SET status = ?, updated_at = ? WHERE id = ? AND workspace_id = ?",
...
export async function DELETE(request: NextRequest) {
  ...
  `)
	...
	db.prepare(
		"UPDATE direct_connections SET status = ?, updated_at = ? WHERE connection_id = ?",
	...
	const agent = db
		...
		.get(conn.agent_id, workspaceId) as any;
	db_helpers.logActivity(
