...
export async function POST(request: NextRequest) {
	...
	if (
		shouldFix("allowed_hosts") &&
		(!allowedHosts || allowAny === "1" || allowAny === "true")
	) {
		try {
			if (allowAny) {
			...
			results.push({
				...
				name: "Host allowlist",
				fixed: true,
				detail: `Set MC_ALLOWED_HOSTS=${mergedHosts.join(",")}`,
				fixSafety: FIX_SAFETY.allowed_hosts,
			});
		} catch (e: any) {
			results.push({
				id: "allowed_hosts",
	...
	if (shouldFix("hsts_enabled") && process.env.MC_ENABLE_HSTS !== "1") {
		try {
			...
			results.push({
				...
				name: "HSTS enabled",
				fixed: true,
				detail: "Set MC_ENABLE_HSTS=1",
				fixSafety: FIX_SAFETY.hsts_enabled,
			});
		} catch (e: any) {
			results.push({
				id: "hsts_enabled",
	...
	if (
		...
		try {
			...
			results.push({
				...
				name: "Secure cookies",
				fixed: true,
				detail: "Set MC_COOKIE_SECURE=1",
				fixSafety: FIX_SAFETY.cookie_secure,
			});
		} catch (e: any) {
			results.push({
				id: "cookie_secure",
	...
	if (shouldFix("rate_limiting") && rateLimitDisabled) {
		try {
			...
			results.push({
				...
				fixed: true,
				detail: "Removed MC_DISABLE_RATE_LIMIT",
				fixSafety: FIX_SAFETY.rate_limiting,
			});
		} catch (e: any) {
			results.push({
				id: "rate_limiting",
	...
	if (
		...
		try {
			...
			results.push({
				...
				name: "API key",
				fixed: true,
				detail: "Generated new random API key",
				fixSafety: FIX_SAFETY.api_key_set,
			});
		} catch (e: any) {
			results.push({
				id: "api_key_set",
	...
	const ocFixIds = [
		...
		"dm_isolation",
		"exec_restricted",
		"control_ui_device_auth",
		"control_ui_insecure_auth",
		"fs_workspace_only",
		"log_redaction",
	];
		...
		if (ocConfig) {
			...
			if (shouldFix("gateway_auth")) {
				...
				if (ocConfig.gateway.auth.mode !== "token") {
					...
					if (!ocConfig.gateway.auth.token) {
						ocConfig.gateway.auth.token = crypto
							.randomBytes(32)
							.toString("hex");
					}
					configChanged = true;
					results.push({
						...
						fixed: true,
						detail: 'Set auth.mode to "token" with generated token',
			...
			if (shouldFix("gateway_bind")) {
				...
				if (
					...
					ocConfig.gateway.bind !== "127.0.0.1"
				) {
					ocConfig.gateway.bind = "loopback";
					configChanged = true;
					results.push({
						...
						fixed: true,
						detail: 'Set bind to "loopback"',
						fixSafety: FIX_SAFETY.gateway_bind,
					});
				}
			}
			...
			if (shouldFix("elevated_disabled")) {
				...
				if (ocConfig.elevated.enabled === true) {
					...
					results.push({
						...
						name: "Elevated mode",
						fixed: true,
						detail: "Disabled elevated mode",
						fixSafety: FIX_SAFETY.elevated_disabled,
					});
				}
			}
			...
			if (shouldFix("dm_isolation")) {
				...
				if (ocConfig.session.dmScope !== "per-channel-peer") {
					...
					results.push({
						...
						fixed: true,
						detail: 'Set dmScope to "per-channel-peer"',
						fixSafety: FIX_SAFETY.dm_isolation,
					});
				}
			}
			...
			if (shouldFix("exec_restricted")) {
				...
				if (
					...
					ocConfig.tools.exec.security !== "deny"
				) {
					ocConfig.tools.exec.security = "allowlist";
					configChanged = true;
					results.push({
						id: "exec_restricted",
						name: "Exec tool restriction",
						fixed: true,
						detail: 'Set exec security to "allowlist"',
						fixSafety: FIX_SAFETY.exec_restricted,
					});
				}
			}
			...
			if (shouldFix("control_ui_device_auth")) {
				if (
					ocConfig.gateway?.controlUi?.dangerouslyDisableDeviceAuth === true
				) {
					...
					results.push({
						...
						fixed: true,
						detail: "Disabled dangerouslyDisableDeviceAuth",
						fixSafety: FIX_SAFETY.control_ui_device_auth,
					});
				}
			}
			...
			if (shouldFix("control_ui_insecure_auth")) {
				if (ocConfig.gateway?.controlUi?.allowInsecureAuth === true) {
					...
					results.push({
						...
						fixed: true,
						detail: "Disabled allowInsecureAuth",
						fixSafety: FIX_SAFETY.control_ui_insecure_auth,
					});
				}
			}
			...
			if (shouldFix("fs_workspace_only")) {
				...
				if (ocConfig.tools.fs.workspaceOnly !== true) {
					...
					results.push({
						...
						fixed: true,
						detail: "Set tools.fs.workspaceOnly to true",
						fixSafety: FIX_SAFETY.fs_workspace_only,
					});
				}
			}
			...
			if (shouldFix("log_redaction")) {
				...
				if (!ocConfig.logging.redactSensitive) {
					...
					results.push({
						...
						name: "Log redaction",
						fixed: true,
						detail: 'Set logging.redactSensitive to "tools"',
						fixSafety: FIX_SAFETY.log_redaction,
					});
				}
			}
	...
	if (shouldFix("world_writable"))
		try {
			...
			const wwOutput = execFileSync(
				"find",
				[cwd, "-maxdepth", "2", "-perm", "-o+w", "-not", "-type", "l"],
				{
					encoding: "utf-8",
					timeout: 5000,
			...
			if (wwOutput) {
				...
				if (fixedCount > 0) {
					results.push({
						...
						fixed: true,
						detail: `Fixed permissions on ${fixedCount} file(s)`,
						fixSafety: FIX_SAFETY.world_writable,
					});
				}
			}
		} catch {
			/* no world-writable files or find not available */
		}
	...
	try {
		...
		db.prepare(
			...
			auth.user.username,
			JSON.stringify({
				fixes: results.filter((r) => r.fixed).map((r) => r.id),
			}),
		);
	} catch {
		/* non-critical */
	...
	const remainingAutoFixable = remainingChecks.filter(
		(check) => check.id in FIX_SAFETY,
	).length;
	const remainingManual = remainingChecks.length - remainingAutoFixable;

	logger.info(
		{ fixed, failed, actor: auth.user.username },
		"Security auto-fix completed",
	);
