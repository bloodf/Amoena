...
function isFixableInScope(
	...
	if (
		safety === "requires-restart" &&
		(scope === "safe+restart" || scope === "all")
	)
		return true;
...
export async function POST(request: NextRequest) {
	...
	if (!action || !["scan", "fix", "scan-and-fix"].includes(action)) {
		return NextResponse.json(
			{ error: 'action must be "scan", "fix", or "scan-and-fix"' },
			{ status: 400 },
		);
	}
	...
	try {
		...
		const allChecks = Object.values(scanResult.categories).flatMap(
			(c) => c.checks,
		);
		...
		const scanResponse = {
			...
			failingChecks: failingChecks.map((c) => ({
				...
				fix: c.fix,
				fixSafety:
					FIX_SAFETY[c.id] ?? c.fixSafety ?? ("manual-only" as FixSafety),
			...
			categories: Object.fromEntries(
				Object.entries(scanResult.categories).map(([key, cat]) => [
					...
					{
						score: cat.score,
						failCount: cat.checks.filter((c) => c.status !== "pass").length,
					},
				]),
			),
		};
		...
		if (action === "scan") {
			const criticalCount = failingChecks.filter(
				(c) => c.severity === "critical",
			).length;
			const highCount = failingChecks.filter(
				(c) => c.severity === "high",
			).length;
			return NextResponse.json({
				scan: scanResponse,
		...
		if (dryRun) {
			return NextResponse.json({
				...
				fixes: {
					...
					skipped,
					requiresRestart: checksToFix.some(
						(c) => FIX_SAFETY[c.id] === "requires-restart",
					),
					requiresManual,
				},
		...
		const requiresRestart = applied.some(
			(r: any) => r.fixed && FIX_SAFETY[r.id] === "requires-restart",
		);
