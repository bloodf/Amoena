...
export async function POST(request: NextRequest) {
	...
	const framework =
		typeof body?.framework === "string" ? body.framework.trim() : "";
	const action = typeof body?.action === "string" ? body.action.trim() : "";
	const payload = body?.payload ?? {};

	if (!framework || !action) {
		return NextResponse.json(
			{ error: "framework and action are required" },
			{ status: 400 },
		);
	}
	...
	try {
		switch (action) {
			case "register": {
				...
				if (!agentId || !name) {
					return NextResponse.json(
						{ error: "payload.agentId and payload.name required" },
						{ status: 400 },
					);
				}
			...
			case "heartbeat": {
				...
				if (!agentId) {
					return NextResponse.json(
						{ error: "payload.agentId required" },
						{ status: 400 },
					);
				}
			...
			case "report": {
				...
				if (!taskId || !agentId) {
					return NextResponse.json(
						{ error: "payload.taskId and payload.agentId required" },
						{ status: 400 },
					);
				}
