...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (!res.ok) {
			logger.warn(
				{ status: res.status },
				"Gateway exec-approvals endpoint returned error",
			);
		...
		if (parsed?.agents && typeof parsed.agents === "object") {
			for (const [agentId, agentConfig] of Object.entries(parsed.agents)) {
				...
				if (Array.isArray(cfg?.allowlist)) {
					agents[agentId] = cfg.allowlist.map((e: any) => ({
	...
	try {
		...
		try {
			...
			if (body.hash) {
				...
				if (body.hash !== serverHash) {
					return NextResponse.json(
						{
							error: "Config has been modified. Please reload and try again.",
							code: "CONFLICT",
						},
						{ status: 409 },
					);
		...
		for (const [agentId, patterns] of Object.entries(body.agents)) {
			...
			if (patterns.length === 0) {
				...
				parsed.agents[agentId].allowlist = patterns.map(
					(p: { pattern: string }) => ({
