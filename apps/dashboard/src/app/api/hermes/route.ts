...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const activeSessions = installed
			? scanHermesSessions(50).filter((s) => s.isActive).length
			: 0;
	...
	try {
		...
		if (action === "install-hook") {
			if (!isHermesInstalled()) {
				return NextResponse.json(
					{ error: "Hermes is not installed (~/.hermes/ not found)" },
					{ status: 400 },
				);
			}
		...
		return NextResponse.json(
			{ error: "Invalid action. Must be: install-hook, uninstall-hook" },
			{ status: 400 },
		);
	} catch (err: any) {
		logger.error({ err }, "Hermes hook management failed");
		return NextResponse.json(
			{ error: err.message || "Hook operation failed" },
			{ status: 500 },
		);
	}
}
