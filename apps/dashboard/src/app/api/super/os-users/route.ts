...
function checkToolExists(homeDir: string, tool: string): boolean {
	...
	try {
		execFileSync("/usr/bin/which", [tool], {
			encoding: "utf-8",
			timeout: 2000,
			stdio: "pipe",
		});
	...
	try {
		if (tool === "amoena") {
			...
			try {
				...
				const msg =
					npmErr?.stderr?.toString?.()?.slice(0, 200) ||
					npmErr?.message ||
					"npm install failed";
				logger.warn(
				...
				return {
					success: true,
					error: `dirs created but npm install failed: ${msg}`,
				};
			}
			return { success: true };
		}
		...
		if (tool === "claude") {
			...
			try {
				...
				try {
					execFileSync(
						"/usr/bin/sudo",
						["-n", "install", "-d", "-o", username, claudeDir],
						{ timeout: 5000, stdio: "pipe" },
					);
				} catch {
		...
		if (tool === "codex") {
			...
			try {
				...
				try {
					execFileSync(
						"/usr/bin/sudo",
						["-n", "install", "-d", "-o", username, codexDir],
						{ timeout: 5000, stdio: "pipe" },
					);
				} catch {
...
function discoverOsUsers(): OsUser[] {
	...
	try {
		if (platform === "darwin") {
			...
			const raw = execFileSync("/usr/bin/getent", ["passwd"], {
				encoding: "utf-8",
				timeout: 5000,
			});
			for (const line of raw.split("\n")) {
...
export async function POST(request: NextRequest) {
	...
	if (!/^[a-z][a-z0-9_-]{1,30}[a-z0-9]$/.test(username)) {
		return NextResponse.json(
			{
				error:
					"Invalid username. Use lowercase letters, numbers, dashes, and underscores (3-32 chars).",
			},
			{ status: 400 },
		);
	}
	...
	if (SERVICE_ACCOUNTS.has(username)) {
		return NextResponse.json(
			{ error: "Cannot use a reserved service account name" },
			{ status: 400 },
		);
	}
	...
	const existingTenant = db
		.prepare("SELECT id FROM tenants WHERE linux_user = ? OR slug = ?")
		.get(username, username) as any;
	if (existingTenant) {
		return NextResponse.json(
			{ error: "This user is already registered as an organization" },
			{ status: 409 },
		);
	}
	...
	if (gatewayMode) {
		try {
			...
			return NextResponse.json(
				{ error: e?.message || "Failed to create tenant bootstrap job" },
				{ status: 400 },
			);
		}
	}
	...
	try {
		if (!alreadyExists) {
			if (platform === "darwin") {
				...
				try {
					execFileSync("/usr/sbin/sysadminctl", args, {
						timeout: 15000,
						stdio: "pipe",
					});
				} catch (_e: any) {
					...
					try {
						execFileSync(
							"/usr/bin/sudo",
							["-n", "/usr/sbin/sysadminctl", ...args],
							{ timeout: 15000, stdio: "pipe" },
						);
					} catch (sudoErr: any) {
						const msg =
							sudoErr?.stderr?.toString?.() ||
							sudoErr?.message ||
							"Failed to create OS user";
				...
				try {
					execFileSync("/usr/bin/sudo", ["-n", "/usr/sbin/useradd", ...args], {
						timeout: 15000,
						stdio: "pipe",
					});
				} catch (e: any) {
					const msg =
						e?.stderr?.toString?.() || e?.message || "Failed to create OS user";
				...
				return NextResponse.json(
					{ error: `OS user creation not supported on ${platform}` },
					{ status: 400 },
				);
			}
		}
    ...
    `)
		...
		const installSummary = Object.entries(installResults)
			.map(([tool, r]) =>
				r.success ? `${tool} installed` : `${tool} failed: ${r.error}`,
			)
			.join(". ");
		...
		return NextResponse.json(
			{
				...
				install_results:
					Object.keys(installResults).length > 0 ? installResults : undefined,
		...
		if (String(e?.message || "").includes("UNIQUE")) {
			return NextResponse.json(
				{ error: "Organization slug or user already exists" },
				{ status: 409 },
			);
		}
