...
function getSkillRoots(): SkillRoot[] {
	...
	const roots: SkillRoot[] = [
		{
			source: "user-agents",
			path: resolveSkillRoot(
				"MC_SKILLS_USER_AGENTS_DIR",
...
async function upsertSkill(root: SkillRoot, name: string, content: string) {
    ...
    `).run(
		...
		/* DB not ready yet — sync will catch it */
	}
...
async function deleteSkill(root: SkillRoot, name: string) {
	...
	try {
		const { getDatabase } = await import("@/lib/db");
		const db = getDatabase();
		db.prepare("DELETE FROM skills WHERE source = ? AND name = ?").run(
			root.source,
			name,
		);
	} catch {
...
export async function GET(request: NextRequest) {
	...
	if (mode === "content") {
		...
		if (!source || !name) {
			return NextResponse.json(
				{ error: "source and valid name are required" },
				{ status: 400 },
			);
		}
	...
	if (mode === "check") {
		...
		try {
			...
			const db = getDatabase();
			db.prepare(
				"UPDATE skills SET security_status = ?, updated_at = ? WHERE source = ? AND name = ?",
	...
	if (dbSkills) {
		...
		const groupMap = new Map<
			string,
			{ source: string; path: string; skills: SkillSummary[] }
		>();
		for (const root of roots) {
			groupMap.set(root.source, {
				source: root.source,
				path: root.path,
				skills: [],
			});
		...
		for (const skill of dbSkills) {
			...
			if (
				!groupMap.has(skill.source) &&
				skill.source.startsWith("workspace-")
			) {
				groupMap.set(skill.source, {
					source: skill.source,
					path: "",
					skills: [],
				});
		...
		return NextResponse.json({
			skills: Array.from(deduped.values()).sort((a, b) =>
...
export async function POST(request: NextRequest) {
	...
	const content =
		contentRaw.trim() || `# ${name || "skill"}\n\nDescribe this skill.\n`;

	if (!root || !name) {
		return NextResponse.json(
			{ error: "Valid source and name are required" },
			{ status: 400 },
		);
	}
...
export async function PUT(request: NextRequest) {
	...
	if (!root || !name || content == null) {
		return NextResponse.json(
			{ error: "Valid source, name, and content are required" },
			{ status: 400 },
		);
	}
