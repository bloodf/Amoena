...
export async function GET(request: NextRequest) {
	...
	const requestedLimit = parseInt(searchParams.get("limit") || "10000", 10);
	...
	switch (type) {
		...
		case "tasks": {
			...
			rows = db
				.prepare(
					`SELECT * FROM tasks ${scopedWhere} ORDER BY created_at DESC LIMIT ?`,
				)
				.all(...params, limit);
			headers = [
				"id",
		...
		case "activities": {
			...
			rows = db
				.prepare(
					`SELECT * FROM activities ${scopedWhere} ORDER BY created_at DESC LIMIT ?`,
				)
				.all(...params, limit);
			headers = [
				"id",
		...
		case "pipelines": {
			...
			const scopedWhere =
				conditions.length > 0
					? `WHERE ${conditions.map((c) => c.replace(/^created_at/, "pr.created_at")).join(" AND ")}`
					: "";
			rows = db
				.prepare(
					`SELECT pr.*, wp.name as pipeline_name FROM pipeline_runs pr LEFT JOIN workflow_pipelines wp ON pr.pipeline_id = wp.id ${scopedWhere} ORDER BY pr.created_at DESC LIMIT ?`,
				)
				.all(...params, limit);
			headers = [
				"id",
