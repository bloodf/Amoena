...
export async function GET(request: NextRequest) {
	...
	return NextResponse.json({
		users: users.filter((u) => (u.workspace_id ?? 1) === workspaceId),
	});
}
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const { username, password, display_name, role, provider, email } =
			result.data;
		...
		logAuditEvent({
			...
			detail: { username, role, provider, email },
...
export async function PUT(request: NextRequest) {
	...
	try {
		const { id, display_name, role, password, is_approved, email, avatar_url } =
			await request.json();
		...
		logAuditEvent({
			...
			target_type: "user",
			target_id: userId,
			detail: { display_name, role, password_changed: !!password, is_approved },
			ip_address: ipAddress,
		});
