...
import { accessRequestActionSchema, validateBody } from "@/lib/validation";
  ...
  `);
	...
	const rows =
		status === "all"
			? db
					.prepare(
						"SELECT * FROM access_requests ORDER BY status = 'pending' DESC, last_attempt_at DESC, id DESC",
					)
					.all()
			: db
					.prepare(
...
export async function POST(request: NextRequest) {
	...
	const providerUserId = reqRow.provider_user_id
		? String(reqRow.provider_user_id)
		: null;
	const displayName = String(
		reqRow.display_name || email.split("@")[0] || "Google User",
	);
	...
	const user = db.transaction(() => {
		const existing = db
			...
			.get(email, "google", providerUserId || "") as any;
