...
function normalizeTokenRecord(
	...
	return {
		id: String(
			record.id ?? `${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
		),
		model,
		sessionId: String(record.sessionId),
		agentName: String(
			record.agentName ?? extractAgentName(String(record.sessionId)),
		),
		...
		taskId:
			record.taskId != null && Number.isFinite(Number(record.taskId))
				? Number(record.taskId)
				: null,
		workspaceId:
			record.workspaceId != null && Number.isFinite(Number(record.workspaceId))
				? Number(record.workspaceId)
				: 1,
...
async function loadTokenDataFromFile(
	workspaceId: number,
	providerSubscriptions: Record<string, boolean>,
	...
	try {
		...
		return parsed
			.map((record: Partial<TokenUsageRecord>) =>
			...
			.filter((record) => {
				...
				return (
					workspaceId === 1 && (!record.workspaceId || record.workspaceId === 1)
				);
			});
	} catch {
		return [];
...
function loadTaskMetadataById(
	workspaceId: number,
	taskIds: number[],
): Record<number, TaskCostMetadata> {
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (action === "stats") {
			...
			const modelGroups = filteredData.reduce(
				(acc, record) => {
			...
			const sessionGroups = filteredData.reduce(
				(acc, record) => {
			...
			const agentGroups = filteredData.reduce(
				(acc, record) => {
		...
		if (action === "agent-costs") {
			...
			for (const [agent, records] of Object.entries(agentGroups)) {
				...
				const modelGroups = records.reduce(
					(acc, r) => {
		...
		if (
					...
					filteredData
						...
						.filter(
							(taskId): taskId is number =>
								Number.isFinite(taskId) && Number(taskId) > 0,
						)
						.map((taskId) => Number(taskId)),
				),
			];
			const taskMetadataById = loadTaskMetadataById(
				workspaceId,
				attributedTaskIds,
			);
			...
			return NextResponse.json({
				...
				timeframe,
				recordCount: filteredData.length,
				attributedRecordCount: filteredData.filter((record) =>
		...
		if (action === "trends") {
			...
			const recentData = filteredData.filter(
				(r) => r.timestamp >= twentyFourHoursAgo,
			);

			const hourlyTrends: Record<
				string,
