...
const COORDINATOR_AGENT =
	String(
		process.env.MC_COORDINATOR_AGENT ||
			process.env.NEXT_PUBLIC_COORDINATOR_AGENT ||
			"coordinator",
	).trim() || "coordinator";
...
function toGatewayAttachments(value: unknown):
	| Array<{
			type: "image";
			mimeType: string;
			fileName?: string;
...
function normalizeToolEvent(raw: any): ToolEvent | null {
	...
	const name = String(
		raw.name || raw.tool || raw.toolName || raw.function || raw.call || "",
	).trim();
	if (!name) return null;
	...
	const output =
		typeof outputRaw === "string"
			...
			: outputRaw !== undefined
				...
				: undefined;
	const status =
		statusRaw !== undefined ? String(statusRaw).slice(0, 60) : undefined;
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		let countQuery =
			"SELECT COUNT(*) as total FROM messages WHERE workspace_id = ?";
		...
		const countRow = db.prepare(countQuery).get(...countParams) as {
			total: number;
		};
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const isCoordinatorOverride =
			requestedFrom.toLowerCase() === COORDINATOR_AGENT.toLowerCase();
		...
		if (body.forward && to) {
			...
			if (!injectionReport.safe) {
				const criticals = injectionReport.matches.filter(
					(m) => m.severity === "critical",
				);
				if (criticals.length > 0) {
					logger.warn(
						{ to, rules: criticals.map((m) => m.rule) },
						"Blocked chat message: injection detected",
					);
					return NextResponse.json(
						{
							error: "Message blocked: potentially unsafe content detected",
							injection: criticals.map((m) => ({
								rule: m.rule,
    ...
    `);
				...
				const coordinatorResolution = resolveCoordinatorDeliveryTarget({
					...
					directAgent: agent
						? {
								...
								session_key:
									typeof agent.session_key === "string"
										? agent.session_key
										: null,
				...
				if (!sessionKey && !amoenaAgentId) {
						...
						if (
							...
							if (
								...
								try {
									...
									logger.error(
										{ err: e },
										"Failed to create gateway failure status reply",
									);
								}
							}
						}
					...
					if (
						...
						try {
							...
							logger.error(
								{ err: e },
								"Failed to create accepted status reply",
							);
						}
		...
		return NextResponse.json(
			{ message: parsedMessage, forward: forwardInfo },
			{ status: 201 },
		);
	} catch (error) {
