...
import { githubSyncSchema, validateBody } from "@/lib/validation";
	...
	try {
		...
		if (action !== "issues") {
			return NextResponse.json(
				{ error: "Unknown action. Use ?action=issues or ?action=stats" },
				{ status: 400 },
			);
		}
		...
		if (!repo || !/^[^/]+\/[^/]+$/.test(repo)) {
			return NextResponse.json(
				{ error: "repo query parameter required (owner/repo format)" },
				{ status: 400 },
			);
		}
		...
		const state =
			(searchParams.get("state") as "open" | "closed" | "all") || "open";
...
export async function POST(request: NextRequest) {
	...
	try {
		switch (action) {
			...
			case "sync-project":
				return await handleSyncProject(
					body,
					auth.user.username,
					auth.user.workspace_id ?? 1,
				);
...
async function handleSync(
	...
	const syncTableHasWorkspace = db
		.prepare(
			"SELECT 1 as ok FROM pragma_table_info('github_syncs') WHERE name = 'workspace_id'",
		)
...
async function handleGitHubStats() {
	...
	return NextResponse.json({
		...
		repos: {
			...
			total_stars: activeRepos.reduce(
				(sum: number, r) => sum + (r.stargazers_count || 0),
				0,
			),
			total_forks: activeRepos.reduce(
				(sum: number, r) => sum + (r.forks_count || 0),
				0,
			),
			total_open_issues: activeRepos.reduce(
				(sum: number, r) => sum + (r.open_issues_count || 0),
				0,
			),
		},
		topLanguages,
...
async function handleInitLabels(body: { repo?: string }, workspaceId: number) {
	const repo = body.repo || process.env.GITHUB_DEFAULT_REPO;
	if (!repo) {
		return NextResponse.json({ error: "repo is required" }, { status: 400 });
	}
...
async function handleSyncProject(
  ...
  `)
	...
	if (!project.github_repo || !project.github_sync_enabled) {
		return NextResponse.json(
			{ error: "GitHub sync not enabled for this project" },
			{ status: 400 },
		);
	}
