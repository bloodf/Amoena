...
function mapAmoenaJob(job: AmoenaCronJob): CronJob {
	...
	const payloadSummary = job.payload.message
		? job.payload.message.slice(0, 200) +
			(job.payload.message.length > 200 ? "..." : "")
		...
		if (action === "history") {
			...
			try {
				...
				entries = entries.filter(
					(r: any) => r.jobId === jobId || r.id === jobId,
				);
				...
				entries.sort(
					(a: any, b: any) =>
						(b.timestamp || b.startedAtMs || 0) -
						(a.timestamp || a.startedAtMs || 0),
				);
	...
	try {
		...
		if (action === "toggle") {
			...
			if (!(await saveCronFile(cronFile))) {
				return NextResponse.json(
					{ error: "Failed to save cron file" },
					{ status: 500 },
				);
			}
		...
		if (action === "clone") {
			...
			const existingNames = new Set(
				cronFile.jobs.map((j) => j.name.toLowerCase()),
			);
