...
export async function POST(request: NextRequest) {
	...
	try {
		...
		for (const field of fieldsToScan) {
			const injectionReport = scanForInjection(field.value, {
				context: "prompt",
			});
			if (!injectionReport.safe) {
				...
				if (criticals.length > 0) {
					logger.warn(
						{ field: field.name, rules: criticals.map((m) => m.rule) },
						`Blocked spawn: injection detected in ${field.name}`,
					);
					return NextResponse.json(
						{
							error: `${field.name} blocked: potentially unsafe content detected`,
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		try {
			...
			const recentLogs = logFiles.sort((a, b) => b.mtime - a.mtime).slice(0, 5);
