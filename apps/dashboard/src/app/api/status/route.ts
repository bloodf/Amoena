...
import { getAgentLiveStatuses, getAllGatewaySessions } from "@/lib/sessions";
import { APP_VERSION } from "@/lib/version";

export async function GET(request: NextRequest) {
	// Docker/Kubernetes health probes must work without auth/cookies.
	const preAction =
		new URL(request.url).searchParams.get("action") || "overview";
	if (preAction === "health") {
    ...
    `)
		...
		try {
			...
			pipelineRecent = (
				db
					.prepare(
						"SELECT COUNT(*) as c FROM pipeline_runs WHERE created_at > ?",
					)
					.get(day) as any
			).c;
		} catch {
		...
		// Latest backup
		let latestBackup: { name: string; size: number; age_hours: number } | null =
			null;
		try {
		...
		try {
			webhookCount = (
				db.prepare("SELECT COUNT(*) as c FROM webhooks").get() as any
			).c;
		} catch {
			// table may not exist
		}
...
async function getGatewayStatus() {
	...
	try {
		gatewayStatus.port_listening = await isPortOpen(
			config.gatewayHost,
			config.gatewayPort,
		);
	} catch (error) {
...
async function getAvailableModels() {
	...
	try {
		...
		const ollamaModels = ollamaOutput
			.split("\n")
			.slice(1) // Skip header
			.filter((line) => line.trim())
			.map((line) => {
...
async function performHealthCheck() {
	...
	const hasCritical = health.checks.some(
		(check: any) => check.status === "critical",
	);
	const hasWarning = health.checks.some(
		(check: any) => check.status === "warning",
	);
	const hasDegraded = health.checks.some(
		(check: any) => check.name === "Database" && check.status === "warning",
	);
...
async function getCapabilities(request?: NextRequest) {
	...
	try {
		...
		if (table?.name) {
			const rows = db.prepare("SELECT host, port FROM gateways").all() as {
				host: string;
				port: number;
			}[];
			if (rows.length > 0) {
	...
	try {
		const db = getDatabase();
		const row = db
			.prepare("SELECT COUNT(*) as c FROM claude_sessions WHERE is_active = 1")
	...
	try {
		...
		const codexPlan = settingsDb
			.prepare(
				"SELECT value FROM settings WHERE key = 'subscription.codex_plan'",
			)
			.get() as { value: string } | undefined;
		if (codexPlan?.value) {
			subscriptions.openai = {
				provider: "openai",
				type: codexPlan.value,
				source: "env" as const,
			};
		}
	} catch {
