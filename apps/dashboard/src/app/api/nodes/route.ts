	...
	if (action === "list") {
		try {
			...
			try {
				...
				logger.warn(
					{ err: rpcErr },
					"node.list RPC failed, returning empty node list",
				);
	...
	if (action === "devices") {
		try {
			...
			try {
				...
				logger.warn(
					{ err: rpcErr },
					"device.pair.list RPC failed, returning empty device list",
				);
				return NextResponse.json({ devices: [] });
			}
		} catch (err) {
...
const ACTION_RPC_MAP: Record<
	DeviceAction,
	{ method: string; paramKey: "requestId" | "deviceId" }
> = {
...
export async function POST(request: NextRequest) {
	...
	if (!id || typeof id !== "string") {
		return NextResponse.json(
			{ error: `Missing required field: ${spec.paramKey}` },
			{ status: 400 },
		);
	}
	...
	try {
		...
		return NextResponse.json(
			{ error: "Gateway device action failed" },
			{ status: 502 },
		);
	}
}
