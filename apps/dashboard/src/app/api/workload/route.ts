...
export async function GET(request: NextRequest) {
	...
	try {
		...
		return NextResponse.json(
			{ error: "Failed to fetch workload signals" },
			{ status: 500 },
		);
	}
}
...
function buildThresholds() {
	return {
		...
		recent_window_seconds: Math.max(
			1,
			Math.floor(numEnv("MC_WORKLOAD_RECENT_WINDOW_SECONDS", 300)),
		),
	};
}
...
function buildCapacityMetrics(
	...
	const tasksLast5m = (
		db
			...
			.get(workspaceId, recentWindow) as any
	).c;
	...
	return {
		...
		error_rate_5m: Math.max(
			0,
			Math.min(1, Math.round(safeErrorRate * 10000) / 10000),
		),
		completions_last_hour: completionsLastHour,
		avg_completion_rate_per_hour:
			Math.round((completionsLastDay / 24) * 100) / 100,
	};
}
...
function buildQueueMetrics(db: any, workspaceId: number): QueueMetrics {
	...
	const byStatus = db
		...
		.all(workspaceId, ...pendingStatuses) as Array<{
		status: string;
		count: number;
	}>;
	...
	return {
		...
		estimated_wait_confidence:
			estimatedWait === null ? "unknown" : "calculated",
	};
}
...
function computeRecommendation(
	...
	if (capacity.error_rate_5m >= THRESHOLDS.error_rate_shed) {
		...
		reasons.push(
			`High error rate: ${(capacity.error_rate_5m * 100).toFixed(1)}%`,
		);
	} else if (capacity.error_rate_5m >= THRESHOLDS.error_rate_throttle) {
		level = escalate(level, "throttle");
		reasons.push(
			`Elevated error rate: ${(capacity.error_rate_5m * 100).toFixed(1)}%`,
		);
	}
	...
	if (agents.busy_ratio >= THRESHOLDS.busy_agent_ratio_shed) {
		level = escalate(level, "shed");
		reasons.push(
			`Agent saturation critical: ${(agents.busy_ratio * 100).toFixed(0)}% busy`,
		);
	} else if (agents.busy_ratio >= THRESHOLDS.busy_agent_ratio_throttle) {
		level = escalate(level, "throttle");
		reasons.push(
			`Agent saturation high: ${(agents.busy_ratio * 100).toFixed(0)}% busy`,
		);
	}
	...
	if (agents.online === 0) {
		level = "pause";
		reasons.push(
			agents.total > 0 ? "No agents online" : "No agents registered",
		);
	}
	...
	return {
		...
		details:
			reasons.length > 0 ? reasons : ["All metrics within normal bounds"],
