...
export async function POST(request: Request) {
	...
	if (action !== "call") {
		return NextResponse.json(
			{ error: "POST only supports action=call" },
			{ status: 400 },
		);
	}
	...
	if (
		!ALLOWED_GATEWAY_PATHS.some(
			(allowed) =>
				normalizedPath === allowed || normalizedPath.startsWith(`${allowed}/`),
		)
	) {
		return NextResponse.json(
			{ error: "Path not in allowed gateway paths" },
			{ status: 403 },
		);
	}
	...
	try {
		...
		return NextResponse.json(
			{ error: "Gateway unreachable", path },
			{ status: 502 },
		);
	}
}
