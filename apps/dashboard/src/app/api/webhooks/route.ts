...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (isBlockedWebhookUrl(url)) {
			return NextResponse.json(
				{ error: "Webhook URL cannot point to internal or private services" },
				{ status: 400 },
			);
		}

		const secret =
			generate_secret !== false ? randomBytes(32).toString("hex") : null;
    ...
    `)
		...
		return NextResponse.json({
			...
			enabled: true,
			message:
				"Webhook created. Save the secret - it won't be shown again in full.",
		});
	} catch (error) {
...
export async function PUT(request: NextRequest) {
	...
	try {
		...
		const { id, name, url, events, enabled, regenerate_secret, reset_circuit } =
			body;
		...
		return NextResponse.json({
			success: true,
			...(newSecret
				? { secret: newSecret, message: "New secret generated. Save it now." }
				: {}),
		});
	} catch (error) {
...
export async function DELETE(request: NextRequest) {
	...
	try {
		...
		db.prepare(
			"DELETE FROM webhook_deliveries WHERE webhook_id = ? AND workspace_id = ?",
		).run(id, workspaceId);
		const result = db
