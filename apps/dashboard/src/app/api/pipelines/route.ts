...
import { createPipelineSchema, validateBody } from "@/lib/validation";
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const templates = db
			.prepare("SELECT id, name FROM workflow_templates")
    ...
    `)
		...
		const parsed = pipelines.map((p) => {
			...
			return {
				...
				steps: steps.map((s) => ({
					...s,
					template_name: nameMap.get(s.template_id) || "Unknown",
				})),
				runs: runMap.get(p.id) || {
					total: 0,
					completed: 0,
					failed: 0,
					running: 0,
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (existing.length !== new Set(templateIds).size) {
			return NextResponse.json(
				{ error: "One or more template IDs not found" },
				{ status: 400 },
			);
		}
    ...
    `)
		...
		return NextResponse.json(
			{ pipeline: { ...pipeline, steps: JSON.parse(pipeline.steps) } },
			{ status: 201 },
		);
	} catch (error) {
...
export async function PUT(request: NextRequest) {
	...
	try {
		...
		db.prepare(
			`UPDATE workflow_pipelines SET ${fields.join(", ")} WHERE id = ? AND workspace_id = ?`,
		).run(...params);
		...
		const updated = db
			...
			.get(id, workspaceId) as Pipeline;
		return NextResponse.json({
			pipeline: { ...updated, steps: JSON.parse(updated.steps) },
		});
	} catch (error) {
...
export async function DELETE(request: NextRequest) {
	...
	try {
		...
		db.prepare(
			"DELETE FROM workflow_pipelines WHERE id = ? AND workspace_id = ?",
		).run(parseInt(id, 10), workspaceId);
