	...
	try {
		...
		if (runId) {
			const run = db
				...
				.get(parseInt(runId, 10), workspaceId) as PipelineRun | undefined;
			if (!run)
				return NextResponse.json({ error: "Run not found" }, { status: 404 });
			return NextResponse.json({
				run: { ...run, steps_snapshot: JSON.parse(run.steps_snapshot) },
			});
		}
		...
		if (pipelineId) {
			...
			params.push(parseInt(pipelineId, 10));
		}
		...
		const pipelines =
			pipelineIds.length > 0
				? (db
						...
						.all(workspaceId, ...pipelineIds) as Array<{
						id: number;
						name: string;
					}>)
				: [];
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		return NextResponse.json(
			{ error: "Invalid action. Use: start, advance, cancel" },
			{ status: 400 },
		);
	} catch (error) {
		logger.error({ err: error }, "POST /api/pipelines/run error");
		return NextResponse.json(
			{ error: "Failed to process pipeline run" },
			{ status: 500 },
		);
	}
}
...
async function spawnStep(
	...
	try {
		...
		const args = [
			"agent",
			"--message",
			`[Pipeline: ${pipelineName} | Step ${stepIdx + 1}] ${template.task_prompt}`,
			"--timeout",
		...
		).run(JSON.stringify(steps), runId, workspaceId);
...
async function startPipeline(
	...
	const pipeline = db
		...
		.get(pipelineId, workspaceId) as any;
	if (!pipeline)
  ...
  `).run(now, now, pipelineId, workspaceId);
	...
	db_helpers.logActivity(
		...
		pipelineId,
		triggeredBy,
		`Started pipeline: ${pipeline.name}`,
		{ run_id: runId },
		workspaceId,
	);
...
async function advanceRun(
	...
	if (run.status !== "running")
		return NextResponse.json(
			{ error: `Run is ${run.status}, not running` },
			{ status: 400 },
		);

	const steps: (RunStepState & { on_failure?: string })[] = JSON.parse(
		run.steps_snapshot,
	);
	...
	if (!success && onFailure === "stop") {
		...
		return NextResponse.json({
			run: { id: runId, status: "failed", steps_snapshot: steps },
		});
	}
	...
	if (nextIdx >= steps.length) {
		...
		return NextResponse.json({
			run: { id: runId, status: finalStatus, steps_snapshot: steps },
		});
	}
	...
	if (template) {
		const pipeline = db
			...
			.get(run.pipeline_id, workspaceId) as any;
		spawnResult = await spawnStep(
			db,
			pipeline?.name || "?",
			template,
...
function cancelRun(
	...
	return NextResponse.json({
		run: { id: runId, status: "cancelled", steps_snapshot: steps },
	});
}
