...
import { AmoenaDoctorBanner } from '@/components/layout/lunaria-doctor-banner';
import { AmoenaUpdateBanner } from '@/components/layout/lunaria-update-banner';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (
      shouldRedirectDashboardToHttps({
    ...
    const connectWithPrimaryGateway = async (): Promise<{
      attempted: boolean;
      connected: boolean;
    }> => {
      try {
        ...
        const gateways = Array.isArray(gatewaysJson?.gateways)
          ? (gatewaysJson.gateways as GatewaySummary[])
          : [];
      ...
      fetch('/api/skills')
        ...
        .then((skillsData) => {
          if (skillsData?.skills)
            setSkillsData(skillsData.skills, skillsData.groups || [], skillsData.total || 0);
        })
        .finally(() => {
          markStep('skills');
        }),
    ]).catch(() => {
      /* panels will lazy-load as fallback */
    });
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <MemorySpotlightPanel open={spotlightOpen} onClose={() => setSpotlightOpen(false)} />
    </div>
  );
}
...
function LocalModeUnavailable({ panel }: { panel: string }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      <p className="text-sm text-muted-foreground">{tp('requiresGateway', { panel })}</p>
      <p className="text-xs text-muted-foreground mt-1">{tp('configureGateway')}</p>
    </div>
  );
}
