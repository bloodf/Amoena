...
export default function Home() {
	...
	useEffect(() => {
		...
		const connectWithEnvFallback = () => {
			...
			const gatewayHost =
				process.env.NEXT_PUBLIC_GATEWAY_HOST || window.location.hostname;
			const gatewayProto =
				process.env.NEXT_PUBLIC_GATEWAY_PROTOCOL ||
				(window.location.protocol === "https:" ? "wss" : "ws");
			const wsUrl =
				explicitWsUrl || `${gatewayProto}://${gatewayHost}:${gatewayPort}`;
			connect(wsUrl);
		};
		...
		const connectWithPrimaryGateway = async (): Promise<{
			...
			try {
				...
				const primaryGateway =
					gateways.find((gw) => Number(gw?.is_primary) === 1) || gateways[0];
		...
		fetch("/api/status?action=capabilities")
			...
			.then(async (data) => {
				...
				if (
					data?.interfaceMode === "essential" ||
					data?.interfaceMode === "full"
				) {
