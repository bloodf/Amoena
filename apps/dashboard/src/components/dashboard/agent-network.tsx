...
import { generateNodePosition, sessionToAgent } from "@/lib/utils";
...
function AgentNode({ data }: { data: any }) {
	...
	const getStatusClasses = () => {
		switch (status) {
			case "active":
				return "border-void-cyan glow-cyan";
			case "idle":
				return "border-void-amber/50";
			case "error":
				return "border-void-crimson badge-glow-error";
			default:
	...
	const getRoleBadge = () => {
		switch (agent.type) {
			case "main":
				return {
					label: "LEAD",
					color: "bg-void-violet/20 text-void-violet border-void-violet/30",
				};
			case "subagent":
				return {
					label: "WORKER",
					color: "bg-void-cyan/20 text-void-cyan border-void-cyan/30",
				};
			case "cron":
				return {
					label: "CRON",
					color: "bg-void-amber/20 text-void-amber border-void-amber/30",
				};
			default:
				return {
					label: "SYSTEM",
	...
	return (
		<div
			className={`void-panel px-3 py-3 border-2 ${getStatusClasses()} min-w-[140px]`}
		>
			<div className="flex items-start justify-between">
				<span
					className={`text-void-cyan ${isWorking ? "animate-glow-pulse" : ""}`}
				>
					{getTypeIcon()}
				</span>
				{isWorking && (
			...
			<div className="mt-2">
				<div className="flex items-center space-x-1 mb-1">
					...
					<span
						className={`px-1.5 py-0.5 text-xs font-bold font-mono border rounded-full ${roleBadge.color}`}
					>
						{roleBadge.label}
					</span>
				</div>
			...
			<div className="void-panel h-96 flex items-center justify-center">
				<div className="text-center text-muted-foreground">
					<svg
						...
						strokeLinecap="round"
						strokeLinejoin="round"
						className="w-10 h-10 mx-auto mb-2 text-void-cyan/40"
					>
