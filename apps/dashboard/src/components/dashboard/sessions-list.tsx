...
function SessionCard({ session }: SessionCardProps) {
	...
	const _statusColor = session.active ? "success" : "warning";
		...
		<div className="bg-card border border-border rounded-lg p-4 hover:bg-secondary/50 transition-colors">
			<div className="flex items-start justify-between">
				<div className="flex items-start space-x-3">
					<div
						className={`text-xl ${session.active ? "working-indicator" : ""}`}
					>
					...
					<div className="flex-1 min-w-0">
						<div className="flex items-center space-x-2 mb-1">
							...
							{/* Role Badge */}
							<span
								className={`px-2 py-0.5 text-xs font-bold border rounded-full ${roleBadge.color}`}
							>
								{roleBadge.label}
							</span>
						</div>
						...
						<div className="flex items-center space-x-2 mt-2">
							<span
								className={`text-xs font-mono ${getModelColor(session.model)}`}
							>
								{session.model}
							</span>
