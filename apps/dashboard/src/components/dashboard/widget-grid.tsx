...
const WIDGET_COMPONENTS: Record<
	string,
	React.ComponentType<{ data: DashboardData }>
> = {
...
export function WidgetGrid({ data }: { data: DashboardData }) {
	...
	const addWidget = (widgetId: string) => {
		setDashboardLayout((currentLayout) => {
			...
			return nextLayout.includes(widgetId)
				? nextLayout
				: [...nextLayout, widgetId];
		});
	};
	...
	const renderWidgets = () => {
		...
		const flushRow = () => {
			...
			elements.push(
				<section
					...
					className="grid xl:grid-cols-12 gap-4"
				>
					{rowWidgets.map(({ id, size }) =>
						renderWidget(id, SIZE_CLASSES[size] || "xl:col-span-4"),
					)}
				</section>,
			);
			rowWidgets = [];
		...
		for (const widgetId of validLayout) {
			...
			if (widget.defaultSize === "full") {
				...
				const span =
					widget.defaultSize === "sm" ? 6 : widget.defaultSize === "md" ? 4 : 8;
