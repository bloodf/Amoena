...
export function MetricCard({
	...
	return (
		<div className={`rounded-lg border p-3.5 ${colorMap[color]}`}>
			...
			{subtitle && (
				<div className="text-2xs opacity-50 font-mono-tight mt-0.5">
					{subtitle}
				</div>
			)}
		</div>
...
export function SignalPill({
	...
	return (
		<div className={`rounded-lg border px-2.5 py-2 ${toneClass}`}>
			...
			<div className="text-xs font-semibold font-mono-tight truncate">
				{value}
			</div>
		</div>
	);
	...
	return (
		<div className="space-y-1">
			<div className="flex items-center justify-between">
				...
				<span className={`text-xs font-medium font-mono-tight ${statusColor}`}>
					{value}
				</span>
			</div>
			{bar != null && (
...
export function StatRow({
	...
	return (
		<div className="flex items-center justify-between">
			<span className="text-xs text-muted-foreground">{label}</span>
			<span
				className={`text-xs font-medium font-mono-tight ${alert ? "text-red-400" : "text-muted-foreground"}`}
			>
				{value}
			</span>
		</div>
	...
	return (
		...
		>
			<div className="w-8 h-8 rounded-md bg-secondary flex items-center justify-center shrink-0 group-hover:bg-primary/10 transition-smooth">
				<div className="w-4 h-4 text-muted-foreground group-hover:text-primary transition-smooth">
					{icon}
				</div>
			</div>
			<div>
...
export function getLocalOsStatus(
	memPct: number | null,
	diskPct: number | null,
): { value: string; status: "good" | "warn" | "bad" } {
	if (memPct == null && diskPct == null)
		return { value: "Unknown", status: "bad" };
