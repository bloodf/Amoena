...
export function Dashboard() {
	...
	const subscriptionPrice =
		subscription?.provider && subscription?.type
			? (SUBSCRIPTION_PRICES[subscription.provider]?.[subscription.type] ??
				null)
			: null;
			...
			requests.push(
				fetch("/api/hermes")
					.then(async (res) => {
						if (!res.ok) return;
						const data = await res.json();
						if (data?.cronJobCount != null)
							setHermesCronJobCount(data.cronJobCount);
					})
					.catch(() => {}),
			);
		} else {
	...
	const claudeHealth = isClaudeLoading
		? { value: "Loading...", status: "warn" as const }
		: getProviderHealth(
				claudeStats?.active_sessions ?? claudeActive,
	...
	const localSessionLogs: LogLike[] = isLocal
		? sessions.reduce<LogLike[]>((acc, session) => {
				...
				const lastPrompt =
					typeof (session as any).lastUserPrompt === "string"
	...
	const mergedRecentLogs: LogLike[] = (
		isLocal ? [...logs, ...localSessionLogs] : logs
	)
		.sort((a, b) => b.timestamp - a.timestamp)
		.filter(
			(entry, index, arr) => arr.findIndex((x) => x.id === entry.id) === index,
		)
		.slice(0, 10);

	const recentErrorLogs = mergedRecentLogs.filter(
		(log) => log.level === "error",
	).length;
	const gatewayHealthStatus = connection.isConnected
		? ("good" as const)
		: ("bad" as const);
			...
			<section className="rounded-xl border border-border bg-card p-4">
				<div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
					...
					<div className="grid grid-cols-2 gap-2 min-w-[280px]">
						<SignalPill
							label="Mode"
							value={isLocal ? "Local" : "Gateway"}
							tone="info"
						/>
						<SignalPill
							label="Events"
							value={`${mergedRecentLogs.length} stream`}
							tone={recentErrorLogs > 0 ? "warning" : "success"}
						/>
						<SignalPill
							label="Queue"
							value={String(backlogCount)}
							tone={backlogCount > 10 ? "warning" : "info"}
						/>
						<SignalPill
							label="Errors"
