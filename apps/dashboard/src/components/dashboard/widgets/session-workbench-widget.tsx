			...
			<div className="divide-y divide-border/50 max-h-80 overflow-y-auto">
				{sessions.length === 0 ? (
					...
					sessions.slice(0, 10).map((session) => (
						<div
							key={session.id}
							className="px-4 py-2.5 hover:bg-secondary/20 transition-smooth"
						>
							<button
								type="button"
							...
							>
								<div
									className={`w-2 h-2 rounded-full shrink-0 ${session.active ? "bg-green-500" : "bg-muted-foreground/30"}`}
								/>
								<div className="flex-1 min-w-0">
									<div className="text-xs font-medium truncate font-mono-tight">
								...
								<div className="text-right shrink-0">
									<div className="text-2xs font-mono-tight text-muted-foreground">
										{session.tokens}
									</div>
									<div className="text-2xs text-muted-foreground">
										{session.age}
