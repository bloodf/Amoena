		...
		return (
			<section className="grid grid-cols-2 xl:grid-cols-6 gap-3">
				...
				<MetricCard
					...
					subtitle={
						hermesCronJobCount > 0
							? `${hermesActive} active · ${hermesCronJobCount} cron`
							: "active sessions"
					}
					icon={<SessionIcon />}
					color="purple"
				...
				<MetricCard
					...
					value={
						isClaudeLoading
							? "..."
							: formatTokensShort(
									(claudeStats?.total_input_tokens ?? 0) +
										(claudeStats?.total_output_tokens ?? 0),
								)
					}
					subtitle={
						isClaudeLoading
							? undefined
							: `${formatTokensShort(claudeStats?.total_input_tokens ?? 0)} in · ${formatTokensShort(claudeStats?.total_output_tokens ?? 0)} out`
					}
					icon={<TokenIcon />}
					color="purple"
				/>
				<MetricCard
					...
					subtitle={
						subscriptionLabel ? `${subscriptionLabel} plan` : "estimated"
					}
					icon={<CostIcon />}
	...
	return (
		<section className="grid grid-cols-2 xl:grid-cols-5 gap-3">
			...
			<MetricCard
				...
				value={onlineAgents}
				subtitle={`${dbStats?.agents.total ?? agents.length} total`}
				icon={<AgentIcon />}
				color="green"
			/>
			<MetricCard
				...
				icon={<TaskIcon />}
				color={backlogCount > 12 ? "red" : "purple"}
			/>
			<MetricCard
				label="System Load"
				value={isSystemLoading ? "..." : `${systemLoad}%`}
				subtitle={`errors ${errorCount}`}
				icon={<ActivityIconMini />}
				color={systemLoad > 85 || errorCount > 0 ? "red" : "blue"}
			/>
		</section>
	);
}
