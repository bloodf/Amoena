...
import { type DashboardData, LogRow } from "../widget-primitives";

export function EventStreamWidget({ data }: { data: DashboardData }) {
	const { isLocal, mergedRecentLogs, recentErrorLogs, isSessionsLoading } =
		data;
				...
				{mergedRecentLogs.length === 0 ? (
					<div className="px-4 py-8 text-center">
						...
						<p className="text-2xs text-muted-foreground/60 mt-1">
							{isLocal
								? "Local Claude/Codex events stream here."
								: "Gateway incidents and warnings stream here."}
						</p>
					</div>
				) : (
