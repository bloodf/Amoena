...
function modeColors(isGateway: boolean) {
	return isGateway
		? {
				text: "text-void-cyan",
				border: "border-void-cyan/30",
				bg: "bg-void-cyan",
				bgLight: "bg-void-cyan/5",
				bgBtn: "bg-void-cyan/20",
				hoverBg: "hover:bg-void-cyan/30",
				hoverBorder: "hover:border-void-cyan/30",
				hoverBgLight: "hover:bg-void-cyan/10",
				dot: "bg-void-cyan",
				dotDim: "bg-void-cyan/40",
			}
		: {
				text: "text-void-amber",
				border: "border-void-amber/30",
				bg: "bg-void-amber",
				bgLight: "bg-void-amber/5",
				bgBtn: "bg-void-amber/20",
				hoverBg: "hover:bg-void-amber/30",
				hoverBorder: "hover:border-void-amber/30",
				hoverBgLight: "hover:bg-void-amber/10",
				dot: "bg-void-amber",
				dotDim: "bg-void-amber/40",
			};
}
...
export function OnboardingWizard() {
	...
	useEffect(() => {
		...
		fetch("/api/onboarding")
			...
			.then((data) => {
				if (data) {
					...
					setStep((current) => {
						const incoming = shouldReplayFromStart
							? 0
		...
		Promise.allSettled([
			fetch("/api/status?action=capabilities").then((r) =>
				r.ok ? r.json() : null,
			),
		...
		fetch("/api/diagnostics")
			...
			.then((data) => {
				if (data?.security?.checks) {
					...
					const authOk =
						checks.find((c) => c.name === "Auth password secure")?.pass ??
						false;
					const apiKeyOk =
						checks.find((c) => c.name === "API key configured")?.pass ?? false;
	...
	return createPortal(
		<div
			className={`fixed inset-0 z-[140] flex items-center justify-center transition-opacity duration-300 ${closing ? "opacity-0" : "opacity-100"}`}
		>
			{/* Backdrop */}
			<div
			...
			>
				...
				{/* Content */}
				<div
					className={`relative px-6 py-4 min-h-[320px] max-h-[70vh] flex flex-col transition-all duration-150 ${
						animating
				...
				>
					{completionMessage && (
						<div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-background/95 backdrop-blur-sm">
							<div
								className={`text-2xl font-bold mb-2 ${isGateway ? "text-void-cyan" : "text-void-amber"}`}
							>
								{t("stationOnline")}
							</div>
					...
					{STEPS[step]?.id === "gateway-link" && (
						<StepGatewayLink
							isGateway={isGateway}
							registration={capabilities.dashboardRegistration}
							onNext={goNext}
							onBack={goBack}
						/>
					)}
		...
		<>
			<div className="flex-1 flex flex-col items-center justify-center text-center gap-4">
				...
				<div className="flex flex-wrap items-center justify-center gap-2">
					<StatusChip
						...
						label={
							capabilities.claudeSessions > 0
								? t("activeSessionsDetected", {
										count: capabilities.claudeSessions,
									})
								: t("noActiveSessions")
						}
					/>
					<StatusChip
						...
						label={
							capabilities.gatewayConnected
								? t("gatewayConnected")
								: t("localModeNoGateway")
						}
					/>
					<StatusChip
					...
					/>
					{capabilities.gatewayConnected &&
						capabilities.dashboardRegistration && (
							<StatusChip
								ok={
									capabilities.dashboardRegistration.registered ||
									capabilities.dashboardRegistration.alreadySet
								}
								label={
									capabilities.dashboardRegistration.registered ||
									capabilities.dashboardRegistration.alreadySet
				...
				<div className="w-full">
					...
					<div className="grid grid-cols-2 gap-3">
						...
						>
							...
							<p
								className={`text-xs font-medium mb-1.5 ${!isGateway ? "text-void-amber" : "text-muted-foreground"}`}
							>
								{t("localMode")}
							</p>
							<ul
								className={`text-2xs space-y-0.5 ${!isGateway ? "text-muted-foreground" : "text-muted-foreground/60"}`}
							>
							...
							{isGateway && (
								<p className="text-2xs text-muted-foreground/40 mt-1.5 italic">
									{t("singlePilot")}
								</p>
							)}
						</div>
						...
						>
							...
							<p
								className={`text-xs font-medium mb-1.5 ${isGateway ? "text-void-cyan" : "text-muted-foreground"}`}
							>
								{t("gatewayMode")}
							</p>
							<ul
			...
			<div className="flex items-center justify-between pt-4 border-t border-border/30">
				...
				<Button
					onClick={onNext}
					size="sm"
					className={`${mc.bgBtn} ${mc.text} border ${mc.border} ${mc.hoverBg}`}
				>
					{t("getStarted")}
				</Button>
			</div>
...
function StatusChip({ ok, label }: { ok: boolean; label: string }) {
	return (
		<div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface-1 border border-border/30">
			<span
				className={`w-2 h-2 rounded-full ${ok ? "bg-green-400" : "bg-surface-2"}`}
			/>
			...
			<div className="flex-1">
				...
				<div className="grid grid-cols-2 gap-3">
					...
					>
						...
						<p
							className={`text-sm font-medium mb-2 ${selected === "essential" ? "text-void-amber" : "text-foreground"}`}
						>
							{t("essential")}
						</p>
					...
					>
						...
						<p
							className={`text-sm font-medium mb-2 ${selected === "full" ? "text-void-cyan" : "text-foreground"}`}
						>
							{t("full")}
						</p>
						<p className="text-xs text-muted-foreground mb-3">
		...
		<>
			<div className="flex-1">
				...
				<div className="space-y-3">
					...
					>
						<span
							className={`font-mono text-sm mt-0.5 ${configured ? "text-green-400" : "text-amber-400"}`}
						>
							[{configured ? "+" : "~"}]
						</span>
						<div>
					...
					>
						<span
							className={`font-mono text-sm mt-0.5 ${configured ? "text-green-400" : "text-muted-foreground"}`}
						>
							[{configured ? "+" : "-"}]
						</span>
						<div>
		...
		<>
			<div className="flex-1 min-h-0 overflow-y-auto">
				...
				{!status ? (
					...
					<div className="space-y-3">
						<div
							className={`flex items-start gap-3 p-3 rounded-lg border ${status.authOk ? "border-green-400/20 bg-green-400/5" : "border-red-400/20 bg-red-400/5"}`}
						>
							<span
								className={`font-mono text-sm mt-0.5 ${status.authOk ? "text-green-400" : "text-red-400"}`}
							>
								[{status.authOk ? "+" : "x"}]
							</span>
							<div>
						...
						<div
							className={`flex items-start gap-3 p-3 rounded-lg border ${status.apiKeyOk ? "border-green-400/20 bg-green-400/5" : "border-red-400/20 bg-red-400/5"}`}
						>
							<span
								className={`font-mono text-sm mt-0.5 ${status.apiKeyOk ? "text-green-400" : "text-red-400"}`}
							>
								[{status.apiKeyOk ? "+" : "x"}]
							</span>
							<div>
