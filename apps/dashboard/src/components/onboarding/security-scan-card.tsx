...
export function SecurityScanCard({
			...
			try {
				...
				if (!res.ok) {
					setFixResult({
						...
						failed: 1,
						remaining: 0,
						remainingAutoFixable: 0,
						remainingManual: 0,
						note: res.status === 401 ? "Admin access required" : "Fix failed",
					});
					return;
				}
		...
		return (
			<div className="flex flex-col items-center gap-4 py-6">
				...
				<Button
					...
					variant="outline"
					size="sm"
					className="border-void-cyan/30 text-void-cyan hover:bg-void-cyan/10"
				>
					Run Security Scan
				</Button>
			</div>
	...
	if (error) {
		return (
			<div className="flex flex-col items-center gap-3 py-6">
				...
				<Button onClick={runScan} variant="outline" size="sm">
					Retry
				</Button>
			</div>
		);
	...
	const failingChecks = Object.values(result.categories).flatMap((cat) =>
		cat.checks.filter((check) => check.status !== "pass"),
	);
	const autoFixableChecks = failingChecks.filter((check) =>
		FIXABLE_IDS.has(check.id),
	);
	const manualChecks = failingChecks.filter(
		(check) => !FIXABLE_IDS.has(check.id),
	);
		...
		<div className="space-y-4">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-3">
					<div
						className={`text-3xl font-bold tabular-nums ${OVERALL_COLOR[result.overall]}`}
					>
						{result.score}
					</div>
					<div>
						<div
							className={`text-sm font-medium capitalize ${OVERALL_COLOR[result.overall]}`}
						>
				...
				return totalFailing > 0 ? (
					<div className="flex items-center justify-between gap-2">
						<div className="text-xs text-muted-foreground">
							...
							<p>
								{autoFixableChecks.length} auto-fixable, {manualChecks.length}{" "}
								manual/review
							</p>
						</div>
						<Button
			...
			{fixResult && (
				<div
					className={`text-xs px-3 py-2 rounded-lg border ${fixResult.failed > 0 ? "bg-amber-500/10 border-amber-500/20 text-amber-300" : "bg-green-500/10 border-green-500/20 text-green-300"}`}
				>
					{fixResult.attempted > 0 && (
						<span>
							{fixResult.attempted} auto-fix attempt
							{fixResult.attempted > 1 ? "s" : ""}.{" "}
						</span>
					)}
					{fixResult.fixed > 0 && (
						<span>
							{fixResult.fixed} issue{fixResult.fixed > 1 ? "s" : ""} fixed.{" "}
						</span>
					)}
					{fixResult.failed > 0 && <span>{fixResult.failed} failed. </span>}
					{fixResult.remaining > 0 && (
						<span>
							{fixResult.remaining} issue{fixResult.remaining > 1 ? "s" : ""}{" "}
							remain.{" "}
						</span>
					)}
					{fixResult.remainingManual > 0 && (
						<span>
							{fixResult.remainingManual} still need manual action or review.{" "}
						</span>
					)}
					{fixResult.note && (
						<span className="text-muted-foreground">{fixResult.note}</span>
					)}
				</div>
			)}
					...
					return (
						...
						>
							...
							>
								...
								<span
									className={`text-xs tabular-nums ${cat.score >= 80 ? "text-green-400" : cat.score >= 50 ? "text-amber-400" : "text-red-400"}`}
								>
									{cat.score}%
								</span>
								{failing.length > 0 && (
								...
								<div className="border-t border-border/30 px-3 py-2 space-y-1.5 bg-surface-1/30">
									{cat.checks.map((check) => (
										<div key={check.id} className="flex items-start gap-2 py-1">
											<span
												className={`font-mono text-xs mt-0.5 w-4 shrink-0 ${STATUS_COLOR[check.status]}`}
											>
											...
											<div className="flex-1 min-w-0">
												...
												{check.fix && check.status !== "pass" && (
													<div className="flex items-center gap-1.5 mt-0.5">
														<p className="text-xs text-void-cyan/70 flex-1 min-w-0">
															Fix: {check.fix}
														</p>
														{FIXABLE_IDS.has(check.id) && (
															<button
																...
																title={
																	FIX_SAFETY[check.id] === "requires-review"
																		? "Requires review — may affect running services"
																		: FIX_SAFETY[check.id] ===
																				"requires-restart"
