...
const ROLE_CONFIG = {
	user: {
		...
		borderClass: "border-l-green-500/40",
	},
	assistant: {
		indicator: "\u25C6",
	...
	system: {
		indicator: "",
		indicatorClass: "",
		borderClass: "border-l-amber-500/20",
	},
} as const;
...
export function SessionMessage({
	...
	return (
		<div className={`flex gap-0 border-l-2 ${config.borderClass} pl-3 py-1.5`}>
			...
			{config.indicator && (
				<div
					className={`w-5 flex-shrink-0 text-center font-mono-tight text-xs ${config.indicatorClass}`}
				>
					{config.indicator}
				</div>
			)}
...
function ToolResultPart({
	...
	return (
		...
		>
			<summary
				className={`cursor-pointer select-none font-mono-tight text-[11px] ${colorClass} hover:brightness-125`}
			>
	...
	return parts.map((part, i) => {
		...
		if (part.startsWith("```") && part.endsWith("```")) {
			...
			return (
				<div
					key={i}
					className="my-1.5 rounded border border-border/30 overflow-hidden"
				>
					{lang && (
			...
			const headerClass =
				level === 1
					...
					: level === 2
						...
						: "text-xs font-medium";
			result.push(
				<span key={`h-${i}`} className={`${headerClass} text-foreground`}>
		...
		if (listMatch) {
			...
			result.push(
				<span key={`li-${i}`} style={{ paddingLeft: `${indent * 4 + 4}px` }}>
					<span className="text-muted-foreground/50">{bullet}</span>{" "}
					{renderInlineText(listMatch[3])}
				</span>,
			);
			continue;
		}
