...
export function ChatInput({
	...
	const handleSend = () => {
		...
		if ((!trimmed && attachments.length === 0) || disabled || isSendingMessage)
			return;
			...
			{attachments.length > 0 && (
				<div className="flex flex-wrap gap-2 mb-2">
					{attachments.map((att, idx) => (
						<div
							key={idx}
							className="relative group rounded-md border border-border/60 bg-surface-1 overflow-hidden"
						>
				...
				<textarea
					...
					placeholder={
						disabled
							...
							: "Message... (@ to mention, Enter to send)"
					}
					disabled={disabled || isSendingMessage}
					rows={1}
