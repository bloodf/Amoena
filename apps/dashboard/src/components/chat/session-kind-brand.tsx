...
const SESSION_KIND_META: Record<
	SessionKind,
	{
		label: string;
		shortLabel: string;
...
function getMeta(kind: string) {
	return SESSION_KIND_META[
		(kind in SESSION_KIND_META ? kind : "gateway") as SessionKind
	];
}
...
export function SessionKindPill({ kind }: { kind: string }) {
	...
	return (
		<span
			className={`rounded px-1 py-px text-[9px] font-medium ${meta.pillClassName}`}
		>
			{meta.shortLabel}
		</span>
	);
