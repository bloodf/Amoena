	...
	return parts.map((part, i) => {
		...
		if (part.startsWith("```") && part.endsWith("```")) {
			...
			return (
				<pre
					key={i}
					className="bg-black/30 rounded-md px-3 py-2 my-1 text-xs font-mono overflow-x-auto whitespace-pre-wrap"
				>
					{code}
				</pre>
			);
...
function ToolCallBubble({ message }: { message: ChatMessage }) {
	...
	const toolName =
		typeof meta.toolName === "string" ? meta.toolName : "unknown_tool";
	const toolArgs = meta.toolArgs;
	const toolOutput = meta.toolOutput;
	const toolStatus =
		meta.toolStatus === "running" ||
		meta.toolStatus === "success" ||
		meta.toolStatus === "error"
			? meta.toolStatus
			: undefined;
	const durationMs =
	...
	const statusIcon =
		toolStatus === "running" ? "..." : toolStatus === "error" ? "x" : ">";
	const statusColor =
		toolStatus === "running"
			? "text-yellow-400"
			: toolStatus === "error"
					...
					<div className="mt-1 ml-5 space-y-1">
						...
						{toolOutput != null && (
							<div>
								...
								<pre
									className={`rounded-md px-2 py-1.5 text-[11px] font-mono overflow-x-auto max-h-48 whitespace-pre-wrap ${
										toolStatus === "error"
...
export function MessageBubble({
	...
	return (
		<div
			className={`flex gap-2 ${isHuman ? "flex-row-reverse" : "flex-row"} ${isGrouped ? "mt-0.5" : "mt-3"}`}
		>
			{/* Avatar */}
			{!isGrouped ? (
				<div
					className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-bold ${theme.bg} ${theme.text} border ${theme.border}`}
				>
			...
			{/* Content */}
			<div
				className={`max-w-[80%] min-w-0 ${isHuman ? "items-end" : "items-start"}`}
			>
				{/* Name + recipient */}
				{!isGrouped && (
					<div
						className={`flex items-center gap-1.5 mb-0.5 ${isHuman ? "flex-row-reverse" : "flex-row"}`}
					>
						...
						<div className="flex flex-wrap gap-1.5 mb-1.5">
							{message.attachments.map((att, idx) =>
								att.type.startsWith("image/") ? (
									...
									<div
										key={idx}
										className="flex items-center gap-1.5 bg-black/20 rounded-md px-2 py-1 text-xs text-muted-foreground"
									>
