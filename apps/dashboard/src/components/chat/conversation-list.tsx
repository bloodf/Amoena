		...
		return [
			{
				...
				active:
					typeof session?.active === "boolean" ? session.active : undefined,
				...
				workingDir:
					...
					session?.workingDir === null
						...
						: undefined,
				lastUserPrompt:
					typeof session?.lastUserPrompt === "string" ||
...
export function ConversationList({
	...
	const saveSessionPref = useCallback(
		async (conv: Conversation, name?: string, color?: string) => {
				...
				conversations.map((c) => {
					...
					return {
						...
						session: {
							...
							displayName: newName,
							colorTag:
								color !== undefined ? color || undefined : c.session.colorTag,
						},
					};
				}),
			);
	...
	const gatewayRows = filteredConversations.filter(
		(c) => c.source === "session" && c.session?.sessionKind === "gateway",
	);
	...
	const localRows = filteredConversations.filter(
		(c) =>
			c.source === "session" &&
			(c.session?.sessionKind === "claude-code" ||
				c.session?.sessionKind === "codex-cli" ||
				c.session?.sessionKind === "hermes"),
	);
	...
	function renderConversationItem(conv: Conversation) {
		...
		return (
			<Button
				...
				onDoubleClick={() => {
					if (conv.session?.prefKey) startRename(conv);
				}}
				...
				<div className="flex items-center gap-2 w-full">
					...
					<div className="relative flex-shrink-0">
						...
						{isSessionRow && conv.session?.active && (
							<div
								className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-card ${STATUS_COLORS.busy}`}
							/>
						)}
						{!isSessionRow && (
							<div
								className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-card ${STATUS_COLORS.offline}`}
							/>
						)}
					</div>
					...
					<div className="flex-1 min-w-0 text-left">
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-1.5 min-w-0">
								{conv.session?.colorTag &&
									TAG_COLORS[conv.session.colorTag] && (
										<span
											className={`h-2 w-2 rounded-full ${TAG_COLORS[conv.session.colorTag]}`}
										/>
									)}
								{isSessionRow &&
									conv.session?.sessionKind &&
								...
								<div className="flex flex-wrap gap-1.5">
									{COLOR_OPTIONS.map((opt) => {
										const isCurrentColor =
											(conv.session?.colorTag || "") === opt.value;
										return (
											<button
												key={opt.value}
											...
											>
												{!opt.value && (
													<svg
														...
														stroke="currentColor"
														strokeWidth="1.5"
														className="m-auto text-muted-foreground/60"
													>
														<path
