...
const LOADER_AGENTS = [
	{
		...
		wrapperClass:
			"absolute left-1/2 top-0 -translate-x-1/2 opacity-0 animate-converge-top",
	...
	},
	{
		...
		wrapperClass:
			"absolute right-0 top-1/2 -translate-y-1/2 opacity-0 animate-converge-right",
	...
	},
	{
		...
		wrapperClass:
			"absolute left-1/2 bottom-0 -translate-x-1/2 opacity-0 animate-converge-bottom",
...
function LoaderDots({ size = "md" }: { size?: "sm" | "md" }) {
	...
	return (
		<div className="flex items-center gap-1.5">
			<div
				className={`${dotSize} rounded-full bg-void-cyan animate-pulse`}
							...
							{LOADER_AGENTS.map((agent) => (
								<div key={agent.key} className={agent.wrapperClass}>
									<div className="relative">
										...
										/>
										<span
											className={`${agent.labelClass} rounded-full border border-border/50 bg-background/85 px-1.5 py-0.5 text-[9px] font-mono uppercase tracking-[0.18em] text-muted-foreground shadow-sm`}
										>
											{agent.name}
										</span>
									</div>
