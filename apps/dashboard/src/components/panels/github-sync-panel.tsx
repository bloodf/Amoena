	...
	const handleToggleSync = async (project: (typeof projects)[number]) => {
		try {
			...
			if (res.ok) {
				...
				showFeedback(
					true,
					`Sync ${project.github_sync_enabled ? "disabled" : "enabled"} for ${project.name}`,
				);
			} else {
				const data = await res.json();
						...
						.map((project) => (
							...
							>
								<div className="flex items-center gap-3">
									<span
										className={`w-2 h-2 rounded-full ${project.github_sync_enabled ? "bg-green-500" : "bg-muted-foreground/30"}`}
									/>
									<div>
