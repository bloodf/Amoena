...
export function StandupPanel() {
	...
	const [standupReport, setStandupReport] = useState<StandupReport | null>(
		null,
	);
	...
	const exportStandup = () => {
		...
		if (standupReport.teamBlockers.length > 0) {
			...
			standupReport.teamBlockers.forEach((task) => {
				lines.push(
					`- **${task.agent}** [${task.priority.toUpperCase()}]: ${task.title}`,
				);
			});
			lines.push("");
		}
				...
				<div className="flex items-center gap-3">
					...
					{view === "current" && (
						<>
							...
							>
								{loading && (
									<div className="animate-spin rounded-full h-3.5 w-3.5 border-2 border-primary-foreground/30 border-t-primary-foreground" />
								)}
								{loading ? t("generating") : t("generate")}
							</Button>

							{standupReport && (
								<Button onClick={exportStandup} variant="success" size="sm">
									{t("export")}
								</Button>
							)}
						</>
			...
			{error && (
				<div className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 m-4 rounded-lg text-sm flex items-center justify-between">
					...
					<Button
						...
						variant="ghost"
						size="icon-xs"
						className="text-red-400/60 hover:text-red-400 ml-2 w-5 h-5"
					>
						×
					</Button>
				</div>
						...
						<div className="p-4 space-y-6">
							...
							<div className="bg-card rounded-lg p-4 border border-border">
								...
								<p className="text-muted-foreground text-sm">
									{t("generatedOn", {
										date: new Date(standupReport.generatedAt).toLocaleString(),
									})}
								</p>
							</div>
							...
							{standupReport.teamAccomplishments.length > 0 && (
								<div className="bg-card rounded-lg p-4 border border-border">
									<h4 className="text-lg font-semibold text-foreground mb-3">
										🎉 {t("teamAccomplishments")}
									</h4>
									<div className="space-y-2">
										{standupReport.teamAccomplishments.map((task) => (
											<div
												key={task.id}
												className="flex justify-between items-center p-2 bg-green-900/20 rounded border-l-4 border-green-500"
											>
								...
								<div className="bg-card rounded-lg p-4 border border-border">
									...
									<div className="space-y-2">
										{standupReport.teamBlockers.map((task) => (
											<div
												key={task.id}
												className="flex justify-between items-center p-2 bg-red-900/20 rounded border-l-4 border-red-500"
											>
												<div>
													<span className="text-foreground">{task.title}</span>
													<span
								...
								<div className="bg-card rounded-lg p-4 border border-border">
									...
									<div className="space-y-2">
										{standupReport.overdueTasks.map((task) => (
											<div
												key={task.id}
												className="flex justify-between items-center p-2 bg-orange-900/20 rounded border-l-4 border-orange-500"
											>
												<div>
													<span className="text-foreground">{task.title}</span>
										...
										<div className="flex justify-between items-start mb-4">
											...
											<div className="text-right text-sm">
												<div className="text-muted-foreground">
													{t("activitySummary", {
														actions: report.activity.actionCount,
														comments: report.activity.commentsCount,
													})}
												</div>
										...
										<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
											...
											<div>
												<h6 className="text-green-400 font-medium mb-2">
													✅{" "}
													{t("sectionCompleted", {
														count: report.completedToday.length,
													})}
												</h6>
						...
						{standupHistory.length === 0 ? (
							...
							<div className="space-y-4">
								{standupHistory.map((history) => (
									<div
										key={history.id}
										className="bg-card rounded-lg p-4 border border-border hover:bg-surface-1 transition-smooth"
									>
										<div className="flex justify-between items-start">
											<div>
												...
												<p className="text-muted-foreground text-sm">
													{t("historyAgentsParticipated", {
														count: history.agentCount,
													})}
												</p>
											</div>
											<div className="text-right">
												{history.summary && (
													<div className="text-sm text-muted-foreground">
														<div>
															{t("historyCompleted", {
																count: history.summary.completed || 0,
															})}
														</div>
														<div>
															{t("historyInProgress", {
																count: history.summary.inProgress || 0,
															})}
														</div>
														<div>
															{t("historyBlocked", {
																count: history.summary.blocked || 0,
															})}
														</div>
													</div>
												)}
