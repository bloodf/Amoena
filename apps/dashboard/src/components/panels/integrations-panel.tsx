		...
		try {
			...
			if (data.categories?.[0]) {
				setActiveCategory((prev) => {
					...
					const ids = (data.categories as Category[]).map(
						(c: Category) => c.id,
					);
	...
	const handleRemove = async (envKeys: string[]) => {
		try {
			...
			if (res.ok) {
				showFeedback(
					true,
					`Removed ${data.count} variable${data.count === 1 ? "" : "s"}`,
				);
				fetchIntegrations();
			} else {
		...
		<div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-xs text-muted-foreground mt-0.5">
						...
						{envPath && (
							<span className="ml-2 font-mono text-muted-foreground/50">
								{envPath}
							</span>
						)}
					</p>
			...
			<div className="flex gap-1 border-b border-border pb-px overflow-x-auto">
				{categories.map((cat) => {
					...
					const catConnected = catIntegrations.filter(
						(i) => i.status === "connected",
					).length;
					return (
						<Button
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
					<div className="bg-card border border-border rounded-lg shadow-xl p-5 max-w-sm mx-4 space-y-4">
						...
						<p className="text-xs text-muted-foreground">
							{t("removeDescription", {
								target:
									confirmRemove.keys.length === 1
										? confirmRemove.keys[0]
										: String(confirmRemove.keys.length),
							})}
						</p>
...
function IntegrationCard({
	...
	const hasEdits = Object.keys(integration.envVars).some(
		(k) => edits[k] !== undefined,
	);
		...
		>
			...
			<div className="flex items-center justify-between mb-3">
				<div className="flex items-center gap-2.5">
					<span
						className={`w-2 h-2 rounded-full shrink-0 ${statusColors[integration.status]}`}
					/>
				...
				{Object.entries(integration.envVars).map(([envKey, info]) => {
					...
					return (
						<div key={envKey} className="flex items-center gap-2">
							<span
								className="text-2xs font-mono text-muted-foreground/70 w-48 truncate shrink-0"
								title={envKey}
							>
								{envKey}
							</span>
