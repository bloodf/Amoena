...
function readActionResult(value: unknown): ActionResult | null {
	...
	return {
		...
		qrDataUrl:
			typeof record.qrDataUrl === "string" ? record.qrDataUrl : undefined,
		connected:
...
function ProbeResult({ probe }: { probe: ChannelStatus["probe"] }) {
	...
	return (
		<div
			className={`text-xs mt-2 px-2 py-1.5 rounded ${probe.ok ? "text-green-400 bg-green-500/10" : "text-red-400 bg-red-500/10"}`}
		>
		...
		<div className="rounded-lg border border-border bg-card p-4">
			<div className="flex items-center justify-between mb-3">
				...
				<div className="flex items-center gap-1.5">
					<span
						className={`w-2 h-2 rounded-full ${isActive ? (status?.connected ? "bg-green-500" : status?.running ? "bg-amber-500" : "bg-muted-foreground/50") : "bg-red-500"}`}
					/>
	...
	return (
		...
		>
			<div className="space-y-0.5">
				...
				<StatusRow
					label="Last connect"
					value={relativeTime(status?.lastConnectedAt)}
				/>
				<StatusRow
					label="Last message"
					value={relativeTime(status?.lastMessageAt)}
				/>
	...
	return (
		...
		>
			<div className="space-y-0.5">
				...
				<StatusRow
					label="Last start"
					value={relativeTime(status?.lastStartAt)}
				/>
			</div>
			<ErrorCallout message={status?.lastError} />
...
function NostrCard({
	...
	const profile: NostrProfile | null =
		primaryAccount?.profile ?? status?.profile ?? null;
	...
	const handleProfileSave = async () => {
		...
		const res = readActionResult(
			await onAction("nostr-profile-save", { accountId, profile: profileForm }),
		);
	...
	const handleProfileImport = async () => {
		...
		const accountId = primaryAccount?.accountId ?? "default";
		const res = readActionResult(
			await onAction("nostr-profile-import", { accountId }),
		);
		setProfileSaving(false);
	...
	return (
		...
		>
			<div className="space-y-0.5">
				...
				<StatusRow
					label="Public Key"
					value={truncatePubkey(status?.publicKey ?? primaryAccount?.publicKey)}
				/>
				<StatusRow
					label="Last start"
					value={relativeTime(status?.lastStartAt)}
			...
			{!editingProfile ? (
				<div className="mt-3 p-2.5 bg-muted/30 rounded text-xs">
					...
					{profile?.displayName || profile?.name ? (
						<div className="space-y-0.5">
							...
							{profile.name && (
								<StatusRow label={t("username")} value={profile.name} />
							)}
							{profile.about && (
								<StatusRow
									label={t("about")}
									value={profile.about.slice(0, 80)}
								/>
							)}
							{profile.nip05 && (
								<StatusRow label="NIP-05" value={profile.nip05} />
							)}
						</div>
					) : (
				...
				<div className="mt-3 p-2.5 bg-muted/30 rounded text-xs space-y-2">
					...
					<ProfileField
						...
						onChange={(v) => setProfileForm((p) => ({ ...p, about: v }))}
						disabled={profileSaving}
						multiline
					/>
					<ProfileField
						...
						onChange={(v) => setProfileForm((p) => ({ ...p, picture: v }))}
						disabled={profileSaving}
					/>
					{showAdvanced && (
						<>
							<ProfileField
								...
								onChange={(v) => setProfileForm((p) => ({ ...p, banner: v }))}
								disabled={profileSaving}
							/>
							<ProfileField
								label={t("website")}
							...
							<ProfileField
								...
								onChange={(v) => setProfileForm((p) => ({ ...p, nip05: v }))}
								disabled={profileSaving}
							/>
							<ProfileField
								label={t("lightning")}
								value={profileForm.lud16 ?? ""}
								onChange={(v) => setProfileForm((p) => ({ ...p, lud16: v }))}
								disabled={profileSaving}
							/>
						</>
					)}
					<div className="flex flex-wrap gap-1.5">
						<Button
							...
							disabled={profileSaving || actionBusy}
							variant="default"
							size="xs"
						>
...
function GenericChannelCard({
	...
	onProbe,
	probing,
}: PlatformCardProps & { label?: string }) {
	return (
		<CardShell
			platform={platform}
			label={label}
			...
			{accounts.map((acct) => (
				...
				>
					...
					{acct.lastInboundAt && (
						<StatusRow
							label="Last inbound"
							value={relativeTime(acct.lastInboundAt)}
						/>
					)}
					{acct.lastError && (
						<div className="text-red-400 break-words mt-1">
							{acct.lastError}
						</div>
					)}
				</div>
...
export function ChannelsPanel() {
	...
	const handleProbe = async (channelId: string) => {
		...
		try {
			await fetch(
				`/api/channels?action=probe&channel=${encodeURIComponent(channelId)}`,
			);
			await fetchChannels();
		} catch {
