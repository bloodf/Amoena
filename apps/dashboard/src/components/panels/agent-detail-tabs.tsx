...
export function OverviewTab({
	...
	const [availableModels, setAvailableModels] = useState<
		Array<{ alias: string; description?: string }>
	>([]);
			...
			<div className="grid md:grid-cols-[1fr_1fr] gap-5">
				...
				<div className="space-y-4">
					...
					{heartbeatData && (
						<div className="text-xs text-muted-foreground bg-surface-1/30 rounded px-3 py-2">
							<span
								className={
									heartbeatData.status === "HEARTBEAT_OK"
										? "text-green-400"
										: "text-yellow-400"
								}
							>
							...
							{heartbeatData.total_items
								? ` · ${t("workItems", { count: heartbeatData.total_items })}`
								: ""}
				...
				<div className="border border-border rounded-lg p-4 flex flex-col">
					<div className="flex items-center justify-between mb-3">
						...
						{messageStatus && (
							<span
								className={`text-xs ${messageStatus === "Sent" ? "text-green-400" : "text-rose-400"}`}
							>
								{messageStatus}
							</span>
						)}
...
export function ActivityTab({ agent }: { agent: Agent }) {
	...
	useEffect(() => {
		const fetchActivities = async () => {
			try {
				const response = await fetch(
					`/api/activities?actor=${agent.name}&limit=50`,
				);
				if (response.ok) {
					const data = await response.json();
	...
	const getActivityIcon = (type: string) => {
		switch (type) {
			...
			case "agent_heartbeat":
				return "*";
			case "agent_soul_updated":
				return "@";
			case "agent_memory_updated":
				return "=";
			default:
				return ".";
		}
...
const TEMPLATES = [
	...
	},
	{
		...
		label: "Developer",
		emoji: "\ud83d\udee0\ufe0f",
		description: "Full-stack builder with Docker bridge",
	...
	},
	{
		...
		emoji: "\u2699\ufe0f",
		description: "Focused developer for specific domains",
		modelTier: "sonnet" as const,
		toolCount: 15,
	...
	},
	{
		...
		label: "Reviewer / QA",
		emoji: "\ud83d\udd2c",
		description: "Read-only code review and quality gates",
	...
	},
	{
		...
		label: "Researcher",
		emoji: "\ud83d\udd0d",
		description: "Browser and web access for research",
		modelTier: "sonnet" as const,
		toolCount: 8,
	...
	},
	{
		...
		emoji: "\u270f\ufe0f",
		description: "Write and edit for content generation",
		modelTier: "haiku" as const,
		toolCount: 9,
	...
	},
	{
		...
		emoji: "\ud83d\udee1\ufe0f",
		description: "Read-only + bash for security scanning",
		modelTier: "sonnet" as const,
		toolCount: 10,
...
export function CreateAgentModal({
	...
	const [progressSteps, setProgressSteps] = useState<ProgressStep[] | null>(
		null,
	);
				...
				setFormData((prev) => ({
					...
					workspaceAccess:
						type === "researcher" || type === "content-creator"
							? "none"
							: type === "reviewer" || type === "security-auditor"
								? "ro"
								: "rw",
					sandboxMode: type === "orchestrator" ? "non-main" : "all",
					dockerNetwork:
						type === "developer" || type === "specialist-dev"
							? "bridge"
							: "none",
				}));
			}
	...
	const handleCreate = async () => {
		...
		try {
			const primaryModel =
				formData.modelPrimary.trim() ||
				DEFAULT_MODEL_BY_TIER[formData.modelTier];
						...
						<div className="grid grid-cols-2 gap-3">
							{TEMPLATES.map((tmpl) => (
								...
								>
									...
									<div className="flex gap-2">
										<span
											className={`px-2 py-0.5 text-xs rounded border ${MODEL_TIER_COLORS[tmpl.modelTier]}`}
										>
								...
								<div className="flex gap-2">
									{(["opus", "sonnet", "haiku"] as const).map((tier) => (
										<Button
											...
											variant={
												formData.modelTier === tier ? "outline" : "secondary"
											}
											className={`flex-1 ${
												formData.modelTier === tier
								...
								<div>
									...
									<select
										...
										onChange={(e) =>
											setFormData((prev) => ({
												...prev,
												workspaceAccess: e.target.value as any,
											}))
										}
										className="w-full bg-surface-1 text-foreground border border-border rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary/50"
									>
								...
								<div>
									...
									<select
										...
										onChange={(e) =>
											setFormData((prev) => ({
												...prev,
												dockerNetwork: e.target.value as any,
											}))
										}
										className="w-full bg-surface-1 text-foreground border border-border rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary/50"
									>
										...
										<div className="grid grid-cols-2 gap-2 text-sm">
											...
											<div>
												...
												<span className="text-foreground">
													{selectedTemplateData?.label || t("custom")}
												</span>
											</div>
											<div>
												<span className="text-muted-foreground">
												...
												<span
													className={`px-2 py-0.5 rounded text-xs ${MODEL_TIER_COLORS[formData.modelTier]}`}
												>
											...
											<div>
												...
												<span className="text-foreground">
													{selectedTemplateData?.toolCount || t("custom")}
												</span>
											</div>
									...
									<label className="flex items-center gap-2 cursor-pointer">
										<input
											...
											onChange={(e) =>
												setFormData((prev) => ({
													...prev,
													write_to_gateway: e.target.checked,
												}))
											}
...
export function ConfigTab({
	...
	const [workspaceDocs, setWorkspaceDocs] = useState<
		Array<{ name: string; exists: boolean; content: string }>
	>([]);
	...
	useEffect(() => {
		const loadWorkspaceDocs = async () => {
			...
			try {
				...
				const entries = Object.entries(payload?.files || {}).map(
	...
	const updateModelConfig = (
		updater: (current: { primary?: string; fallbacks?: string[] }) => {
			primary?: string;
			fallbacks?: string[];
		},
	) => {
	...
	const sandboxNetwork =
		...
		sandbox.network ||
		sandbox.dockerNetwork ||
		sandbox.docker_network ||
		"none";
			...
			{showJson ? (
				...
				<div className="space-y-4">
									...
									<div className="space-y-2">
										{modelFallbacks.map((fallback: string, index: number) => (
											<div key={`${fallback}-${index}`} className="flex gap-2">
												...
												<Button
													onClick={() => {
														const next = modelFallbacks.filter(
									...
									<div className="mt-1">
										...
										<div className="flex flex-wrap gap-1 mt-1">
											{modelFallbacks.map((fb: string, i: number) => (
												<span
													key={i}
													className="px-2 py-0.5 text-xs bg-surface-2 rounded text-muted-foreground font-mono"
												>
													{fb.split("/").pop()}
												</span>
											))}
							...
							<div className="space-y-3">
								<div className="grid grid-cols-3 gap-3">
									<div>
										...
										<input
											value={identityEmoji}
											onChange={(e) =>
												updateIdentityField("emoji", e.target.value)
											}
									...
									<div>
										...
										<input
											value={identity.name || ""}
											onChange={(e) =>
												updateIdentityField("name", e.target.value)
											}
									...
									<div>
										...
										<input
											...
											onChange={(e) =>
												updateIdentityField("theme", e.target.value)
											}
											className="w-full bg-surface-1 text-foreground rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary/50"
								...
								<div>
									...
									<textarea
										...
										onChange={(e) =>
											updateIdentityField("content", e.target.value)
										}
										rows={4}
										className="w-full bg-surface-1 text-foreground border border-border rounded-md px-3 py-2 font-mono text-xs focus:outline-none focus:ring-1 focus:ring-primary/50"
					...
					<div className="bg-surface-1/50 rounded-lg p-4 space-y-4">
						...
						<div className="space-y-2">
							<label className="text-xs text-muted-foreground font-medium">
								{t("otherMarkdownFiles")}
							</label>
							{loadingWorkspaceDocs ? (
								<div className="text-xs text-muted-foreground">
									{t("loadingWorkspaceFiles")}
								</div>
							) : (
								<div className="space-y-2">
									{workspaceDocs
										.filter(
											(doc) => !["identity.md", "agent.md"].includes(doc.name),
										)
										.map((doc) => (
											<div key={doc.name} className="bg-surface-1 rounded p-3">
												<div className="flex items-center justify-between mb-1">
													...
													<span
														className={`text-2xs ${doc.exists ? "text-green-400" : "text-muted-foreground"}`}
													>
														{doc.exists
															? t("chars", { count: doc.content.length })
															: t("missing")}
													</span>
												</div>
							...
							<div className="grid grid-cols-3 gap-3">
								...
								<div>
									...
									<select
										...
										onChange={(e) =>
											updateSandboxField("workspaceAccess", e.target.value)
										}
										className="w-full bg-surface-1 text-foreground rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary/50"
									>
										<option value="">{t("notConfigured")}</option>
								...
								<div>
									...
									<input
										...
										onChange={(e) =>
											updateSandboxField("network", e.target.value)
										}
										className="w-full bg-surface-1 text-foreground rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary/50"
										placeholder={t("none")}
									/>
					...
					<div className="bg-surface-1/50 rounded-lg p-4">
						...
						{editing ? (
							<div className="space-y-3">
								<div>
									<label className="block text-xs text-green-400 font-medium mb-1">
										{t("allowList")}
									</label>
								...
								<div>
									<label className="block text-xs text-red-400 font-medium mb-1">
										{t("denyList")}
									</label>
									<div className="flex flex-wrap gap-1 mb-2">
										{toolDeny.map((tool: string, i: number) => (
											<span
												key={`${tool}-${i}`}
												className="px-2 py-0.5 text-xs bg-red-500/10 text-red-400 rounded border border-red-500/20 flex items-center gap-1"
											>
												{tool}
												<Button
													onClick={() => removeTool("deny", i)}
													variant="ghost"
													size="icon-xs"
								...
								{toolAllow.length > 0 && (
									<div className="mb-2">
										<span className="text-xs text-green-400 font-medium">
											{t("allowCount", { count: toolAllow.length })}:
										</span>
										<div className="flex flex-wrap gap-1 mt-1">
											{toolAllow.map((tool: string) => (
												<span
													key={tool}
													className="px-2 py-0.5 text-xs bg-green-500/10 text-green-400 rounded border border-green-500/20"
												>
													{tool}
												</span>
											))}
								...
								{toolDeny.length > 0 && (
									<div>
										<span className="text-xs text-red-400 font-medium">
											{t("denyCount", { count: toolDeny.length })}:
										</span>
										<div className="flex flex-wrap gap-1 mt-1">
					...
					<div className="bg-surface-1/50 rounded-lg p-4">
						...
						{editing ? (
									...
									<input
										...
										onKeyDown={(e) => {
											if (e.key === "Enter") {
												...
												setConfig((prev: any) => {
													const sa = { ...(prev.subagents || {}) };
													const existing = Array.isArray(sa.allowAgents)
														? sa.allowAgents
														: [];
									...
									<Button
										...
										onClick={(e) => {
											...
											setConfig((prev: any) => {
												...
												return {
													...prev,
													subagents: { ...sa, allowAgents: [...existing, val] },
												};
											});
											input.value = "";
										}}
							...
							<>
								...
								{subagents.model && (
									<div className="text-xs text-muted-foreground mt-1">
										{t("modelLabel")}: {subagents.model}
									</div>
								)}
							</>
						) : (
							<div className="text-xs text-muted-foreground">
								{t("noSubAgentsConfigured")}
							</div>
						)}
					</div>
...
export function FilesTab({ agent }: { agent: Agent }) {
	...
	const loadFiles = async () => {
		...
		try {
			...
			const entries = Object.entries(data.files || {}).map(
				([name, value]: [string, any]) => ({
					name,
					exists: Boolean(value?.exists),
					content: String(value?.content || ""),
						...
						<div className="space-y-3">
							<div className="flex justify-between items-center">
								<div>
									...
									{!activeEntry.exists && (
										<span className="ml-2 px-1.5 py-0.5 text-2xs bg-amber-500/20 text-amber-400 rounded">
											{t("missing")}
										</span>
									)}
								</div>
...
export function ToolsTab({ agent }: { agent: Agent }) {
	...
	const isDirty =
		JSON.stringify(allowList) !== JSON.stringify(toolAllow) ||
		...
		<div className="p-5 space-y-4">
			...
			<div className="bg-surface-1/50 rounded-lg p-4">
				<h5 className="text-sm font-medium text-green-400 mb-2">
					{t("allowListCount", { count: allowList.length })}
				</h5>
				<div className="flex flex-wrap gap-1 mb-3">
					{allowList.map((tool, i) => (
						...
						>
							{tool}
							<button
								onClick={() => removeFromList(allowList, setAllowList, i)}
								className="text-green-400/60 hover:text-green-400 ml-0.5"
							>
								x
							</button>
						</span>
			...
			<div className="bg-surface-1/50 rounded-lg p-4">
				<h5 className="text-sm font-medium text-cyan-400 mb-2">
					{t("alsoAllowCount", { count: alsoAllowList.length })}
				</h5>
				...
				<div className="flex flex-wrap gap-1 mb-3">
					{alsoAllowList.map((tool, i) => (
						<span
							key={`${tool}-${i}`}
							className="px-2 py-0.5 text-xs bg-cyan-500/10 text-cyan-400 rounded border border-cyan-500/20 flex items-center gap-1"
						>
							{tool}
							<button
								onClick={() =>
									removeFromList(alsoAllowList, setAlsoAllowList, i)
								}
			...
			<div className="bg-surface-1/50 rounded-lg p-4">
				<h5 className="text-sm font-medium text-red-400 mb-2">
					{t("denyListCount", { count: denyList.length })}
				</h5>
				<div className="flex flex-wrap gap-1 mb-3">
					{denyList.map((tool, i) => (
						<span
						...
						>
							{tool}
							<button
								onClick={() => removeFromList(denyList, setDenyList, i)}
...
export function ChannelsTab({ agent }: { agent: Agent }) {
	...
	const loadChannels = async () => {
		...
		try {
			...
			const channelMeta: Array<{ id: string; label?: string }> =
				snapshot.channelMeta || [];
			const channelAccounts: Record<string, ChannelAccountInfo[]> =
				...
				<div className="space-y-2">
					{channels.map((channel) => {
						...
						return (
							<div
								key={channel.id}
								className="bg-surface-1/50 rounded-lg p-4 flex items-center justify-between"
							>
								<div>
								...
								<div className="flex gap-3 text-xs text-muted-foreground">
									<span>
										{total > 0
											? t("connectedOf", { connected, total })
											: t("noAccounts")}
									</span>
									<span>
										{configured > 0
											? t("configuredCount", { count: configured })
											: t("notConfigured")}
									</span>
									<span className={enabled > 0 ? "text-green-400" : ""}>
										{total > 0
											? t("enabledCount", { count: enabled })
											: t("disabled")}
									</span>
								</div>
							</div>
		...
		<div className="p-5 space-y-4">
			...
			{agentJobs.length === 0 ? (
				<div className="text-muted-foreground text-sm py-8 text-center">
					{showAll
						? t("noCronJobsFound")
						: t("noCronJobsAssigned", { agent: agent.name })}
				</div>
			) : (
				<div className="space-y-2">
					{agentJobs.map((job) => (
						<div key={job.name} className="bg-surface-1/50 rounded-lg p-4">
							<div className="flex items-start justify-between">
								...
								<div className="text-right text-xs text-muted-foreground space-y-1">
									<div>
										{t("last")}: {formatTime(job.lastRun)}
									</div>
									<div>
										{t("next")}: {formatTime(job.nextRun)}
									</div>
...
export function ModelsTab({ agent }: { agent: Agent }) {
	...
	const modelFallbacks: string[] = Array.isArray(modelCfg.fallbacks)
		? modelCfg.fallbacks
		: [];
	...
	const isDirty =
		...
		JSON.stringify(fallbacks) !== JSON.stringify(modelFallbacks);
	...
	return (
		<div className="p-5 space-y-4">
			...
			<div className="bg-surface-1/50 rounded-lg p-4">
				<h5 className="text-sm font-medium text-foreground mb-2">
					{t("fallbackChainCount", { count: fallbacks.length })}
				</h5>
				...
				{fallbacks.length === 0 ? (
					...
					<div className="space-y-1 mb-3">
						{fallbacks.map((fb, i) => (
							<div
								key={`${fb}-${i}`}
								className="flex items-center gap-2 bg-surface-1 rounded px-3 py-1.5"
							>
