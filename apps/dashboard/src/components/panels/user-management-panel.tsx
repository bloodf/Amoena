...
export function UserManagementPanel() {
	...
	const [reviewingRequestId, setReviewingRequestId] = useState<number | null>(
		null,
	);
	...
	const handleDelete = async (u: UserRecord) => {
		...
		try {
			const res = await fetch(`/api/auth/users?id=${u.id}`, {
				method: "DELETE",
			});
			const data = await res.json().catch(() => ({}));
			if (res.ok) {
	...
	const submitReview = async (
		...
		try {
			...
			const data = await res.json().catch(() => ({}));
			if (!res.ok)
				throw new Error(data.error || t("failedToAction", { action }));
			const req = requests.find((r) => r.id === requestId);
			showFeedback(
				true,
				t("requestActioned", { action, email: req?.email || t("user") }),
			);
	...
	return (
		<div className="p-6 max-w-5xl mx-auto space-y-5">
			...
			{feedback && (
				<div
					className={`px-3 py-2 rounded-md text-sm border ${feedback.ok ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-red-500/10 text-red-400 border-red-500/20"}`}
				>
					{feedback.text}
				</div>
			)}
						...
						<table className="w-full text-sm">
							<thead>
								<tr className="bg-secondary/40 border-b border-border">
									...
									<th className="text-right px-3 py-2 text-xs text-muted-foreground">
										{t("action")}
									</th>
								</tr>
							</thead>
							<tbody>
								{pendingRequests.map((req) => (
									...
									>
										...
										<td className="px-3 py-2 text-xs text-muted-foreground">
											{formatDate(req.last_attempt_at)}
										</td>
				...
				<div className="p-4 rounded-lg bg-secondary/50 border border-border space-y-3">
					...
					<div className="grid grid-cols-2 gap-3">
						...
						<select
							...
							onChange={(e) =>
								setCreateForm((f) => ({ ...f, role: e.target.value as any }))
							}
							className="h-9 px-3 rounded-md bg-secondary border border-border text-sm text-foreground"
						>
							<option value="viewer">{t("roleViewer")}</option>
					...
					<div className="flex justify-end">
						<Button
							...
							disabled={
								!createForm.username || !createForm.password || creating
							}
							size="sm"
						>
				...
				<table className="w-full">
					...
					<tbody>
						{users.map((u) => (
							<tr
								key={u.id}
								className="border-b border-border/50 last:border-0 hover:bg-secondary/20 transition-smooth"
							>
								{editingId === u.id ? (
									<>
										...
										<td className="px-4 py-2.5 hidden md:table-cell">
											<input
												...
												className="h-8 px-2 rounded bg-secondary border border-border text-sm text-foreground w-full"
												disabled={(u.provider || "local") !== "local"}
											/>
										</td>
										<td className="px-4 py-2.5 text-right space-x-2">
											<Button onClick={handleEdit} disabled={saving} size="xs">
												{t("save")}
											</Button>
											<Button
									...
									<>
										...
										<td className="px-4 py-2.5 text-xs">
											<span
												className={`px-2 py-0.5 rounded-full ${u.provider === "google" ? "bg-blue-500/20 text-blue-300" : "bg-gray-500/20 text-gray-300"}`}
											>
												{u.provider || "local"}
											</span>
										</td>
