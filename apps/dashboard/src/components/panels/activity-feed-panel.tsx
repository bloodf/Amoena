...
export function ActivityFeedPanel() {
		...
		async (since?: number) => {
			try {
				...
				if (since && !isAgentView)
					params.append("since", Math.floor(since / 1000).toString());
				...
				if (since && !isAgentView) {
					setActivities((prev) => {
						...
						const uniqueNew = newActivities.filter(
							(a: Activity) => !existingIds.has(a.id),
						);
	...
	useSmartPoll(pollActivities, 30000, {
		enabled: autoRefresh,
	...
	const agentSessions = sessions.filter(
		(s) => selectedAgent && s.key.includes(selectedAgent),
	);
				...
				{loading && activities.length === 0 ? (
					...
					<div className="flex flex-col items-center justify-center h-32 text-muted-foreground/50">
						...
						<p className="text-xs mt-1">
							{selectedAgent
								? t("noActivityForAgent", { agent: selectedAgent })
					...
					<span>
						{isAgentView
							...
							: t("footerShowing", {
									...
									filtered: filter.type ? ` ${t("filtered")}` : "",
								})}
					</span>
					<span>
						{t("lastUpdated", {
