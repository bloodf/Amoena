...
export function ExecApprovalPanel() {
	...
	const pendingCount = execApprovals.filter(
		(a) => a.status === "pending",
	).length;
	...
	const handleAction = (
		...
		updateExecApproval(id, {
			status: newStatus as ExecApprovalRequest["status"],
		});
	};
				...
				<>
					...
					<div className="flex gap-1 mb-4">
						{(["all", "pending", "resolved"] as const).map((tab) => (
							...
							>
								{t(
									`filter${tab.charAt(0).toUpperCase() + tab.slice(1)}` as
										| "filterAll"
										| "filterPending"
										| "filterResolved",
								)}
...
function AllowlistEditor({
	...
	const updatePattern = (agentId: string, index: number, value: string) => {
		setAgents((prev) => ({
			...prev,
			[agentId]: prev[agentId].map((p, i) =>
				i === index ? { pattern: value } : p,
			),
		}));
		setDirty(true);
	};
	...
	const recentCommands = useMemo(() => {
		return execApprovals
			...
			.map((a) => ({
				command: a.command!,
				agentName: a.agentName || a.sessionId,
			}));
	}, [execApprovals]);
...
function ApprovalCard({
	...
	return (
		<div
			className={`rounded-lg border border-border bg-card p-4 border-l-4 ${riskBorder}`}
		>
			{/* Header row */}
			...
			{(approval.cwd || approval.host || approval.resolvedPath) && (
				<div className="text-xs text-muted-foreground mb-2 space-y-0.5">
					{approval.host && (
						<div>
							Host:{" "}
							<span className="font-mono text-foreground">{approval.host}</span>
						</div>
					)}
					{approval.cwd && (
						<div>
							CWD:{" "}
							<span className="font-mono text-foreground">{approval.cwd}</span>
						</div>
					)}
