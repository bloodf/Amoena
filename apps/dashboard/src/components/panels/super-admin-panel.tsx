...
export function SuperAdminPanel() {
	...
	const [localJobEvents, setLocalJobEvents] = useState<
		Record<number, ProvisionEvent[]>
	>({});
	...
	const [decommissionDialog, setDecommissionDialog] =
		useState<DecommissionDialogState>({
			open: false,
			tenant: null,
			dryRun: true,
	...
	const load = useCallback(async () => {
		try {
			const [tenantsRes, jobsRes, gatewaysRes, schedulerRes] =
				await Promise.all([
			...
			const schedulerJson = schedulerRes
				? await schedulerRes.json().catch(() => ({}))
				: {};

			if (!tenantsRes.ok)
				throw new Error(tenantsJson?.error || "Failed to load tenants");
			if (!jobsRes.ok)
				throw new Error(jobsJson?.error || "Failed to load provision jobs");

			let tenantRows = Array.isArray(tenantsJson?.tenants)
				? tenantsJson.tenants
				: [];
			let jobRows = Array.isArray(jobsJson?.jobs) ? jobsJson.jobs : [];
			const gatewayRows = Array.isArray(gatewaysJson?.gateways)
				? gatewaysJson.gateways
				: [];
			const schedulerTasks: SchedulerTask[] = Array.isArray(
				schedulerJson?.tasks,
			)
				? schedulerJson.tasks
				: [];
			...
			if (isLocal) {
				if (tenantRows.length === 0) {
					const primaryGateway = gatewayRows.find(
						(gw: any) => Number(gw?.is_primary) === 1,
					);
				...
				if (jobRows.length === 0 && schedulerTasks.length > 0) {
					jobRows = schedulerTasks.map((task, index) => {
						...
						const lastRunSec = task.lastRun
							? Math.floor(task.lastRun / 1000)
							: null;
						return {
							id,
							tenant_id: -1,
							...
							error_text:
								task.lastResult?.ok === false ? task.lastResult.message : null,
			...
			setGatewayLoadError(
				gatewaysRes.ok
					? null
					: gatewaysJson?.error || "Failed to load gateways",
			);
			setError(null);
		} catch (e: any) {
	...
	const loadJobDetail = useCallback(
		async (jobId: number) => {
			...
			try {
				...
				setSelectedJobEvents(
					Array.isArray(json?.job?.events) ? json.job.events : [],
				);
				setActiveTab("events");
			} catch (e: any) {
		...
		[isLocal, localJobEvents, showFeedback],
	);
	...
	const filteredTenants = useMemo(() => {
		...
		return tenants.filter((tenant) => {
			if (tenantStatusFilter !== "all" && tenant.status !== tenantStatusFilter)
				return false;
			if (!q) return true;
			return [
				tenant.display_name,
	...
	const tenantPages = Math.max(
		1,
		Math.ceil(filteredTenants.length / TENANT_PAGE_SIZE),
	);
	...
	const kpis = useMemo(() => {
		...
		const pending = tenants.filter((t) =>
			["pending", "provisioning", "decommissioning"].includes(t.status),
		).length;
		...
		try {
			const res = await fetch("/api/super/tenants", {
				...
				body: JSON.stringify({
					...
					gateway_port: form.gateway_port
						? Number(form.gateway_port)
						: undefined,
					dashboard_port: form.dashboard_port
	...
	const runJob = async (jobId: number) => {
		setBusyJobId(jobId);
		try {
			const res = await fetch(`/api/super/provision-jobs/${jobId}/run`, {
				method: "POST",
			});
			const json = await res.json().catch(() => ({}));
	...
	const approveAndRunJob = async (jobId: number) => {
		...
		try {
			...
			if (!approveRes.ok)
				throw new Error(
					approveJson?.error || `Failed to approve job #${jobId}`,
				);

			const runRes = await fetch(`/api/super/provision-jobs/${jobId}/run`, {
				method: "POST",
			});
			const runJson = await runRes.json().catch(() => ({}));
			if (!runRes.ok)
				throw new Error(runJson?.error || `Failed to run job #${jobId}`);
	...
	const queueDecommissionFromDialog = async () => {
		...
		if (
			...
			decommissionDialog.confirmText.trim() !== tenant.slug
		) {
		...
		try {
			...
			const json = await res.json().catch(() => ({}));
			if (!res.ok)
				throw new Error(json?.error || "Failed to queue decommission job");

			const jobId = Number(json?.job?.id || 0);
			showFeedback(
				true,
				decommissionDialog.dryRun
					? t("decommissionQueuedDryRun", { slug: tenant.slug })
					: t("decommissionQueued", { slug: tenant.slug }),
			);
			...
			showFeedback(
				false,
				e?.message || `Failed to queue decommission for ${tenant.slug}`,
			);
		}
	};
	...
	const canSubmitDecommission =
		!!decommissionDialog.tenant &&
		(decommissionDialog.dryRun ||
		...
		<div className="p-6 max-w-7xl mx-auto space-y-5">
			<div className="flex items-start justify-between gap-4">
				...
				<div className="flex items-center gap-2">
					<Button size="sm" onClick={() => setCreateExpanded(true)}>
						{t("addWorkspace")}
					</Button>
					...
					<div className="p-4 space-y-3">
						...
						{gatewayLoadError && (
							<div className="px-3 py-2 rounded-md text-xs border bg-amber-500/10 text-amber-300 border-amber-500/20">
								Gateway list unavailable: {gatewayLoadError}. Using fallback
								owner value.
							</div>
						)}
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
							...
							<input
								...
								onChange={(e) =>
									setForm((f) => ({ ...f, linux_user: e.target.value }))
								}
							...
							<select
								...
								onChange={(e) =>
									setForm((f) => ({ ...f, owner_gateway: e.target.value }))
								}
								className="h-9 px-3 rounded-md bg-secondary border border-border text-sm text-foreground"
							>
							...
							<input
								...
								onChange={(e) =>
									setForm((f) => ({ ...f, dashboard_port: e.target.value }))
								}
							...
							<Button onClick={createTenant}>{t("createAndQueue")}</Button>
						</div>
					</div>
				</div>
			)}
					...
					<div className="p-3 space-y-3">
							...
							<table className="w-full text-sm">
								...
								<thead>
									<tr className="bg-secondary/30 border-b border-border">
										<th
											scope="col"
											className="text-left px-3 py-2 text-xs text-muted-foreground"
										>
											{t("colTenant")}
										</th>
										<th
										...
										<th
											scope="col"
											className="text-right px-3 py-2 text-xs text-muted-foreground"
										>
											{t("colAction")}
										</th>
									</tr>
												...
												<td className="px-3 py-2 text-xs">
													{latest ? (
														<Button
															variant="link"
															size="xs"
															onClick={() => loadJobDetail(latest.id)}
						...
						<div className="flex items-center justify-end gap-2 text-xs">
							...
							<Button
								...
								onClick={() =>
									setTenantPage((p) => Math.min(tenantPages, p + 1))
								}
							>
								{t("next")}
							</Button>
								...
								<tbody>
									{pagedJobs.map((job) => {
										...
										return (
											<tr
												key={job.id}
												className={`border-b border-border/50 last:border-0 ${selectedJobId === job.id ? "bg-primary/10" : "hover:bg-secondary/20"}`}
											>
												<td className="px-3 py-2">
													<Button
														variant="link"
													...
													<div className="text-[11px] text-muted-foreground">
														{job.job_type}{" "}
														{job.dry_run ? t("dryRun") : t("live")}
													</div>
												</td>
												<td className="px-3 py-2 text-muted-foreground text-xs">
													{job.tenant_slug || job.tenant_id}
												</td>
															...
															{openActionMenu === menuKey && (
																<div className="absolute right-3 top-10 z-20 w-40 rounded-md border border-border bg-card shadow-xl text-left">
																	...
																	<Button
																		...
																		size="sm"
																		onClick={() =>
																			Number(job.dry_run) === 1
																				? approveAndRunJob(job.id)
																				: setJobState(job.id, "approve")
																		}
																		disabled={
																			busyJobId === job.id ||
																			![
				...
				{activeTab === "events" && (
					<div className="p-3 space-y-2">
						<div className="text-xs text-muted-foreground px-1">
							{selectedJobId
								? t("showingEventsForJob", { jobId: selectedJobId })
								: t("selectJobForEvents")}
						</div>
						<div className="max-h-[420px] overflow-y-auto space-y-2">
							...
							{selectedJobEvents.map((ev) => (
								<div
									key={ev.id}
									className="rounded border border-border/60 bg-secondary/20 px-3 py-2"
								>
					...
					<div className="w-full max-w-2xl rounded-lg border border-border bg-card shadow-xl">
						<div className="px-4 py-3 border-b border-border">
							<h3 className="text-sm font-semibold text-foreground">
								{t("queueDecommissionTitle", {
									name: decommissionDialog.tenant.display_name,
								})}
							</h3>
						...
						<div className="p-4 space-y-4">
									...
									<input
										...
										onChange={(e) =>
											setDecommissionDialog((prev) => ({
												...
												removeStateDirs: e.target.checked
													? false
													: prev.removeStateDirs,
											}))
										}
									/>
									<span>
							...
							<div className="rounded-md border border-border bg-secondary/20 p-3 text-xs text-foreground">
								...
								<ul className="space-y-1 text-muted-foreground">
									<li>
										•{" "}
										{t("impactStopsService", {
											user: decommissionDialog.tenant.linux_user,
										})}
									</li>
									<li>
										•{" "}
									...
									<li>
										•{" "}
										{decommissionDialog.removeLinuxUser
											? t("impactLinuxUserRemoved")
											: decommissionDialog.removeStateDirs
												? t("impactStateDirsRemoved")
												: t("impactRetained")}
									</li>
								</ul>
							</div>
									...
									<div>
										<label className="block text-xs text-muted-foreground mb-1">
											...
											<span className="font-mono text-foreground">
												{decommissionDialog.tenant.slug}
											</span>{" "}
											{t("toConfirmLive")}
										</label>
										<input
						...
						<div className="px-4 py-3 border-t border-border flex items-center justify-end gap-2">
							...
							<Button
								...
								disabled={
									!canSubmitDecommission || decommissionDialog.submitting
								}
							...
							>
								{decommissionDialog.submitting
									? t("queueing")
									: decommissionDialog.dryRun
										? t("queueDryRunDecommission")
										: t("queueLiveDecommission")}
							</Button>
						</div>
					</div>
				</div>
