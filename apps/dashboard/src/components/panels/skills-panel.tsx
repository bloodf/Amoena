...
export function SkillsPanel() {
	...
	const [selectedContent, setSelectedContent] =
		useState<SkillContentResponse | null>(null);
	...
	const filtered = useMemo(() => {
		...
		return list.filter((skill) => {
			const haystack =
				`${skill.name} ${skill.source} ${skill.description || ""}`.toLowerCase();
	...
	useEffect(() => {
		...
		async function run() {
			...
			try {
				...
				if (!cancelled)
					setDrawerError(err?.message || "Failed to load SKILL.md");
			} finally {
	...
	const saveSkill = async () => {
		...
		try {
			...
			setSelectedContent((prev) =>
				prev ? { ...prev, content: draftContent } : prev,
			);
		} catch (err: any) {
		...
		try {
			...
			const stepTimer = setTimeout(() => {
				setInstallModal((prev) =>
					prev?.slug === slug ? { ...prev, step: "scanning" } : prev,
				);
			}, 800);
			const writeTimer = setTimeout(() => {
				setInstallModal((prev) =>
					prev?.slug === slug ? { ...prev, step: "writing" } : prev,
				);
			}, 1600);
			...
			if (!res.ok) {
				...
				setInstallModal({
					...
					name: displayName,
					step: "done",
					message: body?.message || "Installed successfully",
					securityStatus: body?.securityReport?.status,
				});
				await loadSkills();
			}
	...
	const securityBadge = (status?: string | null) => {
		if (!status || status === "unchecked")
			return (
				<span className="text-2xs text-muted-foreground/50">unchecked</span>
			);
		if (status === "clean")
			return <span className="text-2xs text-emerald-400">clean</span>;
		if (status === "warning")
			return <span className="text-2xs text-amber-400">warning</span>;
		if (status === "rejected")
			return <span className="text-2xs text-rose-400">rejected</span>;
		return null;
	};
			...
			<div className="flex flex-wrap items-end justify-between gap-3">
				<div>
					...
					<p className="text-xs text-muted-foreground mt-0.5">
						{t("subtitle")}{" "}
						{dashboardMode === "local" ? t("localMode") : t("gatewayMode")}.
					</p>
				</div>
			...
			{installMessage && (
				<div
					className={`rounded-lg border px-4 py-2 text-xs ${
						...
						installMessage.startsWith("Install error")
									...
									<div className="space-y-1.5">
										...
										<div className="h-1.5 rounded-full bg-secondary overflow-hidden">
											<div
												...
												style={{
													width: `${(scanAll.done / scanAll.total) * 100}%`,
												}}
											/>
										</div>
									</div>
									...
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-3 text-2xs">
											<span className="text-emerald-400">
												{scanAll.results.clean} clean
											</span>
											{scanAll.results.warning > 0 && (
												<span className="text-amber-400">
													{scanAll.results.warning} warning
												</span>
											)}
											{scanAll.results.rejected > 0 && (
												<span className="text-rose-400">
													{scanAll.results.rejected} rejected
												</span>
											)}
											{scanAll.results.error > 0 && (
												<span className="text-destructive">
													{scanAll.results.error} errors
												</span>
											)}
						...
						<div className="grid grid-cols-1 md:grid-cols-[240px_1fr_auto] gap-2">
							...
							>
								<option value="user-agents">
									{SOURCE_LABELS["user-agents"]}
								</option>
								<option value="user-codex">
									{SOURCE_LABELS["user-codex"]}
								</option>
								<option value="project-agents">
									{SOURCE_LABELS["project-agents"]}
								</option>
								<option value="project-codex">
								...
								{(skillGroups || [])
									...
									.map((group) => (
										<button
											...
											onClick={() =>
												setActiveRoot(
													activeRoot === group.source ? null : group.source,
												)
											}
									...
									<div className="divide-y divide-border">
										{filtered.map((skill) => (
											<div key={skill.id} className="px-4 py-3">
												...
												<p className="mt-1 text-2xs text-muted-foreground/70 break-all">
													{skill.path}
												</p>
											</div>
										))}
			...
			{activeTab === "registry" && (
				<>
					...
					{registryResults.length > 0 ? (
						<div className="rounded-lg border border-border bg-card overflow-hidden">
							<div className="px-4 py-3 border-b border-border text-xs text-muted-foreground">
								{registryResults.length} results from{" "}
								{
									{
										clawhub: "ClawdHub",
										"skills-sh": "skills.sh",
										...
										{skill.tags && skill.tags.length > 0 && (
											<div className="mt-1.5 flex flex-wrap gap-1">
												{skill.tags.slice(0, 5).map((tag) => (
													<span
														key={tag}
														className="text-2xs rounded-full bg-secondary/50 border border-border px-1.5 py-0.5 text-muted-foreground"
													>
														{tag}
													</span>
												))}
								...
								<div className="px-5 pb-5 space-y-3">
									...
									<div className="space-y-2">
										<InstallStep
											...
											status={
												installModal.step === "fetching"
													? "active"
													: installModal.step === "error" &&
															!installModal.securityStatus
														? "error"
														: "done"
											}
										/>
										<InstallStep
											...
											status={
												installModal.step === "fetching"
													? "pending"
													: installModal.step === "scanning"
														? "active"
														: installModal.step === "error" &&
																installModal.securityStatus === "rejected"
															? "error"
															: installModal.step === "error" &&
																	!installModal.securityStatus
																? "error"
										...
										<InstallStep
											label={t("stepWriting")}
											status={
												["fetching", "scanning"].includes(installModal.step)
													? "pending"
													: installModal.step === "writing"
														? "active"
														: installModal.step === "error"
															? "error"
															: "done"
									...
									{installModal.securityStatus &&
										installModal.step === "done" && (
											<div className="flex items-center gap-2 text-xs">
												...
												<span
													className={
														installModal.securityStatus === "clean"
															? "text-emerald-400"
															: installModal.securityStatus === "warning"
																? "text-amber-400"
																: "text-rose-400"
													}
												>
								...
								{drawerLoading ? (
									...
									<>
										{selectedContent.security &&
											selectedContent.security.issues.length > 0 && (
												<div
													className={`mx-4 mt-3 rounded-lg border p-3 text-xs ${
												...
												>
													...
													{selectedContent.security.issues.map((issue, i) => (
														...
														>
															<span
																className={`mt-0.5 text-2xs font-mono ${
