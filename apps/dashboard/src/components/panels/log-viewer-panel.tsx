...
export function LogViewerPanel() {
				...
				const params = new URLSearchParams({
					...
					...(tail &&
						currentLogs.length > 0 && {
							since: currentLogs[0]?.timestamp.toString(),
						}),
				});
				...
				if (data.logs && data.logs.length > 0) {
					if (tail) {
						...
						const existingIds = new Set(
							(currentLogs || []).map((l: any) => l?.id).filter(Boolean),
						);
						...
						log.debug(
							`Clearing existing logs and loading ${data.logs.length} logs`,
						);
	...
	const filteredLogs = logs.filter((entry) => {
		...
		if (
			logFilters.search &&
			!entry.message.toLowerCase().includes(logFilters.search.toLowerCase())
		)
			return false;
		if (
			logFilters.session &&
			(!entry.session || !entry.session.includes(logFilters.session))
		)
			return false;
		return true;
	});
	...
	// Debug logging
	log.debug(
		`Store has ${logs.length} logs, filtered to ${filteredLogs.length}`,
	);
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
					...
					<div>
						...
						<input
							...
							onChange={(e) =>
								handleFilterChange({ session: e.target.value || undefined })
							}
								...
								<div className="flex items-start justify-between">
									<div className="flex-1 min-w-0">
										<div className="flex items-center space-x-2 text-xs">
											...
											<span
												className={`font-medium uppercase ${getLogLevelColor(log.level)}`}
											>
												{log.level}
											</span>
											<span className="text-muted-foreground">
