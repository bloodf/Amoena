...
export function CostTrackerPanel() {
	...
	}, [timeframe, timeframeToDays]);

	const loadSessionCosts = useCallback(async () => {
		try {
			const res = await fetch(
				`/api/tokens?action=session-costs&timeframe=${timeframe}`,
			);
			...
			if (Array.isArray(data?.sessions)) {
				...
				setSessionCosts(
					Object.entries(usageStats.sessions).map(([id, stats]) => ({
						sessionId: id,
						model: "",
	...
	const exportData = async (format: "json" | "csv") => {
		...
		try {
			const res = await fetch(
				`/api/tokens?action=export&timeframe=${timeframe}&format=${format}`,
			);
	...
	// Derived data
	const _summary = usageStats?.summary;
				...
				<div className="flex items-center justify-between flex-wrap gap-3">
					...
					<div className="flex items-center gap-3">
						...
						<div className="flex rounded-lg border border-border overflow-hidden">
							{(["overview", "agents", "sessions", "tasks"] as const).map(
								(v) => (
									<button
										key={v}
										onClick={() => setView(v)}
						...
						<div className="bg-secondary rounded-lg p-4">
							...
							<div className="text-xs text-muted-foreground">
								{stats.summary.totalCost > 0
									? (
											(potentialSavings / stats.summary.totalCost) *
											100
										).toFixed(1)
									: "0"}
								% {t("savingsPossible")}
					...
					<div className="space-y-2">
						{modelData.map((m) => {
							...
							const maxCostPer1k = Math.max(
								...modelData.map(
									(d) => (d.cost / Math.max(1, d.tokens)) * 1000,
								),
								0.0001,
							);
							return (
					...
					{agents.map((agent) => {
						...
						return (
							...
							>
								<Button
									onClick={() =>
										setExpandedAgent(isExpanded ? null : agent.agent)
									}
									variant="ghost"
								...
								>
									<div className="flex items-center gap-3 min-w-0">
										...
										<span className="text-xs px-2 py-0.5 rounded-full bg-secondary text-muted-foreground shrink-0">
											{agent.session_count} session
											{agent.session_count !== 1 ? "s" : ""}
										</span>
										<span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 shrink-0">
											{agent.request_count} req
											{agent.request_count !== 1 ? "s" : ""}
										</span>
										{agentTasks.length > 0 && (
						...
						>
							<div className="flex items-center justify-between">
								...
								<div className="text-right shrink-0 ml-3">
									...
									<div className="text-xs text-muted-foreground">
										{formatNumber(task.stats.totalTokens)} {t("tokens")} |{" "}
										{task.stats.requestCount} {t("reqs")}
									</div>
								</div>
							</div>
						</div>
