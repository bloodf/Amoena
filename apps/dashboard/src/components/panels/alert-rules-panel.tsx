		...
		>
			<div className="flex items-start justify-between gap-3">
				<div className="flex-1 min-w-0">
					<div className="flex items-center gap-2">
						<span
							className={`text-2xs px-1.5 py-0.5 rounded border ${ENTITY_COLORS[rule.entity_type] || "bg-muted text-muted-foreground border-border"}`}
						>
							{rule.entity_type}
						</span>
					...
					<div className="flex items-center gap-3 mt-2 text-2xs text-muted-foreground flex-wrap">
						<span className="font-mono bg-secondary/50 px-1.5 py-0.5 rounded">
							...
							{operator?.label || rule.condition_operator}{" "}
					...
					<select
						...
						onChange={(e) =>
							setForm({
								...
								condition_field: ENTITY_FIELDS[e.target.value]?.[0] || "status",
							})
						}
				...
				<div>
					...
					<input
						...
						onChange={(e) =>
							setForm({
								...form,
								cooldown_minutes: parseInt(e.target.value, 10) || 60,
							})
						}
						min={1}
						className="w-full h-8 px-2.5 rounded-md bg-secondary border border-border text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
