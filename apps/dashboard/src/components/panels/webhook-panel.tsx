...
const AVAILABLE_EVENTS = [
	{ value: "*", label: "All events", description: "Receive all event types" },
	{
		value: "agent.error",
		label: "Agent error",
		description: "Agent enters error state",
	},
	{
		value: "agent.status_change",
		label: "Agent status change",
		description: "Any agent status transition",
	},
	{
		value: "security.login_failed",
		label: "Login failed",
		description: "Failed login attempt",
	},
	{
		value: "security.user_created",
		label: "User created",
		description: "New user account created",
	},
	{
		value: "security.user_deleted",
		label: "User deleted",
		description: "User account deleted",
	},
	{
		value: "security.password_change",
		label: "Password changed",
		description: "User password modified",
	},
	{
		value: "notification.mention",
		label: "Mention",
		description: "Agent was @mentioned",
	},
	{
		value: "notification.assignment",
		label: "Assignment",
		description: "Task assigned to agent",
	},
	{
		value: "activity.task_created",
		label: "Task created",
		description: "New task added",
	},
	{
		value: "activity.task_updated",
		label: "Task updated",
...
export function WebhookPanel() {
	...
	const fetchWebhookAutomations = useCallback(async () => {
		...
		try {
			...
			const webhookTasks = tasks.filter(
				(task: SchedulerTask) =>
	...
	async function handleRunAutomation(taskId: string) {
		...
		try {
			...
			setTestResult({
				success: false,
				error: "Failed to run local automation",
			});
		} finally {
		...
		<div className="p-5 space-y-4">
			...
			{newSecret && (
				<div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 space-y-2">
					...
					<Button variant="link" size="xs" onClick={() => setNewSecret(null)}>
						{t("dismiss")}
					</Button>
				</div>
			)}
					...
					<div className="rounded-lg border border-cyan-500/30 bg-cyan-500/5 p-3">
						...
						<div className="space-y-2">
							{webhookAutomations.map((task) => (
								<div
									key={task.id}
									className="rounded border border-cyan-500/20 bg-background/30 p-2.5"
								>
									<div className="flex items-center justify-between gap-2">
										<div className="min-w-0">
											<div className="flex items-center gap-2">
												<span
													className={`w-2 h-2 rounded-full ${task.running ? "bg-blue-400" : task.enabled ? "bg-green-500" : "bg-muted-foreground/40"}`}
												/>
												<span className="text-xs font-medium text-foreground truncate">
													{task.name}
												</span>
												<span className="px-1.5 py-0.5 text-[10px] rounded bg-cyan-500/15 text-cyan-300 font-mono">
													{task.id}
												</span>
											</div>
											<div className="text-2xs text-muted-foreground mt-1">
												{task.nextRun
													? t("nextRun", {
															time: formatTime(task.nextRun / 1000),
														})
													: t("noNextRun")}
						...
						>
							<div className="flex items-start justify-between gap-3">
								...
								>
									<div className="flex items-center gap-2">
										<span
											className={`w-2 h-2 rounded-full ${wh.enabled ? "bg-green-500" : "bg-muted-foreground/30"}`}
										/>
									...
									<div className="flex items-center gap-3 mt-1.5 text-2xs text-muted-foreground">
										<span>
											{wh.events.includes("*")
												? t("allEvents")
												: t("eventCount", { count: wh.events.length })}
										</span>
										<span>
											{t("deliveries", { count: wh.total_deliveries })}
										</span>
										{wh.failed_deliveries > 0 && (
											<span className="text-red-400">
												{t("failed", { count: wh.failed_deliveries })}
											</span>
										)}
										{wh.last_fired_at && (
											<span>
												{t("lastFired", { time: formatTime(wh.last_fired_at) })}
											</span>
										)}
									</div>
								</div>
									...
									{deliveries.length === 0 ? (
										...
										<div className="space-y-1 max-h-60 overflow-y-auto">
											{deliveries.map((d) => (
												<div
													key={d.id}
													className="flex items-center gap-2 text-2xs py-1 px-2 rounded hover:bg-secondary/50"
												>
													<span
				...
				<div className="flex flex-wrap gap-1.5">
					{AVAILABLE_EVENTS.map((ev) => (
						<Button
							...
							type="button"
							variant={
								selectedEvents.includes(ev.value) ? "default" : "secondary"
							}
							size="xs"
