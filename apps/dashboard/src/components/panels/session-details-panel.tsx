...
export function SessionDetailsPanel() {
	...
	const [controllingSession, setControllingSession] = useState<string | null>(
		null,
	);
	const [sessionFilter, setSessionFilter] = useState<"all" | "active" | "idle">(
		"all",
	);
	...
	const parseTokenUsage = (tokenString: string) => {
		...
		const total =
			parseFloat(match[3]) *
			(match[4] === "k" ? 1000 : match[4] === "m" ? 1000000 : 1);
	...
	const filteredSessions = sessions.filter((session) => {
		...
		// Unknown type filter
		if (!includeUnknown && getSessionType(session.key) === "Unknown")
			return false;
	...
	const sortedSessions = [...filteredSessions].sort((a, b) => {
		switch (sortBy) {
			case "tokens":
				return (
					parseTokenUsage(b.tokens).percentage -
					parseTokenUsage(a.tokens).percentage
				);
			case "model":
			...
			<div className="bg-card border border-border rounded-lg p-4">
				<div className="flex flex-wrap items-end gap-4">
					...
					<div className="ml-auto text-sm text-muted-foreground pb-0.5">
						...
						•{" "}
						{t("activeCount", {
							count: sessions.filter((s) => s.active).length,
						})}
					</div>
				</div>
			</div>
										...
										{isExpanded && (
											<div className="pt-4 border-t border-border space-y-4">
												...
												<div className="flex space-x-2">
													...
													>
														{controllingSession === `monitor-${session.id}`
															? t("working")
															: t("monitor")}
													</Button>
													<Button
													...
													>
														{controllingSession === `pause-${session.id}`
															? t("working")
															: t("pause")}
													</Button>
													<Button
														...
														onClick={async (e) => {
															e.stopPropagation();
															if (!window.confirm(t("confirmTerminate")))
																return;
													...
													>
														{controllingSession === `terminate-${session.id}`
															? t("working")
															: t("terminate")}
													</Button>
													...
													{confirmingDelete === session.key ? (
														...
														>
															...
															>
																{controllingSession === `delete-${session.key}`
																	? "..."
																	: t("yes")}
															</Button>
															<Button
					...
					{sessions.some((s) => parseTokenUsage(s.tokens).percentage > 80) && (
						<div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4">
							...
							<div className="text-sm text-muted-foreground">
								{t("highTokenUsageDesc", {
									count: sessions.filter(
										(s) => parseTokenUsage(s.tokens).percentage > 80,
									).length,
								})}
							</div>
						</div>
