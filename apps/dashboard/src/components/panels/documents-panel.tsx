...
export function DocumentsPanel() {
	...
	useEffect(() => {
		...
		const handle = setTimeout(async () => {
			...
			try {
				const res = await fetch(
					`/api/docs/search?q=${encodeURIComponent(query)}&limit=100`,
				);
		...
		<div className="h-full p-4 md:p-6">
			<div className="h-full min-h-[600px] rounded-xl border border-border bg-card overflow-hidden grid grid-cols-1 lg:grid-cols-[340px_1fr]">
				<aside className="border-r border-border p-4 space-y-3 overflow-y-auto">
					...
					{treeError && <div className="text-sm text-red-400">{treeError}</div>}
				...
				<section className="p-4 md:p-6 overflow-y-auto">
					...
					{selectedPath && (
						<div className="space-y-3">
							<div className="rounded-md border border-border bg-secondary/30 px-3 py-2">
								...
								{docMeta && (
									<div className="mt-1 text-xs text-muted-foreground">
										{formatBytes(docMeta.size)} • {t("updated")}{" "}
										{formatTime(docMeta.modified)}
									</div>
								)}
							</div>
							...
							{docError && (
								<div className="text-sm text-red-400">{docError}</div>
							)}
