			...
			if (prev && data.network.length > 0) {
				...
				if (deltaSec > 0) {
					...
					for (const iface of data.network) {
						const prevIface = prev.network.find(
							(p) => p.interface === iface.interface,
						);
						if (prevIface) {
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
				...
				<section className="rounded-xl border border-border bg-card p-4">
					...
					<div className="text-xs text-muted-foreground mb-2">
						{latest.cpu.cores} cores &middot; Load:{" "}
						{latest.cpu.loadAvg.map((l) => l.toFixed(2)).join(", ")}
					</div>
					<div className="h-40">
				...
				<section className="rounded-xl border border-border bg-card p-4">
					...
					<div className="text-xs text-muted-foreground mb-2">
						{formatBytes(latest.memory.usedBytes)} /{" "}
						{formatBytes(latest.memory.totalBytes)}
						{latest.memory.swapTotalBytes > 0 && (
							<>
								{" "}
								&middot; Swap: {formatBytes(latest.memory.swapUsedBytes)} /{" "}
								{formatBytes(latest.memory.swapTotalBytes)}
							</>
						)}
					</div>
					<div className="h-40">
								...
								<div key={d.mountpoint}>
									<div className="flex items-center justify-between text-xs mb-1">
										...
										<span className="tabular-nums">
											{d.usagePercent}% &middot; {formatBytes(d.usedBytes)} /{" "}
						...
						<>
							<div className="text-xs text-muted-foreground mb-2">
								...
								{latest.gpu[0].memoryTotalMB > 0 && (
									<>
										{" "}
										&middot; {latest.gpu[0].memoryUsedMB} MB /{" "}
										{latest.gpu[0].memoryTotalMB} MB
									</>
								)}
							</div>
							{latest.gpu[0].memoryTotalMB > 0 &&
							latest.gpu[0].memoryUsedMB > 0 ? (
								<div className="h-40">
									<ResponsiveContainer width="100%" height="100%">
										<AreaChart
											data={history}
					...
					{latest.processes.length === 0 ? (
						...
						<div className="space-y-0">
							...
							{latest.processes.map((p) => (
								<div
									key={p.pid}
									className="flex items-center text-xs py-1 border-b border-border/50 last:border-0"
								>
									<span
									...
									<span
										className={`w-14 text-right tabular-nums font-mono ${p.cpuPercent >= 50 ? "text-red-400" : p.cpuPercent >= 10 ? "text-amber-400" : ""}`}
									>
										{p.cpuPercent.toFixed(1)}%
									</span>
									<span
										className={`w-14 text-right tabular-nums font-mono ${p.memPercent >= 50 ? "text-red-400" : p.memPercent >= 10 ? "text-amber-400" : ""}`}
									>
					...
					<div className="flex items-center justify-between mb-3">
						...
						{history.length > 0 && (
							<div className="text-right">
								<span className="text-xs text-muted-foreground">
									RX {formatRate(history[history.length - 1].netRxRate)} / TX{" "}
									{formatRate(history[history.length - 1].netTxRate)}
								</span>
							</div>
						)}
					</div>
