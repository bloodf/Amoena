...
const STATUS_COLUMN_KEYS = [
	...
	{
		key: "assigned",
		titleKey: "colAssigned",
		color: "bg-blue-500/20 text-blue-400",
	},
	{
		key: "awaiting_owner",
		titleKey: "colAwaitingOwner",
		color: "bg-orange-500/20 text-orange-400",
	},
	{
		key: "in_progress",
		titleKey: "colInProgress",
...
function useAgentSessions(agentName: string | undefined) {
	...
	useEffect(() => {
		...
		fetch("/api/sessions")
			...
			.then((data) => {
				if (cancelled) return;
				const all = (data.sessions || []) as Array<{
					key: string;
					id: string;
					agent?: string;
					channel?: string;
...
function MentionTextarea({
	...
	const filtered = mentionTargets
		.filter((target) => {
			...
			return (
				target.handle.includes(q) || target.display.toLowerCase().includes(q)
			);
		})
		.slice(0, 8);
	...
	useEffect(() => {
		...
		const estimatedMenuHeight = Math.min(
			Math.max(filtered.length, 1) * 46 + 12,
			224,
		);
...
function DunkItButton({
	taskId,
	onDunked,
}: {
	taskId: number;
	onDunked: (id: number) => void;
}) {
...
export function TaskBoardPanel() {
	...
	const [gnapStatus, setGnapStatus] = useState<{
		enabled: boolean;
		taskCount?: number;
		lastSync?: string;
	} | null>(null);
	...
	const selectedTaskIdFromUrl = Number.parseInt(
	...
	const fetchData = useCallback(async () => {
		try {
			...
			const tasksUrl = tasksQuery.toString()
				? `/api/tasks?${tasksQuery.toString()}`
				: "/api/tasks";

			const [tasksResponse, agentsResponse, projectsResponse] =
				await Promise.all([
					fetch(tasksUrl),
					fetch("/api/agents"),
					fetch("/api/projects"),
			...
			if (taskIds.length > 0) {
				fetch(`/api/quality-review?taskIds=${taskIds.join(",")}`)
					.then((reviewResponse) =>
						reviewResponse.ok ? reviewResponse.json() : null,
					)
					.then((reviewData) => {
	...
	const handleGnapSync = useCallback(async () => {
		...
		try {
			...
			if (res.ok) {
				...
				setGnapStatus((prev) =>
					prev
						? { ...prev, taskCount: data.pushed, lastSync: data.lastSync }
						: prev,
				);
			}
		} catch {
	...
	const tasksByStatus = statusColumns.reduce(
		(acc, column) => {
			acc[column.key] = tasks.filter((task) => {
				const effectiveStatus = detectAwaitingOwner(task)
					? "awaiting_owner"
					: task.status;
		...
		try {
			if (newStatus === "done") {
				const reviewResponse = await fetch(
					`/api/quality-review?taskId=${draggedTask.id}`,
				);
				if (!reviewResponse.ok) {
					throw new Error("Unable to verify Aegis approval");
				}
				const reviewData = await reviewResponse.json();
				const latest = reviewData.reviews?.find(
					(review: any) => review.reviewer === "aegis",
				);
						...
						>
							...
							<div className="flex-1 p-3 space-y-3">
								{Array.from({ length: 3 - colIdx }).map((_, cardIdx) => (
									<div
										key={cardIdx}
										className="bg-surface-1 rounded-lg p-3 border-l-4 border-border space-y-2 animate-pulse"
									>
	...
	return (
		<div className="h-full flex flex-col">
							...
							<div className="flex gap-2">
								...
								<input
									...
									onChange={(e) =>
										setSpawnFormData((prev) => ({
											...prev,
											timeoutSeconds: parseInt(e.target.value, 10) || 300,
										}))
									}
								...
								<Button
									...
									disabled={
										isSpawning ||
										!spawnFormData.task.trim() ||
										!spawnFormData.label.trim()
									}
									size="sm"
								>
			...
			{error && (
				<div
					role="alert"
					className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 m-4 rounded-lg text-sm flex items-center justify-between"
				>
					<span>{error}</span>
					<Button
						variant="ghost"
			...
			{/* Kanban Board */}
			<div
				className="flex-1 min-h-0 flex gap-4 p-4 overflow-x-auto"
				role="region"
								...
								>
												...
												<div className="flex items-center gap-1.5 shrink-0">
													{task.metadata?.recurrence?.enabled && (
														<span
															...
															title={
																task.metadata.recurrence.natural_text ||
																task.metadata.recurrence.cron_expr
															}
														>
															{t("recurring")}
														</span>
													...
													{task.metadata?.recurrence?.parent_task_id && (
														<span
															...
															title={t("spawnedFromTask", {
																id: task.metadata.recurrence.parent_task_id,
															})}
														>
															{t("spawned")}
														</span>
									...
									{task.due_date && (
										<div className="mt-1.5 ml-5.5 text-[10px]">
											...
											>
												{task.due_date * 1000 < Date.now() ? "! " : ""}
												{t("due")} {formatTaskTimestamp(task.due_date)}
											</span>
										</div>
									)}
								</div>
...
function TaskDetailModal({
	...
	const [activeTab, setActiveTab] = useState<
		"details" | "comments" | "quality" | "session"
	>("details");
	...
	const renderComment = (comment: Comment, depth: number = 0) => {
		...
		return (
			<div
				key={comment.id}
				className={`border-l-2 border-border pl-3 ${depth > 0 ? "ml-4" : ""}`}
			>
				...
				<div className="text-sm text-foreground/90 mt-1 whitespace-pre-wrap">
					{text}
				</div>
				...
				<div className="p-6">
					<div className="flex justify-between items-start mb-4">
						...
						<div className="flex gap-2">
							...
							<Button
								...
								size="sm"
								onClick={async () => {
									if (!confirm(t("deleteTaskConfirm", { title: task.title })))
										return;
									try {
							...
							{task.metadata?.dispatch_session_id && (
								<>
									...
									<div className="col-span-2">
										...
										>
											View Session{" "}
											{task.metadata.dispatch_session_id.slice(0, 8)}...
										</Button>
										{task.status === "in_progress" && (
					...
					{activeTab === "comments" && (
						...
						>
							...
							<form onSubmit={handleAddComment} className="mt-4 space-y-3">
								...
								<div>
									...
									<p className="text-[11px] text-muted-foreground mt-1">
										Use <span className="font-mono">@</span> to mention users
										and agents.
									</p>
								</div>
							...
							<div className="mt-5 bg-blue-500/5 border border-blue-500/15 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
								...
								<div>
									<strong className="text-foreground">Comments</strong> are
									persisted on the task and notify all subscribers. Subscribers
									are auto-added when they: create the task, are assigned to it,
									comment on it, or are @mentioned.
								</div>
								<div>
									<strong className="text-foreground">Broadcasts</strong> send a
									one-time notification to all current subscribers without
					...
					{activeTab === "quality" && (
						<div
							id="tabpanel-quality"
							role="tabpanel"
							aria-label={t("tabQualityReview")}
							className="mt-6"
						>
							<h5 className="text-sm font-medium text-foreground mb-2">
								{t("aegisQualityReview")}
							</h5>
							{reviewError && (
							...
							{reviews.length > 0 ? (
								<div className="space-y-2 mb-3">
									{reviews.map((review) => (
										<div
											key={review.id}
											className="text-xs text-foreground/80 bg-surface-1/40 rounded p-2"
										>
											...
											{review.notes && (
												<div className="mt-1">{review.notes}</div>
											)}
										</div>
									))}
								</div>
...
function ClaudeCodeTasksSection() {
	...
	const [data, setData] = useState<{ teams: any[]; tasks: any[] }>({
		teams: [],
		tasks: [],
	});
						...
						Object.entries(tasksByTeam).map(([team, tasks]) => (
							<div key={team}>
								<div className="flex items-center gap-2 mb-2">
									...
									{data.teams.find((t) => t.name === team)?.members?.length >
										0 && (
								...
								<div className="space-y-1">
									{tasks.map((task: any) => (
										...
										>
											<span
												className={`text-[10px] font-mono ${statusColor(task.status)}`}
											>
												{task.status}
											</span>
				...
				<div className="p-4 border-t border-border space-y-2">
					{!loaded ? (
						...
						data.cronJobs.map((job: any) => (
							...
							>
								<span
									className={`text-[10px] font-mono shrink-0 ${job.enabled ? "text-purple-400" : "text-muted-foreground/50"}`}
								>
								...
								<span
									className={`text-[10px] px-1.5 py-0.5 rounded ${job.enabled ? "bg-green-500/15 text-green-400" : "bg-muted text-muted-foreground"}`}
								>
				...
				<form onSubmit={handleSubmit} className="p-6">
					...
					<div className="space-y-4">
						...
						<div>
							...
							/>
							<p className="text-[11px] text-muted-foreground mt-1">
								Tip: type <span className="font-mono">@</span> for mention
								autocomplete.
							</p>
						</div>
						...
						{formData.assigned_to && agentSessions.length > 0 && (
							<div>
								...
								<p className="text-[11px] text-muted-foreground mt-1">
									Send task to an existing agent session instead of creating a
									new one.
								</p>
							</div>
						)}
									...
									{parsedSchedule && (
										<p className="text-xs text-cyan-400 mt-1">
											...
											<span className="text-muted-foreground font-mono">
												({parsedSchedule.cronExpr})
											</span>
										</p>
									)}
									{scheduleError && (
										<p className="text-xs text-red-400 mt-1">
											{scheduleError}. Try: &quot;daily at 9am&quot;,
											&quot;every 2 hours&quot;, &quot;weekly on monday&quot;
										</p>
									)}
								</div>
							)}
					...
					<div className="flex gap-3 mt-6">
						<Button
							type="submit"
							className="flex-1"
							disabled={isRecurring && !parsedSchedule}
						>
