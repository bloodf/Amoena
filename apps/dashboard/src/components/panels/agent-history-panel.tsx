			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-xs text-muted-foreground mt-0.5">
						{t("eventCount", {
							count: total,
							agent: selectedAgent || t("noAgentSelected"),
						})}
					</p>
				</div>
			</div>
									...
									{agentSessions.map((s) => (
										<div key={s.id} className="text-xs space-y-0.5">
											<div className="flex items-center gap-1.5">
												...
												/>
												<span className="font-mono-tight text-foreground truncate">
													{s.kind}
												</span>
											</div>
									...
									<div key={day}>
										...
										<div className="space-y-1 pl-2 border-l-2 border-border/50">
											{dayActivities.map((act) => (
												<div
													key={act.id}
													className="flex items-start gap-2.5 pl-3 py-1.5 hover:bg-secondary/30 rounded-r-lg transition-smooth relative"
												>
													{/* Timeline dot */}
													<span
														className={`absolute -left-[5px] top-3 w-2 h-2 rounded-full bg-card border-2 ${
															act.type === "agent_status_change"
																? "border-yellow-400"
																: act.type.startsWith("task")
																	? "border-blue-400"
													...
													{/* Icon */}
													<span
														className={`w-5 h-5 rounded bg-secondary flex items-center justify-center text-2xs font-mono font-bold shrink-0 ${typeColors[act.type] || "text-muted-foreground"}`}
													>
														{typeIcons[act.type] || "?"}
													</span>
													...
													<div className="flex-1 min-w-0">
														...
														{act.entity?.title && (
															<p className="text-2xs text-muted-foreground mt-0.5 truncate">
																{act.entity.type === "task"
																	? `Task: ${act.entity.title}`
																	: act.entity.title}
															</p>
														)}
													</div>
													...
													<span className="text-2xs text-muted-foreground font-mono-tight shrink-0">
														{new Date(act.created_at * 1000).toLocaleTimeString(
															undefined,
