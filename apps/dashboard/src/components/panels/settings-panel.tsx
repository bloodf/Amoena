...
function parseCoordinatorTargetAgents(
	rawAgents: any[],
): CoordinatorTargetAgent[] {
...
> = {
	...
	security: {
		label: "Security",
		icon: "🔑",
		description: "API key management and security settings",
	},
	retention: {
		label: "Data Retention",
		icon: "🗄",
		description: "How long data is kept before cleanup",
	},
	chat: {
		label: "Chat",
		icon: "💬",
		description: "Coordinator routing and chat behavior settings",
	},
	gateway: {
		label: "Gateway",
		icon: "🔌",
	...
	profiles: {
		...
		icon: "shield",
		description: "Hook profile controls security scanning strictness",
	},
...
const subscriptionDropdowns: Record<
	string,
...
export function SettingsPanel() {
	...
	const [coordinatorTargetAgents, setCoordinatorTargetAgents] = useState<
		CoordinatorTargetAgent[]
	>([]);
	const [coordinatorSessions, setCoordinatorSessions] = useState<
		CoordinatorSession[]
	>([]);
	...
	const [_hermesLoading, _setHermesLoading] = useState(false);
	...
	const getCoordinatorResolutionPreview = useCallback(
		(configuredTarget: string) => {
			const allAgents: CoordinatorAgentRecord[] = coordinatorTargetAgents.map(
				(agent) => ({
					name: agent.name,
			...
			const directAgent =
				allAgents.find(
					(agent) => agent.name.toLowerCase() === COORDINATOR_AGENT,
				) || null;
			const gatewaySessions = coordinatorSessions.filter(
				(session) => (session.source || "gateway") === "gateway",
			);
	...
	const fetchSettings = useCallback(async () => {
		try {
			...
			const hpSetting = (data.settings || []).find(
				(s: Setting) => s.key === "hook_profile",
			);
			...
			try {
				...
				if (agentsRes.ok) {
					...
					setCoordinatorTargetAgents(
						parseCoordinatorTargetAgents(agentsData.agents || []),
					);
				}
			} catch {
				// non-critical
			...
			try {
				...
				if (sessionsRes.ok) {
					...
					const mapped: CoordinatorSession[] = Array.isArray(
						...
						? sessionsData.sessions
								.map((session: any) => ({
									...
									updatedAt: Number(
										session?.lastActivity || session?.startTime || 0,
									),
								...
								.filter(
									(session: CoordinatorSession) => session.key && session.agent,
								)
						: [];
	...
	const handleSave = async () => {
		...
		try {
			...
			if (res.ok) {
				showFeedback(
					true,
					`Saved ${data.count} setting${data.count === 1 ? "" : "s"}`,
				);
				setEdits({});
				fetchSettings();
			} else {
	...
	const categories = categoryOrder.filter(
		(c) => c === "security" || c === "profiles" || grouped[c]?.length > 0,
	);
		...
		<div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
					...
					<div className="flex items-center gap-3 p-3 bg-surface-1/50 border border-border/30 rounded-lg">
							...
							onClick={async () => {
								...
								try {
									...
									if (res.ok) {
										showFeedback(
											true,
											`MC backup created (${(data.backup?.size / 1024).toFixed(0)} KB)`,
										);
									} else {
						...
						<Button
							...
							onClick={async () => {
								...
								try {
									const res = await fetch("/api/backup?target=gateway", {
										method: "POST",
									});
									const data = await res.json();
									if (res.ok) {
										showFeedback(
											true,
											`Gateway backup created: ${data.output}`,
										);
									} else {
						...
						<Button
							...
							onClick={async () => {
								...
								try {
									...
									showFeedback(
										true,
										"Onboarding reset — wizard will appear on next page load",
									);
								} catch {
							...
							<div className="flex items-center justify-between">
								...
								<Button
									...
									onClick={async () => {
										...
										const action = hermesStatus.hookInstalled
											? "uninstall-hook"
											: "install-hook";
										try {
											const res = await fetch("/api/hermes", {
											...
											if (res.ok) {
												showFeedback(
													true,
													data.message ||
														`Hook ${action === "install-hook" ? "installed" : "uninstalled"}`,
												);
												fetchHermesStatus();
											} else {
												showFeedback(
													false,
													data.error || "Hook operation failed",
												);
											}
										} catch {
											showFeedback(false, "Network error");
			...
			<div className="flex gap-1 border-b border-border pb-px">
				{categories.map((cat) => {
					...
					const changedCount = (grouped[cat] || []).filter(
						(s) => edits[s.key] !== undefined && edits[s.key] !== s.value,
					).length;
					return (
						<Button
							key={cat}
					...
					<div className="bg-card border border-border rounded-lg p-4">
						<div className="flex items-start justify-between gap-4">
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs text-muted-foreground mt-0.5">
									Used for programmatic access and agent authentication via
						...
						<div className="mt-3 flex items-center gap-2">
							<code className="text-xs font-mono bg-background border border-border rounded px-2 py-1 text-muted-foreground">
								{apiKeyLoading
									? "Loading..."
									: apiKeyInfo?.masked_key || "No API key configured"}
							</code>
						</div>
						...
						{apiKeyInfo?.last_rotated_at && (
							<div className="text-2xs text-muted-foreground/50 mt-2">
								...
								{new Date(
									apiKeyInfo.last_rotated_at * 1000,
								).toLocaleDateString()}{" "}
								at{" "}
								{new Date(
									apiKeyInfo.last_rotated_at * 1000,
						...
						{!rotateConfirm ? (
							...
							<div className="mt-3 bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
								<p className="text-xs text-amber-300 mb-2">
									Are you sure? Rotating the API key will immediately invalidate
									the current key. All agents and integrations using the old key
									will lose access.
								</p>
								<div className="flex items-center gap-2">
									<Button
						...
						{newApiKey && (
							<div className="mt-3 bg-green-500/10 border border-green-500/20 rounded-lg p-3">
								<p className="text-xs text-green-300 mb-2 font-medium">
									New API key generated. Copy it now -- it will not be shown
									again.
								</p>
								<div className="flex items-center gap-2">
				...
				<div className="space-y-3">
					<div className="bg-card border border-border rounded-lg p-4">
						...
						<p className="text-xs text-muted-foreground mb-4">
							Controls how aggressively security hooks scan tool calls and agent
							outputs.
						</p>
									...
									onClick={async () => {
										...
										try {
											...
											if (res.ok) {
												showFeedback(
													true,
													`Hook profile set to ${profile.label}`,
												);
											} else {
			...
			{activeCategory === "general" && <InterfaceModeSelector />}
			...
			<div className="space-y-3">
				{activeCategory !== "security" &&
					(grouped[activeCategory] || []).map((setting) => {
						...
						const coordinatorTargetOptions =
							setting.key === "chat.coordinator_target_agent"
								? [
										{
											label: "Auto (default/main-session fallback)",
											value: "",
										},
						...
						const dropdownOptions =
							coordinatorTargetOptions || subscriptionDropdowns[setting.key];
							...
							>
								<div className="flex items-start justify-between gap-4">
									<div className="flex-1 min-w-0">
										<div className="flex items-center gap-2">
											...
											{isChanged && (
												<span className="text-2xs px-1.5 py-0.5 rounded bg-primary/15 text-primary">
													modified
												</span>
											)}
										</div>
										...
										<p className="text-2xs text-muted-foreground/60 mt-1 font-mono">
											{setting.key}
										</p>
									</div>
									...
									<div className="flex flex-col items-end gap-1 shrink-0">
										<div className="flex items-center gap-2">
											{dropdownOptions ? (
												<select
													value={currentValue}
													onChange={(e) =>
														handleEdit(setting.key, e.target.value)
													}
										...
										{coordinatorPreview && (
											<p className="text-2xs text-muted-foreground max-w-72 text-right">
												{coordinatorPreview}
											</p>
										)}
									</div>
								...
								{setting.updated_by && setting.updated_at && (
									<div className="text-2xs text-muted-foreground/50 mt-2">
										...
										{new Date(setting.updated_at * 1000).toLocaleDateString()}
									</div>
								)}
							</div>
						);
			...
			{hasChanges && (
				<div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-card border border-border rounded-lg shadow-lg px-4 py-2.5 flex items-center gap-3 z-40">
					...
					<Button onClick={handleDiscard} variant="ghost" size="xs">
						{t("discard")}
					</Button>
				...
				{[
					{
						...
						label: "Essential",
						desc: "Focused view with core panels only — Overview, Agents, Tasks, Chat, Activity, Logs, Settings.",
					},
					{
						value: "full" as const,
						label: "Full",
						desc: "All panels and advanced features including Memory, Cron, Webhooks, Alerts, Audit, and more.",
					},
				].map((option) => (
					<button
						key={option.value}
	...
	const handleDisconnect = async () => {
		...
		try {
			...
			if (res.ok) {
				setFeedback({
					ok: true,
					text: "Google account disconnected. You can now sign in with username and password.",
				});
	...
	return (
		<div className="space-y-3">
			...
			<div className="bg-card border border-border rounded-lg p-4">
				...
				{feedback && (
					<div
						className={`mt-3 rounded-md p-2.5 text-xs font-medium ${
							feedback.ok
								? "bg-green-500/10 text-green-400"
								: "bg-destructive/10 text-destructive"
						}`}
