...
export function PipelineTab() {
	...
	const fetchData = useCallback(async () => {
		const [tRes, pRes, rRes] = await Promise.all([
			...
			fetch("/api/pipelines")
				.then((r) => r.json())
				.catch(() => ({ pipelines: [] })),
			fetch("/api/pipelines/run?limit=10")
				.then((r) => r.json())
				.catch(() => ({ runs: [] })),
		]);
	...
	const addStep = (templateId: number) => {
		...
		setFormSteps((s) => [
			...s,
			{ template_id: templateId, template_name: t.name, on_failure: "stop" },
		]);
	};
	...
	const savePipeline = async () => {
		...
		try {
			...
			if (res.ok) {
				...
				setResult({
					ok: true,
					text: formMode === "edit" ? "Pipeline updated" : "Pipeline created",
				});
			} else {
	...
	const runPipeline = async (id: number) => {
		...
		try {
			...
			if (res.ok) {
				setResult({
					ok: true,
					text: `Pipeline started (run #${data.run?.id})`,
				});
				fetchData();
			} else {
	...
	return (
		<div className="space-y-3">
			...
			{result && (
				<div
					className={`text-xs px-2 py-1 rounded ${result.ok ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}
				>
					{result.text}
				</div>
			)}
			...
			<div className="flex items-center justify-between">
				<span className="text-xs text-muted-foreground">
					{t("pipelineCount", { count: pipelines.length })}
				</span>
				<Button
					onClick={() =>
						formMode !== "hidden" ? closeForm() : setFormMode("create")
					}
					variant="link"
					size="xs"
				>
			...
			{formMode !== "hidden" && (
				<div className="p-3 rounded-lg bg-secondary/50 border border-border space-y-2">
					<span className="text-xs font-medium">
						{formMode === "edit" ? t("editPipeline") : t("newPipeline")}
					</span>
					<input
						value={formName}
					...
					<div className="space-y-1">
						...
						{formSteps.map((step, i) => (
							<div
								key={i}
								className="flex items-center gap-1.5 p-1.5 rounded bg-secondary/80 text-xs"
							>
								...
								<span className="flex-1 truncate text-foreground">
									{step.template_name || `Template #${step.template_id}`}
								</span>
								<select
										...
										setFormSteps((s) =>
											s.map((st, idx) =>
												idx === i
													? {
															...st,
															on_failure: e.target.value as "stop" | "continue",
														}
													: st,
											),
										)
						...
						<select
							onChange={(e) => {
								if (e.target.value) {
									addStep(parseInt(e.target.value, 10));
									e.target.value = "";
								}
							}}
			...
			{pipelines.length === 0 && formMode === "hidden" ? (
				...
				<div className="space-y-1.5 max-h-64 overflow-y-auto">
					{pipelines.map((p) => (
						<div
							key={p.id}
							className="rounded-md bg-secondary/30 hover:bg-secondary/50 transition-smooth group"
						>
											...
											<div key={i} className="flex items-center gap-0.5">
												...
												{i < p.steps.length - 1 && (
													<svg
														...
														stroke="currentColor"
														strokeWidth="1.5"
														className="w-2.5 h-2.5 text-muted-foreground/50 shrink-0"
													>
														<path
															d="M2 4h4M5 2l2 2-2 2"
															strokeLinecap="round"
							...
							{expandedId === p.id && (
								<div className="px-3 pb-3 border-t border-border/50 mt-1 pt-2 space-y-3">
									...
									<div>
										<span className="text-2xs text-muted-foreground">
											Runs: {p.runs.total} total, {p.runs.completed} completed,{" "}
											{p.runs.failed} failed
										</span>
										{runs
											...
											.map((run) => (
												<div
													key={run.id}
													className="mt-1 p-2 rounded bg-secondary/50 text-xs"
												>
													...
													{run.status === "running" && (
														<div className="flex gap-1 mt-1.5">
															<Button
																onClick={() => advanceRun(run.id, true)}
																variant="success"
																size="xs"
																className="bg-green-500/20 text-green-400 hover:bg-green-500/30 h-6 text-2xs"
															>
																Mark Step Done
															</Button>
															<Button
																onClick={() => advanceRun(run.id, false)}
																variant="destructive"
																size="xs"
																className="bg-red-500/20 text-red-400 hover:bg-red-500/30 h-6 text-2xs"
															>
																Mark Step Failed
															</Button>
															<Button
				...
				<div key={i} className="flex items-center gap-1 shrink-0">
					...
					{i < steps.length - 1 && (
						<svg
							viewBox="0 0 20 12"
							fill="none"
							className="w-5 h-3 text-muted-foreground/60 shrink-0"
						>
							<path
			...
			{steps.map((s, i) => (
				<div key={i} className="flex items-center gap-1 shrink-0">
					...
					{i < steps.length - 1 && (
						<svg
							viewBox="0 0 8 8"
							className="w-2 h-2 text-muted-foreground/40 shrink-0"
						>
							<path
...
function RunStatusBadge({ status }: { status: string }) {
	...
	return (
		<span
			className={`text-2xs px-1.5 py-0.5 rounded-full ${styles[status] || "bg-secondary text-muted-foreground"}`}
		>
			{status}
		</span>
	);
		...
		<div className="p-2.5 rounded-lg border border-amber-500/30 bg-amber-500/5">
			<div className="flex items-center justify-between mb-1.5">
				<div className="flex items-center gap-2">
					...
					<span className="text-xs font-medium text-foreground">
						{run.pipeline_name || `Pipeline #${run.pipeline_id}`} — Run #
						{run.id}
					</span>
				</div>
				<span className="text-2xs text-muted-foreground">
			...
			<div className="flex gap-1 mt-2">
				...
				<Button
					...
					variant="secondary"
					size="xs"
					className="h-6 text-2xs ml-auto"
				>
					Cancel
				</Button>
			</div>
