...
interface TaskCostsResponse {
	...
	agents: Record<
		string,
		{ stats: TokenStats; taskCount: number; taskIds: number[] }
	>;
		...
		<div className="space-y-6">
			...
			<div className="bg-card border border-border rounded-lg p-6">
				...
				<div className="space-y-2 max-h-[600px] overflow-y-auto">
					{agents.map((agent) => {
						const costShare =
							(agent.total_cost / Math.max(summary.total_cost, 0.0001)) * 100;
						...
						return (
							...
							>
								<Button
									onClick={() =>
										setExpandedRow(isExpanded ? null : agent.agent)
									}
									variant="ghost"
...
export function AgentCostPanel() {
	...
	const [expandedSection, setExpandedSection] = useState<"models" | "tasks">(
		"tasks",
	);
	const [activeView, setActiveView] = useState<"overview" | "per-agent">(
		"overview",
	);
	...
	const loadData = useCallback(async () => {
		...
		try {
			const [agentRes, taskRes, byAgentRes] = await Promise.all([
				...
				fetch(
					`/api/tokens/by-agent?days=${timeframeToDays(selectedTimeframe)}`,
				),
			]);
	...
	}, [selectedTimeframe, timeframeToDays]);
	...
	const getAgentTasks = useCallback(
		(agentName: string): TaskCostEntry[] => {
			...
			return taskData.tasks.filter((t) =>
				agentEntry.taskIds.includes(t.taskId),
			);
		},
		[taskData],
	);
	...
	const sortedAgents = agents.sort(
		([, a], [, b]) => b.stats.totalCost - a.stats.totalCost,
	);
	...
	const mostEfficient =
		agents.length > 0
			? agents.reduce((best, curr) => {
					const currCostPer1k =
						(curr[1].stats.totalCost / Math.max(1, curr[1].stats.totalTokens)) *
						1000;
					const bestCostPer1k =
						(best[1].stats.totalCost / Math.max(1, best[1].stats.totalTokens)) *
						1000;
	...
	const maxCostPer1k = Math.max(
		...efficiencyData.map((d) => d.costPer1k),
		0.0001,
	);
					...
					<div className="flex items-center gap-4">
						...
						<div className="flex space-x-2">
							{(["hour", "day", "week", "month"] as const).map((tf) => (
								...
								>
									{t(
										`timeframe${tf.charAt(0).toUpperCase() + tf.slice(1)}` as
											| "timeframeHour"
											| "timeframeDay"
											| "timeframeWeek"
											| "timeframeMonth",
				...
				<div className="space-y-6">
					...
					<div className="grid grid-cols-2 md:grid-cols-5 gap-4">
						...
						<div className="bg-card border border-border rounded-lg p-5">
							...
							{mostExpensive && (
								<div className="text-xs text-muted-foreground mt-1">
									...
									{(
										(mostExpensive[1].stats.totalCost /
											Math.max(totalCost, 0.0001)) *
										100
									).toFixed(0)}
									%)
								</div>
						...
						<div className="bg-card border border-border rounded-lg p-5">
							...
							{mostEfficient && (
								<div className="text-xs text-muted-foreground mt-1">
									$
									{(
										(mostEfficient[1].stats.totalCost /
											Math.max(1, mostEfficient[1].stats.totalTokens)) *
										1000
									).toFixed(4)}
									/1K tokens
								</div>
						...
						<div className="bg-card border border-border rounded-lg p-5">
							<div className="text-3xl font-bold text-foreground">
								{taskData
									? `${((1 - taskData.unattributed.totalCost / Math.max(totalCost, 0.0001)) * 100).toFixed(0)}%`
									: "-"}
							</div>
							<div className="text-sm text-muted-foreground">
								{t("taskAttributed")}
							...
							{taskData && taskData.unattributed.totalCost > 0 && (
								<div className="text-xs text-muted-foreground mt-1">
									{t("unattributed", {
										cost: formatCost(taskData.unattributed.totalCost),
									})}
								</div>
							)}
						</div>
						...
						<div className="space-y-2">
							{efficiencyData.map(({ name, costPer1k }) => (
								<div key={name} className="flex items-center text-sm">
									<div className="w-32 truncate text-muted-foreground font-medium">
										{name}
									</div>
									<div className="flex-1 mx-3">
										<div className="w-full bg-secondary rounded-full h-2">
											<div
												className="bg-blue-500 h-2 rounded-full"
												style={{
													width: `${(costPer1k / maxCostPer1k) * 100}%`,
												}}
											/>
										</div>
									</div>
					...
					<div className="bg-card border border-border rounded-lg p-6">
						...
						<div className="space-y-2 max-h-[600px] overflow-y-auto">
							{sortedAgents.map(([name, a], index) => {
								const costShare =
									(a.stats.totalCost / Math.max(totalCost, 0.0001)) * 100;
									...
									>
										...
										>
											...
											<div className="flex items-center gap-6 text-sm">
												<div className="text-right">
													<div className="font-medium text-foreground">
														{formatCost(a.stats.totalCost)}
													</div>
													<div className="text-xs text-muted-foreground">
														{t("ofTotal", { pct: costShare.toFixed(1) })}
													</div>
												</div>
												<div className="text-right">
													<div className="text-muted-foreground">
														{formatNumber(a.stats.totalTokens)} {t("tokens")}
													</div>
													<div className="text-xs text-muted-foreground">
														{t("reqs", { count: a.stats.requestCount })}
													</div>
												</div>
												<svg
										...
										{expandedAgent === name && (
											<div className="px-4 pb-4 border-t border-border bg-secondary/30">
												...
												<div className="flex gap-2 pt-3 mb-3">
													<Button
														variant={
															expandedSection === "tasks" ? "default" : "ghost"
														}
														size="sm"
														onClick={(e) => {
															e.stopPropagation();
													...
													<Button
														variant={
															expandedSection === "models" ? "default" : "ghost"
														}
														size="sm"
														onClick={(e) => {
															e.stopPropagation();
													...
													>
														{t("modelsTab", {
															count: Object.keys(a.models).length,
														})}
													</Button>
												</div>
													...
													<div className="text-sm">
														{agentTasks.length === 0 ? (
															...
															<div className="space-y-1.5">
																{agentTasks.map((task) => {
																	const taskShare =
																		(task.stats.totalCost /
																			Math.max(a.stats.totalCost, 0.0001)) *
																		100;
																	return (
																		<div
																			key={task.taskId}
