...
function getFileColor(filePath: string): string {
	if (filePath.startsWith("sessions/") || filePath.includes("/sessions/"))
		return "#89dceb";
	if (filePath.startsWith("memory/") || filePath.includes("/memory/"))
		return "#94e2d5";
	if (filePath.startsWith("knowledge") || filePath.includes("/knowledge"))
		return "#b4befe";
		...
		if (selectedAgent === "all") {
			agents.forEach((agent, i) => {
				...
				const hubSize = Math.max(
					5,
					Math.min(15, 4 + Math.sqrt(agent.totalChunks) * 0.8),
				);
				...
				files.forEach((file, fi) => {
					const fileSize = Math.max(
						1.5,
						Math.min(5, 1 + Math.sqrt(file.chunks) * 0.6),
					);
			...
			const hubSize = Math.max(
				6,
				Math.min(18, 5 + Math.sqrt(agent.totalChunks) * 0.8),
			);
			...
			displayFiles.forEach((file, fi) => {
				const fileSize = Math.max(
					2,
					Math.min(8, 2 + Math.sqrt(file.chunks) * 0.8),
				);
				const fileColor = getFileColor(file.path);
				const nodeId = `file-${agent.name}-${fi}`;
	...
	const handleNodeClick = useCallback(
		(node: InternalGraphNode) => {
			...
			if (id.startsWith("hub-") && selectedAgent === "all") {
				...
				const { filePath, chunks, textSize } = node.data as {
					filePath: string;
					chunks: number;
					textSize: number;
				};
	...
	const handleNodeHover = useCallback(
		(node: InternalGraphNode) => {
			...
			if (node.data) {
				...
				setHoveredNode({
					label: d.filePath,
					sub: `${d.chunks} chunks / ${formatBytes(d.textSize)}`,
				});
				...
				if (agent) {
					setHoveredNode({
						label: agent.name,
						sub: `${agent.totalChunks} chunks / ${agent.totalFiles} files / ${formatBytes(agent.dbSize)}`,
					});
				}
			}
		},
		...
		return (
			...
			>
				<div className="flex flex-col items-center gap-3">
					...
					<span className="text-[#6c7086] text-sm font-mono">
						{t("loading")}
					</span>
				</div>
			</div>
	...
	if (!agents.length) {
		return (
			...
			>
				...
				<span className="text-[#45475a] text-xs">
					{t("noMemoryDatabasesHint")}
				</span>
			</div>
		);
	}
			...
			{hoveredNode && (
				<div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
					<div className="px-3 py-2 rounded-lg bg-[#1e1e2e]/90 backdrop-blur-xl border border-[#45475a]/40 shadow-2xl shadow-black/40 max-w-md">
						<div className="text-[11px] font-mono text-[#cdd6f4] truncate">
							{hoveredNode.label}
						</div>
						{hoveredNode.sub && (
							<div className="text-[10px] font-mono text-[#6c7086] mt-0.5">
								{hoveredNode.sub}
							</div>
						)}
					</div>
				...
				<div className="absolute bottom-3 left-3 z-10 max-w-sm">
					<div className="px-4 py-3 rounded-lg bg-[#1e1e2e]/90 backdrop-blur-xl border border-[#45475a]/40 shadow-2xl shadow-black/40">
						...
						<div className="flex items-center gap-4 text-[10px] font-mono text-[#6c7086]">
							<span>
								<span className="text-[#cba6f7]">{selectedFile.chunks}</span>{" "}
								{t("chunks")}
							</span>
							<span>
								<span className="text-[#89b4fa]">
									{formatBytes(selectedFile.textSize)}
								</span>{" "}
								{t("text")}
							</span>
						</div>
				...
				<div className="px-3 py-2 rounded-lg bg-[#1e1e2e]/80 backdrop-blur-xl border border-[#45475a]/30">
					<div className="flex items-center gap-3 text-[9px] font-mono text-[#585b70]">
						<span className="flex items-center gap-1">
							<span className="w-1.5 h-1.5 rounded-full bg-[#89dceb]" />
							{t("legendSessions")}
						</span>
						<span className="flex items-center gap-1">
							<span className="w-1.5 h-1.5 rounded-full bg-[#94e2d5]" />
							{t("legendMemory")}
						</span>
						<span className="flex items-center gap-1">
							<span className="w-1.5 h-1.5 rounded-full bg-[#b4befe]" />
							{t("legendKnowledge")}
						</span>
						<span className="flex items-center gap-1">
