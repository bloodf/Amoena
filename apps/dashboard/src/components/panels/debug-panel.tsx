...
function StatusTab() {
	...
	const reachable =
		data && !data.gatewayReachable === false && data.gatewayReachable !== false;
...
function HealthTab() {
	...
	const healthy =
		data?.healthy === true || (data && !data.error && data.healthy !== false);
					...
					<table className="w-full text-left">
						<tbody>
							{Object.entries(data).map(([key, value]) => (
								...
								>
									<td className="py-1 pr-4 font-medium text-muted-foreground whitespace-nowrap">
										{key}
									</td>
...
function ModelsTab() {
	...
	const models: ModelEntry[] = Array.isArray(data?.models)
		? data.models
