...
export function MultiGatewayPanel() {
	...
	const [healthByGatewayId, setHealthByGatewayId] = useState<
		Map<number, GatewayHealthProbe>
	>(new Map());
	...
	const gatewayMatchesConnection = useCallback(
		(gw: Gateway): boolean => {
			...
			if (
				...
				normalizedConn.includes(normalizedHost)
			)
				return true;
	...
	const probeAll = async () => {
		try {
			...
			const rows = Array.isArray(data?.results)
				? (data.results as GatewayHealthProbe[])
				: [];
		...
		<div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					<Button onClick={probeAll} variant="secondary" size="sm">
						{t("probeAll")}
					</Button>
			...
			{shouldShowConnectionSummary && (
				<div className="bg-card border border-border rounded-lg p-4">
					<div className="flex items-center gap-3">
						<span
							className={`w-2.5 h-2.5 rounded-full ${connection.isConnected ? "bg-green-500" : "bg-red-500 animate-pulse"}`}
						/>
						<div>
			...
			{discoveredGateways.filter(
				(dg) => !gateways.some((gw) => gw.port === dg.port),
			).length > 0 && (
				<div>
								...
								>
									<div className="flex items-start justify-between gap-3">
										<div className="flex-1 min-w-0">
											<div className="flex items-center gap-2">
												<span
													className={`w-2 h-2 rounded-full ${dg.active ? "bg-green-500" : "bg-red-500"}`}
												/>
							...
							>
								<div className="flex items-start justify-between gap-3">
									<div className="flex-1 min-w-0">
										<div className="flex items-center gap-2">
											<span
												className={`w-2 h-2 rounded-full ${conn.status === "connected" ? "bg-green-500" : "bg-red-500"}`}
											/>
											<span className="text-sm font-semibold text-foreground">
												{conn.agent_name}
											</span>
											<span className="text-2xs px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30 font-medium">
												{conn.tool_name}
												{conn.tool_version ? ` v${conn.tool_version}` : ""}
											</span>
											<span
												className={`text-2xs px-1.5 py-0.5 rounded font-medium ${
										...
										<div className="flex items-center gap-4 mt-1.5 text-xs text-muted-foreground">
											<span>
												{t("role")}: {conn.agent_role || "cli"}
											</span>
											<span>
												{t("heartbeat")}:{" "}
												{conn.last_heartbeat
													? new Date(
															conn.last_heartbeat * 1000,
														).toLocaleString()
													: t("never")}
											</span>
...
function GatewayCard({
	...
	const timelineEntries =
		historyEntries.length > 0 ? [...historyEntries].slice(0, 10).reverse() : [];
		...
		>
			<div className="flex items-start justify-between gap-3">
				<div className="flex-1 min-w-0">
					<div className="flex items-center gap-2">
						<span
							className={`w-2 h-2 rounded-full ${isCurrentlyConnected ? "bg-green-500" : statusColors[gateway.status] || statusColors.unknown}`}
						/>
						<h3 className="text-sm font-semibold text-foreground">
							{gateway.name}
						</h3>
					...
					<div className="flex items-center gap-4 mt-1.5 text-xs text-muted-foreground">
						...
						<button
							onClick={() => {
								setEditingToken(!editingToken);
								setTokenInput("");
							}}
							className="hover:text-foreground transition-colors cursor-pointer"
							title={
								gateway.token_set ? "Change gateway token" : "Set gateway token"
							}
						>
							{t("token")}: {gateway.token_set ? t("tokenSet") : t("tokenNone")}{" "}
							[edit]
						</button>
						{gateway.latency != null && (
							<span>
								{t("latency")}: {gateway.latency}ms
							</span>
						)}
						<span>
							{t("last")}: {lastSeen}
					...
					<div className="flex flex-wrap items-center gap-2 mt-3 text-2xs text-muted-foreground">
						...
						{latestEntry?.latency != null && (
							<span className="text-2xs font-medium">
								{t("lastLatency", { ms: latestEntry.latency })}
							</span>
						)}
					</div>
				</div>
