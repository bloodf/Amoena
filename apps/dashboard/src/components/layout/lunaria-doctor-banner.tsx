...
export function AmoenaDoctorBanner() {
	...
	const dismissed =
		...
		Date.now() - doctorDismissedAt < TWENTY_FOUR_HOURS;
	...
	const tone =
		doctor.level === "error"
			? {
					...
					secondary:
						"text-red-300 border-red-500/20 hover:border-red-500/40 hover:text-red-200",
				}
			: {
					...
					secondary:
						"text-amber-300 border-amber-500/20 hover:border-amber-500/40 hover:text-amber-200",
				};
	...
	return (
		<div className="mx-4 mt-3 mb-0">
			<div
				className={`flex items-start gap-3 px-4 py-3 rounded-lg border text-sm ${tone.frame}`}
			>
				<span
					className={`mt-1 h-1.5 w-1.5 shrink-0 rounded-full ${tone.dot}`}
				/>
						...
						<div className="mt-2 space-y-1">
							...
							{extraCount > 0 && (
								<p className="text-2xs opacity-75">
									{tc("moreIssues", { count: extraCount })}
								</p>
							)}
						</div>
					)}
			...
			{showDetails && (
				<div
					className={`mt-1 max-h-80 overflow-y-auto rounded-lg border px-4 py-3 text-xs whitespace-pre-wrap ${tone.frame}`}
				>
