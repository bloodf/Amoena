...
export function LiveFeed() {
	...
	const sessionItems = isLocal
		? sessions.slice(0, 10).map((s) => ({
				...
				source:
					s.model?.split("/").pop()?.split("-").slice(0, 2).join("-") ||
					"claude",
	...
	return (
		<div
			className={`w-72 h-full bg-card border-l border-border flex flex-col shrink-0${hasCollapsed ? " slide-in-right" : ""}`}
		>
			{/* Header */}
				...
				{feedItems.length === 0 ? (
					<div className="px-3 py-8 text-center">
						...
						<p className="text-2xs text-muted-foreground/60 mt-1">
							{isLocal ? t("eventsAppearLocal") : t("eventsStreamGateway")}
						</p>
					</div>
				) : (
