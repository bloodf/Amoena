...
const QUICK_NAV_COMMANDS: Array<{
	panel: string;
	titleKey: string;
	title: string;
	aliases: string[];
	...
	},
	{
		...
		titleKey: "goToChat",
		title: "Go to Chat",
		aliases: ["sessions", "messages"],
	},
	{
		panel: "tasks",
		titleKey: "goToTasks",
		title: "Go to Tasks",
		aliases: ["task board", "tickets"],
	},
	{
		panel: "agents",
		titleKey: "goToAgents",
		title: "Go to Agents",
		aliases: ["agent squad", "workers"],
	},
	{
		panel: "activity",
		titleKey: "goToActivityFeed",
		title: "Go to Activity Feed",
		aliases: ["events", "feed"],
	},
	{
		panel: "notifications",
		titleKey: "goToNotifications",
		title: "Go to Notifications",
		aliases: ["alerts inbox"],
	},
	{
		panel: "tokens",
		titleKey: "goToTokenUsage",
		title: "Go to Token Usage",
		aliases: ["cost", "spend"],
	},
	{
		panel: "logs",
	...
	{
		panel: "memory",
		titleKey: "goToMemoryBrowser",
		title: "Go to Memory Browser",
		aliases: ["knowledge", "notes"],
	},
	{
		panel: "integrations",
		titleKey: "goToIntegrations",
		title: "Go to Integrations",
		aliases: ["providers", "api keys"],
	},
	{
		panel: "settings",
	...
	{
		panel: "gateways",
		titleKey: "goToGateways",
		title: "Go to Gateways",
		aliases: ["gateway manager"],
	},
	{
		panel: "github",
		titleKey: "goToGithubSync",
		title: "Go to GitHub Sync",
		aliases: ["github", "sync"],
	},
	{
		panel: "office",
		titleKey: "goToOffice",
		title: "Go to Office",
		aliases: ["workspace", "team"],
	},
	{
		panel: "skills",
		titleKey: "goToSkills",
		title: "Go to Skills",
		aliases: ["skill packs", "agent skills"],
	},
];
...
export function HeaderBar() {
	...
	const getQuickNavResults = useCallback(
		(q: string): SearchResult[] => {
			...
			const ranked: Array<(SearchResult & { _score: number }) | null> =
				QUICK_NAV_COMMANDS.map((cmd, index) => {
					const translatedTitle = th(cmd.titleKey);
					const haystack =
						`${translatedTitle} ${cmd.title} ${cmd.panel} ${cmd.aliases.join(" ")}`.toLowerCase();
					if (!haystack.includes(normalized)) return null;
					const exactPanel = cmd.panel === normalized;
					const startsTitle = translatedTitle
						.toLowerCase()
	...
	useEffect(() => {
		if (!searchOpen) return;
		resultButtonRefs.current[selectedIndex]?.scrollIntoView({
			block: "nearest",
		});
	}, [searchOpen, selectedIndex]);
	...
	const doSearch = useCallback(
		async (q: string) => {
			...
			try {
				...
				const entityResults: SearchResult[] = (data.results || []).map(
					(r: SearchResult) => ({ ...r, source: "entity" }),
				);
	...
	useEffect(() => {
		resultButtonRefs.current = resultButtonRefs.current.slice(
			0,
	...
	return (
		<header
			...
			className="relative z-50 h-14 bg-card/80 backdrop-blur-sm border-b border-border px-3 md:px-4 shrink-0"
		>
					...
					{activeProject ? (
						...
						>
							<span className="text-muted-foreground/60 truncate">
								{activeTenant?.display_name || "Default"}
							</span>
				...
				<div className="hidden md:flex items-center justify-center flex-1 min-w-0 max-w-[28rem] lg:max-w-[34rem] xl:max-w-[42rem]">
					...
					>
						...
						<span className="hidden xl:flex items-center gap-1 ml-2 shrink-0">
							<kbd className="text-2xs px-1.5 py-0.5 rounded bg-muted border border-border font-mono">
								&#8984;K
							</kbd>
							<kbd className="text-2xs px-1.5 py-0.5 rounded bg-muted border border-border font-mono">
								/
									...
									<input
										...
										aria-activedescendant={
											searchResults[selectedIndex]
												? `mc-command-result-${selectedIndex}`
												: undefined
										}
									/>
								</div>
								...
								>
									{searchLoading ? (
										...
										searchResults.map((r, i) => (
											...
											>
												<span
													className={`text-2xs font-medium w-5 h-5 rounded flex items-center justify-center shrink-0 mt-0.5 ${typeColors[r.type] || "bg-muted text-muted-foreground"}`}
												>
													{typeIcons[r.type] || "?"}
												</span>
												<div className="flex-1 min-w-0">
													...
													{r.excerpt && (
														<div className="text-2xs text-muted-foreground/70 truncate mt-0.5">
															{r.excerpt}
														</div>
													)}
												</div>
...
function ModeBadge({
	...
	if (isConnected) {
		...
		statusLabel =
			connection.latency != null ? `${connection.latency}ms` : th("connected");
	...
	return (
		...
		>
			...
			>
				...
				<span className={`font-mono ${textClass} opacity-80`}>
					{statusLabel}
				</span>
			</button>
...
function Stat({
	...
	return (
		<div className="flex items-center gap-1.5 text-xs">
			...
			<span className={`font-medium font-mono-tight ${statusColor}`}>
				{value}
			</span>
		</div>
	);
...
function SseBadge({ connected }: { connected: boolean }) {
	...
	return (
		<div className="flex items-center gap-1.5 text-xs">
			...
			<span
				className={`w-1.5 h-1.5 rounded-full ${connected ? "bg-blue-500" : "bg-muted-foreground/30"}`}
			/>
			<span
				className={`font-medium font-mono-tight ${connected ? "text-blue-400" : "text-muted-foreground"}`}
			>
