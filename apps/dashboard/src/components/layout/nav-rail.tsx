...
export function NavRail() {
	...
	}, [fetchProjects, isAdmin, setActiveProject]);
	...
	const mergedGroups = navGroups.map((g) => {
		...
		const items = translateItems(
			pluginItems.length > 0 ? [...g.items, ...pluginItems] : g.items,
		);
	...
	function flattenItems(items: NavItem[]): NavItem[] {
		return items.flatMap((i) =>
			i.children ? [i, ...flattenItems(i.children)] : [i],
		);
	}
	const filteredAllNavItems = filteredGroups.flatMap((g) =>
			...
			>
				...
				>
					...
					<Button
						...
						title={
							sidebarExpanded ? tn("collapseSidebar") : tn("expandSidebar")
						}
						className="shrink-0"
					>
						<svg
				...
				<div className="flex-1 overflow-y-auto overflow-x-hidden py-1">
					{filteredGroups.map((group, groupIndex) => (
						<div key={group.id}>
							...
							{groupIndex > 0 && (
								<div
									className={`my-1.5 border-t border-border ${sidebarExpanded ? "mx-3" : "mx-2"}`}
								/>
							)}
							...
							>
								<div
									className={`flex flex-col ${sidebarExpanded ? "gap-0.5 px-2" : "items-center gap-1"}`}
								>
					...
					<div className="px-2 pb-2 space-y-2 shrink-0">
						...
						>
							<div className="flex items-center gap-1.5 mb-0.5">
								<span className="text-2xs font-semibold text-foreground group-hover:text-primary transition-colors">
									xint
								</span>
								<span className="text-[9px] px-1 py-px rounded bg-primary/15 text-primary font-mono">
									CLI
								</span>
							</div>
							<p className="text-[10px] text-muted-foreground/70 leading-snug">
								X power tools for agents.
							</p>
						</a>
						<a
						...
						>
							...
							<p className="text-[10px] text-muted-foreground/70 leading-snug">
								AI-native dev shop · Solana experts.
							</p>
						</a>
					</div>
				)}
	...
	if (expanded) {
		return (
			...
			>
				...
				<div className={`shrink-0 ${nested ? "w-4 h-4" : "w-5 h-5"}`}>
					{item.icon}
				</div>
				<span className={`truncate ${nested ? "text-xs" : "text-sm"}`}>
					{item.label}
				</span>
			</Button>
		);
...
function MobileBottomBar({
	...
	const nonPriorityIds = new Set(
		items.filter((i) => !i.priority).map((i) => i.id),
	);
...
function OrgRow({
	...
	return (
		<div>
			...
			>
				<div
					className={`w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold shrink-0 ${colorClass}`}
				>
					{initial}
				</div>
				...
				{isActiveOrg && (
					<svg
						...
						strokeLinecap="round"
						strokeLinejoin="round"
						className="w-3 h-3 shrink-0 ml-auto text-muted-foreground/40"
					>
						<polyline points="4,6 8,10 12,6" />
					</svg>
				)}
				...
				<div className="pl-4 mt-0.5 mb-1">
					...
					{projects.map((project) => (
						...
						>
							...
							>
								{project.ticket_prefix?.slice(0, 2) ||
							...
							<div className="flex items-center gap-1 ml-auto shrink-0">
								{typeof project.task_count === "number" &&
									project.task_count > 0 && (
										<span className="text-[9px] bg-white/10 px-1 rounded text-muted-foreground/50">
											{project.task_count}
										</span>
									)}
								{project.deadline &&
...
function ContextSwitcher({
	...
	const unlinkedOsUsers = osUsers.filter(
		(u) => !linkedUsernames.has(u.username) && !u.is_process_owner,
	);
	...
	const connectionLabel = isLocal
		...
		: isConnected
			? tcs("connected")
			: tcs("disconnected");
	const connectionDotClass = isLocal
		? "bg-void-cyan"
		: isConnected
			? "bg-green-500"
			: "bg-red-500";
	...
	return (
		<div
			className={`shrink-0 relative ${expanded ? "px-3 pb-3" : "flex flex-col items-center pb-3"}`}
		>
			{/* Trigger */}
			<Button
				variant="ghost"
				...
				title={
					expanded
						? undefined
						: `${userName} · ${contextLine} · ${connectionLabel}`
				}
			...
			>
				{/* Avatar */}
				<div
					className={`shrink-0 rounded-full flex items-center justify-center text-[11px] font-semibold relative ${
				...
				>
					...
					<span
						className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-card ${connectionDotClass}`}
					/>
				</div>
			...
			{open && (
				<>
					...
					<div
						className={`absolute z-50 bg-popover border border-border rounded-lg shadow-xl min-w-[220px] max-h-[400px] overflow-y-auto ${
							expanded
								? "bottom-full mb-1 left-3 right-3"
								: "bottom-full mb-1 left-1"
						}`}
					>
						{/* User info header */}
						<div className="px-3 pt-3 pb-2">
							<div className="flex items-center gap-2">
								<div
									className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center text-[11px] font-semibold ${
										currentUser?.avatar_url ? "" : "bg-primary/20 text-primary"
									}`}
								>
									{currentUser?.avatar_url ? (
								...
								<div className="min-w-0">
									...
									<div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
										...
										<span className={`flex items-center gap-1`}>
											<span
												className={`w-1.5 h-1.5 rounded-full inline-block ${connectionDotClass}`}
											/>
											{connectionLabel}
										</span>
									</div>
						...
						<div className="px-3 py-1.5 flex items-center justify-between">
							...
							<div className="flex rounded-md border border-border overflow-hidden">
									...
									onClick={async () => {
										...
										try {
											await fetch("/api/settings", {
												...
												body: JSON.stringify({
													settings: { "general.interface_mode": "essential" },
												}),
											});
										} catch {}
									}}
								...
								>
									<span
										className={`w-1.5 h-1.5 rounded-full ${interfaceMode === "essential" ? "bg-void-amber" : "bg-muted-foreground/30"}`}
									/>
									{tcs("essential")}
								</button>
								<button
								...
								>
									<span
										className={`w-1.5 h-1.5 rounded-full ${interfaceMode === "full" ? "bg-void-cyan" : "bg-muted-foreground/30"}`}
									/>
									{tcs("full")}
								</button>
							</div>
									...
									{unlinkedOsUsers.map((osUser) => {
										...
										return (
											...
											>
												...
												<span
													className={`ml-auto text-[10px] shrink-0 ${disabled ? "text-muted-foreground/20" : "text-muted-foreground/30"}`}
												>
													{statusLabel}
												</span>
											</Button>
										...
										<div className="px-1 pt-1 pb-1 space-y-1.5">
											...
											{isLocal && (
												<div className="space-y-1 px-0.5">
													<div className="text-[10px] text-muted-foreground/60 font-semibold tracking-wider">
														{tcs("installTools")}
													</div>
											...
											{createError && (
												<div className="text-[10px] text-red-400 px-0.5">
													{createError}
												</div>
											)}
												...
												<Button
													...
													onClick={async () => {
														...
														if (!username || !display_name) {
															setCreateError(tcs("usernameAndDisplayRequired"));
															return;
														}
														if (
															!/^[a-z][a-z0-9_-]{1,30}[a-z0-9]$/.test(username)
														) {
															setCreateError(tcs("invalidUsernameFormat"));
															return;
														}
														if (!isLocal && !createForm.gateway_port) {
															setCreateError(tcs("gatewayPortRequired"));
															return;
														}
														setCreating(true);
														setCreateError(null);
