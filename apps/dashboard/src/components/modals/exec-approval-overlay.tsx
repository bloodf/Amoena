...
function MetaRow({ label, value }: { label: string; value?: string | null }) {
	...
	return (
		<div className="flex justify-between text-xs py-0.5">
			...
			<span className="font-mono text-foreground truncate ml-4 max-w-[300px]">
				{value}
			</span>
		</div>
	);
...
export function ExecApprovalOverlay() {
	...
	return (
		...
		>
			<div
				className={`w-[min(540px,95vw)] bg-card border border-border rounded-lg p-5 shadow-2xl border-l-4 ${RISK_BORDER[active.risk]}`}
			>
				{/* Header */}
				<div className="flex items-center justify-between mb-3">
					...
					<div className="flex items-center gap-2">
						<span
							className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${RISK_BADGE[active.risk]}`}
						>
							{active.risk}
						</span>
						{pending.length > 1 && (
				...
				{!active.command &&
					active.toolArgs &&
					Object.keys(active.toolArgs).length > 0 && (
