...
export function ProjectManagerModal({
	...
	const deleteProject = async (project: Project) => {
		if (
			!confirm(
				`Delete project "${project.name}"? Existing tasks will be moved to General.`,
			)
		)
			return;
		try {
			const response = await fetch(`/api/projects/${project.id}?mode=delete`, {
				method: "DELETE",
			});
			const data = await response.json();
			if (!response.ok)
	...
	const startEditing = (project: Project) => {
		...
		setEditForm({
			...
			deadline: project.deadline
				? new Date(project.deadline * 1000).toISOString().split("T")[0]
				: "",
			color: project.color || "",
		...
		try {
			const body: Record<string, unknown> = {
				...
				deadline: editForm.deadline
					? Math.floor(new Date(editForm.deadline).getTime() / 1000)
					: null,
			...
			for (const agentName of toRemove) {
				await fetch(
					`/api/projects/${project.id}/agents?agent_name=${encodeURIComponent(agentName)}`,
					{
						method: "DELETE",
					},
				);
							...
							{projects.map((project) => (
								...
								>
										...
										<div className="flex items-center gap-2.5">
											...
											<div>
												...
												<div className="text-xs text-muted-foreground">
													{project.ticket_prefix} &middot; {project.slug}{" "}
													&middot; {project.status}
													{project.github_repo && (
														<> &middot; {project.github_repo}</>
													)}
												</div>
											</div>
										</div>
												...
												project.assigned_agents.length > 0 && (
													<div className="flex -space-x-1">
														{project.assigned_agents.slice(0, 3).map((a) => (
															<div
																key={a}
																className="w-5 h-5 rounded-full bg-primary/20 text-primary flex items-center justify-center text-[8px] font-bold border border-card"
																title={a}
															>
																{a[0]?.toUpperCase()}
															</div>
									...
									{editingId === project.id && (
										<div
											className="border-t border-border p-3 bg-surface-1/50 space-y-3"
											onClick={(e) => e.stopPropagation()}
										>
													...
													<div>
														...
														<input
															...
															onChange={(e) =>
																setEditForm((prev) => ({
																	...prev,
																	github_default_branch: e.target.value,
																}))
															}
															className="w-full bg-surface-1 text-foreground border border-border rounded-md px-3 py-2 text-sm"
															placeholder="main"
														...
														<button
															...
															onClick={() =>
																setEditForm((prev) => ({
																	...prev,
																	github_sync_enabled:
																		!prev.github_sync_enabled,
																}))
															}
															className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
																editForm.github_sync_enabled
																	? "bg-primary"
													...
													<div className="flex flex-wrap gap-1.5">
														{agents.map((agent) => (
															<button
																...
																type="button"
																onClick={() =>
																	toggleAgentAssignment(agent.name)
																}
											...
											<div className="flex gap-2 pt-1">
												<Button size="sm" onClick={() => saveEdit(project)}>
													Save
												</Button>
												<Button
													size="sm"
