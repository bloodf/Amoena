...
const taskMetadataSchema = z
	.object({
		implementation_repo: z
			.string()
			.min(1, "implementation_repo cannot be empty")
			.max(200)
			.optional(),
		code_location: z
			.string()
			.min(1, "code_location cannot be empty")
			.max(500)
			.optional(),
	})
	.catchall(z.unknown());
...
export const createAgentSchema = z.object({
	...
	status: z
		.enum(["online", "offline", "busy", "idle", "error"])
		.default("offline"),
	config: z
...
export const githubSyncSchema = z.object({
	...
	repo: z
		.string()
		.regex(/^[^/]+\/[^/]+$/, "Repo must be owner/repo format")
		.optional(),
