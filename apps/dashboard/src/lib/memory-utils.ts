...
export function extractSchema(content: string): SchemaBlock | null {
	...
	if (optionalMatch) {
		schema.optional = optionalMatch[1]
			.split(",")
			.map((s) => s.trim())
			.filter(Boolean);
	}
...
export function validateSchema(content: string): SchemaValidationResult {
	...
	if (!fmMatch) {
		return {
			valid: false,
			errors: ["No frontmatter found but _schema declared"],
			schema,
		};
	}
...
export async function runHealthDiagnostics(
	...
	// 1. Schema compliance
	{
		...
		const score =
			filesWithSchema === 0
				? 100
				: Math.round((validSchemas / filesWithSchema) * 100);
		categories.push({
	...
	// 2. Connectivity (wiki-link health)
	{
		...
		const connectedRatio =
			totalFiles > 0 ? (totalFiles - orphanCount) / totalFiles : 1;
	...
	// 3. Broken links
	{
		...
		const totalLinks = Object.values(graph.nodes).reduce(
			(s, n) => s + n.wikiLinks.length,
			0,
		);
	...
	// 4. Staleness (files not modified in 30+ days)
	{
		...
		categories.push({
			...
			suggestions:
				staleFiles.length > 0
					? [
							"Review stale files for relevance",
							"Run a /reweave pass to update older notes",
						]
					: [],
		});
	}
			...
			suggestions:
				unique.length > 0
					? [
							...
							"Avoid generic names like untitled or temp",
						]
					: [],
		});
	}
	...
	// 7. Directory structure
	{
		...
		categories.push({
			...
			suggestions:
				rootRatio > 0.5
					? [
							"Create topic directories to group related notes",
							"Use MOC files as directory indexes",
						]
					: [],
		});
	}
 ...
 */
export async function generateContextPayload(
	baseDir: string,
	...
	const staleCount = files.filter(
		(f) => now - f.modified > staleThreshold,
	).length;
	const orphanCount = graph.orphans.length;
	...
	const quickScore = Math.round(
		((connectedRatio + (1 - staleRatio)) / 2) * 100,
	);
	const overall =
		quickScore >= 70 ? "healthy" : quickScore >= 40 ? "warning" : "critical";
	...
	for (const [dir, paths] of dirGroups) {
		for (let i = 0; i < paths.length; i++) {
			for (let j = i + 1; j < paths.length; j++) {
				...
				if (a && b) {
					const linked =
						a.outgoing.includes(b.path) || b.outgoing.includes(a.path);
					if (!linked) {
						suggestions.push(
...
export async function reweavePass(baseDir: string): Promise<ProcessingResult> {
	...
	for (const f of files) {
		if (now - f.modified > staleThreshold) {
			...
			const newerLinks = [...node.incoming, ...node.outgoing].filter(
				(linked) => {
