...
const migrations: Migration[] = [
	...
	},
	{
		...
		up: (db) => {
			...
			const hasCol = (name: string) =>
				deliveryCols.some((c) => c.name === name);

			if (!hasCol("attempt"))
				db.exec(
					`ALTER TABLE webhook_deliveries ADD COLUMN attempt INTEGER NOT NULL DEFAULT 0`,
				);
			...
			if (!hasCol("is_retry"))
				db.exec(
					`ALTER TABLE webhook_deliveries ADD COLUMN is_retry INTEGER NOT NULL DEFAULT 0`,
				);
			if (!hasCol("parent_delivery_id"))
				db.exec(
					`ALTER TABLE webhook_deliveries ADD COLUMN parent_delivery_id INTEGER`,
				);

			// Add circuit breaker column to webhooks
			const webhookCols = db
	...
	},
	{
      ...
      `);
			const setTaskNo = db.prepare(
				`UPDATE tasks SET project_ticket_no = ? WHERE id = ?`,
			);
			const setProjectCounter = db.prepare(
				`UPDATE projects SET ticket_counter = ?, updated_at = unixepoch() WHERE id = ?`,
			);

			for (const workspace of workspaceRows) {
				ensureDefaultProject.run(workspace.id);
				const defaultProject = getDefaultProject.get(workspace.id) as
					| { id: number; ticket_counter: number }
					| undefined;
        ...
        `)
				...
				for (const project of projectRows) {
					const tasks = listProjectTasks.all(
						workspace.id,
						project.id,
					) as Array<{ id: number }>;
					let counter = 0;
	...
	},
	{
		...
		up: (db) => {
			const hasTokenUsageTable = db
				.prepare(
					`SELECT 1 as ok FROM sqlite_master WHERE type = 'table' AND name = 'token_usage'`,
				)
				.get() as { ok?: number } | undefined;

			if (!hasTokenUsageTable?.ok) return;

			const cols = db.prepare(`PRAGMA table_info(token_usage)`).all() as Array<{
				name: string;
			}>;
	...
	},
	{
		...
		up: (db) => {
			const hasProjects = db
				.prepare(
					`SELECT 1 as ok FROM sqlite_master WHERE type = 'table' AND name = 'projects'`,
				)
				.get() as { ok?: number } | undefined;
	...
	},
	{
		...
		up: (db) => {
			...
			const hasTaskCol = (name: string) =>
				taskCols.some((c) => c.name === name);
      ...
      `);
			...
			const projCols = db
				...
				.all() as Array<{ name: string }>;
			const hasProjCol = (name: string) =>
				projCols.some((c) => c.name === name);

			if (!hasProjCol("github_sync_enabled"))
				db.exec(
					`ALTER TABLE projects ADD COLUMN github_sync_enabled INTEGER NOT NULL DEFAULT 0`,
				);
			if (!hasProjCol("github_labels_initialized"))
				db.exec(
					`ALTER TABLE projects ADD COLUMN github_labels_initialized INTEGER NOT NULL DEFAULT 0`,
				);
			...
			const syncCols = db
				.prepare(`PRAGMA table_info(github_syncs)`)
				.all() as Array<{ name: string }>;
			const hasSyncCol = (name: string) =>
				syncCols.some((c) => c.name === name);

			if (!hasSyncCol("project_id"))
				db.exec(`ALTER TABLE github_syncs ADD COLUMN project_id INTEGER`);
			if (!hasSyncCol("changes_pushed"))
				db.exec(
					`ALTER TABLE github_syncs ADD COLUMN changes_pushed INTEGER NOT NULL DEFAULT 0`,
				);
			if (!hasSyncCol("changes_pulled"))
				db.exec(
					`ALTER TABLE github_syncs ADD COLUMN changes_pulled INTEGER NOT NULL DEFAULT 0`,
				);
			db.exec(
	...
	},
	{
		...
		up: (db) => {
			...
			const hasTenants = db
				.prepare(
					`SELECT 1 as ok FROM sqlite_master WHERE type = 'table' AND name = 'tenants'`,
				)
				.get() as { ok?: number } | undefined;
			...
			const workspaceCols = db
				...
				.all() as Array<{ name: string }>;
			const hasWorkspaceTenantId = workspaceCols.some(
				(c) => c.name === "tenant_id",
			);
			...
			const tenantCount =
				(
					db.prepare(`SELECT COUNT(*) as c FROM tenants`).get() as
						| { c: number }
						| undefined
				)?.c || 0;
        ...
        `)
				...
				const rawHost = String(process.env.MC_HOSTNAME || "default")
					.trim()
					.toLowerCase();
				const slug =
					rawHost
        ...
        `)
			...
			db.prepare(
				`UPDATE workspaces SET tenant_id = ? WHERE tenant_id IS NULL`,
			).run(defaultTenantId);
      ...
      `);
			db.prepare(
				`UPDATE user_sessions SET tenant_id = ? WHERE tenant_id IS NULL`,
			).run(defaultTenantId);

			const workspaceFk = db
				.prepare(`PRAGMA foreign_key_list(workspaces)`)
				.all() as Array<{ table: string; from: string; to: string }>;
			const hasTenantFk = workspaceFk.some(
				(fk) =>
					fk.table === "tenants" && fk.from === "tenant_id" && fk.to === "id",
			);
			const tenantCol = (
	...
	},
	{
		...
		up(db: Database.Database) {
			...
			const existing = db
				.prepare(
					`SELECT 1 as ok FROM sqlite_master WHERE type = 'table' AND name = 'api_keys'`,
				)
				.get() as { ok?: number } | undefined;
	...
	},
	{
		...
		up(db: Database.Database) {
			...
			const rows = db
				.prepare("SELECT id, token FROM user_sessions")
	...
	},
	{
		...
		up(db: Database.Database) {
			...
			db.exec(
				`CREATE INDEX IF NOT EXISTS idx_tasks_stale_inprogress ON tasks(status, updated_at) WHERE status = 'in_progress'`,
			);
		},
	},
	{
