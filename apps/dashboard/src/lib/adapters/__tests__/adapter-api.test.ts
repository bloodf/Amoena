 ...
 * Simulates what an external agent would do to connect to Lunaria.
 *
...
async function simulateAdapterAction(
	...
	switch (action) {
		case "register": {
			...
			if (!agentId || !name)
				return { error: "payload.agentId and payload.name required" };
		...
		case "report": {
			...
			if (!taskId || !agentId)
				return { error: "payload.taskId and payload.agentId required" };
...
describe("Adapter API dispatch", () => {
	...
	describe.each(listAdapters())("Full agent lifecycle: %s", (framework) => {
		it("register → heartbeat → report → assignments → disconnect", async () => {
			mockQuery.mockResolvedValue([
				{ taskId: "1", description: "Do stuff", priority: 1 },
			]);
	...
	describe("input validation", () => {
		...
		it("rejects register without agentId", async () => {
			const result = await simulateAdapterAction("generic", "register", {
				name: "No ID",
			});
		...
		it("rejects report without taskId", async () => {
			const result = await simulateAdapterAction("generic", "report", {
				agentId: "x",
			});
