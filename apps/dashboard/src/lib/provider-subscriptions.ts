...
let detectionCache: { ts: number; value: SubscriptionDetectionResult } | null =
	null;
...
export function detectProviderSubscriptions(
	forceRefresh = false,
	...
	if (
		!forceRefresh &&
		detectionCache &&
		now - detectionCache.ts < CACHE_TTL_MS
	) {
...
export function getProviderSubscriptionFlags(
	forceRefresh = false,
...
export function getPrimarySubscription(
	...
	return (
		detected.anthropic || detected.openai || Object.values(detected)[0] || null
	);
}
