...
export function resolveSeedAuthPassword(
	...
	if (b64 && b64.trim().length > 0) {
		...
		if (!base64Pattern.test(normalized)) {
			logger.warn(
				"AUTH_PASS_B64 is not valid base64; falling back to AUTH_PASS",
			);
		...
		try {
			...
			if (canonical !== normalized) {
				logger.warn(
					"AUTH_PASS_B64 failed base64 verification; falling back to AUTH_PASS",
				);
				return env.AUTH_PASS || null;
			}
			if (decoded.length > 0) return decoded;
			logger.warn(
				"AUTH_PASS_B64 is set but decoded to an empty value; falling back to AUTH_PASS",
			);
		} catch {
			logger.warn(
				"AUTH_PASS_B64 is not valid base64; falling back to AUTH_PASS",
...
function seedAdminUserFromEnv(dbConn: Database.Database): void {
	...
	const count = (
		dbConn.prepare("SELECT COUNT(*) as count FROM users").get() as CountRow
	).count;
	if (count > 0) return;
