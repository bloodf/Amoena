 ...
 */
export function verifyPasswordWithRehashCheck(
	password: string,
	stored: string,
	...
	if (
		...
		timingSafeEqual(derived, storedBuf)
	) {
		return { valid: true, needsRehash: false };
	}

	// Fall back to legacy cost for passwords hashed before the upgrade
	const legacyDerived = scryptSync(password, salt, KEY_LENGTH, {
		N: LEGACY_SCRYPT_COST,
	});
	if (legacyDerived.length !== storedBuf.length)
