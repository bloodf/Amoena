...
const reconnectTimeoutRef: { current: NodeJS.Timeout | undefined } = {
	current: undefined,
};
const pingIntervalRef: { current: NodeJS.Timeout | undefined } = {
	current: undefined,
};
...
const connectRef: { current: (url: string, token?: string) => void } = {
	current: () => {},
};
const lastWebSocketErrorRef: {
	current: { message: string; at: number } | null;
} = { current: null };
const pingCounterRef: { current: number } = { current: 0 };
const pingSentTimestamps: { current: Map<string, number> } = {
	current: new Map(),
};
...
export function useWebSocket() {
	...
	const sendConnectHandshake = useCallback(
		async (ws: WebSocket, nonce?: string) => {
			let device:
				| {
						id: string;
						publicKey: string;
			...
			const connectRequest = {
				...
				params: {
					...
					device,
					deviceToken: tokenOnlyFallbackRef.current
						? undefined
	...
	const _handleGatewayMessage = useCallback(
			...
			switch (message.type) {
				case "session_update":
					if (message.data?.sessions) {
						setSessions(
							message.data.sessions.map((session: any, index: number) => ({
				...
				case "log":
					if (message.data) {
						addLog({
							...
							timestamp:
								message.data.timestamp || message.timestamp || Date.now(),
	...
	const handleGatewayFrame = useCallback(
		(frame: GatewayFrame, ws: WebSocket) => {
			...
			if (frame.type === "res" && frame.id?.startsWith("ping-")) {
				...
				if (!frame.ok && /unknown method:\s*ping/i.test(rawPingError)) {
					...
					pingSentTimestamps.current.clear();
					log.info(
						"Gateway ping RPC unavailable; using passive heartbeat mode",
					);
				}
				handlePong(frame.id);
				return;
			...
			if (frame.type === "res" && !frame.ok) {
				log.error(
					`Gateway error: ${frame.error?.message || JSON.stringify(frame.error)}`,
				);
				...
				if (shouldFallbackToTokenOnly) {
					...
					addLog({
						...
						source: "gateway",
						message:
							"Gateway rejected cached browser device credentials. Retrying with token-only authentication.",
					});
					stopHeartbeat();
					if (
				...
				const nonRetryable = isNonRetryableGatewayError(
					rawMessage,
					frame.error,
				);
			...
			if (frame.type === "event") {
				...
				if (seq !== null) {
					if (lastSeqRef.current !== null && seq > lastSeqRef.current + 1) {
						log.warn(
							`Event sequence gap: expected ${lastSeqRef.current + 1}, received ${seq}`,
						);
					}
				...
				if (frame.event === "tick") {
					...
					if (snapshot?.sessions) {
						setSessions(
							snapshot.sessions.map((session: any, index: number) => ({
								id: session.key || `session-${index}`,
								key: session.key || "",
								kind: session.kind || "unknown",
					...
					if (t) {
						addChatMessage({
							...
							conversation_id:
								t.conversation_id || t.sessionId || "tool-stream",
					...
					addNotification({
						...
						message:
							...
							`Session context compacted (${frame.payload?.percentage || "?"}% reduced)`,
						created_at: Math.floor(Date.now() / 1000),
					});
					...
					addNotification({
						...
						message:
							frame.payload?.message ||
							`Fell back from ${frame.payload?.from || "?"} to ${frame.payload?.to || "?"}`,
						created_at: Math.floor(Date.now() / 1000),
					});
				} else if (
					frame.event === "exec.approval" ||
					...
					if (resolved?.id) {
						const newStatus =
							resolved.decision === "deny" ? "denied" : "approved";
	...
	const shouldSuppressWebSocketError = useCallback(
		(message: string): boolean => {
			...
			if (
				...
				now - previous.at < ERROR_LOG_DEDUPE_MS
			) {
				return true;
			}
			...
			try {
				...
				ws.onclose = (event) => {
					...
					if (attempts < maxReconnectAttempts) {
						const base = Math.min(1000 * 1.7 ** attempts, 15000);
						const timeout = Math.round(base + Math.random() * base * 0.5);
						log.info(
							`Reconnecting in ${timeout}ms (attempt ${attempts + 1}/${maxReconnectAttempts})`,
						);
	...
	const reconnect = useCallback(() => {
		...
		if (reconnectUrl.current) {
			setTimeout(
				() => connect(reconnectUrl.current, authTokenRef.current),
				1000,
			);
		}
	}, [connect, disconnect]);
