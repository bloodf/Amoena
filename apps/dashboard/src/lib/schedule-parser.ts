...
export function parseNaturalSchedule(input: string): ParsedSchedule | null {
	...
	if (lower === "weekly") {
		return {
			cronExpr: "0 9 * * 1",
			humanReadable: "Weekly on Monday at 9:00 AM",
		};
	}
	...
	const dailyAt = lower.match(
		/^(?:every\s+(?:morning|evening|day)|daily)\s+at\s+(.+)$/,
	);
	if (dailyAt) {
		...
		if (time) {
			...
			const displayMin =
				time.minute > 0 ? `:${String(time.minute).padStart(2, "0")}` : "";
			return {
	...
	if (weeklyOn) {
		...
		if (dayNum !== null) {
			const dayName =
				weeklyOn[1].charAt(0).toUpperCase() + weeklyOn[1].slice(1);
			return {
				cronExpr: `0 9 * * ${dayNum}`,
				humanReadable: `Weekly on ${dayName} at 9:00 AM`,
			};
		}
	}
	...
	if (everyDayAt) {
		...
		if (dayNum !== null) {
			...
			if (time) {
				const dayName =
					everyDayAt[1].charAt(0).toUpperCase() + everyDayAt[1].slice(1);
				const label = time.hour < 12 ? "AM" : "PM";
				const displayHour = time.hour % 12 || 12;
				const displayMin =
					time.minute > 0 ? `:${String(time.minute).padStart(2, "0")}` : "";
