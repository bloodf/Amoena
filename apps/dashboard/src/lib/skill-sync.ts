	...
	const roots: Array<{ source: string; path: string }> = [
		{
			source: "user-agents",
			path:
				process.env.MC_SKILLS_USER_AGENTS_DIR ||
		...
		},
		{
			...
			path:
				process.env.MC_SKILLS_PROJECT_CODEX_DIR ||
...
export async function syncSkillsFromDisk(): Promise<{
	...
	try {
		...
		for (const s of diskSkills) {
			if (
				s.source.startsWith("workspace-") &&
				!localSources.includes(s.source)
			) {
    ...
    `);
		const deleteStmt = db.prepare(
			`DELETE FROM skills WHERE source = ? AND name = ?`,
		);
