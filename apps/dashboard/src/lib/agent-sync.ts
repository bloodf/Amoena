	...
	try {
		...
		if (existsSync(safePath)) {
			...
			if (size > MAX_WORKSPACE_FILE_BYTES) {
				logger.warn(
					{ workspace, filename, size },
					`Workspace file exceeds ${MAX_WORKSPACE_FILE_BYTES} byte limit, skipping`,
				);
				return null;
			}
...
export function enrichAgentConfigFromWorkspace(configData: any): any {
	...
	const workspace =
		typeof configData.workspace === "string" ? configData.workspace : undefined;
	...
	const mergedIdentity = {
		...
		...(configData.identity && typeof configData.identity === "object"
			? configData.identity
			: {}),
	};
	const mergedTools = {
		...parseToolsFromFile(toolsFile || ""),
		...(configData.tools && typeof configData.tools === "object"
			? configData.tools
			: {}),
	};
	...
	return {
		...configData,
		identity:
			Object.keys(mergedIdentity).length > 0
				? mergedIdentity
				: configData.identity,
	...
	db.transaction(() => {
		for (const agent of agents) {
			...
			if (existing) {
				...
				const configChanged =
					existingConfig !== configJson || existing.role !== mapped.role;
				const soulChanged =
					mapped.soul_content !== null && mapped.soul_content !== existingSoul;
...
export async function previewSyncDiff(): Promise<SyncDiff> {
	...
	const db = getDatabase();
	const allMCAgents = db
		.prepare("SELECT name, role, config FROM agents")
		.all() as Array<{ name: string; role: string; config: string }>;
	const _mcNames = new Set(allMCAgents.map((a) => a.name));
...
export async function writeAgentToConfig(agentConfig: any): Promise<void> {
	...
	const idx = parsed.agents.list.findIndex(
		(a: any) => a.id === normalizedAgentConfig.id,
	);
	if (idx >= 0) {
...
export async function removeAgentFromConfig(match: {
	...
	const raw = await readFile(configPath, "utf-8");
	const parsed = parseJsonRelaxed<any>(raw);
	const existingList = Array.isArray(parsed?.agents?.list)
		? parsed.agents.list
		: [];
...
function normalizeModelConfig(model: unknown): unknown {
	...
	return {
		...
		...(Array.isArray(normalizedFallbacks)
