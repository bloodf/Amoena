...
const LABEL_STATUS_MAP: Record<string, TaskStatus> = Object.fromEntries(
	Object.entries(STATUS_LABEL_MAP).map(([status, def]) => [
		def.name,
...
const LABEL_PRIORITY_MAP: Record<string, TaskPriority> = Object.fromEntries(
	Object.entries(PRIORITY_LABEL_MAP).map(([priority, def]) => [
		def.name,
...
export const ALL_STATUS_LABEL_NAMES = Object.values(STATUS_LABEL_MAP).map(
	(l) => l.name,
);
export const ALL_PRIORITY_LABEL_NAMES = Object.values(PRIORITY_LABEL_MAP).map(
	(l) => l.name,
);
