...
import { callAmoenaGateway } from './lunaria-gateway';
...
function resolveGatewayAgentId(task: DispatchableTask): string {
  if (task.agent_config) {
    try {
      const cfg = JSON.parse(task.agent_config);
      if (typeof cfg.amoenaId === 'string' && cfg.amoenaId) return cfg.amoenaId;
    } catch {
      /* ignore */
    }
  }
...
function getAgentSoulContent(task: DispatchableTask): string | null {
  try {
    const db = getDatabase();
    const row = db
      .prepare('SELECT soul_content FROM agents WHERE id = ? AND workspace_id = ?')
    ...
    try {
      ...
      if (verdict.status === 'approved') {
        ...
        const currentAttempts =
          (
            db.prepare('SELECT dispatch_attempts FROM tasks WHERE id = ?').get(task.id) as
...
export async function dispatchAssignedTasks(): Promise<{
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      const taskMeta = (() => {
        try {
          const row = db.prepare('SELECT metadata FROM tasks WHERE id = ?').get(task.id) as
            | { metadata: string }
            | undefined;
...
function scoreAgentForTask(
  ...
  if (agent.status === 'offline' || agent.status === 'error' || agent.status === 'sleeping')
    return -1;
