...
function buildTaskPrompt(
	...
	const ticket =
		task.ticket_prefix && task.project_ticket_no
...
function getAgentSoulContent(task: DispatchableTask): string | null {
	try {
		...
		const row = db
			...
			.get(task.agent_id, task.workspace_id) as
...
async function callClaudeDirectly(
	...
	if (!apiKey)
		throw new Error(
			"ANTHROPIC_API_KEY not set — cannot dispatch without gateway",
		);
	...
	logger.info(
		...
		"Dispatching task via direct Claude API",
	);
 ...
 */
export async function runAegisReviews(): Promise<{
	ok: boolean;
	message: string;
}> {
	const db = getDatabase();
      ...
      `).run(task.id, verdict.status, verdict.notes, task.workspace_id);
			...
			if (verdict.status === "approved") {
				...
				const currentAttempts =
					(
						db
							.prepare("SELECT dispatch_attempts FROM tasks WHERE id = ?")
							.get(task.id) as { dispatch_attempts: number } | undefined
					)?.dispatch_attempts ?? 0;
				...
				if (newAttempts >= maxAegisRetries) {
					...
					db.prepare(
						...
						"failed",
						`Aegis rejected ${newAttempts} times. Last: ${verdict.notes}`,
						newAttempts,
						now,
						task.id,
					);
        ...
        `).run(
					task.id,
					`Quality Review Rejected (attempt ${newAttempts}/${maxAegisRetries}):\n${verdict.notes}`,
					now,
					task.workspace_id,
				);
			}
			...
			logger.info(
				{ taskId: task.id, verdict: verdict.status },
				"Aegis review completed",
			);
		} catch (err: any) {
 ...
 */
export async function requeueStaleTasks(): Promise<{
	ok: boolean;
	message: string;
}> {
	const db = getDatabase();
	...
	for (const task of staleTasks) {
		...
		if (newAttempts >= maxDispatchRetries) {
			...
			db.prepare(
				...
				"assigned",
				`Requeued: agent "${task.assigned_to}" went offline while task was in_progress`,
				newAttempts,
				now,
				task.id,
			);
...
export async function dispatchAssignedTasks(): Promise<{
	ok: boolean;
	message: string;
}> {
	const db = getDatabase();
  ...
  `)
	...
	for (const task of tasks) {
		if (typeof task.tags === "string") {
			try {
				task.tags = JSON.parse(task.tags as string);
			} catch {
				task.tags = undefined;
			}
		}
	...
	for (const task of tasks) {
      ...
      `)
			...
			const rejectionFeedback =
				rejectionRow?.content?.replace(/^Quality Review Rejected:\n?/, "") ||
				null;
			...
			const targetSession: string | null =
				typeof taskMeta?.target_session === "string" && taskMeta.target_session
			...
			if (useDirectApi && !targetSession) {
				...
				if (status !== "started" && status !== "ok" && status !== "in_flight") {
					throw new Error(
						`chat.send to session ${targetSession} returned status: ${status}`,
					);
				}
				...
				if (
					...
					finalPayload?.result?.meta?.agentMeta?.sessionId
				) {
					agentResponse.sessionId =
						finalPayload.result.meta.agentMeta.sessionId;
				}
			...
			const truncated =
				agentResponse.text.length > 10_000
      ...
      `).run(
			...
			const errorMsg = err.message || "Unknown error";
			logger.error(
				{ taskId: task.id, agent: task.agent_name, err },
	...
	if (agent.config) {
		try {
			...
			for (const cap of caps) {
				if (typeof cap === "string" && text.includes(cap.toLowerCase()))
					score += 15;
			}
		} catch {
			/* ignore */
 ...
 */
export async function autoRouteInboxTasks(): Promise<{
	ok: boolean;
	message: string;
}> {
	const db = getDatabase();
  ...
  `)
	...
	if (agents.length === 0) {
		return {
			ok: true,
			message: `${inboxTasks.length} inbox task(s) but no available agents`,
		};
	}
	...
	for (const task of inboxTasks) {
		...
		const inProgressCount = (
			db
				...
				.get(best.name, task.workspace_id) as { c: number }
		).c;
			...
			const alt = scored.find((s) => {
				const c = (
					db
						...
						.get(s.agent.name, task.workspace_id) as { c: number }
				).c;
				return c < 3;
			});
