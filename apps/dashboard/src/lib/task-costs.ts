...
export function buildTaskCostReport(
	records: TokenCostRecord[],
	taskMetadata: Record<number, TaskCostMetadata>,
): TaskCostReport {
	const attributedRecords = records.filter((record) =>
		Number.isFinite(record.taskId),
	);
	const unattributedRecords = records.filter(
		(record) => !Number.isFinite(record.taskId),
	);
		...
		.map(([taskIdRaw, taskRecords]) => {
			...
			return {
				...
				project: {
					...
					ticketRef: formatTicketRef(
						meta.project_prefix,
						meta.project_ticket_no,
					),
				},
	...
	for (const [agent, agentRecords] of Object.entries(byAgent)) {
		const taskIds = [...(agentTaskIds[agent] || new Set<number>())].sort(
			(a, b) => a - b,
		);
		agents[agent] = {
	...
	for (const [projectKey, projectRecords] of Object.entries(byProject)) {
		const taskIds = [...(projectTaskIds[projectKey] || new Set<number>())].sort(
			(a, b) => a - b,
		);
		projects[projectKey] = {
	...
	return {
		summary: calculateStats(
			attributedRecords.filter(
				(record) =>
					Number.isFinite(record.taskId) && taskMetadata[Number(record.taskId)],
			),
		),
		tasks,
		agents,
