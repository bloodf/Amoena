...
describe("parseMentions", () => {
	it("extracts multiple mentions", () => {
		expect(db_helpers.parseMentions("@alice hello @bob")).toEqual([
			"alice",
			"bob",
		]);
	});
	...
	it("handles mentions at start and end of string", () => {
		expect(db_helpers.parseMentions("@start and @end")).toEqual([
			"start",
			"end",
		]);
	});
	...
	it("supports hyphen, underscore, and dots in handles", () => {
		expect(
			db_helpers.parseMentions("ping @agent-code_reviewer.v2 now"),
		).toEqual(["agent-code_reviewer.v2"]);
	});

	it("deduplicates repeated mentions case-insensitively", () => {
		expect(db_helpers.parseMentions("@Alice please sync with @alice")).toEqual([
			"Alice",
		]);
	});
