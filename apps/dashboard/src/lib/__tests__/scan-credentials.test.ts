...
import { redactSecrets, scanForSecrets } from "@/lib/secret-scanner";
...
describe("scanForSecrets", () => {
	...
	it("detects Stripe test keys with warning severity", () => {
		...
		expect(hits.find((h) => h.type === "stripe_test_key")?.severity).toBe(
			"warning",
		);
	});
	...
	it("detects mongodb+srv connection strings", () => {
		const hits = scanForSecrets(
			"mongodb+srv://admin:pw123@cluster0.mongodb.net/db",
		);
		expect(hits.some((h) => h.type === "db_connection_string")).toBe(true);
	});
	...
	it("returns no false positives on normal text", () => {
		const hits = scanForSecrets(
			"Hello, this is a normal message about deploying our application to production.",
		);
...
describe("scanForSecrets - new patterns", () => {
	it("detects Slack webhook URLs", () => {
		...
		expect(hits.find((h) => h.type === "slack_webhook")?.severity).toBe(
			"critical",
		);
	});
