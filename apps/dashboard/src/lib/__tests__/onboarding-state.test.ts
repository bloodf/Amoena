...
describe("onboarding-state", () => {
	...
	it("computes current step index from completed steps", () => {
		...
		expect(
			nextIncompleteStepIndex(BASE_STEPS, [
				"welcome",
				"interface-mode",
				"credentials",
			]),
	...
	it("shows onboarding only for admin users when not completed/skipped", () => {
		expect(
			shouldShowOnboarding({ completed: false, skipped: false, isAdmin: true }),
		).toBe(true);
		expect(
			shouldShowOnboarding({ completed: true, skipped: false, isAdmin: true }),
		).toBe(false);
		expect(
			shouldShowOnboarding({ completed: false, skipped: true, isAdmin: true }),
		).toBe(false);
		expect(
			shouldShowOnboarding({
				completed: false,
