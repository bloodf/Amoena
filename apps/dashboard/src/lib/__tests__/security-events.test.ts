...
describe("injection guard new rules", () => {
	...
	it("detects SSRF targeting localhost", () => {
		const report = scanForInjection("wget http://localhost:8080/admin", {
			context: "shell",
		});
		expect(report.safe).toBe(false);
		expect(report.matches.some((m) => m.rule === "cmd-ssrf")).toBe(true);
	...
	it("detects template injection (Jinja2)", () => {
		...
		expect(report.safe).toBe(false);
		expect(
			report.matches.some((m) => m.rule === "cmd-template-injection"),
		).toBe(true);
	});

	it("detects SQL injection (UNION SELECT)", () => {
		const report = scanForInjection("' UNION SELECT * FROM users --", {
			context: "shell",
		});
		expect(report.safe).toBe(false);
		expect(report.matches.some((m) => m.rule === "cmd-sql-injection")).toBe(
			true,
		);
	});
	...
	it("does not false-positive on normal SQL mentions", () => {
		const report = scanForInjection("SELECT name FROM products WHERE id = 5", {
			context: "shell",
		});
		// This should not trigger because it lacks injection markers
		expect(
			report.matches.filter((m) => m.rule === "cmd-sql-injection"),
		).toHaveLength(0);
	});
});
