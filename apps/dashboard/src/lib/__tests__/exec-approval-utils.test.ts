...
describe("matchesGlobPattern", () => {
	...
	it("matches npm install with argument", () => {
		expect(matchesGlobPattern("npm install *", "npm install lodash")).toBe(
			true,
		);
	});
	...
	it("matches multi-word prefix with wildcard", () => {
		expect(matchesGlobPattern("docker compose *", "docker compose up -d")).toBe(
			true,
		);
	});
...
describe("findMatchingPatterns", () => {
	it("returns only matching patterns", () => {
		expect(findMatchingPatterns(["git *", "npm *"], "git status")).toEqual([
			"git *",
		]);
	});

	it("returns multiple matches including catch-all", () => {
		expect(findMatchingPatterns(["git *", "*"], "git status")).toEqual([
			"git *",
			"*",
		]);
	});
