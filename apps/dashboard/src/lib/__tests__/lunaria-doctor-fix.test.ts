...
describe("archiveOrphanTranscriptsForStateDir", () => {
	it("archives only unreferenced primary transcript files", () => {
		...
		fs.writeFileSync(
			path.join(sessionsDir, "keep-session.jsonl"),
			'{"type":"session"}\n',
		);
		fs.writeFileSync(
			path.join(sessionsDir, "custom-session.jsonl"),
			'{"type":"session"}\n',
		);
		fs.writeFileSync(
		...
		fs.writeFileSync(
			path.join(
				sessionsDir,
				"orphan-session.jsonl.reset.2026-03-11T00-00-00.000Z",
			),
			"old\n",
		);
		...
		expect(fs.existsSync(path.join(sessionsDir, "keep-session.jsonl"))).toBe(
			true,
		);
		expect(fs.existsSync(path.join(sessionsDir, "custom-session.jsonl"))).toBe(
			true,
		);
		expect(fs.existsSync(path.join(sessionsDir, "orphan-session.jsonl"))).toBe(
			false,
		);
		expect(
			fs
				.readdirSync(sessionsDir)
				.some((name) => name.startsWith("orphan-session.jsonl.deleted.")),
		).toBe(true);
	});
});
