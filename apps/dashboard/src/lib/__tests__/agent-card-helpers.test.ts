...
describe("formatModelName", () => {
	it("strips provider prefix from model ID", () => {
		expect(
			formatModelName({ model: { primary: "anthropic/claude-opus-4-5" } }),
		).toBe("claude-opus-4-5");
	});

	it("returns model name when no prefix", () => {
		expect(formatModelName({ model: { primary: "gpt-4o" } })).toBe("gpt-4o");
	});

	it("handles deeply nested provider path", () => {
		expect(formatModelName({ model: { primary: "openai/gpt-4/turbo" } })).toBe(
			"turbo",
		);
	});
...
describe("buildTaskStatParts", () => {
	...
	it("assigns correct colors to active/review/done", () => {
		...
		expect(result.find((p) => p.label === "active")?.color).toBe(
			"text-amber-300",
		);
		expect(result.find((p) => p.label === "review")?.color).toBe(
			"text-violet-300",
		);
		expect(result.find((p) => p.label === "done")?.color).toBe(
			"text-emerald-300",
		);
	});
});
...
describe("extractWsHost", () => {
	...
	it("extracts host from wss:// URL", () => {
		expect(extractWsHost("wss://gateway.example.com")).toBe(
			"gateway.example.com",
		);
	});

	it("extracts host with port from wss:// URL", () => {
		expect(extractWsHost("wss://gateway.example.com:4443")).toBe(
			"gateway.example.com:4443",
		);
	});
