		...
		return {
			get: (...args: unknown[]) => {
				if (
					...
					normalized.includes("where id = ? and tenant_id = ?")
				) {
				...
				if (
					normalized.includes("from projects p") &&
					normalized.includes("join workspaces w")
				) {
				...
				if (
					normalized.startsWith(
						"select action, actor, target_type, target_id, detail from audit_log",
					)
				) {
