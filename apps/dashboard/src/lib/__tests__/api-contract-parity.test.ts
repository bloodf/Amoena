...
describe("api-contract-parity helpers", () => {
	it("maps Next.js route files to OpenAPI-style API paths", () => {
		expect(routeFileToApiPath("src/app/api/agents/route.ts")).toBe(
			"/api/agents",
		);
		expect(routeFileToApiPath("src/app/api/tasks/[id]/route.ts")).toBe(
			"/api/tasks/{id}",
		);
		expect(routeFileToApiPath("src/app/api/files/[...slug]/route.ts")).toBe(
			"/api/files/{slug}",
		);
		expect(
			routeFileToApiPath("src/app/api/optional/[[...tail]]/route.ts"),
		).toBe("/api/optional/{tail}");
	});
