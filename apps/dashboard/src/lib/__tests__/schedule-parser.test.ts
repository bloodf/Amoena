...
import { isCronDue, parseNaturalSchedule } from "../schedule-parser";
...
describe("parseNaturalSchedule", () => {
	...
	it('parses "every N minutes"', () => {
		expect(parseNaturalSchedule("every 5 minutes")?.cronExpr).toBe(
			"*/5 * * * *",
		);
		expect(parseNaturalSchedule("every 1 minute")?.cronExpr).toBe(
			"*/1 * * * *",
		);
		expect(parseNaturalSchedule("every 30 minutes")?.cronExpr).toBe(
			"*/30 * * * *",
		);
	});
	...
	it('parses "weekly on DAYNAME"', () => {
		expect(parseNaturalSchedule("weekly on monday")?.cronExpr).toBe(
			"0 9 * * 1",
		);
		expect(parseNaturalSchedule("weekly on friday")?.cronExpr).toBe(
			"0 9 * * 5",
		);
		expect(parseNaturalSchedule("weekly on sunday")?.cronExpr).toBe(
			"0 9 * * 0",
		);
	});
