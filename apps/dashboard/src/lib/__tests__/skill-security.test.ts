...
describe("checkSkillSecurity", () => {
	...
	it('detects "ignore previous instructions" injection', () => {
		...
		expect(
			report.issues.some((i) => i.rule === "prompt-injection-system"),
		).toBe(true);
	});
	...
	it("detects role manipulation injection", () => {
		...
		const report = checkSkillSecurity(content);
		expect(report.status).toBe("rejected");
		expect(report.issues.some((i) => i.rule === "prompt-injection-role")).toBe(
			true,
		);
	});
	...
	it("detects rm -rf in bash code block", () => {
		...
		const report = checkSkillSecurity(content);
		expect(report.status).toBe("rejected");
		expect(report.issues.some((i) => i.rule === "shell-exec-dangerous")).toBe(
			true,
		);
	});
	...
	it("detects data exfiltration instruction", () => {
		...
		const report = checkSkillSecurity(content);
		expect(report.status).toBe("rejected");
		expect(report.issues.some((i) => i.rule === "data-exfiltration")).toBe(
			true,
		);
	});
	...
	it("warns on hardcoded API key", () => {
		...
		expect(report.issues.some((i) => i.rule === "credential-harvesting")).toBe(
			true,
		);
	});
	...
	it("warns on base64 decode usage", () => {
		...
		const report = checkSkillSecurity(content);
		expect(report.status).toBe("warning");
		expect(report.issues.some((i) => i.rule === "obfuscated-content")).toBe(
			true,
		);
	});
	...
	it("warns on hidden injection in HTML comment", () => {
		...
		const report = checkSkillSecurity(content);
		expect(report.status).toBe("warning");
		expect(report.issues.some((i) => i.rule === "hidden-instructions")).toBe(
			true,
		);
	});
	...
	it("warns on sudo usage", () => {
		...
		const report = checkSkillSecurity(content);
		expect(report.status).toBe("warning");
		expect(report.issues.some((i) => i.rule === "excessive-permissions")).toBe(
			true,
		);
	});
	...
	it("flags external fetch URLs as info", () => {
		...
		expect(
			report.issues.some(
				(i) => i.rule === "network-fetch" && i.severity === "info",
			),
		).toBe(true);
	});
	...
	it("detects SSRF targeting localhost", () => {
		...
		const report = checkSkillSecurity(content);
		expect(report.status).toBe("rejected");
		expect(report.issues.some((i) => i.rule === "ssrf-internal-network")).toBe(
			true,
		);
	});
	...
	it("detects SSRF targeting AWS metadata endpoint", () => {
		...
		const report = checkSkillSecurity(content);
		expect(report.status).toBe("rejected");
		expect(report.issues.some((i) => i.rule === "ssrf-metadata-endpoint")).toBe(
			true,
		);
	});
	...
	it("includes line numbers for found issues", () => {
		...
		const report = checkSkillSecurity(content);
		const injection = report.issues.find(
			(i) => i.rule === "prompt-injection-system",
		);
