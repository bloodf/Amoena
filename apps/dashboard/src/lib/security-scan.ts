...
function scoreCategory(checks: Check[]): Category {
	...
	return {
		score:
			weightedMax > 0 ? Math.round((weightedScore / weightedMax) * 100) : 100,
		checks,
	};
}
...
function scanCredentials(): Category {
	...
	checks.push({
		...
		fix:
			!apiKey || apiKey === "generate-a-random-key"
...
function scanNetwork(): Category {
	...
	checks.push({
		...
		detail:
			cookieSecure === "1" || cookieSecure === "true"
				? "Cookies marked secure"
				: "Cookies not explicitly set to secure",
		fix: !(cookieSecure === "1" || cookieSecure === "true")
			? "Set MC_COOKIE_SECURE=1 in .env (requires HTTPS)"
			: "",
		severity: "medium",
	});
...
function scanLunaria(): Category {
	...
	const tokenOk =
		gwAuth?.mode === "token" && (gwAuth?.token ?? "").trim().length > 0;
	const passwordOk =
		gwAuth?.mode === "password" && (gwAuth?.password ?? "").trim().length > 0;
	...
	checks.push({
		...
		fix:
			gwBind !== "loopback"
				? 'Set gateway.bind to "loopback" to prevent external access'
				: "",
		severity: "critical",
	});
	...
	checks.push({
		...
		fix:
			toolsProfile === "all"
				? 'Use a restrictive tools profile like "messaging" or "coding"'
				: "",
		severity: "low",
	});
	...
	checks.push({
		...
		detail:
			elevated === true
				? "Elevated mode is enabled"
				: "Elevated mode is disabled",
		fix:
			elevated === true
				? "Set elevated.enabled to false unless explicitly needed"
				: "",
		severity: "high",
	});
	...
	checks.push({
		...
		fix:
			execSecurity !== "deny" && execSecurity !== "allowlist"
	...
	checks.push({
		...
		detail: logRedact
			? `Log redaction: ${logRedact}`
			: "Sensitive data redaction is not configured",
		fix: !logRedact
			? 'Set logging.redactSensitive to "tools" to prevent secrets leaking into logs'
			: "",
		severity: "low",
	});
	...
	checks.push({
		...
		detail: sandboxMode
			...
			: "No default sandbox mode configured",
		fix:
			sandboxMode !== "all"
	...
	if (Array.isArray(safeBins) && safeBins.length > 0) {
		...
		const unsafeInterpreters = safeBins.filter((b: string) =>
			interpreters.includes(b),
		);
		const safeBinProfiles = ocConfig?.tools?.exec?.safeBinProfiles || {};
		const unprofiledInterps = unsafeInterpreters.filter(
			(b: string) => !safeBinProfiles[b],
		);
		checks.push({
...
function scanRuntime(): Category {
	...
	try {
		...
		if (existsSync(backupDir)) {
			...
			if (files.length > 0) {
				...
				checks.push({
					...
					fix:
						ageHours >= 24
							? "Enable auto_backup in Settings or run: curl -X POST /api/backup"
							: "",
					severity: "medium",
				});
			} else {
				checks.push({
					...
					fix: "Enable auto_backup in Settings",
					severity: "medium",
				});
			}
		} else {
		...
		checks.push({
			...
			name: "Recent backup exists",
			status: "warn",
			detail: "Could not check backups",
			fix: "",
			severity: "medium",
		});
	}
...
function scanOS(): Category {
	...
	if (isLinux && uid !== undefined && uid !== 0) {
		...
		checks.push({
			...
			detail: hasCaps
				? `Node binary has capabilities: ${caps}`
				: "Node binary has no special capabilities",
	...
	if (uptimeSeconds === null) {
		...
		checks.push({
			...
			fix:
				uptimeDays >= 30
					? "Consider rebooting to apply kernel and system updates"
					: "",
			severity: "low",
			platform: "all",
		});
	...
	if (isLinux) {
		...
		const ntpActive =
			ntpStatus?.toLowerCase().includes("yes") ||
			ntpStatus?.toLowerCase().includes("active");
		checks.push({
			id: "ntp_sync",
			...
			detail: ntpActive
				...
				: "NTP sync status unknown or inactive",
	...
	if (isLinux) {
		...
		checks.push({
			...
			fix:
				!hasUfw && !hasIptables && !hasNft
					? "Enable a firewall: sudo ufw enable"
					: "",
			severity: "critical",
			platform: "linux",
		});
	} else if (isDarwin) {
		const pfStatus = tryExec(
		...
		checks.push({
			...
			detail:
				autoUpdate === "1"
					...
					: "Automatic update status unknown",
			fix:
				autoUpdate !== "1"
	...
	if (isLinux || isDarwin) {
		...
		const wwFiles = tryExec(
			`find "${cwd}" -maxdepth 2 -perm -o+w -not -type l 2>/dev/null | head -5`,
		);
	...
	if (isLinux) {
		...
		checks.push({
			...
			fix: coreToFile
				? 'Restrict core dumps: echo "|/bin/false" > /proc/sys/kernel/core_pattern'
				: "",
			severity: "medium",
			fixSafety: "manual-only",
			platform: "linux",
		...
		// MAC framework
		const selinux = cachedExec(
			"selinux",
			"cat /sys/fs/selinux/enforce 2>/dev/null",
		);
		const apparmor = cachedExec(
			"apparmor",
		...
		checks.push({
			...
			detail: tmpNoexec
				...
				: "/tmp may allow execution — consider noexec mount",
			fix: !tmpNoexec
				? "Add noexec,nosuid,nodev to /tmp mount options in /etc/fstab"
				: "",
			severity: "medium",
			fixSafety: "manual-only",
			platform: "linux",
	...
	if (isDarwin) {
		...
		checks.push({
			...
			detail: sipEnabled
				? "SIP is enabled"
				: "SIP is disabled — system files are unprotected",
			fix: !sipEnabled
				? "Re-enable SIP from Recovery Mode: csrutil enable"
				: "",
			severity: "critical",
			fixSafety: "manual-only",
			platform: "darwin",
		...
		const stealthStatus = cachedExec(
			"stealth",
		...
		checks.push({
			...
			detail:
				guestDisabled || guestAccount === null
	...
	if (isWindows) {
		const defenderStatus = cachedExec(
			"win_defender",
			'powershell -NoProfile -Command "(Get-MpComputerStatus).RealTimeProtectionEnabled" 2>nul',
		);
		checks.push({
			id: "win_defender",
			...
			fix:
				defenderStatus !== "True"
					? "Enable Windows Defender real-time protection in Windows Security settings"
					: "",
			severity: "critical",
			fixSafety: "manual-only",
			platform: "win32",
		...
		const fwProfiles = cachedExec(
			"win_firewall",
			'powershell -NoProfile -Command "(Get-NetFirewallProfile | Where-Object {$_.Enabled -eq $true}).Count" 2>nul',
		);
		...
		checks.push({
			...
			detail:
				fwCount >= 3
					...
					: `${fwCount} of 3 firewall profiles active`,
			fix:
				fwCount < 3
					? "Enable all firewall profiles in Windows Defender Firewall settings"
					: "",
			severity: "critical",
			fixSafety: "manual-only",
			platform: "win32",
		...
		const bitlocker = cachedExec(
			"win_bitlocker",
			'powershell -NoProfile -Command "(Get-BitLockerVolume -MountPoint C:).ProtectionStatus" 2>nul',
		);
		checks.push({
			id: "win_bitlocker",
			...
			detail:
				bitlocker === "On"
					? "BitLocker is active on C:"
					: "BitLocker is not active on C:",
			fix:
				bitlocker !== "On"
					? "Enable BitLocker in Control Panel > BitLocker Drive Encryption"
					: "",
			severity: "high",
			fixSafety: "manual-only",
			platform: "win32",
		...
		const uac = cachedExec(
			"win_uac",
			'powershell -NoProfile -Command "(Get-ItemProperty HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System).EnableLUA" 2>nul',
		);
		checks.push({
			id: "win_uac",
			name: "UAC enabled",
			...
			detail:
				uac === "1" ? "User Account Control is enabled" : "UAC is disabled",
			fix:
				uac !== "1"
					? "Enable UAC in Control Panel > User Account Control Settings"
					: "",
			severity: "high",
			fixSafety: "manual-only",
			platform: "win32",
		...
		const rdp = cachedExec(
			"win_rdp",
			"powershell -NoProfile -Command \"(Get-ItemProperty 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server').fDenyTSConnections\" 2>nul",
		);
		checks.push({
			id: "win_rdp_disabled",
			...
			fix:
				rdp !== "1"
					? "Disable RDP if not needed: System Properties > Remote > disable Remote Desktop"
					: "",
			severity: "medium",
			fixSafety: "manual-only",
			platform: "win32",
		...
		const smb1 = cachedExec(
			"win_smb1",
			'powershell -NoProfile -Command "(Get-SmbServerConfiguration).EnableSMB1Protocol" 2>nul',
		);
		checks.push({
