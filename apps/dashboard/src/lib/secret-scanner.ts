...
const SECRET_PATTERNS: SecretPattern[] = [
	...
	// GitHub tokens
	{
		type: "github_token",
		severity: "critical",
		regex: /gh[ps]_[A-Za-z0-9_]{36,255}/g,
	},
	{
	...
	// Generic API key patterns (key=... or api_key=...)
	{
		...
		severity: "warning",
		regex:
			/(?:api[_-]?key|apikey|api[_-]?secret)\s*[=:]\s*['"]?[A-Za-z0-9_-]{20,64}['"]?/gi,
	},
	...
	// Database connection strings
	{
		...
		severity: "critical",
		regex:
			/(?:postgres|postgresql|mysql|mongodb|mongodb\+srv|redis):\/\/[^\s'"]{10,}/gi,
	},
	...
	// Discord webhooks
	{
		...
		severity: "critical",
		regex:
			/https:\/\/discord(?:app)?\.com\/api\/webhooks\/\d{17,}\/[A-Za-z0-9_-]{60,}/g,
	},
	...
	// SSH private key content (full PEM block)
	{
		...
		severity: "critical",
		regex:
			/-----BEGIN\s(?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----[\s\S]{20,}?-----END\s(?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----/g,
	},
];
