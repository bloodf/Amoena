...
export async function syncLocalAgents(): Promise<{
	ok: boolean;
	message: string;
}> {
	try {
 ...
 */
export function writeLocalAgentSoul(
	agentDir: string,
	soulContent: string,
): void {
	...
	const targetPath = existsSync(soulPath)
		? soulPath
		: existsSync(agentMdPath)
			? agentMdPath
			: soulPath;
	...
	try {
		...
		db.prepare(
			`UPDATE agents SET content_hash = ?, updated_at = ? WHERE workspace_path = ? AND source = 'local'`,
