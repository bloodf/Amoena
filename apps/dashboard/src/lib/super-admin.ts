...
function getTenantWorkspaceDirname(): string {
	return (
		String(process.env.MC_TENANT_WORKSPACE_DIRNAME || "workspace").trim() ||
		"workspace"
	);
}
...
function normalizeOwnerGateway(value: any, _slug: string): string {
...
function ensureProvisionArtifacts(job: any) {
	...
	if (
		...
		throw new Error(
			"Missing/invalid gateway_port for gateway unit provisioning",
		);
	}
...
export function listProvisionJobs(
  ...
  `)
		.all(...params) as Array<
		ProvisionJob & { tenant_slug: string; tenant_display_name: string }
	>;
...
export function createTenantAndBootstrapJob(
	...
	if (!isValidSlug(slug)) {
		throw new Error(
			"Invalid slug. Use lowercase letters, numbers, and dashes (3-32 chars).",
		);
	}
...
export async function executeProvisionJob(jobId: number, actor: string) {
	...
	const plan = Array.isArray(job.plan_json)
		? (job.plan_json as ProvisionStep[])
		: [];
