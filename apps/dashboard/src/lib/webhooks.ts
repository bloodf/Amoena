...
const MAX_RETRIES =
	parseInt(process.env.MC_WEBHOOK_MAX_RETRIES || "5", 10) || 5;
...
export function initWebhookListener() {
	eventBus.on("server-event", (event: ServerEvent) => {
		...
		const isAgentError =
			event.type === "agent.status_changed" && event.data?.status === "error";
		const workspaceId =
			typeof event.data?.workspace_id === "number"
				? event.data.workspace_id
				: 1;

		fireWebhooksAsync(webhookEventType, event.data, workspaceId).catch(
			(err) => {
...
async function fireWebhooksAsync(
	...
	const resolvedWorkspaceId =
		workspaceId ??
		(typeof payload?.workspace_id === "number" ? payload.workspace_id : 1);
	...
	try {
		...
		webhooks = db
			.prepare("SELECT * FROM webhooks WHERE enabled = 1 AND workspace_id = ?")
	...
	await Promise.allSettled(
		matchingWebhooks.map((wh) =>
			deliverWebhook(wh, eventType, payload, { allowRetry: true }),
		),
	);
}
...
async function deliverWebhook(
    ...
    `).run(statusCode ?? -1, webhook.id, webhook.workspace_id ?? 1);
		...
		if (allowRetry) {
			if (success) {
				...
				).run(webhook.id, webhook.workspace_id ?? 1);
			} else {
				...
				if (attempt < MAX_RETRIES - 1) {
					...
					const wh = db
						...
						.get(webhook.id, webhook.workspace_id ?? 1) as
						| { consecutive_failures: number }
						| undefined;
					if (wh && wh.consecutive_failures >= MAX_RETRIES) {
						db.prepare(
							`UPDATE webhooks SET enabled = 0, updated_at = unixepoch() WHERE id = ? AND workspace_id = ?`,
						).run(webhook.id, webhook.workspace_id ?? 1);
						logger.warn(
    ...
    `).run(
		...
		logger.error(
			{ err: logErr, webhookId: webhook.id },
			"Webhook delivery logging/pruning failed",
		);
	}
