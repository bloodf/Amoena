		...
		function connect() {
			...
			es.onerror = () => {
				...
				if (attempts >= SSE_MAX_RECONNECT_ATTEMPTS) {
					log.error(
						`Max reconnect attempts (${SSE_MAX_RECONNECT_ATTEMPTS}) reached`,
					);
					return;
				}
				...
				log.warn(
					`Reconnecting in ${delay}ms (attempt ${attempts + 1}/${SSE_MAX_RECONNECT_ATTEMPTS})`,
				);
				...
				case "chat.message":
					if (event.data?.id) {
						addChatMessage({
							...
							created_at:
								event.data.created_at || Math.floor(Date.now() / 1000),
						});
					}
					break;
