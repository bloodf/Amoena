 ...
 */
export function getAllGatewaySessions(
	...
	if (
		!force &&
		_sessionCache &&
		now - _sessionCache.ts < SESSION_CACHE_TTL_MS
	) {
			...
			try {
				...
				for (const [key, entry] of Object.entries(data)) {
					...
					sessions.push({
						...
						channel:
							s.deliveryContext?.channel || s.lastChannel || s.channel || "",
						model:
							typeof s.model === "object" && s.model?.primary
								? String(s.model.primary)
								: String(s.model || ""),
...
export function pruneGatewaySessionsOlderThan(retentionDays: number): {
	deleted: number;
	filesTouched: number;
} {
 ...
 */
export function getAgentLiveStatuses(): Map<
	string,
	{
