...
function writePersisted(values: PersistedValues): void {
	try {
		...
		const lines = ["# Auto-generated values. Overridden by env vars when set."];
...
const PLACEHOLDER_AUTH_SECRETS = new Set(["random-secret-for-legacy-cookies"]);
const PLACEHOLDER_API_KEYS = new Set(["generate-a-random-key"]);
...
export function ensureAutoGeneratedCredentials(): void {
	...
	if (!currentAuthSecret || PLACEHOLDER_AUTH_SECRETS.has(currentAuthSecret)) {
		if (persisted.AUTH_SECRET) {
			...
			dirty = true;
			logger.info(
				"Auto-generated AUTH_SECRET (persisted to .data/.auto-generated)",
			);
		}
	}
	...
	if (!currentApiKey || PLACEHOLDER_API_KEYS.has(currentApiKey)) {
		if (persisted.API_KEY) {
			...
			dirty = true;
			logger.info(
				"Auto-generated API_KEY (persisted to .data/.auto-generated)",
			);
		}
	}
