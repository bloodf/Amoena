 ...
 */
export function findTailscaleServePort(
 ...
 */
export function hasGwPathHandler(
	web: Record<string, any> | null | undefined,
): boolean {
	if (!web) return false;
	for (const hostConfig of Object.values(web) as any[]) {
...
export function refreshTailscaleCache(): void {
	...
	if (!_tailscaleServeJsonCache || now > _tailscaleServeJsonCache.expiresAt) {
		_tailscaleServeJsonCache = {
			value: execTailscaleServeJson(),
			expiresAt: now + TAILSCALE_CACHE_TTL_MS,
		};
...
export function isTailscaleServe(): boolean {
	...
	if (!_tailscaleServeCache || now > _tailscaleServeCache.expiresAt) {
		_tailscaleServeCache = {
			value: detectTailscaleServe(getCachedTailscaleWeb()),
			expiresAt: now + TAILSCALE_CACHE_TTL_MS,
		};
	}
