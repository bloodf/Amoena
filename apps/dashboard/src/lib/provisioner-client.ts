...
const DEFAULT_SOCKET =
	process.env.MC_PROVISIONER_SOCKET || "/run/mc-provisioner.sock";
...
export async function runProvisionerCommand(input: {
	...
	if (!response.ok) {
		const detail = [response.error, response.stderr, response.stdout]
			.filter(Boolean)
			.join(" | ");
...
function callProvisioner(
	payload: ProvisionerRequest,
): Promise<ProvisionerResponse> {
		...
		socket.on("data", (chunk) => {
			...
			if (idx !== -1) {
				...
				try {
					...
					reject(
						new Error(
							`Invalid provisioner response: ${(err as Error).message}`,
						),
					);
				}
			}
