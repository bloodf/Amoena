...
function allowedRoots(baseDir: string): string[] {
	const candidateRoots = DOC_ROOT_CANDIDATES.filter((root) =>
		existsSync(join(baseDir, root)),
	);
...
export async function readDocsContent(
	relativePath: string,
): Promise<{ content: string; size: number; modified: number; path: string }> {
...
export async function searchDocs(
	query: string,
	limit = 100,
): Promise<Array<{ path: string; name: string; matches: number }>> {
	const baseDir = config.memoryDir;
	if (!baseDir || !existsSync(baseDir)) return [];
