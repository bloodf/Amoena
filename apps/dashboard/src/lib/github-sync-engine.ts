...
export async function pushTaskToGitHub(
	...
	if (task.github_issue_number) {
		...
		try {
			...
			logger.error(
				{ err, repo, issue: task.github_issue_number },
				"Failed to fetch issue for update",
			);
			return;
		}
    ...
    `).run(now, task.id);
		...
		logger.info(
			{ repo, issue: task.github_issue_number },
			"Pushed task update to GitHub",
		);
    ...
    `).run(created.number, repo, now, task.id);
		...
		logger.info(
			{ repo, issue: created.number, taskId: task.id },
...
export async function pullFromGitHub(
  ...
  `)
	...
	for (const issue of issues) {
      ...
      `)
			...
			const issueUpdatedAt = Math.floor(
				new Date(issue.updated_at).getTime() / 1000,
			);
			...
			if (!existingTask) {
				...
				const tags = labelNames.filter(
					(l) =>
						!ALL_STATUS_LABEL_NAMES.includes(l) &&
						!ALL_PRIORITY_LABEL_NAMES.includes(l),
				);
        ...
        `).run(
				...
				if (
					existingTask.github_synced_at &&
					Math.abs(existingTask.github_synced_at - issueUpdatedAt) < 10
				) {
					continue;
				}
        ...
        `).run(
			...
			logger.error(
				{ err, issue: issue.number, repo },
				"Failed to sync GitHub issue",
			);
		}
	}
  ...
  `).run(repo, now, pulled, project.id, pushed, pulled, workspaceId);
	...
	logger.info(
		{ repo, pulled, pushed, projectId: project.id },
