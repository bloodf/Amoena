...
export function parseTokenUsage(tokens: string): {
	used: number;
	total: number;
	percentage: number;
} {
	...
	const used = parseInt(match[1], 10) * (match[1].includes("k") ? 1000 : 1);
	const total = parseInt(match[2], 10) * (match[2].includes("k") ? 1000 : 1);
...
export function normalizeModel(model: unknown): string {
	...
	if (model && typeof model === "object" && "primary" in model)
		return String((model as any).primary);
	return "";
}
