...
export function getDriftTimeline(
    ...
    `)
		...
		timeline.push({
			...
			successRate:
				(tools?.total ?? 0) > 0
					? Math.round((tools.successes / tools.total) * 10000) / 100
					: 100,
			completionRate:
				(tasks?.total ?? 0) > 0
					? Math.round((tasks.completed / tasks.total) * 10000) / 100
					: 100,
		});
	}
