...
function getSettingNumber(key: string, defaultValue: number): number {
	try {
		...
		const row = db
			.prepare("SELECT value FROM settings WHERE key = ?")
			.get(key) as { value: string } | undefined;
		if (row) return parseInt(row.value, 10) || defaultValue;
		return defaultValue;
	} catch {
		return defaultValue;
	}
...
async function runCleanup(): Promise<{ ok: boolean; message: string }> {
	try {
		...
		return {
			ok: true,
			message: `Cleaned ${totalDeleted} stale record${totalDeleted === 1 ? "" : "s"}`,
		};
	} catch (err: any) {
...
async function runHeartbeatCheck(): Promise<{ ok: boolean; message: string }> {
	try {
		const db = getDatabase();
		const now = Math.floor(Date.now() / 1000);
		const timeoutMinutes = getSettingNumber(
    ...
    `);
		...
		db.transaction(() => {
			for (const agent of staleAgents) {
				...
				logActivity.run(
					agent.id,
					`Agent "${agent.name}" marked offline (no heartbeat for ${timeoutMinutes}m)`,
				);
		...
		return {
			ok: true,
			message: `Marked ${staleAgents.length} agent(s) offline: ${names.join(", ")}`,
		};
	} catch (err: any) {
...
async function syncAgentLiveStatuses(): Promise<number> {
	...
	const db = getDatabase();
	const agents = db
		.prepare("SELECT id, name, config FROM agents")
		.all() as Array<{
		id: number;
		name: string;
		config: string | null;
	...
	const update = db.prepare(
		"UPDATE agents SET status = ?, last_seen = ?, last_activity = ?, updated_at = ? WHERE id = ?",
	);
	let refreshed = 0;
...
export function initScheduler() {
	...
	logger.info(
		"Scheduler initialized - backup at ~3AM, cleanup at ~4AM, heartbeat every 5m, webhook/claude/skill/local-agent/gateway-agent sync every 60s",
	);
}
						...
						: id === "webhook_retry"
							...
							: id === "claude_session_scan"
								...
								: id === "skill_sync"
									...
									: id === "local_agent_sync"
										? await syncLocalAgents()
										: id === "gateway_agent_sync"
											? await syncAgentsFromConfig("scheduled").then(
													async (r) => {
											...
											: id === "task_dispatch"
												? await autoRouteInboxTasks().then(
														async (routeResult) => {
															...
															].filter((m) => m && !m.includes("No "));
															return {
																ok: routeResult.ok && dispatchResult.ok,
																message:
																	parts.join(" | ") ||
																	"No tasks to route or dispatch",
															};
														},
													)
												: id === "aegis_review"
...
export async function triggerTask(
	...
	if (taskId === "gateway_agent_sync")
		return syncAgentsFromConfig("manual").then((r) => ({
			ok: true,
			message: `Gateway sync: ${r.created} created, ${r.updated} updated, ${r.synced} total`,
		}));
	if (taskId === "task_dispatch")
		return autoRouteInboxTasks().then(async (r) => {
