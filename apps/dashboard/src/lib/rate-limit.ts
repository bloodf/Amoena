...
const TRUSTED_PROXIES = new Set(
	(process.env.MC_TRUSTED_PROXIES || "")
		.split(",")
		.map((s) => s.trim())
		.filter(Boolean),
);
		...
		if (entry.count > options.maxRequests) {
			try {
				logSecurityEvent({
					...
					detail: JSON.stringify({ ip: key }),
					ip_address: typeof key === "string" ? key : "unknown",
					workspace_id: 1,
					tenant_id: 1,
				});
			} catch {}
			...
			return NextResponse.json(
				{
					error:
						options.message ||
						`${who} has exceeded the rate limit. Please try again later.`,
				},
				{
					status: 429,
					headers: {
