...
const INTERVAL_MS = parseInt(
	process.env.GITHUB_SYNC_INTERVAL_MS || "60000",
	10,
);
...
export function getSyncPollerStatus(): {
	running: boolean;
	interval: number;
	lastRun?: number;
} {
    ...
    `)
		...
		for (const project of projects) {
			try {
				...
				logger.error(
					{ err, projectId: project.id, repo: project.github_repo },
					"Sync poller: project sync failed",
				);
			}
		}
