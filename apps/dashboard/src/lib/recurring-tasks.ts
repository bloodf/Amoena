...
export async function spawnRecurringTasks(): Promise<{
	ok: boolean;
	message: string;
}> {
	try {
    ...
    `)
		...
		for (const template of templates) {
			...
			const lastSpawnedAtMs = recurrence.last_spawned_at
				? recurrence.last_spawned_at * 1000
				: 0;
      ...
      `)
          ...
          `).run(template.project_id, template.workspace_id);
				...
				const ticketRow = template.project_id
					? (db
							...
							.get(template.project_id, template.workspace_id) as
		...
		return {
			ok: true,
			message:
				spawned > 0 ? `Spawned ${spawned} recurring task(s)` : "No tasks due",
		};
	} catch (err: any) {
