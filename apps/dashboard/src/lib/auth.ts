...
const PROXY_AUTH_TRUSTED_IPS = new Set(
	(process.env.MC_PROXY_AUTH_TRUSTED_IPS || "")
		.split(",")
		.map((s) => s.trim())
		.filter(Boolean),
);
...
function resolveTenantForWorkspace(workspaceId: number): number {
	const db = getDatabase();
	const row = db
		.prepare(`SELECT tenant_id FROM workspaces WHERE id = ? LIMIT 1`)
		.get(workspaceId) as { tenant_id?: number } | undefined;
...
export function createSession(
	...
	const resolvedWorkspaceId =
		...
		((
			...
			getDefaultWorkspaceContext().workspaceId);
...
export function authenticateUser(
	...
	if (!row) {
		...
		try {
			logSecurityEvent({
				...
				severity: "warning",
				source: "auth",
				detail: JSON.stringify({ username, reason: "user_not_found" }),
				workspace_id: 1,
				tenant_id: 1,
			});
		} catch {}
	...
	if ((row.provider || "local") !== "local") {
		...
		try {
			logSecurityEvent({
				...
				severity: "warning",
				source: "auth",
				detail: JSON.stringify({ username, reason: "wrong_provider" }),
				workspace_id: 1,
				tenant_id: 1,
			});
		} catch {}
	...
	if ((row.is_approved ?? 1) !== 1) {
		...
		try {
			logSecurityEvent({
				...
				severity: "warning",
				source: "auth",
				detail: JSON.stringify({ username, reason: "not_approved" }),
				workspace_id: 1,
				tenant_id: 1,
			});
		} catch {}
	...
	const { valid, needsRehash } = verifyPasswordWithRehashCheck(
		password,
		row.password_hash,
	);
	if (!valid) {
	...
	if (needsRehash) {
		try {
			...
			/* non-fatal — will rehash on next login */
		}
	}
	return {
		id: row.id,
		...
		role: row.role,
		workspace_id: row.workspace_id || getDefaultWorkspaceContext().workspaceId,
		tenant_id: resolveTenantForWorkspace(
			row.workspace_id || getDefaultWorkspaceContext().workspaceId,
		),
		provider: row.provider || "local",
		email: row.email ?? null,
		avatar_url: row.avatar_url ?? null,
...
export function createUser(
	...
	const workspaceId =
		options?.workspace_id || getDefaultWorkspaceContext().workspaceId;
	const result = db
		.prepare(`
...
function resolveOrProvisionProxyUser(username: string): User | null {
    ...
    `)
		...
		if (
			!defaultRole ||
			!(["viewer", "operator", "admin"] as const).includes(
				defaultRole as User["role"],
			)
		) {
			return null;
		}
	...
	if (proxyAuthHeader) {
		...
		if (trustedIps.size > 0) {
			...
			if (!trustedIps.has(clientIp)) {
				...
				const proxyUsername = (
					request.headers.get(proxyAuthHeader) || ""
				).trim();
				if (proxyUsername) {
		...
		try {
			logSecurityEvent({
				...
				detail: JSON.stringify({
					hint: "Consider using agent-scoped API keys for least-privilege access",
				}),
      ...
      `)
			...
			if (row && !row.revoked_at && (!row.expires_at || row.expires_at > now)) {
				...
				const agent = db
					...
					.get(row.agent_id, row.workspace_id) as
...
export function requireRole(
	...
	if (process.env.AMOENA_LOCAL_MODE === "true" || process.env.AMOENA_LOCAL_MODE === "1") {
