...
export function routeFileToApiPath(
	...
	const segments = relative.split("/").filter(Boolean).map(normalizeSegment);
...
export function collectRouteOperations(projectRoot: string): RouteOperation[] {
	...
	for (const file of routeFiles) {
		...
		const apiPath = routeFileToApiPath(
...
export function compareApiContractParity(params: {
	...
	const ignored = new Set(
		(params.ignore ?? []).map((x) => normalizeOperation(x)),
	);
	const routeOperations = Array.from(
		new Set(
			params.routeOperations.map((op) =>
				toContractOperation(op.method, op.path),
			),
		),
	).sort();
	const openapiOperations = Array.from(
		new Set(params.openapiOperations.map((op) => normalizeOperation(op))),
	).sort();
	...
	for (const op of openapiOperations) {
		if (ignored.has(op)) {
			if (!ignoredOperations.includes(op as ContractOperation))
...
export function runApiContractParityCheck(params: {
	...
	return compareApiContractParity({
		routeOperations,
		openapiOperations,
