...
const RULES: InjectionRule[] = [
	...
	},
	{
		...
		description:
			"Prompt delimiter injection (XML-style role tags or code block system markers)",
		contexts: ["prompt", "display"],
	},
	{
	...
	},
	{
		...
		description:
			"Attempts to access environment variables or sensitive system files",
		contexts: ["prompt", "shell"],
	},
	...
	},
	{
		...
		severity: "warning",
		pattern:
			/\b(?:webhook|callback|postback)\s*[:=]\s*https?:\/\/(?!(?:localhost|127\.0\.0\.1))/i,
		description:
			"External webhook URL that could be used for data exfiltration",
		contexts: ["prompt", "shell"],
	},
	...
	// ── Encoding / obfuscation ──────────────────────────────────
	{
		...
		severity: "warning",
		pattern:
			/(?:base64\s+-d|atob\s*\(|Buffer\.from\s*\([^)]+,\s*['"]base64['"])/i,
	...
	},
	{
		...
		description:
			"Heavy hex/unicode escape sequences that may hide malicious content",
		contexts: ["prompt", "shell", "display"],
	},
];
...
export function scanForInjection(
	...
	const unsafe = matches.some(
		(m) =>
			m.severity === "critical" || (!criticalOnly && m.severity === "warning"),
	);
...
export function injectionRefinement(
	context: GuardOptions["context"] = "prompt",
) {
...
export function sanitizeForPrompt(input: string): string {
	return input
		.replace(
			/<\/?(?:system|user|assistant|human|ai|instruction|context)>/gi,
			"",
		)
		.replace(
			/\[(?:SYSTEM|INST|HIDDEN|ADMIN)\s*(?:OVERRIDE|MESSAGE|INSTRUCTION)?[\]:]/gi,
			"",
		);
}

/** Scan for injection and log security event if unsafe */
export function scanAndLogInjection(
	text: string,
	options?: GuardOptions,
	context?: { agentName?: string; source?: string; workspaceId?: number },
): InjectionReport {
	...
	if (!report.safe) {
		try {
			...
			logSecurityEvent({
				...
				severity: report.matches.some((m) => m.severity === "critical")
					? "critical"
					: "warning",
				source: context?.source || "injection-guard",
				...
				workspace_id: context?.workspaceId || 1,
				tenant_id: 1,
			});
		} catch {}
	}
