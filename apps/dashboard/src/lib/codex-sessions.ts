...
function parseCodexSessionFile(
	...
	for (const line of lines) {
		...
		if (entryType === "event_msg" && payload) {
			...
			if (totals) {
				...
				const allTokens =
					asNumber(totals.total_tokens) || inTokens + cached + outTokens;
	...
	const projectSlug = projectPath ? basename(projectPath) : "codex-local";
	const parsedFirstMs = firstMessageAt
		? clampTimestamp(new Date(firstMessageAt).getTime())
		: 0;
	const parsedLastMs = lastMessageAt
		? clampTimestamp(new Date(lastMessageAt).getTime())
		: 0;
	...
	const isActive =
		effectiveLastMs > 0 && Date.now() - effectiveLastMs < ACTIVE_THRESHOLD_MS;
	...
	return {
		...
		outputTokens,
		totalTokens,
		firstMessageAt: effectiveFirstMs
			? new Date(effectiveFirstMs).toISOString()
			: null,
		lastMessageAt: effectiveLastMs
			? new Date(effectiveLastMs).toISOString()
			: null,
		isActive,
	};
}
