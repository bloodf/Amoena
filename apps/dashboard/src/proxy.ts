...
function getRequestHostCandidates(request: NextRequest): string[] {
	...
	const candidates = rawCandidates.map(normalizeHostname).filter(Boolean);
...
function nextResponseWithNonce(request: NextRequest): {
...
export function proxy(request: NextRequest) {
	...
	const allowAnyHost =
		envFlag("MC_ALLOW_ANY_HOST") || process.env.NODE_ENV !== "production";
	...
	const isAllowedHost =
		...
		requestHosts.some(
			(hostName) =>
				implicitAllowedHosts.some((candidate) =>
					hostMatches(candidate, hostName),
	...
	const isPublicHealthProbe =
		pathname === "/api/status" &&
		request.nextUrl.searchParams.get("action") === "health";
	if (
		pathname === "/login" ||
		pathname === "/setup" ||
	...
	const sessionToken =
		request.cookies.get(MC_SESSION_COOKIE_NAME)?.value ||
	...
	if (pathname.startsWith("/api/")) {
		...
		const hasValidApiKey = Boolean(
			configuredApiKey && apiKey && safeCompare(apiKey, configuredApiKey),
		);
