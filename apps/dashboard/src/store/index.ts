...
// Billing/provisioning entity that can own multiple Lunaria workspaces.
...
interface LunariaStore {
	...
	setSubscription: (
		sub: { type: string; provider?: string; rateLimitTier?: string } | null,
	) => void;
	...
	lunariaUpdateDismissedVersion: string | null;
	setLunariaUpdate: (
		info: {
			installed: string;
			latest: string;
	...
	dismissLunariaUpdate: (version: string) => void;

	// Lunaria Doctor banner dismiss (persisted with 24h expiry)
	...
	updateExecApproval: (
		id: string,
		updates: Partial<ExecApprovalRequest>,
	) => void;
	...
	setSecurityPosture: (
		posture: { score: number; level: string } | undefined,
	) => void;
...
export const useLunaria = create<LunariaStore>()(
	subscribeWithSelector((set, get) => ({
		...
		setLocalSessionsAvailable: (available) =>
			set({ localSessionsAvailable: available }),
		...
		lunariaUpdateDismissedVersion: (() => {
			if (typeof window === "undefined") return null;
			try {
				return localStorage.getItem("mc-lunaria-update-dismissed");
			} catch {
				return null;
			}
		})(),
		setLunariaUpdate: (info) => set({ lunariaUpdate: info }),
		dismissLunariaUpdate: (version) => {
			try {
				localStorage.setItem("mc-lunaria-update-dismissed", version);
			} catch {}
			set({ lunariaUpdateDismissedVersion: version });
		},

		// Lunaria Doctor banner dismiss
		doctorDismissedAt: (() => {
			if (typeof window === "undefined") return null;
			try {
		...
		setShowProjectManagerModal: (show) =>
			set({ showProjectManagerModal: show }),
		...
		addExecApproval: (approval) =>
			set((state) => {
				...
				return {
					execApprovals: [approval, ...state.execApprovals].slice(0, 200),
				};
			}),
		...
		addChatMessage: (message) =>
			set((state) => {
				...
				if (
					message.id > 0 &&
					state.chatMessages.some((m) => m.id === message.id)
				) {
					return state;
				}
