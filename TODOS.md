...
## Amoena Migration Plan — Engineering Review Items
...
- **What:** Wrap all hardcoded English strings across Amoena-derived screens in `t()` calls and extract to translation files
- **Why:** User chose full app i18n (not just Amoena screens). Amoena has zero i18n — ~500+ strings across 90+ components need extraction.
...
- **Context:** Can be done incrementally: Phase 1 (branding strip) is a natural time to touch every file. Extract strings to `packages/i18n/locales/en.json` organized by screen/component namespace. Use `react-i18next` or equivalent. Amoena components in `packages/ui/src/components/ui/` and `apps/desktop/src/renderer/` all need wrapping.
- **Depends on:** Phase 1 completion (files are accessible after fork)
- **Added:** 2026-03-19 via /plan-eng-review

## Amoena Migration Plan — CEO Review Expansions
