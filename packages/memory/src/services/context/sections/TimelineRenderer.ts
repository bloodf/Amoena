...
function renderDayTimelineMarkdown(
	...
	for (const item of dayItems) {
		if (item.type === "summary") {
			...
			output.push(
				...Markdown.renderMarkdownSummaryItem(summary, formattedTime),
			);
		} else {
			...
			if (shouldShowFull) {
				...
				output.push(
					...Markdown.renderMarkdownFullObservation(
						obs,
						timeDisplay,
						detailField,
						config,
	...
	for (const item of dayItems) {
		if (item.type === "summary") {
			...
			if (shouldShowFull) {
				const detailField = getDetailField(obs, config);
				output.push(
					...Color.renderColorFullObservation(
						obs,
						time,
						showTime,
						detailField,
