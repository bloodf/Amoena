...
import type { ContextConfig, PriorMessages, TokenEconomics } from "../types.js";
...
export function renderFooter(
	...
	if (
		!shouldShowContextEconomics(config) ||
	...
	if (useColors) {
		return Color.renderColorFooter(
			economics.totalDiscoveryTokens,
			economics.totalReadTokens,
		);
	}
	return Markdown.renderMarkdownFooter(
		economics.totalDiscoveryTokens,
		economics.totalReadTokens,
	);
}
