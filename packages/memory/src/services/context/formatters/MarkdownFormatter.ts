...
export function renderMarkdownFullObservation(
	...
	const icon = ModeManager.getInstance().getTypeIcon(obs.type);
	const time = timeDisplay ? compactTime(timeDisplay) : '"';
	const { readTokens, discoveryDisplay } = formatObservationTokenDisplay(
		obs,
		config,
	);
 ...
 */
export function renderMarkdownSummaryField(
	label: string,
	value: string | null,
): string[] {
	if (!value) return [];
 ...
 */
export function renderMarkdownPreviouslySection(
 ...
 */
export function renderMarkdownFooter(
	totalDiscoveryTokens: number,
	_totalReadTokens: number,
): string[] {
