...
export function renderColorLegend(): string[] {
	...
	const typeLegendItems = mode.observation_types
		.map((t) => `${t.emoji} ${t.id}`)
		.join(" | ");
...
export function renderColorContextEconomics(
	...
	if (
		economics.totalDiscoveryTokens > 0 &&
...
export function renderColorTableRow(
	...
	const { readTokens, discoveryTokens, workEmoji } =
		formatObservationTokenDisplay(obs, config);

	const timePart = showTime
		? `${colors.dim}${time}${colors.reset}`
		: " ".repeat(time.length);
	const readPart =
		config.showReadTokens && readTokens > 0
			? `${colors.dim}(~${readTokens}t)${colors.reset}`
			: "";
	const discoveryPart =
 ...
 */
export function renderColorSummaryField(
	label: string,
	value: string | null,
	color: string,
): string[] {
 ...
 */
export function renderColorPreviouslySection(
 ...
 */
export function renderColorFooter(
