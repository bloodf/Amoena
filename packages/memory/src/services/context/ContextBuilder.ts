...
} from "./sections/FooterRenderer.js";
import { renderHeader } from "./sections/HeaderRenderer.js";
import {
	renderSummaryFields,
	shouldShowSummary,
} from "./sections/SummaryRenderer.js";
	...
	try {
		...
		if (error.code === "ERR_DLOPEN_FAILED") {
			try {
				...
				logger.debug(
					"SYSTEM",
					"Marker file cleanup failed (may not exist)",
					{},
			...
			logger.error(
				"SYSTEM",
				"Native module rebuild needed - restart Claude Code to auto-fix",
			);
			return null;
		}
		throw error;
