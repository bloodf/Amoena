	...
	async startSession(
		...
		try {
			...
			if (!session.memorySessionId) {
				...
				logger.info(
					"SESSION",
					`MEMORY_ID_GENERATED | sessionDbId=${session.sessionDbId} | provider=Gemini`,
				);
			}
			...
			for await (const message of this.sessionManager.getMessageIterator(
				...
				if (message.type === "observation") {
					...
					if (obsResponse.content) {
						...
						logger.warn(
							"SDK",
							"Empty Gemini observation response, skipping processing to preserve message",
							{
								sessionId: session.sessionDbId,
								messageId:
									session.processingMessageIds[
										session.processingMessageIds.length - 1
									],
							},
						);
					...
					if (summaryResponse.content) {
						...
						logger.warn(
							"SDK",
							"Empty Gemini summary response, skipping processing to preserve message",
							{
								sessionId: session.sessionDbId,
								messageId:
									session.processingMessageIds[
...
export function isGeminiAvailable(): boolean {
	...
	const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
	return !!(
		settings.CLAUDE_MEM_GEMINI_API_KEY || getCredential("GEMINI_API_KEY")
	);
}
