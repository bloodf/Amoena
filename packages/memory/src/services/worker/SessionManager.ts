...
import { ensureProcessExit, getProcessBySession } from "./ProcessRegistry.js";
...
export class SessionManager {
	...
	initializeSession(
		...
		if (session) {
			...
			if (currentUserPrompt) {
				...
				logger.debug(
					"SESSION",
					"No currentUserPrompt provided for existing session",
					{
						sessionDbId,
						promptNumber,
		...
		session = {
			...
			lastPromptNumber:
				...
				this.dbManager
					.getSessionStore()
					.getPromptNumberFromUserPrompts(dbSession.content_session_id),
			startTime: Date.now(),
		...
		logger.debug(
			"SESSION",
			"Creating new session object (memorySessionId cleared to prevent stale resume)",
			{
				sessionDbId,
				contentSessionId: dbSession.content_session_id,
	...
	async deleteSession(sessionDbId: number): Promise<void> {
		...
		if (session.generatorPromise) {
			const generatorDone = session.generatorPromise.catch(() => {
				logger.debug("SYSTEM", "Generator already failed, cleaning up", {
					sessionId: session.sessionDbId,
				});
			});
			const timeoutDone = new Promise<void>((resolve) => {
				AbortSignal.timeout(30_000).addEventListener("abort", () => resolve(), {
					once: true,
				});
			});
			await Promise.race([generatorDone, timeoutDone]).then(
				...
				() => {
					logger.warn(
						"SESSION",
						"Generator did not exit within 30s after abort, forcing cleanup (#1099)",
						{ sessionDbId },
					);
				},
			);
		...
		if (tracked && tracked.process.exitCode === null) {
			logger.debug(
				"SESSION",
				`Waiting for subprocess PID ${tracked.pid} to exit`,
				{
					sessionId: sessionDbId,
					pid: tracked.pid,
				},
		...
		try {
			...
			logger.warn(
				"SESSION",
				"Supervisor reapSession failed (non-blocking)",
				{
					sessionId: sessionDbId,
				},
				error as Error,
	...
	async *getMessageIterator(
		...
		const processor = new SessionQueueProcessor(
		...
		for await (const message of processor.createIterator({
			...
			onIdleTimeout: () => {
				logger.info(
					"SESSION",
					"Triggering abort due to idle timeout to kill subprocess",
					{ sessionDbId },
				);
