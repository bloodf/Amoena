...
import type { TimelineData, TimelineItem } from "./TimelineBuilder.js";
...
export class SearchOrchestrator {
	...
	private async executeWithFallback(
		...
		if (!options.query) {
			logger.debug(
				"SEARCH",
				"Orchestrator: Filter-only query, using SQLite",
				{},
			);
		...
		if (this.chromaStrategy) {
			...
			logger.debug(
				"SEARCH",
				"Orchestrator: Chroma failed, falling back to SQLite",
				{},
			);
	...
	getTimeline(
		...
		return this.timelineBuilder.filterByDepth(
			items,
			anchorId,
			anchorEpoch,
			depthBefore,
