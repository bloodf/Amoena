	...
	implements SearchStrategy
{
	...
	async search(options: StrategySearchOptions): Promise<StrategySearchResult> {
		...
		try {
			...
			logger.debug("SEARCH", "ChromaSearchStrategy: Querying Chroma", {
				query,
				searchType,
			});
			...
			if (categorized.sessionIds.length > 0) {
				sessions = this.sessionStore.getSessionSummariesByIds(
					categorized.sessionIds,
					{
						orderBy,
						limit,
						project,
			...
			logger.error(
				"SEARCH",
				"ChromaSearchStrategy: Search failed",
				{},
				error as Error,
			);
	...
	private filterByRecency(chromaResults: {
		...
		for (const meta of chromaResults.metadatas) {
			if (
				meta?.sqlite_id !== undefined &&
				!metadataByIdMap.has(meta.sqlite_id)
			) {
