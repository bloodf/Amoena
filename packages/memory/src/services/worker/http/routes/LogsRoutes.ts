...
export function readLastLines(
	filePath: string,
	lineCount: number,
): { lines: string; totalEstimate: number } {
...
export class LogsRoutes extends BaseRouteHandler {
	 ...
	 */
	private handleGetLogs = this.wrapHandler(
		(req: Request, res: Response): void => {
			...
			const requestedLines = parseInt(
				(req.query.lines as string) || "1000",
				10,
			);
			const maxLines = Math.min(requestedLines, 10000); // Cap at 10k lines

			const { lines: recentLines, totalEstimate } = readLastLines(
				logFilePath,
				maxLines,
			);
			const returnedLines =
				recentLines === "" ? 0 : recentLines.split("\n").length;
	 ...
	 */
	private handleClearLogs = this.wrapHandler(
