...
export class DataRoutes extends BaseRouteHandler {
	...
	setupRoutes(app: express.Application): void {
		...
		app.get(
			...
			this.handleGetProcessingStatus.bind(this),
		);
		...
		app.post(
			...
			this.handleProcessPendingQueue.bind(this),
		);
		app.delete(
			"/api/pending-queue/failed",
			this.handleClearFailedQueue.bind(this),
		);
	 ...
	 */
	private handleGetObservations = this.wrapHandler(
		(req: Request, res: Response): void => {
			const { offset, limit, project } = this.parsePaginationParams(req);
			const result = this.paginationHelper.getObservations(
				offset,
				limit,
				project,
			);
	 ...
	 */
	private handleGetSummaries = this.wrapHandler(
		(req: Request, res: Response): void => {
			const { offset, limit, project } = this.parsePaginationParams(req);
	 ...
	 */
	private handleGetPrompts = this.wrapHandler(
		(req: Request, res: Response): void => {
			const { offset, limit, project } = this.parsePaginationParams(req);
	 ...
	 */
	private handleGetObservationById = this.wrapHandler(
		(req: Request, res: Response): void => {
	 ...
	 */
	private handleGetObservationsByIds = this.wrapHandler(
		(req: Request, res: Response): void => {
	 ...
	 * GET /api/session/:id
	 */
	private handleGetSessionById = this.wrapHandler(
		(req: Request, res: Response): void => {
			const id = this.parseIntParam(req, res, "id");
			if (id === null) return;
	 ...
	 */
	private handleGetSdkSessionsByIds = this.wrapHandler(
		(req: Request, res: Response): void => {
	 ...
	 * GET /api/prompt/:id
	 */
	private handleGetPromptById = this.wrapHandler(
		(req: Request, res: Response): void => {
			const id = this.parseIntParam(req, res, "id");
			if (id === null) return;
	 ...
	 */
	private handleGetStats = this.wrapHandler(
		(_req: Request, res: Response): void => {
			...
			// Get database stats
			const totalObservations = db
				.prepare("SELECT COUNT(*) as count FROM observations")
				.get() as { count: number };
			const totalSessions = db
				.prepare("SELECT COUNT(*) as count FROM sdk_sessions")
				.get() as { count: number };
			const totalSummaries = db
				.prepare("SELECT COUNT(*) as count FROM session_summaries")
				.get() as { count: number };
	 ...
	 * GET /api/projects
	 */
	private handleGetProjects = this.wrapHandler(
		(_req: Request, res: Response): void => {
			const db = this.dbManager.getSessionStore().db;
	 ...
	 */
	private handleGetProcessingStatus = this.wrapHandler(
		(_req: Request, res: Response): void => {
	 ...
	 */
	private handleSetProcessing = this.wrapHandler(
		(_req: Request, res: Response): void => {
	 ...
	 */
	private handleImport = this.wrapHandler(
		(req: Request, res: Response): void => {
	 ...
	 */
	private handleGetPendingQueue = this.wrapHandler(
		(_req: Request, res: Response): void => {
			const {
				PendingMessageStore,
			...
			res.json({
				queue: {
					...
					totalPending: queueMessages.filter(
						(m: { status: string }) => m.status === "pending",
					).length,
					totalProcessing: queueMessages.filter(
						(m: { status: string }) => m.status === "processing",
					).length,
					totalFailed: queueMessages.filter(
	 ...
	 */
	private handleProcessPendingQueue = this.wrapHandler(
		async (req: Request, res: Response): Promise<void> => {
			...
			const result =
				await this.workerService.processPendingQueues(sessionLimit);
	 ...
	 */
	private handleClearFailedQueue = this.wrapHandler(
		(_req: Request, res: Response): void => {
			const {
				PendingMessageStore,
			} = require("../../../sqlite/PendingMessageStore.js");
	 ...
	 * Returns the number of messages cleared
	 */
	private handleClearAllQueue = this.wrapHandler(
		(_req: Request, res: Response): void => {
			const {
				PendingMessageStore,
			} = require("../../../sqlite/PendingMessageStore.js");
			...
			logger.warn(
				"QUEUE",
				"Cleared ALL queue messages (pending, processing, failed)",
				{ clearedCount },
			);
