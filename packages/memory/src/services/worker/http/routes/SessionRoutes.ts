...
export class SessionRoutes extends BaseRouteHandler {
	...
	private ensureGeneratorRunning(sessionDbId: number, source: string): void {
		...
		if (this.spawnInProgress.get(sessionDbId)) {
			logger.debug("SESSION", "Spawn already in progress, skipping", {
				sessionDbId,
				source,
			});
			return;
		...
		if (timeSinceActivity > SessionRoutes.STALE_GENERATOR_THRESHOLD_MS) {
			logger.warn(
				"SESSION",
				"Stale generator detected, aborting to prevent queue stall (#1099)",
				{
			...
			this.spawnInProgress.set(sessionDbId, true);
			this.startGeneratorWithProvider(
				session,
				selectedProvider,
				"stale-recovery",
			);
		...
		if (
			...
			session.currentProvider !== selectedProvider
		) {
			logger.info(
				"SESSION",
				`Provider changed, will switch after current generator finishes`,
				{
					sessionId: sessionDbId,
	...
	private startGeneratorWithProvider(
		...
		if (session.abortController.signal.aborted) {
			logger.debug(
				"SESSION",
				"Resetting aborted AbortController before starting generator",
				{
		...
		logger.info(
			"SESSION",
			`Generator auto-starting (${source}) using ${agentName}`,
			{
				sessionId: session.sessionDbId,
		...
		session.generatorPromise = agent
			.startSession(session, this.workerService)
			.catch((error) => {
				...
				const pendingStore = this.sessionManager.getPendingMessageStore();
				try {
					const failedCount = pendingStore.markSessionMessagesFailed(
						session.sessionDbId,
					);
					if (failedCount > 0) {
						logger.error(
							"SESSION",
							`Marked messages as failed after generator error`,
							{
								sessionId: session.sessionDbId,
								failedCount,
							},
			...
			.finally(async () => {
				...
				if (wasAborted) {
					logger.info("SESSION", `Generator aborted`, {
						sessionId: sessionDbId,
					});
				} else {
					logger.error("SESSION", `Generator exited unexpectedly`, {
						sessionId: sessionDbId,
					});
				}
					...
					try {
						...
						if (pendingCount > 0) {
							...
							if (this.crashRecoveryScheduled.has(sessionDbId)) {
								logger.debug("SESSION", "Crash recovery already scheduled", {
									sessionDbId,
								});
								return;
							}

							session.consecutiveRestarts =
								(session.consecutiveRestarts || 0) + 1;
							...
							if (session.consecutiveRestarts > MAX_CONSECUTIVE_RESTARTS) {
								logger.error(
									...
									`CRITICAL: Generator restart limit exceeded - stopping to prevent runaway costs`,
									{
										...
										action:
											"Generator will NOT restart. Check logs for root cause. Messages remain in pending state.",
									},
								);
							...
							logger.info(
								"SESSION",
								`Restarting generator after crash/exit with pending work`,
								{
									sessionId: sessionDbId,
									pendingCount,
									consecutiveRestarts: session.consecutiveRestarts,
							...
							const backoffMs = Math.min(
								1000 * 2 ** (session.consecutiveRestarts - 1),
								8000,
							);
							...
							logger.debug(
								"SESSION",
								"Aborted controller after natural completion",
								{
									sessionId: sessionDbId,
								},
							);
						...
						logger.debug(
							"SESSION",
							"Error during recovery check, aborting to prevent leaks",
							{
								sessionId: sessionDbId,
	 ...
	 */
	private handleSessionInit = this.wrapHandler(
		(req: Request, res: Response): void => {
			...
			const latestPrompt = this.dbManager
				.getSessionStore()
				.getLatestUserPrompt(session.contentSessionId);
			...
			this.eventBroadcaster.broadcastSessionStarted(
				sessionDbId,
				session.project,
			);
	 ...
	 */
	private handleObservations = this.wrapHandler(
		(req: Request, res: Response): void => {
			const sessionDbId = this.parseIntParam(req, res, "sessionDbId");
			if (sessionDbId === null) return;

			const { tool_name, tool_input, tool_response, prompt_number, cwd } =
				req.body;
	 ...
	 * CRITICAL: Ensures SDK agent is running to process the queue (ALWAYS SAVE EVERYTHING)
	 */
	private handleSummarize = this.wrapHandler(
		(req: Request, res: Response): void => {
			const sessionDbId = this.parseIntParam(req, res, "sessionDbId");
			if (sessionDbId === null) return;
	 ...
	 * Get session status
	 */
	private handleSessionStatus = this.wrapHandler(
		(req: Request, res: Response): void => {
			const sessionDbId = this.parseIntParam(req, res, "sessionDbId");
			if (sessionDbId === null) return;
	 ...
	 * Delete a session
	 */
	private handleSessionDelete = this.wrapHandler(
	 ...
	 */
	private handleSessionComplete = this.wrapHandler(
		async (req: Request, res: Response): Promise<void> => {
			const sessionDbId = this.parseIntParam(req, res, "sessionDbId");
			if (sessionDbId === null) return;
	 ...
	 */
	private handleObservationsByClaudeId = this.wrapHandler(
		(req: Request, res: Response): void => {
			const { contentSessionId, tool_name, tool_input, tool_response, cwd } =
				req.body;
			...
			if (fileOperationTools.has(tool_name) && tool_input) {
				...
				if (filePath?.includes("session-memory")) {
					logger.debug(
						"SESSION",
						"Skipping meta-observation for session-memory file",
						{
							tool_name,
							file_path: filePath,
						},
	 ...
	 */
	private handleSummarizeByClaudeId = this.wrapHandler(
		(req: Request, res: Response): void => {
	 ...
	 */
	private handleCompleteByClaudeId = this.wrapHandler(
		async (req: Request, res: Response): Promise<void> => {
			const { contentSessionId } = req.body;

			logger.info("HTTP", "→ POST /api/sessions/complete", {
				contentSessionId,
			});
	 ...
	 */
	private handleSessionInitByClaudeId = this.wrapHandler(
		(req: Request, res: Response): void => {
			const { contentSessionId } = req.body;
			...
			const contextInjected =
				this.sessionManager.getSession(sessionDbId) !== undefined;
