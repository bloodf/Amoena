...
function getTrackedProcesses(): TrackedProcess[] {
	return getSupervisor()
		...
		.map((record) => {
			const processRef = getSupervisor()
				.getRegistry()
				.getRuntimeProcess(record.id);
			if (!processRef) {
				return null;
			}
...
export function registerProcess(
	...
	logger.info("PROCESS", `Registered PID ${pid} for session ${sessionDbId}`, {
		pid,
		sessionDbId,
	});
}
 ...
 */
export function getProcessBySession(
	sessionDbId: number,
): TrackedProcess | undefined {
	const matches = getTrackedProcesses().filter(
		(info) => info.sessionDbId === sessionDbId,
	);
	if (matches.length > 1) {
		logger.warn(
			"PROCESS",
			`Multiple processes found for session ${sessionDbId}`,
			{
...
export async function waitForSlot(
	...
	logger.info(
		"PROCESS",
		`Pool limit reached (${activeCount}/${maxConcurrent}), waiting for slot...`,
	);
	...
	return new Promise<void>((resolve, reject) => {
		const timeout = setTimeout(() => {
			...
			reject(
				new Error(`Timed out waiting for agent pool slot after ${timeoutMs}ms`),
			);
		}, timeoutMs);
...
export async function ensureProcessExit(
	...
	logger.warn(
		"PROCESS",
		`PID ${pid} did not exit after ${timeoutMs}ms, sending SIGKILL`,
		{ pid, timeoutMs },
	);
	try {
		proc.kill("SIGKILL");
	...
	try {
		...
		for (const line of stdout.trim().split("\n")) {
			...
			if (minutes >= 1) {
				logger.info(
					"PROCESS",
					`Killing idle daemon child PID ${pid} (idle ${minutes}m)`,
					{ pid, minutes },
				);
				try {
	...
	try {
		...
		for (const line of stdout.trim().split("\n")) {
			...
			if (match && parseInt(match[2], 10) === 1) {
				...
				logger.warn("PROCESS", `Killing system orphan PID ${orphanPid}`, {
					pid: orphanPid,
				});
				try {
 ...
 */
export async function reapOrphanedProcesses(
	activeSessionIds: Set<number>,
): Promise<number> {
	let killed = 0;

	// Registry-based: kill processes for dead sessions
	for (const record of getSupervisor()
		.getRegistry()
		.getAll()
		.filter((entry) => entry.type === "sdk")) {
		...
		logger.warn(
			"PROCESS",
			`Killing orphan PID ${pid} (session ${sessionDbId} gone)`,
			{ pid, sessionDbId },
		);
		try {
			if (processRef) {
...
export function createPidCapturingSpawn(sessionDbId: number) {
	return (spawnOptions: {
		...
		const child = useCmdWrapper
			? spawn(
					"cmd.exe",
					["/d", "/c", spawnOptions.command, ...spawnOptions.args],
					{
		...
		if (child.stderr) {
			child.stderr.on("data", (data: Buffer) => {
				logger.debug(
					"SDK_SPAWN",
					`[session-${sessionDbId}] stderr: ${data.toString().trim()}`,
				);
			});
		}
		...
		if (child.pid) {
			...
			child.on("exit", (code: number | null, signal: string | null) => {
				if (code !== 0) {
					logger.warn(
						"SDK_SPAWN",
						`[session-${sessionDbId}] Claude process exited`,
	...
	const interval = setInterval(async () => {
		try {
			...
			if (killed > 0) {
				logger.info(
					"PROCESS",
					`Reaper cleaned up ${killed} orphaned processes`,
					{ killed },
				);
			}
		} catch (error) {
