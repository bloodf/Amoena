...
export function resolveWorkerRuntimePath(
	...
	const candidatePaths = [
		...
		env.USERPROFILE
			? path.join(env.USERPROFILE, ".bun", "bin", "bun.exe")
			: undefined,
		env.LOCALAPPDATA
			? path.join(env.LOCALAPPDATA, "bun", "bun.exe")
			: undefined,
		env.LOCALAPPDATA
			? path.join(env.LOCALAPPDATA, "bun", "bin", "bun.exe")
			: undefined,
	];
	...
	try {
		if (isWindows) {
			...
			if (!stdout.trim() || stdout.trim() === "null") {
				logger.debug(
					"SYSTEM",
					"No orphaned claude-mem processes found (Windows)",
				);
				return;
			}
			...
			for (const line of lines) {
				...
				if (ageMinutes >= ORPHAN_MAX_AGE_MINUTES) {
					...
					logger.debug("SYSTEM", "Found orphaned process", {
						pid,
						ageMinutes,
						command: match[3].substring(0, 80),
					});
				}
			}
		}
		...
		logger.error(
			"SYSTEM",
			"Failed to enumerate orphaned processes",
			{},
			error as Error,
		);
		return;
...
export async function aggressiveStartupCleanup(): Promise<void> {
	...
	try {
		if (isWindows) {
			...
			for (const proc of processList) {
				...
				const isAggressive = AGGRESSIVE_CLEANUP_PATTERNS.some((p) =>
					commandLine.includes(p),
				);

				if (isAggressive) {
					// Kill immediately — no age check
					pidsToKill.push(pid);
					logger.debug("SYSTEM", "Found orphaned process (aggressive)", {
						pid,
					...
					if (creationMatch) {
						...
						if (ageMinutes >= ORPHAN_MAX_AGE_MINUTES) {
							pidsToKill.push(pid);
							logger.debug("SYSTEM", "Found orphaned process (age-gated)", {
								pid,
								ageMinutes: Math.round(ageMinutes),
							});
						}
	...
	logger.info(
		"SYSTEM",
		"Aggressive startup cleanup: killing orphaned processes",
		{
			platform: isWindows ? "Windows" : "Unix",
			count: pidsToKill.length,
			pids: pidsToKill,
	...
	logger.info("SYSTEM", "Aggressive startup cleanup complete", {
		count: pidsToKill.length,
	});
}
...
export function runOneTimeChromaMigration(dataDirectory?: string): void {
	...
	const markerPath = path.join(
		effectiveDataDir,
		CHROMA_MIGRATION_MARKER_FILENAME,
	);
	...
	logger.warn(
		"SYSTEM",
		"Running one-time chroma data wipe (upgrade from pre-v10.3)",
		{ chromaDir },
	);
...
export function spawnDaemon(
	...
	if (isWindows) {
		...
		if (!runtimePath) {
			logger.error(
				"SYSTEM",
				"Failed to locate Bun runtime for Windows worker spawn",
			);
			return undefined;
		}
