...
function extractSignatureFromLines(
	lines: string[],
	startRow: number,
	endRow: number,
	maxLen: number = 200,
	...
	if (!sig.trimEnd().endsWith("{") && !sig.trimEnd().endsWith(":")) {
		const chunk = lines
			.slice(startRow, Math.min(startRow + 10, endRow + 1))
			.join("\n");
...
function findPythonDocstringFromLines(
	lines: string[],
	startRow: number,
	endRow: number,
): string | undefined {
...
function isExported(
	...
	switch (language) {
		...
		case "typescript":
		case "tsx":
			return exportRanges.some(
				(r) => startRow >= r.startRow && endRow <= r.endRow,
			);
		case "python":
...
function buildSymbols(
	...
	for (const match of matches) {
		...
		const docstring =
			language === "python"
				? findPythonDocstringFromLines(lines, startRow, endRow)
				: undefined;
	...
	for (const container of containers) {
		for (const sym of symbols) {
			...
			if (
				sym.lineStart > container.startRow &&
				sym.lineEnd <= container.endRow
			) {
...
export function formatFoldedView(file: FoldedFile): string {
	...
	parts.push(
		`📁 ${file.filePath} (${file.language}, ${file.totalLines} lines)`,
	);
	parts.push("");
