...
import {
	ENV_EXACT_MATCHES,
	ENV_PREFIXES,
} from "../../supervisor/env-sanitizer.js";
...
export class Server {
	...
	private setupCoreRoutes(): void {
		...
		this.app.get("/api/instructions", async (req: Request, res: Response) => {
			...
			try {
				...
				if (operation) {
					...
					// Path boundary check
					const OPERATIONS_BASE_DIR = path.resolve(
						__dirname,
						"../skills/mem-search/operations",
					);
		...
		this.app.get(
			...
			(_req: Request, res: Response) => {
				...
				const deadProcessPids = processes
					.filter((p) => p.status === "dead")
					.map((p) => p.pid);

				// Check if CLAUDECODE_* env vars are leaking into this process
				const envClean = !Object.keys(process.env).some(
					(key) =>
						ENV_EXACT_MATCHES.has(key) ||
	...
	private extractInstructionSection(content: string, topic: string): string {
		const sections: Record<string, string> = {
			workflow: this.extractBetween(
				content,
				"## The Workflow",
				"## Search Parameters",
			),
			search_params: this.extractBetween(
				content,
				"## Search Parameters",
				"## Examples",
			),
			examples: this.extractBetween(
				content,
				"## Examples",
				"## Why This Workflow",
			),
