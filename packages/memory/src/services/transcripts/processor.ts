...
import { matchesRule, resolveFieldSpec, resolveFields } from "./field-utils.js";
import type { SchemaEvent, TranscriptSchema, WatchTarget } from "./types.js";
...
export class TranscriptEventProcessor {
	...
	private resolveSessionId(
		...
		const fieldSpec =
			...
			(schema.sessionIdPath ? { path: schema.sessionIdPath } : undefined);
		const resolved = resolveFieldSpec(fieldSpec, entry, ctx);
		if (typeof resolved === "string" && resolved.trim()) return resolved;
		if (typeof resolved === "number") return String(resolved);
		if (sessionIdOverride?.trim()) return sessionIdOverride;
		return null;
	}
	...
	private resolveCwd(
		...
		const fieldSpec =
			event.fields?.cwd ??
			(schema.cwdPath ? { path: schema.cwdPath } : undefined);
		const resolved = resolveFieldSpec(fieldSpec, entry, ctx);
		if (typeof resolved === "string" && resolved.trim()) return resolved;
	...
	private resolveProject(
		...
		const ctx = { watch, schema, session } as any;
		const fieldSpec =
			event.fields?.project ??
			(schema.projectPath ? { path: schema.projectPath } : undefined);
		const resolved = resolveFieldSpec(fieldSpec, entry, ctx);
		if (typeof resolved === "string" && resolved.trim()) return resolved;
	...
	private async handleEvent(
		...
		if (!sessionId) {
			logger.debug("TRANSCRIPT", "Skipping event without sessionId", {
				event: event.name,
				watch: watch.name,
			});
			return;
	...
	private async handleToolUse(
		...
		const toolId =
			typeof fields.toolId === "string" ? fields.toolId : undefined;
		const toolName =
			typeof fields.toolName === "string" ? fields.toolName : undefined;
	...
	private async sendFileEdit(
		session: SessionState,
		fields: Record<string, unknown>,
	): Promise<void> {
		const filePath =
			typeof fields.filePath === "string" ? fields.filePath : undefined;
		if (!filePath) return;
	...
	private async updateContext(
		...
		try {
			...
			const agentsPath = expandHomePath(
				watch.context.path ?? `${cwd}/AGENTS.md`,
			);
			writeAgentsMd(agentsPath, content);
			logger.debug("TRANSCRIPT", "Updated AGENTS.md context", {
				agentsPath,
				watch: watch.name,
			});
		} catch (error) {
