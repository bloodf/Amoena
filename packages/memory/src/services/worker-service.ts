...
// Integration imports
import { handleCursorCommand } from "./integrations/CursorHooksInstaller.js";
// Server imports
...
export class WorkerService {
	...
	private registerRoutes(): void {
		...
		this.server.app.get("/api/context/inject", async (_req, res, next) => {
	...
	private async initializeBackground(): Promise<void> {
		try {
			...
			const { SettingsDefaultsManager } = await import(
				"../shared/SettingsDefaultsManager.js"
			);
			...
			const { PendingMessageStore } = await import(
				"./sqlite/PendingMessageStore.js"
			);
			...
			try {
				...
				logger.warn(
					"WORKER",
					"MCP server connection failed, cleaning up subprocess",
					{
						error:
			...
			.finally(async () => {
				...
				if (pendingCount > 0) {
					...
					if (session.consecutiveRestarts > MAX_PENDING_RESTARTS) {
						logger.error(
							"SYSTEM",
							"Exceeded max pending-work restarts, stopping to prevent infinite loop",
							{
								sessionId: session.sessionDbId,
								pendingCount,
...
async function ensureWorkerStarted(port: number): Promise<boolean> {
	...
	if (pidFileStatus === "alive") {
		...
		logger.warn(
			"SYSTEM",
			"Live PID detected but worker did not become healthy before timeout",
		);
		return false;
	}
...
async function main() {
	...
	switch (command) {
		...
		case "restart": {
			...
			await httpShutdown(port);
			const restartFreed = await waitForPortFree(
				port,
