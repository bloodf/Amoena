	...
	loadMode(modeId: string): ModeConfig {
		...
		if (!inheritance.hasParent) {
			try {
				...
				logger.warn(
					"SYSTEM",
					`Mode file not found: ${modeId}, falling back to 'code'`,
				);
		...
		try {
			...
			logger.warn(
				"SYSTEM",
				`Parent mode '${parentId}' not found for ${modeId}, falling back to 'code'`,
			);
		...
		try {
			...
			logger.debug(
				"SYSTEM",
				`Loaded override file: ${overrideId} for parent ${parentId}`,
			);
		} catch (_error) {
			logger.warn(
				"SYSTEM",
		...
		if (!overrideConfig) {
			logger.warn(
				"SYSTEM",
				`Invalid override file: ${overrideId}, using parent mode '${parentId}' only`,
			);
			this.activeMode = parentMode;
			return parentMode;
		}
