...
function repairMalformedSchema(db: Database): void {
	try {
		...
		logger.warn("DB", "Detected malformed database schema, attempting repair", {
			error: message,
		});
		...
		if (!match) {
			logger.error(
				"DB",
				"Could not parse malformed schema error, cannot auto-repair",
				{ error: message },
			);
			throw error;
		}
...
`,
			...
			execFileSync("python3", [scriptPath, dbPath, objectName], {
				timeout: 10000,
			});
			logger.info(
				"DB",
				`Dropped orphaned schema object "${objectName}" and reset migration versions via Python sqlite3. All migrations will re-run (they are idempotent).`,
			);
		} catch (pyError: unknown) {
			const pyMessage =
				pyError instanceof Error ? pyError.message : String(pyError);
			logger.error("DB", "Python sqlite3 repair failed", { error: pyMessage });
			throw new Error(
				`Schema repair failed: ${message}. Python repair error: ${pyMessage}`,
			);
		} finally {
 ...
 */
function repairMalformedSchemaWithReopen(
	dbPath: string,
	db: Database,
): Database {
	try {
...
export class DatabaseManager {
	...
	private async runMigrations(): Promise<void> {
		...
		const maxApplied =
			appliedVersions.length > 0 ? Math.max(...appliedVersions) : 0;
