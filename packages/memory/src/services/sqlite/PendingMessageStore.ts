...
export class PendingMessageStore {
    ...
    `);
		...
		if (resetResult.changes > 0) {
			logger.info(
				"QUEUE",
				`STUCK_RESET | hasAnyPendingWork reset ${resetResult.changes} stuck processing message(s) older than 5 minutes`,
			);
		}
	...
	toPendingMessage(persistent: PersistentPendingMessage): PendingMessage {
		return {
			...
			tool_input: persistent.tool_input
				? JSON.parse(persistent.tool_input)
				: undefined,
			tool_response: persistent.tool_response
				? JSON.parse(persistent.tool_response)
				: undefined,
