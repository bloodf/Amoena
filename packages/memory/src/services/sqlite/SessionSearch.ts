...
export class SessionSearch {
	...
	private ensureFTSTables(): void {
		...
		const tables = this.db
			...
			.all() as TableNameRow[];
		const hasFTS = tables.some(
			(t) =>
				t.name === "observations_fts" || t.name === "session_summaries_fts",
		);
		...
		if (!this.isFts5Available()) {
			logger.warn(
				"DB",
				"FTS5 not available on this platform — skipping FTS table creation (search uses ChromaDB)",
			);
			return;
		}
	...
	private buildFilterClause(
		...
		if (filters.concepts) {
			const concepts = Array.isArray(filters.concepts)
				? filters.concepts
				: [filters.concepts];
		...
		if (filters.files) {
			const files = Array.isArray(filters.files)
				? filters.files
				: [filters.files];
	...
	searchObservations(
      ...
      `;
		...
		logger.warn(
			"DB",
			"Text search not supported - use ChromaDB for vector search",
		);
		return [];
	}
	 ...
	 */
	private hasDirectChildFileSession(
    ...
    `;
		...
		let sessions = this.db
			.prepare(sessionsSql)
			.all(...sessionParams) as SessionSummarySearchResult[];

		// Post-filter to direct children if isFolder mode
		if (isFolder) {
			sessions = sessions
				.filter((s) => this.hasDirectChildFileSession(s, filePath))
				.slice(0, limit);
		}
