...
const DEFAULT_CHROMA_DATA_DIR = path.join(
	os.homedir(),
	".claude-mem",
	"chroma",
);
...
export class ChromaMcpManager {
	...
	private async ensureConnected(): Promise<void> {
		...
		const timeSinceLastFailure =
			Date.now() - this.lastConnectionFailureTimestamp;
		if (
			this.lastConnectionFailureTimestamp > 0 &&
			timeSinceLastFailure < RECONNECT_BACKOFF_MS
		) {
			throw new Error(
	...
	private async connectInternal(): Promise<void> {
		...
		const uvxSpawnCommand = isWindows
		...
		const timeoutPromise = new Promise<never>((_, reject) => {
			timeoutId = setTimeout(
				() =>
					reject(
						new Error(
							`MCP connection to chroma-mcp timed out after ${MCP_CONNECTION_TIMEOUT_MS}ms`,
						),
					),
		...
		try {
			...
			logger.warn(
				"CHROMA_MCP",
				"Connection failed, killing subprocess to prevent zombie",
				{
					error:
		...
		this.transport.onclose = () => {
			if (this.transport !== currentTransport) {
				logger.debug(
					"CHROMA_MCP",
					"Ignoring stale onclose from previous transport",
				);
				return;
			}
			logger.warn(
	...
	private buildCommandArgs(): string[] {
		...
		const pythonVersion =
			process.env.CLAUDE_MEM_PYTHON_VERSION ||
		...
		if (chromaMode === "remote") {
			...
			const chromaTenant =
				settings.CLAUDE_MEM_CHROMA_TENANT || "default_tenant";
			const chromaDatabase =
				settings.CLAUDE_MEM_CHROMA_DATABASE || "default_database";
	...
	async callTool(
		...
		const firstTextContent = contentArray.find(
	...
	async stop(): Promise<void> {
		...
		try {
			...
			logger.debug(
				"CHROMA_MCP",
				"Error during client close (subprocess may already be dead)",
				{},
				error as Error,
			);
		}
	...
	private getCombinedCertPath(): string | undefined {
		const combinedCertPath = path.join(
			os.homedir(),
			".claude-mem",
			"combined_certs.pem",
		);
		...
		try {
			...
			logger.info(
				"CHROMA_MCP",
				"Created combined SSL certificate bundle for Zscaler",
				{
			...
			logger.debug(
				"CHROMA_MCP",
				"Could not create combined cert bundle",
				{},
				error as Error,
			);
			return undefined;
	...
	private getSpawnEnv(): Record<string, string> {
		...
		logger.info(
			"CHROMA_MCP",
			"Using combined SSL certificates for enterprise compatibility",
			{
