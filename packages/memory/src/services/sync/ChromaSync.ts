...
export class ChromaSync {
	...
	private async addDocuments(documents: ChromaDocument[]): Promise<void> {
		...
		for (let i = 0; i < documents.length; i += this.BATCH_SIZE) {
			...
			const cleanMetadatas = batch.map((d) =>
				Object.fromEntries(
					Object.entries(d.metadata).filter(
			...
			try {
				...
				logger.error(
					"CHROMA_SYNC",
					"Batch add failed, continuing with remaining batches",
					{
						collection: this.collectionName,
						batchStart: i,
	...
	async ensureBackfilled(projectOverride?: string): Promise<void> {
		...
		try {
			...
			const existingObsIds = Array.from(existing.observations).filter(
      ...
      `)
			...
			const existingSummaryIds = Array.from(existing.summaries).filter(
				(id) => Number.isInteger(id) && id > 0,
			);
      ...
      `)
			...
			const existingPromptIds = Array.from(existing.prompts).filter(
				(id) => Number.isInteger(id) && id > 0,
			);
	...
	static async backfillAllProjects(): Promise<void> {
		...
		try {
			...
			logger.info(
				"CHROMA_SYNC",
				`Backfill check for ${projects.length} projects`,
			);
			...
			for (const { project } of projects) {
				try {
					...
					logger.error(
						"CHROMA_SYNC",
						`Backfill failed for project: ${project}`,
						{},
						error as Error,
					);
