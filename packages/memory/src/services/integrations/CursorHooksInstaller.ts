...
export function registerCursorProject(
	...
	logger.info("CURSOR", "Registered project for auto-context updates", {
		projectName,
		workspacePath,
	});
}
 ...
 */
export async function updateCursorContextForProject(
	projectName: string,
	_port: number,
): Promise<void> {
	const registry = readCursorRegistry();
	...
	try {
		...
		logger.debug("CURSOR", "Updated context file", {
			projectName,
...
export function configureCursorMcp(target: CursorInstallTarget): number {
	...
	if (!mcpServerPath) {
		...
		console.error(
			"   Expected at: ~/.claude/plugins/marketplaces/thedotmack/plugin/scripts/mcp-server.cjs",
		);
		return 1;
	}
	...
	try {
		...
		console.log(
			`  Configured MCP server in ${target === "user" ? "~/.cursor" : ".cursor"}/mcp.json`,
		);
...
export async function installCursorHooks(
	...
	if (!targetDir) {
		console.error(
			`Invalid target: ${target}. Use: project, user, or enterprise`,
		);
		return 1;
	}
	...
	if (!workerServicePath) {
		...
		console.error(
			"   Expected at: ~/.claude/plugins/marketplaces/thedotmack/plugin/scripts/worker-service.cjs",
		);
		return 1;
	}
...
`);
		...
		if (target === "enterprise") {
			console.error(
				"   Tip: Enterprise installation may require sudo/admin privileges",
			);
		}
		return 1;
	}
...
async function setupProjectContext(
	...
	try {
		...
		logger.debug(
			"CURSOR",
			"Worker not running during install",
			{},
			error as Error,
		);
	}
...
export function checkCursorHooksStatus(): number {
	...
	if (process.platform === "darwin") {
		locations.push({
			name: "Enterprise",
			dir: "/Library/Application Support/Cursor",
		});
	} else if (process.platform === "linux") {
	...
	for (const loc of locations) {
		...
		if (existsSync(hooksJson)) {
			...
			try {
				...
				const firstCommand =
					hooksContent?.hooks?.beforeSubmitPrompt?.[0]?.command || "";

				if (
					firstCommand.includes("worker-service.cjs") &&
					firstCommand.includes("hook cursor")
				) {
					...
					const hasBash = bashScripts.some((s) =>
						existsSync(path.join(hooksDir, s)),
					);
					const hasPs = psScripts.some((s) =>
						existsSync(path.join(hooksDir, s)),
					);

					if (hasBash || hasPs) {
						console.log(
							`   Mode: Legacy shell scripts (consider reinstalling for unified CLI)`,
						);
						if (hasBash && hasPs) {
			...
			if (loc.name === "Project") {
				...
				if (existsSync(contextFile)) {
					...
					console.log(
						`   Context: Not yet generated (will be created on first prompt)`,
					);
				}
			}
		} else {
...
export async function detectClaudeCode(): Promise<boolean> {
	try {
		...
		const { stdout } = await execAsync("which claude || where claude", {
			timeout: 5000,
		});
		if (stdout.trim()) {
			return true;
