	...
	while ((match = observationRegex.exec(text)) !== null) {
		...
		if (type) {
			if (validTypes.includes(type.trim())) {
				...
				logger.error(
					"PARSER",
					`Invalid observation type: ${type}, using "${fallbackType}"`,
					{ correlationId },
				);
			}
		} else {
			logger.error(
				"PARSER",
				`Observation missing type field, using "${fallbackType}"`,
				{ correlationId },
			);
		}
...
export function parseSummary(
	...
	if (!summaryMatch) {
		...
		if (/<observation>/.test(text)) {
			logger.warn(
				"PARSER",
				"Summary response contained <observation> tags instead of <summary> — prompt conditioning may need strengthening",
				{ sessionId },
			);
		}
		return null;
