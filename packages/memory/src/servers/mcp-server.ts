...
import {
	formatSearchResults,
	searchCodebase,
} from "../services/smart-file-read/search.js";
		...
		inputSchema: {
			...
			properties: {
				...
				file_pattern: {
					type: "string",
					description:
						'Substring filter for file paths (e.g. ".ts", "src/services")',
				},
			},
			required: ["query"],
		},
	...
	},
	{
		...
		handler: async (args: any) => {
			...
			if (parsed.symbols.length > 0) {
				const available = parsed.symbols
					.map((s) => `  - ${s.name} (${s.kind})`)
					.join("\n");
				return {
					content: [
...
function startParentHeartbeat() {
	...
	heartbeatTimer = setInterval(() => {
		if (process.ppid === 1 || process.ppid !== initialPpid) {
			logger.info(
				"SYSTEM",
				"Parent process died, self-exiting to prevent orphan",
				{
					initialPpid,
