...
import { getTimeout, HOOK_TIMEOUTS } from "./hook-constants.js";
...
const HEALTH_CHECK_TIMEOUT_MS = (() => {
	...
	if (envVal) {
		...
		logger.warn(
			"SYSTEM",
			"Invalid CLAUDE_MEM_HEALTH_TIMEOUT_MS, using default",
			{
				value: envVal,
				min: 500,
				max: 300000,
...
function getPluginVersion(): string {
	try {
		...
		if (code === "ENOENT" || code === "EBUSY") {
			logger.debug("SYSTEM", "Could not read plugin version (shutdown race)", {
				code,
			});
			return "unknown";
		}
...
async function getWorkerVersion(): Promise<string> {
	const response = await workerHttpRequest("/api/version", {
		timeoutMs: HEALTH_CHECK_TIMEOUT_MS,
	});
	if (!response.ok) {
