...
export class SettingsDefaultsManager {
	...
	static getAllDefaults(): SettingsDefaults {
		return { ...SettingsDefaultsManager.DEFAULTS };
	}
	...
	static get(key: keyof SettingsDefaults): string {
		return process.env[key] ?? SettingsDefaultsManager.DEFAULTS[key];
	}
	...
	static getInt(key: keyof SettingsDefaults): number {
		const value = SettingsDefaultsManager.get(key);
	...
	private static applyEnvOverrides(
		...
		for (const key of Object.keys(SettingsDefaultsManager.DEFAULTS) as Array<
	...
	static loadFromFile(settingsPath: string): SettingsDefaults {
		try {
			if (!existsSync(settingsPath)) {
				...
				return SettingsDefaultsManager.applyEnvOverrides(defaults);
			}
			...
			const result: SettingsDefaults = { ...SettingsDefaultsManager.DEFAULTS };
			for (const key of Object.keys(SettingsDefaultsManager.DEFAULTS) as Array<
				keyof SettingsDefaults
			>) {
			...
			return SettingsDefaultsManager.applyEnvOverrides(result);
		} catch (error) {
			console.warn(
			...
			return SettingsDefaultsManager.applyEnvOverrides(
