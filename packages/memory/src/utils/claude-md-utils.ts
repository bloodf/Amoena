...
export function writeClaudeMdToFolder(
	...
	if (!existsSync(folderPath)) {
		logger.debug("FOLDER_INDEX", "Skipping non-existent folder", {
			folderPath,
		});
		return;
	}
...
function isExcludedFolder(folderPath: string, excludePaths: string[]): boolean {
	...
	for (const excludePath of excludePaths) {
		...
		if (
			normalizedFolder === normalizedExclude ||
...
export async function updateFolderClaudeMdFiles(
	...
	try {
		...
		if (Array.isArray(parsed)) {
			folderMdExcludePaths = parsed.filter(
		...
		logger.warn(
			"FOLDER_INDEX",
			"Failed to parse CLAUDE_MEM_FOLDER_MD_EXCLUDE setting",
		);
	}
	...
	for (const filePath of filePaths) {
		...
		if (basename === "CLAUDE.md") {
			...
			logger.debug(
				"FOLDER_INDEX",
				"Detected active CLAUDE.md, will skip folder",
				{ folderPath },
			);
		}
	}
	...
	for (const filePath of filePaths) {
		...
		if (folderPath && folderPath !== "." && folderPath !== "/") {
			...
			if (isExcludedUnsafeDirectory(folderPath)) {
				logger.debug(
					"FOLDER_INDEX",
					"Skipping unsafe directory for CLAUDE.md",
					{ folderPath },
				);
				continue;
			}
			...
			if (foldersWithActiveClaudeMd.has(folderPath)) {
				logger.debug(
					"FOLDER_INDEX",
					"Skipping folder with active CLAUDE.md to avoid race condition",
					{ folderPath },
				);
				continue;
			}
			...
			if (
				folderMdExcludePaths.length > 0 &&
				isExcludedFolder(folderPath, folderMdExcludePaths)
			) {
				logger.debug("FOLDER_INDEX", "Skipping excluded folder", {
					folderPath,
				});
				continue;
			}
	...
	for (const folderPath of folderPaths) {
		try {
			...
			if (hasNoActivity && !fileExists) {
				logger.debug("FOLDER_INDEX", "Skipping empty CLAUDE.md creation", {
					folderPath,
				});
				continue;
			}
