...
function countTags(content: string): number {
	...
	const systemInstructionCount = (content.match(/<system_instruction>/g) || [])
		.length;
	const systemInstructionHyphenCount = (
		content.match(/<system-instruction>/g) || []
	).length;
	return (
		privateCount +
		contextCount +
		systemInstructionCount +
		systemInstructionHyphenCount
	);
}
