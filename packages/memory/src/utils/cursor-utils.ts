 ...
 */
export function readCursorRegistry(
	registryFile: string,
): CursorProjectRegistry {
	try {
		...
		logger.error(
			"CONFIG",
			"Failed to read Cursor registry, using empty registry",
			{
				file: registryFile,
 ...
 */
export function writeCursorRegistry(
	registryFile: string,
	registry: CursorProjectRegistry,
): void {
 ...
 */
export function unregisterCursorProject(
	registryFile: string,
	projectName: string,
): void {
	const registry = readCursorRegistry(registryFile);
...
export function removeMcpConfig(mcpJsonPath: string): void {
	...
	try {
		const config: CursorMcpConfig = JSON.parse(
