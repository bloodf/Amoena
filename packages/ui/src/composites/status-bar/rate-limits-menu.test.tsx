...
describe('RateLimitsMenu', () => {
  test('renders toggle button with provider info', () => {
    render(<RateLimitsMenu open={false} onToggle={vi.fn(() => {})} onClose={vi.fn(() => {})} />);
  ...
  test('calls onToggle when button is clicked', () => {
    ...
    render(<RateLimitsMenu open={false} onToggle={onToggle} onClose={vi.fn(() => {})} />);
    fireEvent.click(screen.getAllByRole('button')[0]);
    expect(onToggle).toHaveBeenCalled();
  });

  test('shows dropdown content when open=true', () => {
    render(<RateLimitsMenu open={true} onToggle={vi.fn(() => {})} onClose={vi.fn(() => {})} />);
