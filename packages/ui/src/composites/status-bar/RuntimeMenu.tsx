...
export function RuntimeMenu({
  ...
  return (
    <div className="relative">
      <Button onClick={onToggle} variant="ghost" size="sm" className="h-6 gap-1.5 px-2 py-1 font-mono text-[11px] text-muted-foreground">
        <RuntimeIcon size={11} className={runtimeConfig[runtimeLocation].className} />
        <span className={runtimeConfig[runtimeLocation].className}>{runtimeConfig[runtimeLocation].label}</span>
        <ChevronDown size={8} />
      </Button>
      <StatusBarDropdown open={open} onClose={onClose} className="bottom-full left-0 mb-1 w-44">
        ...
        {(Object.entries(runtimeConfig) as [RuntimeLocation, (typeof runtimeConfig)[RuntimeLocation]][]).map(([key, config]) => {
          const Icon = config.icon;
          return (
            <button key={key} onClick={() => onSelect(key)} className={`flex w-full items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-surface-2 ${runtimeLocation === key ? "bg-surface-2" : ""}`}>
              <Icon size={12} className={config.className} />
              <span className={`text-[12px] ${config.className}`}>{config.label}</span>
              {runtimeLocation === key ? <Check size={13} className="ml-auto text-primary" /> : null}
            </button>
          );
        })}
      </StatusBarDropdown>
