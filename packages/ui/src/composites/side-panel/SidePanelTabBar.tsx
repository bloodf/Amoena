...
export function SidePanelTabBar({
  ...
  return (
    <div ref={containerRef} className="flex flex-shrink-0 overflow-hidden border-b border-border" onDragLeave={onDragLeave}>
      {visibleTabs.map((tab, index) => (
        <div key={tab.id} className="relative min-w-0 flex-1">
          {dropIndex === index && dragIndex !== null && dragIndex !== index ? <div className="absolute bottom-1 left-0 top-1 z-10 w-[2px] rounded-full bg-primary" /> : null}
          <button
            draggable
      ...
      {showOverflow ? (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              aria-label="More side panel tabs"
              className={cn(
          ...
          <DropdownMenuContent align="end" className="min-w-[120px]">
            {overflowTabs.map((tab) => (
              <DropdownMenuItem key={tab.id} onClick={() => onSelect(tab.id)} className={cn("flex items-center gap-2 text-[12px]", activeTab === tab.id && "font-medium text-primary")}>
                <tab.icon size={12} />
                {tab.label}
              </DropdownMenuItem>
            ))}
