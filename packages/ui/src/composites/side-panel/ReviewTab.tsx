...
export function ReviewTab() {
  ...
  const rejectAll = () =>
    setFiles((prev) => prev.map((f) => ({ ...f, status: 'rejected' as const })));
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="p-3 border-b border-border flex-shrink-0 space-y-2">
        ...
        <div className="flex gap-1">
          <button
            onClick={applyAll}
            className="flex-1 text-[10px] px-2 py-1.5 rounded bg-success/20 text-success hover:bg-success/30 transition-colors font-medium"
          >
          ...
          <button
            onClick={rejectAll}
            className="flex-1 text-[10px] px-2 py-1.5 rounded border border-destructive/40 text-destructive hover:bg-destructive/10 transition-colors font-medium"
          >
            ...
            className={cn(
              ...
              (() => {
                ...
                if (f.status === 'rejected') {
                  return 'bg-destructive/5 border-destructive/30 opacity-60';
                }
                if (f.hasConflict) {
          ...
          >
            ...
            <div className="flex gap-0.5 flex-shrink-0">
              {f.status === 'pending' ? (
                <>
                  <button
                    onClick={() => updateFile(f.path, 'approved')}
                  ...
                  <button
                    onClick={() => updateFile(f.path, 'rejected')}
        ...
        {(() => {
          if (hasConflicts) {
            return (
              <button
                disabled
                className="w-full flex items-center justify-center gap-1.5 py-2 text-[12px] font-medium rounded bg-surface-3 text-muted-foreground cursor-not-allowed"
              >
