...
import { GitBranch, GitCommit, RotateCcw, Diff, FileCode, Plus, ChevronRight, ChevronDown, Circle } from "lucide-react";
...
const timeline: Checkpoint[] = [
  ...
  {
    id: "cp3", label: "JWT token rotation", timestamp: "3:52 PM", tokensUsed: "5.8k", filesChanged: 3, isCurrent: false,
    children: [{
      id: "cp3b", label: "Alt: Session-based auth", timestamp: "3:53 PM", tokensUsed: "3.2k", filesChanged: 2,
      isCurrent: false, branch: "experiment/session-auth",
    }],
  },
...
export function TimelineTab() {
  ...
  const selectedCp = timeline.find(t => t.id === selectedCheckpoint) ||
        ...
        <div className={cn("flex items-start gap-2", depth > 0 && "ml-5")}>
          <div className="flex flex-col items-center flex-shrink-0 pt-1">
            <div className={cn(
              ...
              cp.isCurrent ? "border-primary bg-primary shadow-[0_0_6px_hsl(var(--primary)/0.5)]" :
              isSelected ? "border-primary/60 bg-primary/30" :
              depth > 0 ? "border-purple/60 bg-purple/20" :
          ...
          <button
            ...
            className={cn(
              ...
              isSelected ? "border-primary/30 bg-primary/5" : "border-transparent hover:bg-surface-2"
            )}
          >
            <div className="flex items-center justify-between mb-0.5">
              <div className="flex items-center gap-1.5 min-w-0">
                ...
                <span className={cn("text-[11px] font-medium truncate", cp.isCurrent ? "text-primary" : "text-foreground")}>
                  {cp.label}
                  {cp.isCurrent && <span className="text-[8px] text-primary ml-1 flex items-center gap-0.5"><Circle size={5} className="fill-primary text-primary" /> now</span>}
                </span>
              </div>
              <span className="text-[9px] text-muted-foreground font-mono flex-shrink-0 ml-1">{cp.timestamp}</span>
            </div>
            <div className="flex items-center gap-2 text-[9px] text-muted-foreground">
              <span className="font-mono">{cp.tokensUsed} tok</span>
              {cp.filesChanged > 0 && (
                <span className="flex items-center gap-0.5"><FileCode size={8} />{cp.filesChanged}</span>
              )}
            </div>
          </button>
          ...
          {isSelected && !cp.isCurrent && (
            <div className="flex items-center gap-0.5 pt-1.5 flex-shrink-0">
              <button className="p-1 text-muted-foreground cursor-pointer hover:text-primary hover:bg-primary/10 rounded focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors" aria-label="Restore checkpoint">
                <RotateCcw size={10} />
              </button>
              <button className="p-1 text-muted-foreground cursor-pointer hover:text-purple hover:bg-purple/10 rounded focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors" aria-label="Fork from checkpoint">
                <GitBranch size={10} />
              </button>
            </div>
          )}
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {timeline.map(cp => renderCheckpoint(cp))}
      </div>
            ...
            <div className="space-y-0.5">
              {changedFiles.map((file) => (
                <div key={file.path}>
                  ...
                  >
                    {expandedDiff === file.path ? <ChevronDown size={9} className="text-muted-foreground mr-1" /> : <ChevronRight size={9} className="text-muted-foreground mr-1" />}
                    <FileCode size={10} className={cn("flex-shrink-0 mr-1.5", file.status === "added" ? "text-green" : "text-warning")} />
                    <span className="text-[10px] font-mono text-foreground flex-1 truncate">{file.path}</span>
                    <span className="text-[9px] font-mono text-green mr-1">+{file.additions}</span>
                    {file.deletions > 0 && <span className="text-[9px] font-mono text-destructive">-{file.deletions}</span>}
                  </button>
                    ...
                    <div className="ml-4 mt-0.5 mb-1 border border-border rounded overflow-hidden text-[9px] font-mono leading-[1.5]">
                      {diffLines.map((line, i) => (
                        <div key={i} className={cn(
                          ...
                          <span className="w-6 text-right pr-1 text-muted-foreground/40 select-none border-r border-border">{line.line}</span>
                          <span className={cn("px-1 w-3 select-none",
                            line.type === "addition" ? "text-green" : line.type === "deletion" ? "text-destructive" : "text-muted-foreground/30"
                          )}>
                            {line.type === "addition" ? "+" : line.type === "deletion" ? "-" : " "}
                          </span>
                          <span className={cn("flex-1 truncate",
                            line.type === "addition" ? "text-green" : line.type === "deletion" ? "text-destructive" : "text-foreground"
                          )}>{line.content}</span>
                        </div>
                      ))}
                    </div>
