  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="ml-auto font-mono text-[10px] text-muted-foreground">{filteredLog.length} requests</span>
      </div>
      ...
      <div className="overflow-hidden rounded border border-border">
        <div className="grid grid-cols-[70px_130px_90px_80px_80px_60px_60px_1fr] border-b border-border bg-surface-2 px-3 py-2 font-mono text-[9px] uppercase tracking-wider text-muted-foreground">
          <span>Time</span><span>Model</span><span>Provider</span><span>Input</span><span>Output</span><span>Latency</span><span>Cost</span><span>Session</span>
        </div>
        {filteredLog.map((request, index) => (
          <div key={`${request.time}-${request.session}-${index}`} className={index > 0 ? "grid grid-cols-[70px_130px_90px_80px_80px_60px_60px_1fr] items-center border-t border-border px-3 py-2 text-[11px] transition-colors hover:bg-surface-2" : "grid grid-cols-[70px_130px_90px_80px_80px_60px_60px_1fr] items-center px-3 py-2 text-[11px] transition-colors hover:bg-surface-2"}>
            ...
            <span className="font-mono text-foreground">{(request.inputTokens / 1000).toFixed(1)}k</span>
            <span className="font-mono text-foreground">{(request.outputTokens / 1000).toFixed(1)}k</span>
