import { Cell, ChartTooltip as Tooltip, Pie, PieChart, RechartsResponsiveContainer as ResponsiveContainer } from "@/primitives/chart";
      ...
      <div className="grid grid-cols-2 gap-6">
        <SurfacePanel>
          ...
          <div className="h-[240px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={platformBreakdown} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={3} dataKey="value">
        ...
        <div className="space-y-3">
          {providerQuotas.map((provider) => {
            ...
            return (
              <div key={provider.name} className="rounded border border-border bg-surface-1 p-3 transition-colors hover:border-primary/30">
                <div className="mb-1 flex items-center gap-2">
                  <div className="h-2 w-2 rounded-full" style={{ backgroundColor: `hsl(${provider.color})` }} />
                  <span className="flex-1 text-[13px] font-medium text-foreground">{provider.name}</span>
                  <span className="font-mono text-[12px] text-foreground">${provider.totalSpent.toFixed(2)}</span>
                </div>
