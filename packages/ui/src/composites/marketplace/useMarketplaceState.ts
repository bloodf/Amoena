...
export function useMarketplaceState(initialItems: MarketplaceItem[]) {
  ...
  const filtered = useMemo(() => {
    ...
    if (authorFilter === "official") result = result.filter((item) => item.author === "lunaria-team");
  ...
  const featuredItems = useMemo(() => items.filter((item) => item.featured && !item.installed), [items]);
  const hasActiveFilters = trustFilter !== "all" || authorFilter !== "all" || searchQuery.trim() !== "";
  ...
  const handleInstall = (item: MarketplaceItem) => {
    setItems((previous) => previous.map((entry) => (entry.id === item.id ? { ...entry, installed: true } : entry)));
    setReviewItem(null);
    if (selectedItem?.id === item.id) setSelectedItem({ ...item, installed: true });
  };

  const handleUninstall = (item: MarketplaceItem) => {
    setItems((previous) => previous.map((entry) => (entry.id === item.id ? { ...entry, installed: false } : entry)));
