...
export function PermissionPresetPicker({ presets, selected, onSelect }: PermissionPresetPickerProps) {
  return (
      ...
      {presets.map((preset) => (
        <button
          ...
          className={cn(
            ...
            selected === preset.id ? "border-primary bg-primary/5" : "border-border hover:border-primary/30",
            preset.id === "full" && selected === preset.id && "border-warning bg-warning/5",
          )}
        >
          <Shield
            size={12}
            className={cn(selected === preset.id ? (preset.id === "full" ? "text-warning" : "text-primary") : "text-muted-foreground")}
          />
