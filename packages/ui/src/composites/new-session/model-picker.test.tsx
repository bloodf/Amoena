...
describe("ModelPicker", () => {
  test("renders all model options", () => {
    render(<ModelPicker models={models} selectedModel="claude-4-sonnet" onSelect={vi.fn(() => {})} />);
  ...
  test("highlights selected model with primary border", () => {
    render(<ModelPicker models={models} selectedModel="gpt-5" onSelect={vi.fn(() => {})} />);
  ...
  test("calls onSelect when model clicked", () => {
    const onSelect = vi.fn((_model: string) => {});
  ...
  test("renders empty when no models", () => {
    const { container } = render(<ModelPicker models={[]} selectedModel="" onSelect={vi.fn(() => {})} />);
