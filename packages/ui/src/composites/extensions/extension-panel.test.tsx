...
describe('ExtensionPanel', () => {
  ...
  test('shows enabled/disabled status', () => {
    ...
    const statusButtons = buttons.filter(
      (b) =>
        b.textContent?.toLowerCase().includes('enabled') ||
  ...
  test('calls onUninstall on uninstall click', () => {
    const handlers = makeHandlers();
    render(<ExtensionPanel extensions={extensions} {...handlers} />);
    const uninstallButtons = screen
      .getAllByRole('button')
      .filter((b) => b.textContent?.toLowerCase().includes('uninstall'));
    ...
    const installBtn = screen
      ...
      .find(
        (b) =>
          ...
          b.textContent?.includes('extensions.install'),
      );
