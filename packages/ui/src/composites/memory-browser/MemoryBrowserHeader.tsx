...
import { ScreenDescription } from "@/components/screen";
  ...
  return (
    <ScreenHeader className="border-b border-border px-4 py-2">
      <ScreenHeaderCopy>
        ...
        <ScreenDescription className="text-[11px]">Browse, pin, and manage structured memory.</ScreenDescription>
      </ScreenHeaderCopy>
      <ScreenActions>
        <Button onClick={onAdd} variant="outline" size="sm" className="h-7 gap-1.5 text-[11px] text-primary">
        ...
        <div className="flex items-center gap-1 rounded bg-surface-2 p-0.5">
          <Button onClick={() => onViewModeChange("list")} variant={viewMode === "list" ? "secondary" : "ghost"} size="sm" className="h-7 gap-1.5 text-[11px]">
            <List size={12} /> List
          </Button>
          <Button onClick={() => onViewModeChange("graph")} variant={viewMode === "graph" ? "secondary" : "ghost"} size="sm" className="h-7 gap-1.5 text-[11px]">
