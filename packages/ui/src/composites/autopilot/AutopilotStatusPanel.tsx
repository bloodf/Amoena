...
function getStatusBorderClasses(state: AutopilotState): string {
  if (state === 'executing') return 'border-green/40 bg-green/5';
  if (state === 'waiting_approval') return 'border-warning/40 bg-warning/5';
...
export function AutopilotStatusPanel({
  ...
  return (
    <div className="p-6 border-b border-border" aria-labelledby="autopilot-heading">
      ...
      <div
        className={cn(
          ...
          getStatusBorderClasses(state),
        )}
        role="status"
        aria-live="polite"
        aria-label={`Autopilot status: ${sc.label}`}
      >
        <div
          className={cn(
            'w-3 h-3 rounded-full',
            sc.bgColor,
            (state === 'executing' || state === 'planning') && 'animate-pulse',
          )}
          aria-hidden="true"
        />
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {(state === 'executing' || state === 'planning') && (
          <>
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              className="h-8 gap-1.5 border-destructive text-destructive hover:bg-destructive/10"
            >
              <Square size={13} /> Stop
            </Button>
          </>
        ...
        {state === 'waiting_approval' && (
          <div className="flex items-center gap-2 w-full">
            <Button
              onClick={onApprove}
              size="sm"
              className="h-8 flex-1 justify-center gap-1.5 bg-success text-success-foreground hover:bg-success/90"
            >
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              className="h-8 flex-1 justify-center gap-1.5 border-destructive text-destructive hover:bg-destructive/10"
            >
              <X size={13} /> Deny
            </Button>
          </div>
        ...
        {state === 'blocked' && (
          <Button
            ...
            variant="outline"
            size="sm"
            className="h-8 gap-1.5 border-warning text-warning hover:bg-warning/10"
          >
