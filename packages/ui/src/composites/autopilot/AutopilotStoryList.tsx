...
export function AutopilotStoryList({ steps }: { steps: AutopilotStoryStep[] }) {
  return (
    <div className="p-4 border-b border-border flex-1">
      <h3 className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Story Breakdown</h3>
      <div className="space-y-1">
        {steps.map((step, index) => (
          <div key={`${step.label}-${index}`} className="flex items-center gap-2 py-1.5 px-2 rounded hover:bg-surface-2 transition-colors">
            <div className={cn(
