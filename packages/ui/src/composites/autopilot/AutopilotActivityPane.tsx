...
import type { AutopilotActivityItem, AutopilotPipelinePhase, AutopilotState, AutopilotSubAgent } from "./types";
...
const subAgentStatusConfig: Record<AutopilotSubAgent["status"], { color: string; bgColor: string; label: string }> = {
...
function SubAgentSwarmGrid({ agents }: { agents: AutopilotSubAgent[] }) {
  return (
    <div className="mb-4">
      <h3 className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Active Agents</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {agents.map(agent => {
          const sc = subAgentStatusConfig[agent.status];
          const progress = agent.stepsTotal > 0 ? (agent.stepsCompleted / agent.stepsTotal) * 100 : 0;
          return (
            <div key={agent.id} className="rounded border border-border bg-surface-2 p-3">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className={cn("text-[9px] px-1.5 py-0.5 rounded font-mono", sc.color, sc.bgColor)}>{sc.label}</span>
              </div>
              <div className="text-[10px] text-muted-foreground mb-2">{agent.role}</div>
              {agent.currentTask && (
                <div className="text-[10px] text-foreground/70 mb-2 italic truncate">{agent.currentTask}</div>
              )}
              <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                <span className="font-mono">{agent.stepsCompleted}/{agent.stepsTotal} steps</span>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Live Activity</h2>
        <div className="flex items-center gap-3">
          <button onClick={onOpenTaskBoard} className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium text-primary border border-primary/30 rounded cursor-pointer hover:bg-primary/10 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors">
      ...
      <div className="mb-4 p-3 rounded border border-border bg-surface-2">
        ...
        <div className="grid grid-cols-4 gap-3 mt-2">
          <div><div className="text-[10px] text-muted-foreground">Steps</div><div className="text-[13px] font-mono text-foreground">3/7</div></div>
          <div><div className="text-[10px] text-muted-foreground">Tokens</div><div className="text-[13px] font-mono text-foreground">6.0k</div></div>
          <div><div className="text-[10px] text-muted-foreground">Files</div><div className="text-[13px] font-mono text-foreground">4</div></div>
          <div><div className="text-[10px] text-muted-foreground">Duration</div><div className="text-[13px] font-mono text-foreground">4:12</div></div>
        </div>
      </div>
      ...
      <div className="space-y-1">
        {activityLog.map((item, index) => (
          <div key={`${item.time}-${item.target}-${index}`} className="flex items-center px-3 py-2 rounded hover:bg-surface-2 transition-colors">
            <span className="text-[11px] font-mono text-muted-foreground w-16">{item.time}</span>
            <Circle size={6} className={cn("fill-current mx-2", item.status === "completed" && "text-green", item.status === "pending_approval" && "text-warning animate-pulse")} />
            ...
            {item.status === "pending_approval" && (
              <div className="flex items-center gap-1.5">
                <button onClick={() => onApprove(index)} className="px-2 py-0.5 text-[10px] font-medium bg-success text-success-foreground rounded cursor-pointer hover:bg-success/90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors">
                  <Check size={10} /> Approve
                </button>
                <button onClick={() => onDeny(index)} className="px-2 py-0.5 text-[10px] font-medium border border-destructive text-destructive rounded cursor-pointer hover:bg-destructive/10 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors">
                  <X size={10} /> Deny
                </button>
              </div>
            )}
        ...
        {activityLog.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <div className="text-[11px] text-muted-foreground mt-1">Start an autopilot run to see activity here</div>
          </div>
        )}
      </div>
    </div>
