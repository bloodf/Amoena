  ...
  return (
    <ScreenSidebar className="w-[280px] p-0">
      <div className="flex items-center justify-between p-3">
        <span className="flex-1 px-3 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Workspaces</span>
        <Button onClick={onCreate} variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground">
          <Plus size={13} />
        </Button>
      </div>
          ...
          >
            <div className="flex min-w-0 flex-1 flex-col gap-1">
              <div className="flex items-center gap-2">
                ...
                <span className="flex-1 truncate text-[13px] text-foreground">{workspace.name}</span>
                <StatusPill label={health.label} className={cn("text-[9px]", health.color, health.bgColor)} />
              </div>
              <div className="ml-5 flex items-center gap-3">
                <span className="font-mono text-[10px] text-muted-foreground">{workspace.branch}</span>
                <span className="text-[10px] text-muted-foreground">{workspace.disk}</span>
                {workspace.pending ? <Circle size={5} className={cn("fill-current", workspace.conflicts ? "text-destructive" : "text-warning")} /> : null}
              </div>
            </div>
          </ScreenNavButton>
        );
