import { Check, GitBranch, ImagePlus, Shield, ToggleLeft, Users, Zap } from 'lucide-react';
    ...
    <div className="relative">
      <ComposerDropdown open={open} onClose={onClose} className="bottom-full left-0 mb-1 w-56">
        ...
        >
          ...
          <div
            className={cn(
              'relative ml-auto h-4 w-8 rounded-full',
              planMode ? 'bg-primary/30' : 'bg-surface-3',
            )}
          >
            <div
              className={cn(
                'absolute top-0.5 h-3 w-3 rounded-full',
                planMode ? 'right-0.5 bg-primary' : 'left-0.5 bg-muted-foreground',
              )}
            />
          </div>
        </button>
      ...
      {agents.map((agent) => (
        <button
          ...
          className={cn(
            ...
            agent.id === activeAgentId && 'bg-surface-2',
          )}
        >
    ...
    <ComposerDropdown open={open} onClose={onClose} className="bottom-full left-0 mb-1 w-52">
      ...
      {models.map((model) => (
        <button
          ...
          className={cn(
            'flex w-full items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-surface-2',
            model.id === activeModelId && 'bg-surface-2',
          )}
        >
          <Zap
            size={11}
            className={model.id === activeModelId ? 'text-primary' : 'text-muted-foreground'}
          />
  ...
  return (
    <ComposerDropdown open={open} onClose={onClose} className="bottom-full left-0 mb-1 w-52">
      ...
      {reasoningLevels.map((level) => (
        <button
          key={level.id}
          onClick={() => onSelectReasoning(level.id)}
          className={cn(
            'flex w-full items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-surface-2',
            level.id === reasoningLevel && 'bg-surface-2',
          )}
        >
  ...
  return (
    <ComposerDropdown open={open} onClose={onClose} className="bottom-full left-0 mb-1 w-48">
      {options.map((option) => (
        <button
          key={option.id}
          onClick={() => onSelect(option.id as 'local' | 'worktree' | 'cloud')}
          className={cn(
            'flex w-full items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-surface-2',
            option.id === current && 'bg-surface-2',
          )}
        >
    ...
    <ComposerDropdown open={open} onClose={onClose} className="bottom-full left-0 mb-1 w-52">
      {options.map((option) => (
        ...
        >
          <Shield
            size={11}
            className={option.id === 'full' ? 'text-warning' : 'text-muted-foreground'}
          />
          <span className="flex-1 text-[12px] text-foreground">{option.label}</span>
          {option.id === current ? <Check size={12} className="text-primary" /> : null}
        </button>
