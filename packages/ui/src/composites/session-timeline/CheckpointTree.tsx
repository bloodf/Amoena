            ...
            <div className="mb-1 flex-1 py-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
                  ...
                  Context compacted — {checkpoint.compaction.observationCount} observations summarized
                </span>
                <div className="h-px flex-1 bg-border" />
              </div>
            </div>
        ...
        <div className={cn("flex items-start gap-3", depth > 0 && "ml-6")}>
            ...
            <div
              className={cn(
                ...
                checkpoint.isCurrent
                  ? "border-primary bg-primary shadow-[0_0_8px_hsl(var(--primary)/0.5)]"
                  : isSelected
          ...
          >
            <div className="mb-0.5 flex items-center justify-between">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <span className={cn("truncate text-[12px] font-medium", checkpoint.isCurrent ? "text-primary" : "text-foreground")}>
                  {checkpoint.label}
                  {checkpoint.isCurrent ? (
              ...
              <span className="ml-2 flex-shrink-0 font-mono text-[10px] text-muted-foreground">{checkpoint.timestamp}</span>
            </div>
          ...
          {isSelected && !checkpoint.isCurrent ? (
            <div className="flex flex-shrink-0 items-center gap-1 pt-2">
              <button className="rounded p-1.5 text-muted-foreground cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors hover:bg-primary/10 hover:text-primary" aria-label="Restore checkpoint">
                <RotateCcw size={12} />
              </button>
              <button className="rounded p-1.5 text-muted-foreground cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors hover:bg-purple/10 hover:text-purple" aria-label="Fork from checkpoint">
                <GitBranch size={12} />
              </button>
              <button className="rounded p-1.5 text-muted-foreground cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors hover:bg-surface-2 hover:text-foreground" aria-label="View diff">
                <Diff size={12} />
              </button>
            </div>
          ) : null}
