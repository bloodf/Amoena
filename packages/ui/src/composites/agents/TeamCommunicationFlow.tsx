import { ArrowRight, Bot, Circle, Clock, Pause, Zap } from 'lucide-react';
          ...
          <div className="mt-4 pt-3 border-t border-border">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[9px] text-muted-foreground">
                Weighted by decision authority
              </span>
            </div>
            <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-surface-3">
              {team.agents.map((agent) => {
                const totalWeight = team.agents.reduce(
                  (sum, a) => sum + (a.decisionWeight ?? 0.5),
                  0,
                );
                const pct =
                  totalWeight > 0 ? ((agent.decisionWeight ?? 0.5) / totalWeight) * 100 : 0;
                return (
                  <div
                    key={agent.id}
                    className={cn(
                      ...
                      !['tui-claude', 'tui-opencode', 'tui-gemini'].includes(agent.tuiColor) &&
                        'bg-primary/60',
                    )}
                    style={{ width: `${pct}%` }}
            ...
            <div className="flex justify-between mt-1">
              {team.agents.map((agent) => (
                <span key={agent.id} className="text-[8px] text-muted-foreground">
                  {agent.name}
                </span>
              ))}
            </div>
