...
const teams: AgentTeam[] = [
  {
    ...
    status: "active",
    agents: [
      { id: "a1", name: "Claude", role: "Lead", model: "claude-4", tuiColor: "#FF0000", status: "working" },
      { id: "a2", name: "Gemini", role: "Worker", model: "gemini-2", tuiColor: "#00FF00", status: "idle" },
    ],
    totalTokens: "12.5k",
  ...
  },
  {
    ...
    status: "paused",
    agents: [
      { id: "a3", name: "GPT", role: "Developer", model: "gpt-5", tuiColor: "#0000FF", status: "paused" },
    ],
    totalTokens: "8.2k",
...
describe("TeamListPane", () => {
  test("renders the Agent Teams heading", () => {
    render(<TeamListPane teams={teams} selectedTeamId="team-1" onSelectTeam={mock(() => {})} />);
  ...
  test("calls onSelectTeam when a team is clicked", () => {
    const onSelectTeam = mock((_id: string) => {});
  ...
  test("renders the navigation landmark", () => {
    render(<TeamListPane teams={teams} selectedTeamId="team-1" onSelectTeam={mock(() => {})} />);
    expect(screen.getByLabelText("Agent teams")).toBeTruthy();
  });

  test("renders empty list gracefully", () => {
    render(<TeamListPane teams={[]} selectedTeamId="" onSelectTeam={mock(() => {})} />);
    expect(screen.getByText("Agent Teams")).toBeTruthy();
  });
});
