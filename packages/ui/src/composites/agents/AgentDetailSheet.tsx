    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          ...
          <button onClick={onClose} className="p-1 text-muted-foreground cursor-pointer hover:text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors" aria-label="Close agent settings">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="space-y-4 px-5 py-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between text-[12px]">
              ...
              <span className={cn("rounded px-1.5 py-0.5 text-[10px] font-mono", sourceColors[agent.source])}>
                {agent.source}
              </span>
            </div>
            <div className="flex items-center justify-between text-[12px]">
              <span className="text-muted-foreground">Tools</span>
              <span className="text-[11px] font-mono text-foreground">{agent.tools.join(", ")}</span>
            </div>
            {agent.session && (
              <div className="flex items-center justify-between text-[12px]">
          ...
          {(agent.division || agent.collaborationStyle || agent.communicationPreference || agent.decisionWeight !== undefined || agent.vibe) && (
              ...
              {agent.decisionWeight !== undefined && (
                <div className="space-y-1">
                  <div className="flex items-center justify-between text-[12px]">
                    ...
                    <span className="text-[10px] font-mono text-foreground">{Math.round(agent.decisionWeight * 100)}%</span>
                  </div>
                ...
                <div className="space-y-1">
                  ...
                  {agent.strengths?.length ? (
                    agent.strengths.map((strength) => (
                      <div key={strength} className="rounded bg-surface-3 px-2 py-1 text-[10px] text-foreground">
                        {strength}
                      </div>
                    ))
                  ) : (
                ...
                <div className="space-y-1">
                  ...
                  {agent.limitations?.length ? (
                    agent.limitations.map((limitation) => (
                      <div key={limitation} className="rounded bg-surface-3 px-2 py-1 text-[10px] text-foreground">
                        {limitation}
                      </div>
                    ))
                  ) : (
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              {(agent.status === "idle" || agent.status === "created" || agent.status === "paused" || agent.status === "stopped" || agent.status === "failed" || agent.status === "cancelled") && (
                <button
              ...
              {(agent.status === "active" || agent.status === "running" || agent.status === "paused") && (
                <button
