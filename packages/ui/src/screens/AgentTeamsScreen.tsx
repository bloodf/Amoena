...
const defaultTeams: AgentTeam[] = [
  {
    ...
    description:
      'Complete rewrite of authentication system with JWT rotation, OAuth2, and MFA support',
    status: 'active',
    totalTokens: '48.2k',
    ...
    agents: [
      {
        ...
        tuiColor: 'tui-claude',
        status: 'working',
        currentTask: 'Designing token rotation flow',
        progress: 65,
        tokensUsed: '18.4k',
      ...
      },
      {
        ...
        tuiColor: 'tui-claude',
        status: 'waiting',
        currentTask: 'Waiting for auth schema from Architect',
        tokensUsed: '14.1k',
        messagesExchanged: 8,
      ...
      },
      {
        ...
        status: 'idle',
        currentTask: 'Reviewed 2 PRs, waiting for next',
        tokensUsed: '8.2k',
        messagesExchanged: 5,
      ...
      },
      {
        ...
        tuiColor: 'tui-gemini',
        status: 'completed',
        currentTask: 'API docs for /auth endpoints',
        tokensUsed: '7.5k',
        messagesExchanged: 3,
    ...
    agents: [
      {
        ...
        tuiColor: 'tui-claude',
        status: 'idle',
        currentTask: 'Paused - query optimization',
        tokensUsed: '12.3k',
        messagesExchanged: 9,
        collaborationStyle: 'directive',
        communicationPreference: 'structured',
      ...
      },
      {
        ...
        tuiColor: 'tui-opencode',
        status: 'idle',
        currentTask: 'Paused - Redis integration',
        tokensUsed: '10.4k',
        messagesExchanged: 6,
        collaborationStyle: 'collaborative',
        communicationPreference: 'concise',
        ...
        <div className="p-6 space-y-6">
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2">
              {team.status === 'active' ? (
                <button
                  className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] bg-warning/10 text-warning border border-warning/30 rounded cursor-pointer hover:bg-warning/20 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors"
                  aria-label={t('ui.pauseTeam')}
                >
                  <Pause size={12} /> {t('ui.pauseTeam')}
                </button>
              ) : (
                <button
                  className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] bg-primary/10 text-primary border border-primary/30 rounded cursor-pointer hover:bg-primary/20 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring transition-colors"
                  aria-label={t('ui.resumeTeam')}
                >
                  <Play size={12} /> {t('ui.resumeTeam')}
                </button>
              )}
            </div>
          </div>
