...
import { ScreenMain, ScreenRoot } from "@/components/screen";
...
import { createImportedAgent, createManagedAgent, initialManagedAgents } from "@/composites/agents/data";
import type { AgentStatus, AgentTeam, ManagedAgent } from "@/composites/agents/types";
...
export function AgentManagementScreen({
  ...
  const updateAgentStatus = (name: string, status: AgentStatus) => {
    setAgents(prev => prev.map(a => a.name === name ? { ...a, status } : {
      ...a, children: a.children?.map(c => c.name === name ? { ...c, status } : c)
    }));
  ...
  const updateAgentPermission = (name: string, permission: string) => {
    setAgents(prev => prev.map(a => a.name === name ? { ...a, permission } : {
      ...a, children: a.children?.map(c => c.name === name ? { ...c, permission } : c)
    }));
  ...
  const deleteAgent = (name: string) => {
    setAgents(prev => prev.filter(a => a.name !== name).map(a => ({
      ...a, children: a.children?.filter(c => c.name !== name)
    })));
          ...
          <ScreenMain className="overflow-y-auto">
            {filteredAgents.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16">
                ...
                <div className="text-[13px] text-muted-foreground">No agents match the current filters</div>
              </div>
            ) : (
              filteredAgents.map(agent => (
                <div key={agent.name}>
                  ...
                  />
                  {expandedAgents.has(agent.name) && agent.children?.map(sub => (
                    <AgentRow key={sub.name} agent={sub} depth={1} expanded={false} onToggle={() => {}} onOpenSettings={(a) => setSettingsAgent(a)} />
                  ))}
                </div>
              ))
            )}
