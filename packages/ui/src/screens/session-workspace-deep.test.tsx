...
describe("SessionWorkspace deep flows", () => {
  ...
  test("closes a session tab and falls back to another tab", () => {
    ...
    const closers = screen.getAllByRole("button").filter((button) => button.getAttribute("aria-label") === "New Session" || button.getAttribute("aria-label") === "Hide side panel");
    expect(closers.length).toBeGreaterThan(0);

    const closeChips = screen.getAllByRole("button", { hidden: true }).filter((button) => button.textContent?.trim() === "");
  ...
  test("resizes terminal and side panel plus reorders tabs", () => {
    ...
    const separators = Array.from(document.querySelectorAll("[class*='cursor-row-resize'], [class*='cursor-col-resize']"));
    ...
    const tabButtons = screen.getAllByRole("button").filter((button) =>
      ...
      ["JWT Auth Refactor", "Rate Limiter Design", "API Routes"].some((label) => button.textContent?.includes(label)),
    );
    fireEvent.dragStart(tabButtons[0]);
    fireEvent.dragOver(tabButtons[1], { preventDefault: () => {} });
    fireEvent.dragEnd(tabButtons[0]);

    expect(screen.getAllByText(/JWT Auth Refactor|Rate Limiter Design|API Routes/).length).toBeGreaterThan(0);
  });
  ...
  test("updates session controls through the embedded composer and supports keyboard tab closing", () => {
    ...
    const closeAffordance = screen.getAllByRole("button", { hidden: true }).find((node) => node.getAttribute("role") === "button");
    if (closeAffordance) {
