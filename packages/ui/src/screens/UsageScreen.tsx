...
export function UsageScreen() {
  ...
  const totalTokens = useMemo(() => {
    return usageDailyUsage.reduce((sum, day) => sum + day.claude + day.openai + day.gemini + day.codex, 0);
  }, []);

  const totalCost = useMemo(() => {
    return usageDailyCost.reduce((sum, day) => sum + day.cost, 0);
  }, []);

  const totalRequests = usageApiRequestLog.length * 12;

  const filteredLog = usageApiRequestLog.filter(
    (request) => (providerFilter === "all" || request.provider === providerFilter) && (sessionFilter === "all" || request.session === sessionFilter),
  );
        ...
        <ScreenStack>
          <ScreenHeader>
            <ScreenHeaderText>
              ...
              <ScreenSubtitle>Track token consumption, spending, and API usage across sessions and providers.</ScreenSubtitle>
            </ScreenHeaderText>
              ...
              {usageTimeRanges.map((range) => (
                <button
                  ...
                  className={cn(
                    ...
                    timeRange === range.id ? "bg-surface-1 text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground",
                  )}
                >
                  {range.label}
                </button>
          ...
          <UsageTabs tabs={usageTabs as unknown as { id: TabId; label: string }[]} activeTab={activeTab} onChange={setActiveTab} />
          ...
          {activeTab === "sessions" ? <UsageSessionsPanel sessions={usageSessionBreakdown} customTooltip={<UsageStackedTooltip />} /> : null}
          ...
          {activeTab === "platforms" ? (
            <UsagePlatformsPanel platformBreakdown={usagePlatformBreakdown} providerQuotas={usageProviderQuotas} />
          ) : null}
        </ScreenStack>
      </ScreenContainer>
    </ScreenRoot>
