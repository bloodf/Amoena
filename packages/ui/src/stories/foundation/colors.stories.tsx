...
import { amoenaThemeTokens } from '@lunaria/tokens';
...
function Swatch({ name, hsl }: SwatchProps) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <span className="text-[10px] font-mono text-muted-foreground">hsl({hsl})</span>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex h-16 rounded-md overflow-hidden border border-border">
        ...
        >
          <span
            ...
            style={{
              color: `hsl(${darkHsl === '0 0% 100%' || (darkHsl && parseInt(darkHsl.split(' ').pop() ?? '0') > 60) ? '0 0% 10%' : '0 0% 90%'})`,
            }}
          >
            dark
          </span>
        ...
        >
          <span
            className="text-[9px] font-mono"
            style={{
              color: `hsl(${lightHsl === '0 0% 100%' || (lightHsl && parseInt(lightHsl.split(' ').pop() ?? '0') > 60) ? '0 0% 10%' : '0 0% 90%'})`,
            }}
          >
            light
          </span>
      ...
      <div className="flex gap-2">
        <span className="text-[10px] font-mono text-muted-foreground truncate">D: {darkHsl}</span>
      </div>
      <span className="text-[10px] font-mono text-muted-foreground truncate">L: {lightHsl}</span>
    </div>
  );
}
...
const colorGroups: { title: string; tokens: string[] }[] = [
  ...
  {
    title: 'Status',
    tokens: ['warning', 'warning-foreground', 'success', 'success-foreground'],
  },
];
  ...
  render: () => (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <p className="text-muted-foreground mb-8">
        All semantic color tokens from the Amoena design system. Each swatch shows dark (left) and
        light (right) theme values as HSL.
      </p>
      {colorGroups.map((group) => (
        <ColorSection key={group.title} title={group.title} tokens={group.tokens} />
      ))}
    </div>
  ),
};
...
export const LiveThemeColors: Story = {
  ...
  render: () => {
    ...
    return (
      <div className="p-8 max-w-6xl mx-auto">
        <h2 className="text-2xl font-bold text-foreground mb-2">Live Theme Colors</h2>
        <p className="text-muted-foreground mb-6">
          These swatches use CSS custom properties and respond to the active Storybook theme toggle.
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {liveTokens.map((token) => (
            <Swatch key={token} name={token} hsl={`var(--${token})`} />
          ))}
        </div>
      </div>
    );
