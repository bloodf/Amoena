...
describe('Amoena stateful panels', () => {
  ...
  test('renders general, editor, terminal, session, and providers settings content', () => {
    ...
    expect(screen.getByText('Max context tokens')).toBeTruthy();
    session.unmount();
  ...
  test('message queue reorders, pauses, and fully removes items', async () => {
    ...
    const draggableRows = screen
      .getAllByTitle(/Pause|Resume/i)
      .map((button) => button.closest("div[class*='group']")!)
      .filter(Boolean);
  ...
  test('terminal panel can add, close tabs, reorder, and close panel', async () => {
    ...
    const tabButtons = screen
      .getAllByRole('button')
      .filter(
        (button) => button.textContent?.includes('bash') || button.textContent?.includes('node'),
      );
