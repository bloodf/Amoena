...
import { 
  Home, MessageSquare, PlayCircle, Users, Brain, GitBranch, 
  Layout, Package, SlidersHorizontal, 
  Search, Sun, Moon, BarChart3, ClipboardList
} from "lucide-react";
...
function SidebarItem({ icon: Icon, label, path, onNavigate }: { icon: any; label: string; path: string; onNavigate?: () => void }) {
  return (
    <NavLink
      to={path}
      end={path === "/"}
...
export function SidebarRail({ onOpenCommandPalette, onNavigate }: { onOpenCommandPalette?: () => void; onNavigate?: () => void }) {
    ...
    <div className="flex h-full w-[180px] flex-col border-r border-border bg-surface-0 flex-shrink-0">
      ...
      <div className="flex flex-col gap-0.5 pb-3">
        ...
        >
          ...
          <kbd className="text-[9px] font-mono text-muted-foreground/60 bg-surface-2 px-1 py-0.5 rounded border border-border">⌘K</kbd>
        </button>
      </div>
    </div>
  );
