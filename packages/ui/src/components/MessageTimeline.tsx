...
export function MessageTimeline({
  ...
  return (
    <div className="relative h-full">
      <div ref={scrollRef} onScroll={handleScroll} className="h-full overflow-y-auto px-4 py-4 space-y-3">
        {messages.map((msg) => {
                ...
                <div className="max-w-[85%] space-y-1">
                  {msg.model ? (
                    <div className="mb-1 flex items-center gap-1.5">
                      ...
                      />
                      <span className="text-[11px] font-mono text-muted-foreground">{msg.model}</span>
                      {reasoningActive ? (
                        <span className="rounded bg-primary/10 px-1.5 py-0.5 text-[9px] font-mono text-primary">
                          REASONING
                        </span>
                      ) : null}
                    </div>
