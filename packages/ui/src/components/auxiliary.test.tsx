...
describe('Amoena auxiliary surfaces', () => {
  ...
  test('status bar expands rate limits', async () => {
    ...
    const trigger = screen
      .getAllByRole('button')
      .find((button) => button.textContent?.includes('86%'));
  ...
  test('install review sheet shows permissions', () => {
    render(<InstallReviewSheet item={marketplaceItem} onClose={() => {}} onConfirm={() => {}} />);
  ...
  test('command palette shows grouped results and closes via action', async () => {
    ...
    const settingsEntry = screen
      .getAllByText('Settings')
      .find((node) => node.className.includes('text-[12.5px]'));
