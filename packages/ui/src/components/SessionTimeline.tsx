...
import { sessionTimelineChangedFiles, sessionTimelineCheckpoints, sessionTimelineDiffLines } from "@/composites/session-timeline/data";
...
export function SessionTimeline() {
  ...
  const selected = sessionTimelineCheckpoints.find((checkpoint) => checkpoint.id === selectedCheckpoint);
    ...
    <div className="flex h-full">
      ...
      <div className="w-[360px] border-r border-border flex flex-col flex-shrink-0">
        <div className="flex items-center justify-between p-3 border-b border-border">
          ...
          <button className="flex items-center gap-1 px-2 py-1 text-[10px] border border-border text-muted-foreground rounded hover:bg-surface-2 transition-colors">
            <Plus size={10} />
            Checkpoint
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-3">
          <CheckpointTree checkpoints={sessionTimelineCheckpoints} selectedCheckpoint={selectedCheckpoint} onSelect={setSelectedCheckpoint} />
        </div>
      </div>
