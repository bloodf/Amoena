...
const defaultQueue: QueueItem[] = [
  { id: "q1", message: "Refactor the authentication middleware to use JWT validation", status: "running" },
...
export function MessageQueue() {
  ...
  return (
    <div className="border-t border-border bg-surface-0">
      ...
      {!collapsed && (
        <div className="px-2 pb-2 max-h-[160px] overflow-y-auto" onDragLeave={() => setDropIndex(null)}>
          {queue.map((item, index) => (
            <div key={item.id} className="relative">
              ...
              >
                ...
                <div className={cn("w-1.5 h-1.5 rounded-full flex-shrink-0", statusConfig[item.status].color)} />
                <span className="text-[10px] text-muted-foreground w-4 flex-shrink-0 font-mono">{index + 1}</span>
